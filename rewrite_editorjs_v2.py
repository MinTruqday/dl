...
new_render_block = '''    @staticmethod
    def _render_block(block: dict) -> str:
        ...
        if t in ("smartArtCycle", "smartArtHierarchy", "smartArtList", "smartArtMatrix", "smartArtProcess", "smartArtPyramid", "smartArtRelationship", "wordArt", "shape", "drawing"):
            ...
            return f'<div style="border:2px solid #aaa;padding:20px;margin:15px 0;text-align:center;background:#fafafa;border-radius:5px;"><h4 style="margin:0;">{title}</h4><div style="font-size:0.8em;color:#888;">[So do / Hinh khoi]</div></div>'
...
if start_idx == -1:
    # try regex
    match = re.search(r"\\s+@staticmethod\\n\\s+def _render_block", content)
    if match:
        start_idx = match.start()

end_idx = content.find("    @staticmethod\\n    def _convert_blocks_to_html")
if end_idx == -1:
    match = re.search(r"\\s+@staticmethod\\n\\s+def _convert_blocks_to_html", content)
    if match:
        end_idx = match.start()
