...
export const createNestedCommentAPI = async (itemId: string, payload: { text: string; parent_id?: string | null; item_type?: string }) => {
    const token = getToken();
...
export async function replyStoryAPI(storyId: string, message: string) {
    const res = await fetch(`${API_URL}/social/stories/${storyId}/reply?message=${encodeURIComponent(message)}`, {
        method: "POST",
        headers: getAuthHeaders()
    });
    if (!res.ok) throw new Error("Gửi trả lời thất bại.");
...
export async function getTrendingTagsAPI() {
    const res = await fetch(`${API_URL}/social/trending-tags`, { headers: getAuthHeaders() });
    if (!res.ok) throw new Error("Không thể tải xu hướng.");
    return await res.json();
}

export async function getSuggestedDocumentsAPI() {
    const res = await fetch(`${API_URL}/social/suggested-documents`, { headers: getAuthHeaders() });
...
export async function reportPostAPI(postId: string, reason: string) {
    const res = await fetch(`${API_URL}/social/posts/${postId}/report?reason=${encodeURIComponent(reason)}`, {
        method: "POST",
        headers: getAuthHeaders()
    });
...
export async function getFeedCommentsAPI_V2(itemId: string, itemType: string) {
    const res = await fetch(`${API_URL}/social/${itemType}/${itemId}/comments`, { headers: getAuthHeaders() });
...
export async function getFeedAPI(tab: string, skip: number, limit: number, itemType?: string, sort?: string) {
