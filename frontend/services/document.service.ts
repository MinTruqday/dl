...
export async function restoreVersionAPI(versionId: string) {
    const token = getToken();
    const res = await fetch(`${API_URL}/documents/versions/${versionId}/restore`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` }
    });
...
export async function monetizeDocumentAPI(id: string, price: number) {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || "Thiết lập giá trị giao dịch thất bại.");
    return data;
}
...
export async function transferDocumentAPI(id: string, newOwnerId: string) {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || "Chuyển nhượng quyền sở hữu thất bại.");
    return data;
}
...
export async function shareToFeedAPI(id: string) {
    const token = getToken();
    const res = await fetch(`${API_URL}/documents/${id}/share-feed`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }});
    return res.json();
}

export async function getDocumentAnalyticsAPI(id: string) {
    const token = getToken();
    const res = await fetch(`${API_URL}/documents/${id}/analytics`, { headers: { Authorization: `Bearer ${token}` }});
