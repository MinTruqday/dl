export default function MetricStrip({ items }) {
    ...
    return (<dl className={`grid border-y border-border ${columns}`}>
      {items.map((item, index) => (<div key={item.label} className={`min-w-0 py-4 sm:px-5 ${dividers(index)} ${index % smColumns === 0 ? "sm:pl-0" : ""} ${index % lgColumns === 0 ? "lg:pl-0" : ""} ${index % smColumns === smColumns - 1 ? "sm:pr-0" : ""} ${index % lgColumns === lgColumns - 1 ? "lg:pr-0" : ""}`}>
