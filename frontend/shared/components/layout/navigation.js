...
export const navigationGroups = [
  {
    ...
    items: [
      ...
      },
      {
        id: "review-queue",
        label: "Chờ rà soát",
        href: "/giao-vien/cau-hoi/ra-soat",
        icon: Shield,
        roles: ["author", "admin"],
        personas: ["teacher"],
      },
  ...
  return groups
    .map((group) => ({
      ...
      items: group.items.filter((item) => {
        ...
        if (item.roles && !item.roles.includes(String(user?.role || "").toLowerCase()))
          return false;
        if (
          !item.personas ||
          !personas.length ||
