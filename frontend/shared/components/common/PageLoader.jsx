export default function PageLoader({ rows = 5, compact = false, }) {
    return (<div className={`w-full ${compact ? "py-4" : "py-8"}`} role="status" aria-label="Đang tải dữ liệu">
      ...
      <div className="overflow-hidden rounded-panel border border-border bg-surface">
        ...
        <div className="divide-y divide-border">
          {Array.from({ length: rows }).map((_, index) => (<div key={index} className="grid min-h-16 grid-cols-[minmax(0,1fr)_7rem] items-center gap-4 px-4">
