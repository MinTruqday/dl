...
export default class DocLibUndo {
  ...
  constructor({ editor, maxHistorySize = 50 }: { editor: any, maxHistorySize?: number }) {
    this.editor = editor;
  ...
  private async initialize() {
      ...
      setTimeout(() => {
          ...
          if (this.holder) {
              ...
              this.observer.observe(this.holder, { childList: true, subtree: true, characterData: true });
          }
      }, 500);
  }
  ...
  private updateUI() {
      ...
      if (this.redoBtn) this.redoBtn.disabled = this.position >= this.history.length - 1;
  }
}
