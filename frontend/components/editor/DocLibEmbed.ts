...
export default class DocLibEmbed implements BlockTool {
  ...
  private data: { service: string, source: string, embed: string, width: number, height: number, caption: string };

  static get toolbox() {
    return {
      title: 'DocLib Embed',
      icon: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>'
    };
  }
  ...
  render() {
    ...
    if (!document.getElementById('doclib-embed-styles')) {
        ...
        style.innerHTML = `
            ...
            .doclib-embed-caption:empty::before { content: 'Nhập chú thích video...'; color: #94a3b8; pointer-events: none; }
  ...
  private buildUI() {
      ...
      if (this.data.embed) {
          ...
          input.placeholder = 'Nhập link YouTube (VD: https://youtube.com/watch?v=...)';
          ...
          const processEmbed = () => {
              ...
              if (val.includes('youtube.com/watch?v=') || val.includes('youtu.be/')) {
                  const videoId = val.split('v=')[1]?.split('&')[0] || val.split('youtu.be/')[1]?.split('?')[0];
                  if (videoId) {
                  ...
                  input.value = '';
                  input.placeholder = 'Link không hợp lệ! Vui lòng nhập link YouTube.';
              }
          };
