...
export default class DocLibImageCrop implements BlockTool {
  ...
  static get toolbox() {
    return {
      ...
      icon: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v14a2 2 0 0 0 2 2h14"></path><path d="M18 22V8a2 2 0 0 0-2-2H2"></path></svg>'
    };
  }
  ...
  private buildUI() {
      ...
      if (this.data.url) {
          ...
          if (!this.readOnly) {
              ...
              const createSlider = (labelStr: string, min: number, max: number, value: number, onChange: (val: number) => void) => {
