...
import { MessageCircle, X, Send, Cpu, Zap, Coins, Paperclip, Image as ImageIcon, FileText, Loader2, Maximize2, Minimize2, History as HistoryIcon, Edit2, ChevronLeft, Trash2 } from "lucide-react";
...
export default function AiChat() {
  ...
  const [messages, setMessages] = useState<{ id?: string; role: string; content: string; thoughts?: string[] }[]>([]);
  ...
  const handleSubmit = async (e?: React.FormEvent, retryText?: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try { const errJson = await res.json(); errorText = errJson.detail || errorText; } catch {}
        setMessages((prev) => {
          const updated = [...prev];
          ...
          if (type === "status" && data) {
            try {
              ...
              setMessages((prev) => {
                ...
                if (lastMsg.role === "assistant" && !lastMsg.thoughts?.includes(nodeVi)) {
  ...
  return (
    <div className="font-sans">
      <button
        ...
        className={`fixed bottom-6 right-6 z-[100] w-14 h-14 border border-zinc-200 flex items-center justify-center transition-all duration-300 active:scale-90 ${isOpen ? "bg-black text-white hover:bg-zinc-800" : "bg-white text-black hover:bg-zinc-50"}`}
      >
        {isOpen ? <X className="w-6 h-6" /> : <MessageCircle className="w-6 h-6" />}
      </button>

      {isOpen && (
        <div className={`fixed bottom-24 right-6 z-[100] ${isExpanded ? 'w-[850px]' : 'w-[450px]'} h-[700px] bg-white border border-zinc-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-8 fade-in duration-300 transition-all`}>
          ...
          <div className="px-5 py-5 border-b border-zinc-100 flex items-center justify-between shrink-0 bg-white">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setView(view === 'chat' ? 'history' : 'chat')} className={`p-1.5 transition-colors rounded-sm ${view === 'history' ? 'bg-black text-white' : 'text-zinc-400 hover:text-black'}`}>
                <HistoryIcon className="w-4 h-4" />
              </button>
              <button onClick={() => setIsExpanded(!isExpanded)} className="p-1.5 text-zinc-400 hover:text-black transition-colors">
                {isExpanded ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
              </button>
              <button onClick={() => setIsOpen(false)} className="p-1.5 text-zinc-400 hover:text-black transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>

          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 min-h-0 bg-zinc-50/20 scrollbar-thin scrollbar-thumb-zinc-200">
            {view === 'history' ? (
              <div className="space-y-4 animate-in fade-in duration-300">
                <button onClick={() => setView('chat')} className="flex items-center gap-2 text-[10px] font-bold text-zinc-400 hover:text-black uppercase tracking-widest mb-4">
                ...
                {sessions.length === 0 ? (
                  ...
                  <div key={s._id} className={`p-6 border bg-white transition-all cursor-pointer group relative ${currentSessionId === s._id ? 'border-black' : 'border-zinc-100 hover:border-black'}`}>
                    <div onClick={() => {
                    ...
                    <button
                      onClick={async (e) => {
                        ...
                        try {
                          ...
                          console.error("Lỗi xóa phiên hội thoại:", err);
                        }
                      }}
                      className="absolute top-4 right-4 p-1.5 text-zinc-300 hover:text-black opacity-0 group-hover:opacity-100 transition-all"
                    >
              ...
              messages.map((msg, idx) => {
                ...
                return (
                <div key={idx} className={`max-w-[98%] text-sm leading-relaxed animate-in fade-in duration-300 flex flex-col ${msg.role === "user" ? "self-end items-end" : "self-start items-start"}`}>
                  <div className={`border relative group ${isTyping ? "px-4 py-3 bg-white border-zinc-100 inline-flex items-center" : msg.role === "user" ? "px-5 py-4 bg-black text-white border-black" : "px-5 py-4 bg-white border-zinc-100 text-black w-full"}`}>
                    {msg.role === "user" && !isSending && (
                      <button onClick={() => setEditingMessageId(msg.id || null)} className="absolute -left-10 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-2 text-zinc-300 hover:text-black transition-all">
                        <Edit2 className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {editingMessageId && editingMessageId === msg.id ? (
                      <div className="flex flex-col gap-3 py-1">
                        <textarea defaultValue={msg.content} className="w-full bg-zinc-900 text-white p-3 text-[13px] border border-zinc-700 focus:outline-none min-h-[80px]" onKeyDown={(e: any) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSubmit(undefined, e.target.value))} />
                        <div className="flex justify-end gap-2">
                          <button onClick={() => setEditingMessageId(null)} className="text-[10px] font-bold uppercase px-3 py-1.5 text-zinc-400 hover:text-white transition-colors">Hủy</button>
                          <button onClick={(ev) => { const ta = ev.currentTarget.parentElement?.parentElement?.querySelector('textarea') as HTMLTextAreaElement; handleSubmit(undefined, ta.value); }} className="text-[10px] font-bold uppercase px-3 py-1.5 bg-white text-black">Gửi lại</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        <div className={`w-full prose prose-sm max-w-none ${msg.role === "user" ? "prose-invert" : "prose-zinc"}`}>
                          {msg.role === "assistant" && !msg.content && isSending ? (
                            <div className="flex items-center gap-1.5 py-2">
                              <div className="w-2 h-2 bg-zinc-300 rounded-sm animate-bounce" style={{ animationDelay: "0ms" }} />
                              <div className="w-2 h-2 bg-zinc-300 rounded-sm animate-bounce" style={{ animationDelay: "150ms" }} />
                              <div className="w-2 h-2 bg-zinc-300 rounded-sm animate-bounce" style={{ animationDelay: "300ms" }} />
                            </div>
                          ) : (
                            <ReactMarkdown
                              ...
                              components={{
                                ...
                                code({ node, inline, className, children, ...props }: any) {
                                  ...
                                  if (!inline && match) {
                                    const lang = match[1];
                                    return (
                                      <div className="my-3 bg-zinc-900 border border-zinc-800 overflow-x-auto">
                                        <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800">
                                          <span className="text-[10px] font-bold text-zinc-500 uppercase">{lang}</span>
                                          <button onClick={() => { navigator.clipboard.writeText(content); showToast("Đã sao chép", "info"); }} className="text-[10px] font-bold text-zinc-500 hover:text-white transition-colors uppercase">Sao chép</button>
                                        </div>
                                        <pre className="p-4 overflow-x-auto"><code className="text-[13px] font-mono text-zinc-300">{content}</code></pre>
                                      </div>
                                    );
                                  }
                                  return <code className={`${className} bg-zinc-100 px-1 py-0.5 text-black font-mono text-[13px]`} {...props}>{children}</code>;
                                },
                                table: ({ children }) => <div className="overflow-x-auto my-4 border border-zinc-100"><table className="min-w-full divide-y divide-zinc-200">{children}</table></div>,
          ...
          <div className="p-5 bg-white border-t border-zinc-100 shrink-0 relative">
              ...
              <div className="flex gap-3 mb-4 overflow-x-auto pb-1 scrollbar-none">
                {selectedImage && (
                  <div className="relative group shrink-0">
                    ...
                    <button onClick={() => setSelectedImage(null)} className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-black text-white flex items-center justify-center hover:bg-zinc-800 transition-colors"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {selectedFile && (
                  <div className="relative group shrink-0 h-14 px-4 bg-zinc-50 border border-zinc-200 flex items-center gap-3">
                    ...
                    <span className="text-[12px] font-bold text-zinc-600 truncate max-w-[120px]">{selectedFile.name}</span>
                    <button onClick={() => setSelectedFile(null)} className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-black text-white flex items-center justify-center hover:bg-zinc-800 transition-colors"><X className="w-3 h-3" /></button>
                  </div>
                )}
              </div>
            )}
            ...
            {showAttachments && (
              <div className="absolute bottom-full left-4 mb-4 bg-white border border-zinc-200 p-2 flex gap-2 animate-in fade-in slide-in-from-bottom-4 duration-300 z-50">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center gap-2 p-4 hover:bg-zinc-50 transition-all min-w-[80px]">
                  <div className="w-12 h-12 border border-zinc-200 flex items-center justify-center"><FileText className="w-6 h-6 text-black" /></div>
                  <span className="text-[11px] font-bold text-black">Tài liệu</span>
                </button>
                <button onClick={() => imageInputRef.current?.click()} className="flex flex-col items-center gap-2 p-4 hover:bg-zinc-50 transition-all min-w-[80px]">
                  <div className="w-12 h-12 border border-zinc-200 flex items-center justify-center"><ImageIcon className="w-6 h-6 text-black" /></div>
            ...
            <form onSubmit={handleSubmit} className="flex gap-2">
              <div className="flex-1 h-14 bg-zinc-50 border border-zinc-200 flex items-center px-4 gap-3 focus-within:border-black focus-within:bg-white transition-all">
                <button type="button" onClick={handleAttach} className="text-zinc-400 hover:text-black transition-colors shrink-0">
                  <Paperclip className="w-5 h-5" />
                </button>
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Nhập câu hỏi" disabled={isSending} className="flex-1 h-full text-sm bg-transparent outline-none font-medium" />
              </div>
              <button type="submit" disabled={isSending || !input.trim() || (usePro && (user?.wallet_balance || 0) < 10)} className="w-14 h-14 bg-black text-white flex items-center justify-center hover:bg-zinc-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-95">
