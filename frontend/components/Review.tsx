        ...
        >
          <div className="space-y-6 text-center md:text-left">
            ...
            <div className="flex gap-4 justify-center md:justify-start">
              {[1, 2, 3, 4, 5].map((s) => (
                ...
                >
                  <Star
                    className={`w-10 h-10 ${s <= rating ? "fill-black" : "fill-none"}`}
                  />
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <textarea
              ...
              placeholder=""
              className="w-full bg-white border border-zinc-100 p-8 text-sm font-medium focus:border-black focus:outline-none min-h-[200px] rounded-sm placeholder:text-zinc-200"
              required
            />
          </div>
        ...
        {loading ? (
          <div className="space-y-10">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-40 bg-white animate-pulse border border-zinc-100 rounded-sm"
              />
            ))}
          </div>
        ) : reviews.length > 0 ? (
            ...
            >
              <div className="flex items-start justify-between gap-6">
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 border border-zinc-100 bg-white flex items-center justify-center overflow-hidden shrink-0 rounded-sm">
                    {rev.avatar_url ? (
                      <img
                        src={rev.avatar_url}
                        alt=""
                  ...
                  <div>
                    <div className="text-sm font-bold text-black uppercase tracking-tight">
                      {rev.full_name || "Cộng tác viên ẩn danh"}
                    </div>
              ...
              <p className="text-base text-zinc-600 leading-loose pl-18 font-medium max-w-4xl">
                {rev.comment}
              </p>
            </div>
          ))
