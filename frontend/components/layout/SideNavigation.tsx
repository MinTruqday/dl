...
import { Home, MessageSquare, Library, Files, PenTool, Trophy } from 'lucide-react';
import { cn } from '../../lib/utils';

const NAV_ITEMS = [
  { href: '/', label: 'Trang chủ', icon: Home },
  { href: '/feed', label: 'Bảng tin', icon: MessageSquare },
    ...
    <aside className="w-64 border-r border-border bg-white h-full shrink-0 hidden md:flex flex-col py-4 overflow-y-auto">
      <div className="px-3 flex flex-col gap-1">
        {NAV_ITEMS.filter(item => {
            if (item.href === '/studio') return user?.role === 'author' || user?.role === 'admin';
            if (['/library', '/documents', '/profile', '/wallet'].includes(item.href)) return !!user;
            return true;
          }).map((item) => {
