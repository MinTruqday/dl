...
export default function Moderation({ initialTab }: { initialTab?: ModTab }) {
  ...
  const [activeTab, setActiveTab] = useState<ModTab>(
    initialTab || tabFromUrl || "documents",
  );
  ...
  const [confirmModal, setConfirmModal] = useState<{
    type: "approve" | "reject" | "report_keep" | "report_takedown" | "collection";
    data: any;
  } | null>(null);
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 py-12 font-sans text-black selection:bg-black selection:text-white">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            ...
            <p className="text-zinc-400 text-sm font-bold uppercase tracking-widest flex items-center gap-2">
              Hệ thống kiểm soát nội dung DocLib{" "}
              <ShieldCheck className="w-3.5 h-3.5 text-zinc-100" />
            </p>
          </div>

          <div className="flex items-center gap-4">
            <div className="flex bg-white p-1 rounded-sm border border-zinc-100">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest rounded-sm ${
                    activeTab === tab.id
                      ? "bg-white text-black "
                      : "text-zinc-400 "
                  }`}
            ...
            <button
              ...
              className="h-12 px-8 bg-black text-white text-[10px] font-bold tracking-widest uppercase active:scale-[0.98] flex items-center gap-3 rounded-sm disabled:opacity-50"
            >
              {isRefreshing ? (
        ...
        {activeTab === "documents" && (
          <div className="space-y-10 animate-in fade-in ">
            <div className="flex items-center justify-between px-2">
              <h2 className="text-[11px] font-bold text-black tracking-[0.2em] uppercase">
                Hàng đợi phê duyệt ({pendingDocuments.length})
              </h2>
            ...
            {pendingDocuments.length === 0 ? (
              <div className="py-48 text-center border border-dashed border-zinc-200 bg-white/10 rounded-sm">
              ...
              <div className="grid gap-6">
                {pendingDocuments.map((doc: any) => (
                  <div
                    key={doc._id}
                    className="group flex flex-col p-10 border border-zinc-100 bg-white space-y-10 rounded-sm"
                  >
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-10">
                      <div className="flex items-center gap-8 min-w-0">
                        ...
                        <div className="min-w-0 space-y-3">
                          <h3 className="font-bold text-2xl text-black truncate tracking-tighter">
                            {doc.title}
                          </h3>
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        <button
                          onClick={() =>
                            window.open(
                              `/documents/viewer/${doc._id}`,
                              "_blank",
                            )
                          }
                          className="h-12 px-8 border border-zinc-100 text-[10px] font-bold uppercase tracking-widest rounded-sm flex items-center gap-2"
                        >
                    ...
                    <div className="bg-white p-8 border-l-[3px] border-zinc-100 text-[13px] text-zinc-500 font-medium leading-relaxed italic rounded-sm">
                      "
                      {doc.description ||
                        "Tác phẩm này chưa được tác giả cung cấp mô tả chi tiết."}
                      "
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-10 border-t border-zinc-50">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest italic">
                          Yêu cầu xác thực nội dung tri thức
                        </span>
                      </div>
                      <div className="flex items-center gap-4 w-full md:w-auto">
                        <button
                          onClick={() => setConfirmModal({ type: "reject", data: doc })}
                          className="flex-1 md:flex-none h-14 px-12 border border-zinc-100 text-[10px] font-bold uppercase tracking-widest text-zinc-300 rounded-sm active:scale-95 transition-transform"
                        >
                          Từ chối
                        </button>
                        <button
                          onClick={() => setConfirmModal({ type: "approve", data: doc })}
                          className="flex-1 md:flex-none h-14 px-16 bg-black text-white text-[10px] font-bold uppercase tracking-[0.2em] active:scale-95 rounded-sm transition-transform"
                        >
                          Phê duyệt
                        </button>
                      </div>
                  ...
                  <tbody className="divide-y divide-zinc-50">
                    {reports.length === 0 ? (
                      <tr>
                        <td
                          colSpan={4}
                          className="px-10 py-48 text-center text-[10px] font-bold text-zinc-200 uppercase tracking-widest italic"
                        >
                        ...
                        <tr key={r._id} className=" group">
                          <td className="px-10 py-10">
                            <div className="flex items-center gap-6">
                              ...
                              <div className="flex flex-col gap-1.5">
                                ...
                                <span className="text-[9px] font-bold text-zinc-200 truncate max-w-[150px] tracking-tight">
                                  {r.item_id}
                                </span>
                              </div>
                            </div>
                          ...
                          <td className="px-10 py-10">
                            <div className="space-y-2 max-w-md">
                              <p className="font-bold text-black text-[11px] leading-tight uppercase tracking-tight">
                                {r.reason}
                              </p>
                              <p className="text-[10px] text-zinc-400 font-medium italic line-clamp-1">
                                "{r.description}"
                              </p>
                            </div>
                          </td>
                          ...
                          <td className="px-10 py-10 text-right">
                            <div className="flex justify-end gap-3">
                              <button
                                onClick={() => setConfirmModal({ type: "report_keep", data: r })}
                                className="h-10 px-6 border border-zinc-100 text-[10px] font-bold uppercase tracking-widest text-zinc-300 rounded-sm active:scale-95 transition-transform"
                              >
                                Bỏ qua
                              </button>
                              <button
                                onClick={() => setConfirmModal({ type: "report_takedown", data: r })}
                                className="h-10 px-8 bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-sm active:scale-95 transition-transform"
                              >
                                Gỡ bỏ
                              </button>
                            </div>
          ...
          <div className="space-y-12 animate-in fade-in ">
            <div className="grid md:grid-cols-3 gap-8">
              <div className="md:col-span-1 space-y-10">
                <div className="space-y-4">
                  <h2 className="text-[11px] font-bold text-black tracking-[0.2em] uppercase px-2">
                    Cấu hình thu thập
                  </h2>
                  <div className="bg-white border border-zinc-100 p-8 space-y-8 rounded-sm">
                    <div className="space-y-4">
                      ...
                      <select
                        ...
                        className="w-full h-12 px-4 bg-white border border-zinc-100 text-[11px] font-bold uppercase tracking-widest outline-none focus:border-black rounded-sm"
                      >
                      ...
                      <>
                        ...
                        <div className="space-y-4">
                          ...
                          <input
                            ...
                            placeholder=""
                            className="w-full h-12 px-4 bg-white border border-zinc-100 text-[11px] font-medium outline-none focus:border-black rounded-sm"
                          />
                        </div>
                      </>
                    )}
                    ...
                    <button
                      onClick={() => setConfirmModal({ type: "collection", data: collectionForm })}
                      disabled={isRefreshing}
                      className="w-full h-14 bg-black text-white text-[10px] font-bold uppercase tracking-[0.2em] active:scale-95 flex items-center justify-center gap-3 rounded-sm disabled:opacity-50"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="bg-white border border-zinc-100 p-8 rounded-sm space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-xl font-bold tracking-tight text-black uppercase">
                          Đang hoạt động
                        </p>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="flex items-center justify-between p-8 border border-zinc-100 bg-white rounded-sm "
                      >
                        <div className="flex items-center gap-6">
                          <div className="w-12 h-12 bg-white flex items-center justify-center border border-zinc-100 rounded-sm">
                          ...
                          <div className="space-y-1">
                            <h4 className="font-bold text-black uppercase tracking-tight text-sm">
                              {source.name}
                            </h4>
            ...
            {activityLogs.length === 0 ? (
              ...
              <div className="grid gap-4">
                {activityLogs.map((log: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-8 border border-zinc-100 bg-white rounded-sm group"
                  >
                    <div className="flex items-center gap-8">
                      <div className="w-12 h-12 border border-zinc-50 flex items-center justify-center font-bold text-[13px] text-zinc-100 ">
                        {idx + 1}
                      </div>
                      <div className="space-y-1.5">
                        <p className="text-sm font-bold text-black tracking-tight uppercase transition-transform ">
      ...
      >
        <ModalHeader>
          <ModalTitle>
            {confirmModal?.type === "approve" ? "Phê duyệt nội dung" : 
             confirmModal?.type === "reject" ? "Từ chối nội dung" :
             confirmModal?.type === "report_keep" ? "Giữ lại nội dung" :
             confirmModal?.type === "report_takedown" ? "Gỡ bỏ vi phạm" :
             "Kích hoạt thu thập"}
          </ModalTitle>
        </ModalHeader>
        <ModalContent>
          <p className="text-sm font-bold text-zinc-400 uppercase tracking-widest leading-relaxed">
            {confirmModal?.type === "approve" ? `Bạn có chắc chắn muốn phê duyệt tác phẩm "${confirmModal.data?.title}" để phát hành công khai?` :
             confirmModal?.type === "reject" ? `Bạn có chắc chắn muốn từ chối phát hành tác phẩm "${confirmModal.data?.title}"?` :
             confirmModal?.type === "report_keep" ? "Bạn quyết định giữ lại nội dung này và đánh dấu báo cáo là không hợp lệ?" :
             confirmModal?.type === "report_takedown" ? "Hành động này sẽ gỡ bỏ nội dung bị báo cáo khỏi hệ thống ngay lập tức." :
        ...
        <ModalFooter className="flex gap-4">
          ...
          <button
            onClick={() => {
              if (confirmModal?.type === "approve") reviewDocument(confirmModal.data._id, "PUBLISHED");
              else if (confirmModal?.type === "reject") reviewDocument(confirmModal.data._id, "REJECTED");
              else if (confirmModal?.type === "report_keep") resolveReport(confirmModal.data._id, "KEEP");
              else if (confirmModal?.type === "report_takedown") resolveReport(confirmModal.data._id, "TAKEDOWN");
              else if (confirmModal?.type === "collection") handleTriggerCollection();
            }}
            disabled={isProcessing}
          ...
          >
            {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : "Xác nhận thực hiện"}
          </button>
        </ModalFooter>
      </Modal>
    </div>
