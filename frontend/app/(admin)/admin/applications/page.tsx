...
export default function AuthorApplicationsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setNotification({ type: "error", text: "Không thể tải danh sách đơn ứng tuyển." });
    } finally {
  ...
  const handleReview = async (appId: string, status: string) => {
    try {
      ...
      setNotification({ type: "success", text: "Đã cập nhật trạng thái hồ sơ ứng tuyển." });
      fetchData();
    } catch (err: any) {
      setNotification({ type: "error", text: err.message || "Lỗi xử lý hồ sơ." });
    }
  };
