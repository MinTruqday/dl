...
export default function ReportsManagementPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setNotification({ type: "error", text: "Không thể kết nối máy chủ báo cáo." });
    } finally {
  ...
  const handleResolve = async (reportId: string, action: string) => {
    try {
      ...
      setNotification({ type: "success", text: action === "RESOLVED" ? "Đã xử lý vi phạm thành công." : "Đã bỏ qua báo cáo." });
      fetchData();
    } catch (err: any) {
