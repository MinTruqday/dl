...
export default function UsersManagementPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setNotification({ type: "error", text: "Không thể tải danh sách nhân sự." });
    } finally {
  ...
  const handleUpdateRole = async (userId: string, role: string) => {
    try {
      ...
      setNotification({ type: "success", text: "Cập nhật quyền hạn thành công." });
      fetchData();
    } catch (err: any) {
      setNotification({ type: "error", text: err.message || "Lỗi cập nhật quyền." });
    }
  };
  ...
  const handleUpdateStatus = async (userId: string, isActive: boolean) => {
    try {
      ...
      setNotification({ type: "success", text: isActive ? "Đã kích hoạt tài khoản." : "Đã vô hiệu hóa tài khoản." });
      fetchData();
    } catch (err: any) {
