...
export default function ApprovalPage() {
  ...
  return (
    <div className="flex flex-col h-full font-sans">
        ...
        {loading ? (
          ...
          <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-[#F5F5F7] rounded-[18px] p-12 text-center">
            ...
            <p className="text-[15px] text-[#6E6E73] max-w-sm">
              Tất cả bản thảo đã được kiểm duyệt. Hệ thống hiện không có tác
              phẩm nào đang chờ xử lý.
            </p>
          </div>
        ) : (
          <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 pb-6">
            {pendingDocs.map((doc: any) => (
              <div
                key={doc._id}
                className="bg-[#F5F5F7] border-[#E8E8ED] p-6 rounded-[18px] flex flex-col group hover: transition-all duration-300"
              >
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="space-y-2 flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-1 bg-[#FF9F0A]/10 text-[#FF9F0A] text-[12px] font-medium rounded-md">
                        Chờ duyệt
                      </span>
                    ...
                    <h4 className="font-semibold text-[17px] text-[#1D1D1F] line-clamp-2 leading-relaxed">
                      {doc.title || "Tác phẩm chưa có tiêu đề"}
                    </h4>
                    <p className="text-[13px] text-[#6E6E73] truncate">
                      Tác giả:{" "}
                      <span className="font-medium text-[#1D1D1F]">
                        {doc.author_name || "Ẩn danh"}
                      </span>
                    </p>
                  </div>
                  <button
                    onClick={() =>
                      window.open(`/tai-lieu/viewer/${doc._id}`, "_blank")
                    }
                    className="w-10 h-10 bg-[#F5F5F7] hover:bg-[#0071E3] text-[#6E6E73] hover:text-white rounded-full flex items-center justify-center shrink-0 transition-colors"
                    title="Xem chi tiết"
                  >
                ...
                <div className="bg-[#F5F5F7] p-4 rounded-[10px] mb-6 flex-1">
                  <p className="text-[13px] text-[#1D1D1F] line-clamp-3 leading-relaxed font-medium">
                    {doc.description ||
                      "Không có mô tả chi tiết cho bản thảo này."}
                  </p>
                </div>
                <div className="flex gap-3 mt-auto pt-4 border-[#E8E8ED]">
                  <button
                    onClick={() =>
                      setConfirmModal({ type: "reject", data: doc })
                    }
                    className="flex-1 h-[44px] bg-white border-[#FF3B30]/30 text-[#FF3B30] hover:bg-[#FFEBEB] text-[15px] font-medium rounded-full transition-colors flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 h-[44px] bg-[#0071E3] text-white hover:bg-[#0077ED] text-[15px] font-medium rounded-full transition-colors flex items-center justify-center gap-2"
                  >
      ...
      >
        <ModalHeader
          className={`p-6 ${confirmModal?.type === "reject" ? "bg-[#FF3B30]/10" : "bg-[#34C759]/10"}`}
        >
          <ModalTitle
            className={`text-[17px] font-semibold flex items-center gap-2 ${confirmModal?.type === "reject" ? "text-[#FF3B30]" : "text-[#34C759]"}`}
          >
          ...
          <ModalDescription
            className={`text-[13px] mt-2 ml-7 ${confirmModal?.type === "reject" ? "text-[#FF3B30]" : "text-[#34C759]"}`}
          >
            Kiểm duyệt bản thảo
          </ModalDescription>
        </ModalHeader>
        <ModalContent className="p-6">
          <div className="bg-[#F5F5F7] p-4 rounded-[10px] mb-4">
            <h4 className="text-[15px] font-semibold text-[#1D1D1F] mb-1 line-clamp-1">
              {confirmModal?.data?.title}
            </h4>
            <p className="text-[13px] text-[#6E6E73]">
              Tác giả: {confirmModal?.data?.author_name}
            </p>
          </div>
        ...
        <ModalFooter className="flex gap-3 border-[#E8E8ED] p-6 bg-[#F5F5F7]">
          ...
          <button
            ...
            className={`flex-1 h-[44px] text-white text-[15px] font-medium rounded-full flex items-center justify-center disabled:opacity-50 transition-colors gap-2 ${confirmModal?.type === "approve" ? "bg-[#0071E3] hover:bg-[#0077ED]" : "bg-[#FF3B30] hover:bg-[#E0332A]"}`}
          >
            {isProcessing && <Loader2 className="w-5 h-5 animate-spin" />} Xác
            nhận
          </button>
        </ModalFooter>
      </Modal>
