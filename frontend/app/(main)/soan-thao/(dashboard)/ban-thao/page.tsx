        ...
        {loading ? (
          ...
          <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-[#F5F5F7] rounded-[18px] p-12 text-center">
            ...
            <p className="text-[15px] text-[#6E6E73] max-w-sm mb-6">
              Bạn chưa có tác phẩm nào đang trong quá trình soạn thảo. Bắt đầu
              sáng tác ngay.
            </p>
            <button
              onClick={() => router.push("/cung-cap")}
              className="h-[44px] px-6 bg-[#0071E3] text-white text-[15px] font-medium rounded-full hover:bg-[#0077ED] transition-colors flex items-center gap-2"
            >
          ...
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 pb-6">
            {drafts.map((draft: any) => (
              <button
                ...
                onClick={() =>
                  router.push(`/soan-thao?tai-lieu=${draft._id || draft.id}`)
                }
                className="group flex flex-col bg-[#F5F5F7] border-[#E8E8ED] rounded-[18px] hover: transition-all duration-300 overflow-hidden text-left hover:-translate-y-1"
              >
                ...
                <div className="p-5 flex flex-col flex-1 gap-4 w-full bg-white">
                  <p className="text-[13px] font-medium text-[#6E6E73] mb-4 line-clamp-2 leading-relaxed">
