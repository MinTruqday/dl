...
export default function StatsPage() {
 ...
 <div key={i} className="bg-[#F5F5F7] border-[#E8E8ED] p-6 flex flex-col justify-between h-[140px] rounded-[18px] relative overflow-hidden">
 <div className="flex justify-between items-start relative z-10">
 <span className="text-[13px] font-medium text-[#6E6E73]">{s.label}</span>
 <div className={`w-10 h-10 rounded-[10px] ${s.bg} flex items-center justify-center`}><s.icon className={`w-5 h-5 ${s.color}`} /></div>
 </div>
 ...
 <p className="text-[13px] font-medium text-[#6E6E73] mb-4">Hiệu suất tác phẩm</p>
 </div>
 <div className="flex-1 overflow-auto custom-scrollbar">
 {(stats?.documents || []).length === 0 ? (
 <div className="h-full flex flex-col items-center justify-center p-12 text-center">
 <div className="w-16 h-16 bg-[#F5F5F7] flex items-center justify-center rounded-[18px] mb-4"><BookOpen className="w-8 h-8 text-[#C7C7CC]" /></div>
 <p className="text-[13px] font-medium text-[#6E6E73] mb-4 mb-2">Chưa có dữ liệu</p>
 ...
 <div onClick={(e) => { e.stopPropagation(); setPricingDocId(doc.id); setPricingDocTitle(doc.title); setNewPrice(doc.price_dl || 0); setShowPricingModal(true); }} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-[#E8E8ED] text-[#6E6E73] transition-colors" title="Thiết lập giá"><Banknote className="w-5 h-5" /></div>
 ...
 <Modal isOpen={showAnalyticsModal} onClose={() => setShowAnalyticsModal(false)} className="max-w-3xl rounded-[18px] border-[#E8E8ED] bg-[#F5F5F7] p-0 overflow-hidden">
 ...
 <div className="flex items-center gap-2 pb-3"><Eye className="w-5 h-5 text-[#1D1D1F]" /><p className="text-[13px] font-medium text-[#6E6E73] mb-4">Tương tác độc giả</p></div>
 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
 <div className="p-5 bg-[#F5F5F7] rounded-[18px] flex flex-col justify-between h-[120px]"><Eye className="w-5 h-5 text-[#0071E3] mb-2" /><div><p className="text-[12px] font-medium text-[#6E6E73] mb-1">Lượt xem</p><p className="text-[24px] font-semibold text-[#1D1D1F]">{(selectedAnalytics?.views || 0).toLocaleString()}</p></div></div>
 <div className="p-5 bg-[#F5F5F7] rounded-[18px] flex flex-col justify-between h-[120px]"><Clock className="w-5 h-5 text-[#AF52DE] mb-2" /><div><p className="text-[12px] font-medium text-[#6E6E73] mb-1">Đọc TB</p><p className="text-[24px] font-semibold text-[#1D1D1F]">{selectedAnalytics?.avg_read_time || "0 phút"}</p></div></div>
 <div className="p-5 bg-[#F5F5F7] rounded-[18px] flex flex-col justify-between h-[120px]"><Bookmark className="w-5 h-5 text-[#34C759] mb-2" /><div><p className="text-[12px] font-medium text-[#6E6E73] mb-1">Lượt lưu</p><p className="text-[24px] font-semibold text-[#1D1D1F]">{selectedAnalytics?.saves || 0}</p></div></div>
 <div className="p-5 bg-[#F5F5F7] rounded-[18px] flex flex-col justify-between h-[120px]"><MessageSquare className="w-5 h-5 text-[#FF9F0A] mb-2" /><div><p className="text-[12px] font-medium text-[#6E6E73] mb-1">Bình luận</p><p className="text-[24px] font-semibold text-[#1D1D1F]">{selectedAnalytics?.comments || 0}</p></div></div>
 </div>
 </div>
 <div className="space-y-4">
 <div className="flex items-center gap-2 pb-3"><BookOpen className="w-5 h-5 text-[#1D1D1F]" /><p className="text-[13px] font-medium text-[#6E6E73] mb-4">Chỉ số học thuật</p></div>
 <div className="grid grid-cols-2 gap-4">
 <div className="p-6 bg-[#F5F5F7] rounded-[18px] flex items-center justify-between"><div><p className="text-[12px] font-medium text-[#6E6E73] mb-1">Tổng số từ</p><p className="text-[24px] font-semibold text-[#1D1D1F]">{(selectedAcademic?.word_count || 0).toLocaleString()}</p></div><div className="w-12 h-12 bg-white rounded-full flex items-center justify-center"><FileText className="w-5 h-5 text-[#1D1D1F]" /></div></div>
 <div className="p-6 bg-[#F5F5F7] rounded-[18px] flex items-center justify-between"><div><p className="text-[12px] font-medium text-[#6E6E73] mb-1">Độ đọc hiểu</p><p className="text-[24px] font-semibold text-[#1D1D1F] flex items-baseline gap-1">{selectedAcademic?.readability_score || 0}<span className="text-[15px] text-[#6E6E73]">/100</span></p></div><div className="w-12 h-12 bg-white rounded-full flex items-center justify-center"><Percent className="w-5 h-5 text-[#1D1D1F]" /></div></div>
 </div>
 </div>
 </div>
 )}
 ...
 <ModalFooter className="border-[#E8E8ED] p-6 bg-[#F5F5F7] flex justify-end"><button onClick={() => setShowAnalyticsModal(false)} className="h-[44px] px-8 bg-[#0071E3] text-white text-[15px] font-medium rounded-full hover:bg-[#0077ED] transition-colors">Đóng báo cáo</button></ModalFooter>
 </Modal>

 <Modal isOpen={showWithdrawalModal} onClose={() => !requestingWithdrawal && setShowWithdrawalModal(false)} className="max-w-md rounded-[18px] border-[#E8E8ED] bg-[#F5F5F7] p-0 overflow-hidden">
 ...
 <div className="bg-[#34C759]/10 p-4 rounded-[10px] flex items-center justify-between border-[#34C759]/20"><span className="text-[13px] font-medium text-[#1D1D1F]">Số dư khả dụng:</span><span className="text-[17px] font-semibold text-[#34C759]">{revenue?.available_balance || 0} dl</span></div>
 <div className="space-y-4">
 <div className="space-y-2"><label className="text-[13px] font-medium text-[#1D1D1F]">Số tiền cần rút (dl)</label><div className="relative"><Banknote className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#6E6E73]" /><input type="number" value={withdrawalAmount || ""} onChange={(e) => setWithdrawalAmount(parseInt(e.target.value) || 0)} placeholder="" className="w-full h-[48px] pl-12 pr-4 text-[15px] text-[#1D1D1F] rounded-[10px] outline-none focus:border-[#0071E3] bg-[#F5F5F7] focus:bg-white transition-colors" /></div></div>
 <div className="space-y-2"><label className="text-[13px] font-medium text-[#1D1D1F]">Tên ngân hàng</label><input value={bankInfo.bank_name} onChange={(e) => setBankInfo({ ...bankInfo, bank_name: e.target.value })} placeholder="" className="w-full h-[48px] px-4 text-[15px] text-[#1D1D1F] rounded-[10px] outline-none focus:border-[#0071E3] bg-[#F5F5F7] focus:bg-white transition-colors" /></div>
 <div className="space-y-2"><label className="text-[13px] font-medium text-[#1D1D1F]">Số tài khoản</label><input value={bankInfo.account_number} onChange={(e) => setBankInfo({ ...bankInfo, account_number: e.target.value })} placeholder="" className="w-full h-[48px] px-4 text-[15px] text-[#1D1D1F] rounded-[10px] outline-none focus:border-[#0071E3] bg-[#F5F5F7] focus:bg-white transition-colors" /></div>
 <div className="space-y-2"><label className="text-[13px] font-medium text-[#1D1D1F]">Tên chủ tài khoản</label><input value={bankInfo.account_name} onChange={(e) => setBankInfo({ ...bankInfo, account_name: e.target.value })} placeholder="" className="w-full h-[48px] px-4 text-[15px] text-[#1D1D1F] rounded-[10px] outline-none focus:border-[#0071E3] bg-[#F5F5F7] focus:bg-white transition-colors uppercase" /></div>
 </div>
 </div>
 </ModalContent>
 <ModalFooter className="flex gap-3 border-[#E8E8ED] p-6 bg-[#F5F5F7]"><button onClick={() => setShowWithdrawalModal(false)} disabled={requestingWithdrawal} className="flex-1 h-[44px] bg-white text-[15px] font-medium text-[#1D1D1F] rounded-full hover:bg-[#F5F5F7] transition-colors disabled:opacity-50">Hủy bỏ</button><button onClick={handleWithdrawal} disabled={requestingWithdrawal || withdrawalAmount <= 0} className="flex-1 h-[44px] bg-[#34C759] text-white text-[15px] font-medium rounded-full flex items-center justify-center hover:bg-[#2EB150] transition-colors disabled:opacity-50 gap-2">{requestingWithdrawal ? <Loader2 className="w-5 h-5 animate-spin" /> : "Gửi yêu cầu"}</button></ModalFooter>
 </Modal>

 <Modal isOpen={showPricingModal} onClose={() => setShowPricingModal(false)} className="max-w-md rounded-[18px] border-[#E8E8ED] bg-[#F5F5F7] p-6">
 ...
 <div className="relative"><input type="number" min="0" value={newPrice} onChange={(e) => setNewPrice(Number(e.target.value))} className="w-full h-[52px] pl-4 pr-12 rounded-[10px] text-[15px] font-medium bg-[#F5F5F7] focus:bg-white focus:border-[#0071E3] outline-none transition-all" placeholder="" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-[15px] font-medium text-[#6E6E73]">dl</span></div>
 <p className="text-[13px] text-[#6E6E73] mt-2">Lưu ý: Nhập 0 để phát hành miễn phí.</p>
 </div>
 </ModalContent>
 <ModalFooter className="flex gap-3 justify-end"><button onClick={() => setShowPricingModal(false)} className="px-6 py-3 rounded-full text-[15px] font-medium text-[#1D1D1F] hover:bg-[#F5F5F7] transition-colors">Hủy</button><button onClick={handleSetPricing} disabled={settingPrice} className="px-6 py-3 rounded-full bg-[#0071E3] text-white text-[15px] font-medium disabled:opacity-50 flex items-center gap-2 hover:bg-[#0077ED] transition-colors">{settingPrice ? <Loader2 className="w-5 h-5 animate-spin" /> : <Banknote className="w-5 h-5" />} Xác nhận</button></ModalFooter>
 </Modal>
 </div>
 );
}
