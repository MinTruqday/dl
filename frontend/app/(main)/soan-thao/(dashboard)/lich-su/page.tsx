...
function renderLineDiff(textA: string, textB: string) {
 ...
 <div className="w-10 shrink-0 bg-[#F5F5F7] border-[#E8E8ED] flex items-center justify-center text-[12px] text-[#6E6E73]">{idx + 1}</div>
 <div className={`flex-1 p-3 border-[#E8E8ED] whitespace-pre-wrap break-all ${row.type === "diff" && row.a ? "bg-[#FF3B30]/10 text-[#FF3B30] -2 -[#FF3B30] font-medium" : "text-[#1D1D1F]"}`}>
 {row.type === "diff" && row.a ? <span className="text-[#FF3B30] select-none mr-2">-</span> : null}{row.a}
 </div>
 <div className={`flex-1 p-3 whitespace-pre-wrap break-all ${row.type === "diff" && row.b ? "bg-[#34C759]/10 text-[#34C759] -2 -[#34C759] font-medium" : "text-[#1D1D1F]"}`}>
...
export default function HistoryPage() {
 ...
 <button onClick={handleCompareVersions} disabled={selectedVersions.length !== 2 || isComparing} className={`h-[44px] px-6 text-[15px] font-medium rounded-full flex items-center justify-center gap-2 transition-colors ${selectedVersions.length === 2 ? "bg-[#0071E3] text-white hover:bg-[#0077ED]" : "bg-[#F5F5F7] text-[#C7C7CC] cursor-not-allowed "}`}>
 ...
 <button onClick={(e) => { e.stopPropagation(); setConfirmRestore(v.id); }} className="h-[44px] px-6 bg-white text-[13px] font-medium text-[#1D1D1F] rounded-full hover:bg-[#F5F5F7] transition-all flex items-center justify-center gap-2 sm:opacity-0 sm:group-hover:opacity-100">
 ...
 <button onClick={() => setConfirmRestore(null)} className="flex-1 h-[44px] bg-white text-[15px] font-medium text-[#1D1D1F] rounded-full transition-colors hover:bg-[#E8E8ED]">Hủy bỏ</button>
