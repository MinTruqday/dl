...
import { User, Camera, LogOut, Loader2, BadgeCheck, ShieldCheck, Crown } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.data?.url) {
        setAvatarUrl(res.data.url.startsWith("http") ? res.data.url : `${API_URL}/storage/${res.data.url}`);
  ...
  return (
    <div className="w-full max-w-[1000px] mx-auto px-6 py-8 min-h-[calc(100dvh-44px)] flex flex-col font-sans text-[#1D1D1F]">
        ...
        <aside className="md:col-span-4 xl:col-span-4 space-y-6">
          <div className="bg-[#F5F5F7] rounded-[24px] p-8 flex flex-col items-center text-center">
            ...
            <div className="w-full mb-8">
              ...
              <p className="text-[14px] text-[#6E6E73]">@{user.slug || "nguoidung"}</p>
              
              <div className="mt-4 flex justify-center">
                <span className={`px-3 py-1 text-[13px] font-medium rounded-full ${
                  user.role === "admin" ? "bg-[#E8E8ED] text-[#1D1D1F]" :
                  user.role === "author" ? "bg-[#EBF4FF] text-[#0071E3]" :
                  user.role === "potential_author" ? "bg-[#FFF4E5] text-[#FF9500]" :
            ...
            <button onClick={logoutState} className="w-full py-3 bg-white text-[#FF3B30] text-[15px] font-medium flex items-center justify-center gap-2 rounded-full transition-colors hover:bg-[#F5F5F7]">
        ...
        <main className="md:col-span-8 xl:col-span-8 space-y-6">
          <div className="bg-[#F5F5F7] rounded-[24px] p-8 space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[13px] font-medium text-[#6E6E73] ml-1 block">Tên hiển thị</label>
                <input
                  value={fullName}
            ...
            <div className="space-y-2">
              <label className="text-[13px] font-medium text-[#6E6E73] ml-1 block">Tiểu sử</label>
              <textarea
                value={bio}
            ...
            <div className="flex justify-end pt-4">
              <button onClick={handleSave} disabled={isSaving} className="pill-button">
          ...
          {user.role === "reader" && (
            <div className="bg-[#EBF4FF] rounded-[24px] p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <button onClick={handleBecomeAuthor} disabled={isSaving} className="pill-button shrink-0 bg-[#0071E3] hover:bg-[#0055C6]">
                {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : "Nâng cấp"}
              </button>
            </div>
          )}
            ...
            <div className="bg-[#FFF4E5] rounded-[24px] p-8 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button onClick={handleApplyAuthor} disabled={isApplying} className="pill-button bg-[#FF9500] hover:bg-[#CC7700] shrink-0">
                  {isApplying ? <Loader2 className="w-5 h-5 animate-spin" /> : "Gửi yêu cầu"}
                </button>
              </div>
            </div>
          )}
