...
export default function StoragePage() {
  ...
  const fetchQuota = async () => {
    try {
      ...
      showToast(e.message || "Lỗi tải thông tin dung lượng", "error");
    }
  };
  ...
  const fetchMoveFolders = async (folderId?: string) => {
    try {
      ...
      setMoveFolders(
        data.filter((item) => item.is_folder && item._id !== moveItem?._id),
      );
    } catch (error: any) {
  ...
  const handleDelete = async (item: StorageItem) => {
    if (viewMode === "trash") {
      if (!window.confirm(`Bạn có chắc muốn xóa vĩnh viễn ${item.name}?`))
        return;
      try {
      ...
      if (!window.confirm(`Bạn có chắc muốn đưa ${item.name} vào thùng rác?`))
        return;
      try {
  ...
  const handleMove = async () => {
    ...
    try {
      ...
      await updateStorageItemAPI(moveItem._id, {
        parent_id: payloadParentId as any,
      });
    ...
    try {
      if (useAISearch) {
        ...
        if (res.ok && data.data && Array.isArray(data.data)) {
          ...
          const normalData = await searchStorageItemsAPI(
            searchQuery.trim(),
        ...
        const data = await searchStorageItemsAPI(
          searchQuery.trim(),
          searchType || undefined,
        );
        setItems(data);
  ...
  return (
    <div className="w-full max-w-[1280px] mx-auto px-6 py-6 h-[calc(100dvh-var(--navbar-height))] flex flex-col gap-6 font-sans text-black selection:bg-black selection:text-white">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-0">
        <main
          className={`flex flex-col gap-6 h-full min-h-0 overflow-y-auto custom-scrollbar pb-6 pr-2 animate-in fade-in slide-in-from-bottom-8 duration-300 ${detailsItem ? "lg:col-span-8" : "lg:col-span-12"}`}
          style={{ animationFillMode: "both" }}
        >
          <div
            className="border border-zinc-200 bg-white rounded-2xl shadow-sm p-5 space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-300"
          ...
          <div
            className="border border-zinc-200 bg-white rounded-2xl shadow-sm p-5 flex flex-col flex-1 min-h-0 animate-in fade-in slide-in-from-bottom-8 duration-300"
            style={{ animationDelay: "150ms", animationFillMode: "both" }}
          >
            {viewMode === "files" && (
              <div className="mb-4 flex gap-2 items-center flex-wrap">
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="px-4 h-9 bg-black text-white text-sm font-medium border border-black rounded-xl hover:bg-zinc-800 transition-colors"
                  >
                    Tìm
                  </button>
                </form>
                    ...
                    <tbody>
                      {items.length === 0 ? (
                        ...
                        items.map((item) => (
                          <tr
                            key={item._id}
                            className="border-b border-zinc-200 last:border-0 hover:bg-zinc-50 cursor-pointer transition-colors"
                                  ...
                                  {item.tags && item.tags.length > 0 && (
                                    <div className="flex gap-1 flex-wrap">
                                      {item.tags.map((t) => (
                                        <span
                                          key={t}
                                          className="text-[9px] bg-zinc-100 px-1.5 py-0.5 rounded border border-zinc-200 text-zinc-500 font-mono"
                                        >
                                          {t}
                                        </span>
                                      ))}
                              ...
                              <div className="flex justify-end gap-2">
                                {viewMode === "trash" ? (
                                  ...
                                  <>
                                    ...
                                    >
                                      <Share2
                                        className={`w-4 h-4 ${item.is_public ? "text-black" : ""}`}
                                      />
                                    </button>
                                    {!item.is_folder && (
                                      <button
                    ...
                    >
                      ...
                      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                        <button
                          ...
                          className="p-1.5 bg-white border border-zinc-200 rounded-lg hover:bg-zinc-100 transition-colors"
                        >
                        ...
                        >
                          <Star
                            className={`w-3.5 h-3.5 ${item.is_starred ? "text-black fill-black" : "text-zinc-600"}`}
                          />
                        </button>
                        <button
                          onClick={(e) => {
                      ...
                      <div className="w-16 h-16 bg-zinc-50 flex flex-col items-center justify-center mb-4 border border-zinc-200 rounded-xl relative">
                        {item.color && (
                          <div
                            className="absolute top-0 right-0 w-3 h-3 rounded-full translate-x-1/3 -translate-y-1/3 border-2 border-white"
                            style={{ backgroundColor: item.color }}
                          />
                        )}
                        {item.is_shortcut ? (
                      ...
                      <div className="w-full">
                        {item.is_folder ? (
                          <button
                            ...
                            className="text-sm font-semibold block w-full truncate hover:text-zinc-600 transition-colors"
                          >
                            {item.name}
                          </button>
                        ) : item.url ? (
                          ...
                          <span className="text-sm font-semibold block w-full truncate">
                            {item.name}
                          </span>
                        )}
          ...
          >
            ...
            <ModalFooter>
              <button
                ...
                className="px-4 py-2 text-sm border border-zinc-200 bg-white"
              >
                Hủy
              </button>
              <button
                onClick={handleCreateFolder}
                className="px-4 py-2 text-sm border border-black bg-black text-white"
              >
                Tạo
              </button>
            </ModalFooter>
              ...
              {activeSidebarTab === "info" ? (
                <div className="flex flex-col flex-1 overflow-y-auto custom-scrollbar">
                  <div className="p-6 flex flex-col items-center justify-center border-b border-zinc-100 bg-white">
                    ...
                    <h3 className="font-semibold text-center break-all">
                      {detailsItem.name}
                    </h3>
                  ...
                  <div className="p-6 flex flex-col gap-4 text-sm">
                    <div className="flex justify-between items-center bg-zinc-50 p-2 border border-zinc-200">
                      ...
                      {detailsItem.ai_processed ? (
                        <span className="text-[10px] bg-black text-white px-2 py-1 uppercase font-semibold">
                          Đã xử lý
                        </span>
                      ) : (
                        <span className="text-[10px] bg-zinc-200 text-zinc-600 px-2 py-1 uppercase font-semibold">
                          Đang chờ
                        </span>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <span>
                        {new Date(detailsItem.created_at).toLocaleString(
                          "vi-VN",
                        )}
                      </span>
                    </div>
                    <div>
                      ...
                      <span>
                        {new Date(detailsItem.updated_at).toLocaleString(
                          "vi-VN",
                        )}
                      </span>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {detailsItem.tags.map((t) => (
                            <span
                              key={t}
                              className={`text-[10px] px-2 py-1 font-mono border ${t === "VIOLATION_FLAGGED" ? "bg-black border-black text-white font-bold" : "bg-zinc-100 border-zinc-300 text-zinc-600"}`}
                            >
                              {t}
                            </span>
                          ))}
                    ...
                    {detailsItem.entities &&
                      Object.keys(detailsItem.entities).length > 0 && (
                        <div className="border border-zinc-200 bg-zinc-50 p-3 mt-2">
                          <span className="block text-xs font-semibold text-black mb-2 uppercase">
                            Thực thể (AI trích xuất)
                          </span>
                          <div className="flex flex-col gap-2">
                            {detailsItem.entities.people &&
                              detailsItem.entities.people.length > 0 && (
                                <div>
                                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">
                                    Nhân vật / Người:{" "}
                                  </span>
                                  <span className="text-xs font-mono">
                                    {detailsItem.entities.people.join(", ")}
                                  </span>
                                </div>
                              )}
                            {detailsItem.entities.organizations &&
                              detailsItem.entities.organizations.length > 0 && (
                                <div>
                                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">
                                    Tổ chức:{" "}
                                  </span>
                                  <span className="text-xs font-mono">
                                    {detailsItem.entities.organizations.join(
                                      ", ",
                                    )}
                                  </span>
                                </div>
                              )}
                            {detailsItem.entities.dates &&
                              detailsItem.entities.dates.length > 0 && (
                                <div>
                                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">
                                    Ngày tháng:{" "}
                                  </span>
                                  <span className="text-xs font-mono">
                                    {detailsItem.entities.dates.join(", ")}
                                  </span>
                                </div>
                              )}
                            {detailsItem.entities.amounts &&
                              detailsItem.entities.amounts.length > 0 && (
                                <div>
                                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">
                                    Số tiền:{" "}
                                  </span>
                                  <span className="text-xs font-mono">
                                    {detailsItem.entities.amounts.join(", ")}
                                  </span>
                                </div>
                              )}
                          </div>
                    ...
                    {detailsItem.broken_links &&
                      detailsItem.broken_links.length > 0 && (
                        <div className="border border-black bg-zinc-100 p-3 mt-2">
                          <span className="block text-xs font-bold text-black mb-2 uppercase">
                            Cảnh báo liên kết hỏng
                          </span>
                    ...
                    {detailsItem.versions &&
                      detailsItem.versions.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-zinc-200">
                          <span className="block text-xs font-semibold text-zinc-500 mb-3">
                            Lịch sử phiên bản
                          </span>
                    ...
                    {relatedItems.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-zinc-200">
                        <span className="block text-xs font-semibold text-zinc-500 mb-3">
                          Tài liệu liên quan (AI)
                        </span>
                        <div className="flex flex-col gap-2">
                          {relatedItems.map((item) => (
                            <div
                              key={item._id}
                              className="p-2 border border-zinc-200 bg-zinc-50 hover:bg-zinc-100 cursor-pointer flex items-center gap-2"
                              onClick={() => setDetailsItem(item)}
                            >
                ...
                <div className="flex flex-col flex-1 bg-white overflow-hidden">
                  <div className="flex-1 p-5 overflow-y-auto custom-scrollbar flex flex-col gap-4 text-sm font-sans bg-zinc-50/30">
                    {chatHistory.length === 0 ? (
                      ...
                      chatHistory.map((msg, i) => (
                        ...
                        >
                          <div
                            className={`p-3 max-w-[85%] rounded-2xl ${msg.role === "user" ? "bg-black text-white rounded-tr-sm" : "bg-white border border-zinc-200 text-black rounded-tl-sm shadow-sm"}`}
                          >
                            {msg.content}
                          </div>
                        </div>
                  ...
                  <div className="p-4 border-t border-zinc-100 bg-white">
                    <form
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (res.ok) {
                            ...
                            setChatHistory((prev) => [
                              ...prev,
                              {
                                role: "bot",
                                content:
                                  "Xin lỗi, hiện tại không thể kết nối tới máy chủ AI.",
                              },
                            ]);
                          }
                        } catch (e) {
                          setChatHistory((prev) => [
                            ...prev,
                            { role: "bot", content: "Đã xảy ra lỗi hệ thống." },
                          ]);
                        }
                      }}
                    ...
                    >
                      ...
                      <button
                        type="submit"
                        className="bg-black text-white px-4 py-2 text-sm font-medium border border-black rounded-xl hover:bg-zinc-800 transition-colors"
                      >
                        Gửi
                      </button>
                    </form>
                        ...
                        onClick={async () => {
                          setChatHistory((prev) => [
                            ...prev,
                            {
                              role: "user",
                              content:
                                "Hãy tạo một bản dịch tài liệu này sang tiếng Việt.",
                            },
                          ]);
                          try {
                            ...
                            setChatHistory((prev) => [
                              ...prev,
                              {
                                role: "bot",
                                content:
                                  "Tuyệt vời, tôi đã tạo xong một bản dịch tiếng Việt cho tài liệu này và lưu vào cùng thư mục.",
                              },
                            ]);
                            fetchItems(currentFolderId);
                          } catch (err: any) {
                            setChatHistory((prev) => [
                              ...prev,
                              {
                                role: "bot",
                                content:
                                  "Xin lỗi, không thể dịch tài liệu lúc này.",
                              },
                            ]);
                          }
                        }}
