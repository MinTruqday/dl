...
export default function StudioCollabPage() {
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto px-6 py-6 h-[calc(100dvh-44px)] font-sans text-[#1D1D1F] flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-semibold tracking-tight text-[#1D1D1F]">Cộng tác không gian</h1>
          <p className="text-[15px] text-[#6E6E73] mt-1">Quản lý dự án, cộng tác viên, và theo dõi tiến độ công việc.</p>
        </div>
        ...
        <aside className="lg:col-span-4 flex flex-col space-y-6 overflow-y-auto no-scrollbar pb-6 pr-2">
            ...
            {selectedDocumentId && isOwnerOfSelected() && (
              <div className="space-y-3 pt-4 border-t border-[#E8E8ED]">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={() => handleUpdateAccessLevel("invite_only")} className={`flex items-center gap-2 px-4 py-3 text-[14px] font-medium rounded-[14px] transition-colors ${accessLevel === "invite_only" ? "bg-[#0071E3] text-white" : "bg-white text-[#1D1D1F] hover:bg-[#E8E8ED]"}`}><Lock className="w-4 h-4" /> Chỉ người được mời</button>
                  <button onClick={() => handleUpdateAccessLevel("anyone_with_link")} className={`flex items-center gap-2 px-4 py-3 text-[14px] font-medium rounded-[14px] transition-colors ${accessLevel === "anyone_with_link" ? "bg-[#0071E3] text-white" : "bg-white text-[#1D1D1F] hover:bg-[#E8E8ED]"}`}><Globe className="w-4 h-4" /> Có link tham gia</button>
                </div>
              </div>
            )}
          </div>
          ...
          {selectedDocumentId && isOwnerOfSelected() && (
            <div className="bg-[#F5F5F7] rounded-[24px] p-6 space-y-4 shadow-sm">
              ...
              ) : <button onClick={handleGenerateCode} className="w-full py-3 bg-white text-[14px] font-medium rounded-[14px] border border-[#E8E8ED]">Tạo mã mời</button>}
            </div>
          )}
                    ...
                    return (
                      <div key={c.collaboration_id} className="bg-white p-4 rounded-[16px] shadow-sm">
                        <div className="flex justify-between items-start gap-2">
                          ...
                          {isOwnerOfSelected() ? (
                            <select value={c.role} onChange={(e) => handleUpdateRole(c.collaboration_id, e.target.value)} className="bg-[#F5F5F7] text-[12px] px-2 py-1 rounded-[8px] outline-none border border-[#E8E8ED]">
      ...
      <Modal isOpen={!!transferId} onClose={() => setTransferId(null)} className="max-w-md bg-[#F5F5F7] rounded-[24px] p-0 shadow-2xl border-none">
