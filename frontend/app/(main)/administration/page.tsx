...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [uData, rData, aData, hData] = await Promise.all([
        getAdminUsersAPI(),
        getAdminReportsAPI(),
      ...
      if (uData) setUsers(uData.data || uData);
      if (rData) setReports(rData.data || rData);
      if (aData) setApplications(aData.data || aData);
      if (hData) setHealth(hData.data || hData);

      if (user?.role === "admin") {
        const [cData, mData, auditData, collStats, pData, bData] = await Promise.all([
          getAdminConfigAPI(),
          getMaintenanceModeAPI(),
          getAuditLogsAPI(undefined),
          getCollectorStatsAPI(),
          getPayoutsAPI(),
          getBannersAPI()
        ]);
        
        if (cData) setConfig(cData.data || cData);
        if (mData) setMaintenanceMode(mData.data?.enabled || mData.enabled || false);
        if (auditData) setAuditLogs(auditData.data || auditData);
        if (collStats) setCollectorStats(collStats.data || collStats);
        if (pData) setPayouts(pData.data || pData);
        if (bData) setBanners(bData.data || bData);
      }
    } catch (err: any) {
  ...
  const toggleMaintenance = async () => {
    try {
      await toggleMaintenanceModeAPI(!maintenanceMode, "Hệ thống đang bảo trì định kỳ");
      setMaintenanceMode(!maintenanceMode);
      showToast(maintenanceMode ? "Đã tắt chế độ bảo trì." : "Hệ thống đã vào trạng thái bảo trì.", "success");
      fetchData();
    } catch (err: any) {
  ...
  const updateStatus = async (userId: string, isActive: boolean) => {
    try {
      ...
      showToast(isActive ? "Đã khôi phục tài khoản." : "Đã vô hiệu hóa tài khoản.", "success");
      fetchData();
    } catch (err: any) {
  ...
  const reviewApplication = async (appId: string, status: string) => {
    try {
      await reviewAuthorApplicationAPI(appId, status, status === "APPROVED" ? "Đã duyệt" : "Không đủ tiêu chuẩn");
      showToast("Đã xử lý hồ sơ ứng tuyển.", "success");
      fetchData();
    } catch (err: any) {
  ...
  const reviewPayout = async (payoutId: string, status: string) => {
    try {
      await reviewPayoutAPI(payoutId, status);
      showToast("Đã phê duyệt yêu cầu thanh toán.", "success");
      fetchData();
    } catch (err: any) {
  ...
  const updateConfig = async (newConfig: any) => {
    try {
      await updateAdminConfigAPI(newConfig);
  ...
  const triggerCollection = async () => {
    setIsRefreshing(true);
    try {
        await triggerCollectionAPI(collectionSource, collectionUrl, collectionType, "Document");
        showToast("Yêu cầu thu thập dữ liệu đã được gửi thành công.", "success");
        setCollectionUrl("");
        fetchData();
    } catch (err: any) {
        console.error("Lỗi thu thập:", err);
        showToast(err.message || "Lỗi khi kích hoạt thu thập.", "error");
    } finally {
        setIsRefreshing(false);
    }
  };
