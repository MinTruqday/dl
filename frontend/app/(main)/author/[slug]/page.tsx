...
export default function AuthorProfilePage() {
  ...
  return (
    <Workspace>
      ...
      >
        <div className="bg-white border border-zinc-200 rounded-sm p-8 md:p-12 flex flex-col md:flex-row items-center md:items-start gap-12 mb-16 ">
          <div className="w-32 h-32 md:w-40 md:h-40 bg-white border border-zinc-100 overflow-hidden shrink-0 grayscale ">
            {author.avatar_url ? (
              <img
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="inline-flex items-center px-4 py-1.5 bg-white border border-zinc-100 text-zinc-400 text-[10px] font-bold mb-4">
              Hồ sơ tác giả
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-8 text-[11px] font-bold text-zinc-400 mb-10">
              <span className="flex items-center gap-2">
                <Users className="w-4 h-4" /> {author.followers_count || 0}{" "}
                người theo dõi
              </span>
              <span className="flex items-center gap-2">
                <BookOpen className="w-4 h-4" /> {documents.length} tài liệu
                xuất bản
              </span>
            </div>
            <div className="flex items-center justify-center md:justify-start gap-4">
              <Button
                ...
                className={`px-10 h-14 flex items-center gap-3 text-[11px] font-bold active:scale-95 ${isFollowing ? "bg-zinc-100 text-black border border-zinc-200" : "bg-black text-white"}`}
              >
          ...
          <nav className="-mb-px flex gap-12 justify-center md:justify-start">
            {[
              ...
              <button
                ...
                className={`whitespace-nowrap py-6 border-b-2 font-bold text-[11px] active:scale-95 ${activeTab === tab.id ? "border-black text-black" : "border-transparent text-zinc-400 "}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          {activeTab === "works" && (
            <div className="animate-in fade-in slide-in-from-bottom-4 ">
              {documents.length > 0 ? (
                  ...
                  {documents.map((doc) => (
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        ...
                        <h3 className="font-bold text-black text-sm tracking-tight line-clamp-2 underline-offset-4 decoration-1">
                          {doc.title}
                        </h3>
                      </div>
                    </a>
                ...
                <div className="text-center py-32 text-zinc-300 border border-dashed border-zinc-200 bg-white/20">
              ...
              <div className="bg-white border border-zinc-100 p-10 max-w-3xl">
                ...
                <div className="prose prose-zinc max-w-none text-zinc-600 leading-relaxed text-sm font-medium">
                  {author.bio ? (
                    ...
                    <p className="italic text-zinc-300 text-center py-20 font-bold">
