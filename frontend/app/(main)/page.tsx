...
export default function ExplorePage() {
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 py-12 font-sans text-black selection:bg-black selection:text-white">
          ...
          <div className="absolute inset-0 flex items-center px-16 overflow-hidden">
            ...
            <div className="relative z-10 max-w-3xl space-y-6">
              <div className="flex items-center gap-3 text-white/20 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <span className="text-[10px] font-bold tracking-[0.6em] uppercase">
                  Khám phá vũ trụ tri thức
                </span>
              </div>
              <h2 className="text-7xl font-bold text-white tracking-tighter leading-[0.9] animate-in fade-in slide-in-from-bottom-4 duration-300">
                Mở rộng <br />{" "}
                <span className="text-white/20 italic">Giới hạn tri thức</span>
              </h2>
              <p className="text-white/40 text-lg font-medium leading-relaxed max-w-xl animate-in fade-in slide-in-from-bottom-6 duration-300">
                Hàng ngàn tài liệu, nghiên cứu và bài viết chuyên sâu từ cộng
                đồng chuyên gia hàng đầu đang chờ đón bạn.
              </p>
      ...
      >
        ...
        <div className="flex items-center gap-6">
          <div className="flex border border-zinc-100 p-1 bg-zinc-50/50">
            ...
            >
              <LayoutGrid
                className={`w-4 h-4 ${viewMode === "grid" ? "text-black" : "text-zinc-300"}`}
              />
            </button>
            <button
            ...
            >
              <ListIcon
                className={`w-4 h-4 ${viewMode === "list" ? "text-black" : "text-zinc-300"}`}
              />
            </button>
          </div>
        </div>
      ...
      <div
        className="mb-12 transition-all duration-300 delay-200"
        style={{
          opacity: visible ? 1 : 0,
          transform: visible ? "translateY(0)" : "translateY(10px)",
        }}
      >
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          >
            <Zap
              className={`w-4 h-4 ${useSemantic ? "text-yellow-400 fill-yellow-400" : "text-zinc-300"}`}
            />
          ...
          <div className="space-y-6">
            ...
            <nav className="flex flex-col gap-1">
              ...
              >
                Tất cả tài liệu
                <ChevronRight
                  className={`w-3.5 h-3.5 transition-transform ${!selectedCategory ? "rotate-90" : ""}`}
                />
              </button>
              {categories.map((cat) => (
                <button
                ...
                >
                  {cat}
                  <ChevronRight
                    className={`w-3.5 h-3.5 transition-transform ${selectedCategory === cat ? "rotate-90" : ""}`}
                  />
                </button>
              ))}
            </nav>
            ...
            <div className="space-y-3">
              {featuredAuthors.map((author) => (
                <Link
                  ...
                  className="flex items-center gap-4 group p-3 border border-transparent hover:border-zinc-100 transition-all bg-zinc-50/30"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold text-black uppercase tracking-tight truncate">
                      {author.full_name}
                    </p>
                    <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-widest mt-0.5">
                      {author.document_count || 0} Tác phẩm
                    </p>
                  </div>
                </Link>
              ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {recommendations.map((doc, i) => (
                  <Link
                    ...
                    className="flex gap-6 p-6 border border-zinc-100 hover:border-black transition-all group bg-white"
                  >
                    ...
                    <div className="flex-1 space-y-3">
                      <span className="text-[9px] font-bold text-zinc-300 uppercase tracking-[0.3em]">
                        {doc.categories?.[0] || "Tri thức"}
                      </span>
                      <h3 className="text-base font-bold leading-tight group-hover:underline underline-offset-4 decoration-1 uppercase tracking-tight">
                        {doc.title}
                      </h3>
            ...
            {loading ? (
              ...
              >
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className={`bg-zinc-50 border border-zinc-100 animate-pulse ${viewMode === "grid" ? "aspect-[3/4]" : "h-32"}`}
                  />
                ))}
              </div>
            ) : documents.length > 0 ? (
              ...
              >
                {documents.map((document, i) => (
                  ...
                  >
                    <div
                      className={`${viewMode === "grid" ? "aspect-[3/4] w-full" : "w-32 h-44 shrink-0"} bg-zinc-50 border border-zinc-100 relative overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-300`}
                    >
                      {document.cover_url ? (
                        <img
                    ...
                    >
                      ...
                      <h3
                        className={`${viewMode === "grid" ? "text-base" : "text-xl"} font-bold leading-tight group-hover:underline underline-offset-4 decoration-1 text-black tracking-tight uppercase`}
                      >
                        {document.title}
                      </h3>
                      <p className="text-[13px] text-zinc-400 line-clamp-2 leading-relaxed font-medium">
                        {document.description ||
                          "Khám phá nội dung chi tiết của tài liệu này trên DocLib"}
                      </p>
                    </div>
                  </Link>
                ))}
              ...
              <div className="py-48 flex flex-col items-center justify-center border border-dashed border-zinc-100 bg-zinc-50/30">
                ...
                <p className="text-[11px] font-bold text-zinc-300 uppercase tracking-widest text-center max-w-xs leading-loose">
                  Hãy thử thay đổi danh mục hoặc từ khóa để tìm kiếm tri thức
                  khác
                </p>
              </div>
            )}
