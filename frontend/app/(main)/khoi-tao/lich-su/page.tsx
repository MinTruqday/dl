    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {selectedDocumentId ? (
              ...
              versions.map((v) => (
                <div 
                  ...
                  className={`bg-white border p-6 flex items-center justify-between rounded-2xl cursor-pointer transition-all ${
                    selectedVersions.includes(v.id) ? "border-black ring-1 ring-black shadow-sm" : "border-zinc-200 hover:border-zinc-300"
                  }`}
                >
                   <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 flex items-center justify-center rounded-xl border ${selectedVersions.includes(v.id) ? "bg-black text-white border-black" : "bg-zinc-50 text-zinc-500 border-zinc-200"}`}>
                   ...
                   <div className="flex gap-3">
                      <button 
                        onClick={(e) => { e.stopPropagation(); setConfirmRestore(v.id); }}
        ...
        <div className="bg-white border border-zinc-200 p-16 rounded-2xl flex flex-col items-center justify-center gap-4 text-center">
          ...
          <p className="text-sm font-medium text-zinc-500">Vui lòng chọn một tác phẩm để xem lịch sử</p>
        </div>
      )}

      <Modal isOpen={!!confirmRestore} onClose={() => setConfirmRestore(null)} className="max-w-sm">
        <ModalHeader>
          <ModalTitle>Khôi phục phiên bản</ModalTitle>
        </ModalHeader>
        <ModalContent>
          <p className="text-xs font-medium text-zinc-500 leading-relaxed">Bạn có chắc muốn khôi phục về phiên bản này? Nội dung hiện tại sẽ bị ghi đè và bạn sẽ mất các thay đổi mới nhất.</p>
        </ModalContent>
        <ModalFooter>
          <button onClick={() => setConfirmRestore(null)} className="flex-1 py-2 border border-zinc-200 bg-white text-xs font-medium text-black rounded-xl">Hủy</button>
          <button onClick={executeRestore} className="flex-1 py-2 bg-black text-white text-xs font-medium border border-black rounded-xl">Khôi phục</button>
        </ModalFooter>
      </Modal>
      ...
      <Modal isOpen={!!diffData} onClose={() => setDiffData(null)} className="max-w-5xl h-[80vh] flex flex-col">
        ...
        <ModalFooter>
           <button onClick={() => setDiffData(null)} className="px-6 py-2 bg-black text-white text-xs font-medium rounded-xl">Đóng cửa sổ</button>
        </ModalFooter>
      </Modal>
    </div>
  );
