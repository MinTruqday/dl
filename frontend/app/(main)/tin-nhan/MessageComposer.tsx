          ...
          <div className="relative flex items-center gap-1">
            ...
            {menu && (
              <div className="absolute bottom-11 left-0 z-20 w-48 rounded-panel border border-border bg-surface p-1 shadow-xl">
                {[
                  [ImageIcon, "Hình ảnh", "image/*"],
                  [Video, "Video", "video/*"],
                  [FileText, "Tệp", ".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.zip"],
                  [Link2, "Tệp bất kỳ", "*/*"],
                ].map(([Icon, label, accept]: any) => (
                  <button
                    key={label}
                    type="button"
                    className="flex w-full items-center gap-3 rounded-control px-3 py-2 text-left text-[13px] text-ink hover:bg-surface-quiet"
