...
import { API_URL, WS_URL, getToken } from "@/features/auth/services/user_authentication.service";
import { useToast } from "@/shared/contexts/ToastContext";
import { Modal, ModalHeader, ModalTitle, ModalDescription, ModalContent } from "@/shared/components/ui/Modal";
import {
 ImageIcon, Book, Loader2, ArrowLeft, Search, Plus, Send, User, MoreVertical,
 ChevronRight, X, Eye, Share2, Paperclip, Languages, ShieldAlert, Pin, Users,
 Mic, Flame, Volume2, VolumeX, ThumbsUp, Heart, Play, Pause, Trash2,
 MoreHorizontal, Reply, PinOff, Download, Edit2, Undo2, CheckCheck, MessageSquare
} from "lucide-react";
...
const CustomAudioPlayer = ({ src, isSender }: { src: string; isSender: boolean }) => {
 ...
 <button onClick={togglePlay} className={`flex shrink-0 items-center justify-center w-8 h-8 rounded-full ${isSender ? "bg-white text-[#0071E3]" : "bg-[#0071E3] text-white"}`}>
 {isPlaying ? <Pause size={14} className="fill-current" /> : <Play size={14} className="ml-0.5 fill-current" />}
 </button>
 <div className="flex-1 flex items-center gap-3">
 <div className="flex-1 h-1.5 rounded-full relative overflow-hidden" style={{ background: isSender ? "rgba(255,255,255,0.3)" : "rgba(0,113,227,0.1)" }}>
 <div className="absolute top-0 left-0 h-full rounded-full transition-all duration-100" style={{ width: `${progress}%`, background: isSender ? "white" : "#0071E3" }}></div>
 </div>
 <span className="text-[12px] font-medium opacity-80 min-w-[32px] text-right">{formatTime(audioRef.current?.currentTime || 0)}</span>
 </div>
 <audio ref={audioRef} src={src} onTimeUpdate={handleTimeUpdate} onEnded={() => { setIsPlaying(false); setProgress(100); }} className="hidden" />
 </div>
 );
};
...
export default function MessagesPage() {
 ...
 if (selectedConvRef.current?.other_user_id !== senderId) conv.unread_count = (conv.unread_count || 0) + 1;
 ...
 if (lastMsgId) socket.send(JSON.stringify({ action: "sync", data: { last_message_id: lastMsgId } }));
 };
 ...
 socketRef.current.send(JSON.stringify({ action: "mark_read", data: { other_user_id: selectedConvRef.current.other_user_id } }));
 }
 }
 ...
 if (selectedConvRef.current) setSelfDestructSeconds(data.self_destruct_seconds || 0);
 }
 } catch (err) {}
 };
 return () => { clearInterval(pingInterval); socketRef.current = null; socket.close(); };
 }, [user?._id, updateConversationInPlace]);

 const selectConversation = async (conv: any) => {
 if (selectedConvRef.current && newMessage.trim()) await saveDraftAPI(selectedConvRef.current.other_user_id, newMessage.trim());
 ...
 if (isBlocked) { showToast("Không thể gửi khi bị chặn.", "error"); return; }
 ...
 setMessages((prev) => prev.map((m) => (m._id || m.id) === (editingMsg._id || editingMsg.id) ? { ...m, content: newMessage.trim(), is_edited: true } : m));
 setEditingMsg(null);
 setNewMessage("");
 } catch (err: any) { showToast("Chỉnh sửa thất bại.", "error"); } finally { setSending(false); }
 return;
 }
 setSending(true);
 try {
 ...
 const resUpload = await fetch(`${API_URL}/tai-len/file`, { method: "POST", headers: { Authorization: `Bearer ${getToken()}` }, body: formData });
 const uploadData = await resUpload.json();
 imageUrl = uploadData.data.url;
 setUploadingImage(false);
 }
 const res = await sendMessageAPI(selectedConv.other_user_id, newMessage.trim(), imageUrl, replyingTo?._id || replyingTo?.id);
 ...
 } catch (err: any) { showToast("Gửi thất bại.", "error"); } finally { setSending(false); setUploadingImage(false); }
 };
 ...
 const resUpload = await fetch(`${API_URL}/tai-len/file`, { method: "POST", headers: { Authorization: `Bearer ${getToken()}` }, body: formData });
 const uploadData = await resUpload.json();
 const res = await sendMessageAPI(selectedConv.other_user_id, "Tin nhắn thoại", undefined, undefined, uploadData.data.url);
 const msg = res.data || res;
 setMessages((prev) => [...prev, msg]);
 updateConversationInPlace(selectedConv.other_user_id, msg);
 } catch (err) { showToast("Lỗi gửi giọng nói.", "error"); } finally { setSending(false); }
 };
 recorder.start();
 setMediaRecorder(recorder);
 setIsRecording(true);
 setRecordingDuration(0);
 recordTimerRef.current = setInterval(() => setRecordingDuration((prev) => prev + 1), 1000);
 } catch (err) { showToast("Không thể ghi âm.", "error"); }
 };

 const handleStopRecording = () => {
 if (mediaRecorder && isRecording) { mediaRecorder.stop(); setIsRecording(false); clearInterval(recordTimerRef.current); }
 };

 const handleCancelRecording = () => {
 if (mediaRecorder && isRecording) { cancelRecordingRef.current = true; mediaRecorder.stop(); setIsRecording(false); clearInterval(recordTimerRef.current); }
 };

 const scrollToMessage = (id: string) => {
 const el = messageRefs.current[id];
 if (el) { el.scrollIntoView({ behavior: "smooth", block: "center" }); el.classList.add("bg-[#F5F5F7]"); setTimeout(() => el.classList.remove("bg-[#F5F5F7]"), 2000); }
 };
 ...
 setMessages((prev) => {
 const newMsgs = prev.map((m) => (m._id || m.id) === messageId ? { ...m, is_pinned: !m.is_pinned } : m);
 if (selectedConv) setSelectedConv({ ...selectedConv, pinned_messages: newMsgs.filter((m) => m.is_pinned) });
 return newMsgs;
 });
 } catch (err: any) { showToast("Ghim thất bại.", "error"); }
 };

 const handleRecall = async (messageId: string) => {
 try {
 await recallMessageAPI(messageId);
 setMessages((prev) => prev.map((m) => (m._id || m.id) === messageId ? { ...m, is_recalled: true, content: "Tin nhắn đã thu hồi" } : m));
 showToast("Đã thu hồi.", "success");
 } catch (err: any) { showToast("Thu hồi thất bại.", "error"); }
 };

 const handleSearchMessages = async (q: string) => {
 setSearchMsgQuery(q);
 if (!selectedConv || q.length < 1) return setSearchedMsgResults([]);
 setSearchedMsgResults(messages.filter((m) => m.content && !m.is_recalled && m.content.toLowerCase().includes(q.toLowerCase())));
 };

 const handleAddReaction = async (messageId: string, reaction: string) => {
 try {
 const msg = messages.find((m) => (m._id || m.id) === messageId);
 const existing = msg?.reactions?.find((r: any) => r.user_id === user?._id);
 const finalReaction = existing?.reaction === reaction ? "" : reaction;
 const res = await addReactionAPI(messageId, finalReaction);
 setMessages((prev) => prev.map((m) => (m._id || m.id) === messageId ? { ...m, reactions: (res.data || res).reactions } : m));
 } catch (err: any) { showToast("Thất bại.", "error"); }
 };
 ...
 } catch (err: any) { showToast("Lỗi chia sẻ.", "error"); }
 };

 const handleBlockUser = async () => {
 if (!selectedConv) return;
 try {
 if (isBlocked) { await unblockUserAPI(selectedConv.other_user_id); setIsBlocked(false); showToast("Đã bỏ chặn.", "success"); }
 else { await blockUserAPI(selectedConv.other_user_id); setIsBlocked(true); showToast("Đã chặn.", "success"); }
 } catch (err: any) { showToast("Thao tác thất bại.", "error"); }
 };
 ...
 setActiveConvMenuId(null);
 } catch (err: any) { showToast("Không thể ghim.", "error"); }
 };
 ...
 setActiveConvMenuId(null);
 } catch (err) { showToast("Không thể đánh dấu", "error"); }
 };
 ...
 } catch (err) { showToast("Không thể xóa", "error"); }
 };

 const handleTranslate = async (messageId: string, lang: string) => {
 try {
 const res = await translateMessageAPI(messageId, lang);
 setMessages((prev) => prev.map((m) => (m._id || m.id) === messageId ? { ...m, translated_content: (res.data || res).translated_content } : m));
 showToast("Đã dịch.", "success");
 } catch (err: any) { showToast("Không thể dịch.", "error"); }
 };
 ...
 showToast((res.data || res).is_muted ? "Đã tắt âm." : "Đã bật âm.", "success");
 } catch (err: any) { showToast("Không thể điều chỉnh.", "error"); }
 };
 ...
 setShowSelfDestructMenu(false);
 showToast(seconds > 0 ? `Đã đặt tự hủy sau ${seconds}s.` : "Đã tắt tự hủy.", "success");
 } catch (err: any) { showToast("Cài đặt thất bại.", "error"); }
 };

 const openGroupModal = async () => {
 setShowGroupModal(true); setLoadingGroupUsers(true);
 try {
 ...
 setShowGroupModal(false); setGroupName(""); setSelectedMembers([]); loadConversations();
 selectConversation({ other_user_id: created._id || created.id, other_user: { username: created.group_name, full_name: created.group_name, avatar_url: "", is_group: true }, last_message: null, pinned_messages: [], unread_count: 0 });
 } catch (err: any) { showToast("Tạo thất bại.", "error"); }
 };
 ...
 } catch (err: any) { showToast("Tìm kiếm thất bại.", "error"); } finally { setSearching(false); }
 };

 const startNewChat = (otherUser: any) => {
 const existing = conversations.find((c) => c.other_user_id === (otherUser._id || otherUser.id));
 if (existing) selectConversation(existing);
 else { setSelectedConv({ other_user_id: (otherUser._id || otherUser.id), other_user: otherUser, last_message: null, pinned_messages: [] }); setMessages([]); }
 setShowNewChatModal(false); setSearchQuery(""); setSearchResults([]);
 };
 ...
 <Modal isOpen={showNewChatModal} onClose={() => setShowNewChatModal(false)} className="max-w-xl rounded-[18px] bg-[#F5F5F7] p-0 border-none -2xl">
 ...
 <span className="text-[15px] font-medium text-[#1D1D1F]">{u.full_name || u.username}</span>
 <span className="text-[13px] text-[#6E6E73]">ID: {u.slug || u.username}</span>
 </div>
 </div>
 <ChevronRight className="w-5 h-5 text-[#6E6E73]" />
 </div>
 )) : <p className="text-center text-[13px] text-[#6E6E73] py-12">Không tìm thấy kết quả</p>}
 </div>
 </ModalContent>
 </Modal>

 <Modal isOpen={showGroupModal} onClose={() => setShowGroupModal(false)} className="max-w-md rounded-[18px] bg-[#F5F5F7] p-0 border-none -2xl">
 <ModalHeader className="p-6">
 ...
 <input type="checkbox" checked={selectedMembers.includes(u._id || u.id)} onChange={(e) => { if (e.target.checked) setSelectedMembers([...selectedMembers, u._id || u.id]); else setSelectedMembers(selectedMembers.filter((id) => id !== (u._id || u.id))); }} className="w-4 h-4 rounded text-[#0071E3] focus:ring-[#0071E3]" />
 <span className="text-[15px] text-[#1D1D1F]">{u.full_name || u.username}</span>
 </div>
 ))}
 </div>
 <button onClick={handleCreateGroup} className="pill-button w-full">Khởi tạo</button>
 </ModalContent>
 </Modal>

 <Modal isOpen={showShareDocModal} onClose={() => setShowShareDocModal(false)} className="max-w-xl rounded-[18px] bg-[#F5F5F7] p-0 border-none -2xl">
 <ModalHeader className="p-6">
 <ModalTitle className="text-[20px] font-semibold text-[#1D1D1F]">Chia sẻ tài liệu</ModalTitle>
 </ModalHeader>
 ...
 )) : <p className="text-center text-[13px] text-[#6E6E73] py-6">Không có tài liệu</p>}
 </ModalContent>
 </Modal>

 <div className="flex flex-1 min-h-0 gap-6">
 <div className={`w-full md:w-[320px] bg-[#F5F5F7] rounded-[18px] flex flex-col overflow-hidden shrink-0 ${selectedConv ? "hidden md:flex" : "flex"}`}>
 <div className="p-6 pb-4 flex items-center justify-between">
 <h2 className="text-[20px] font-semibold text-[#1D1D1F]">Hộp thư</h2>
 <div className="flex gap-2">
 <button onClick={openGroupModal} className="p-2 bg-white rounded-full text-[#0071E3] hover:opacity-80 transition-opacity"><Users className="w-4 h-4" /></button>
 <button onClick={() => setShowNewChatModal(true)} className="p-2 bg-[#0071E3] rounded-full text-white hover:bg-[#0055C6] transition-colors"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2 hide-scrollbar">
 {loadingConv ? <div className="p-12 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-[#6E6E73]" /></div> : sortedConversations.length > 0 ? sortedConversations.map((conv) => {
 ...
 <span className="text-[15px] font-medium text-[#1D1D1F] truncate pr-2">{conv.other_user?.full_name || conv.other_user?.username}</span>
 <span className="text-[12px] text-[#6E6E73] shrink-0">{conv.last_message?.created_at ? parseUTC(conv.last_message.created_at).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" }) : ""}</span>
 </div>
 <div className="flex justify-between items-center">
 <p className={`text-[13px] truncate ${conv.unread_count > 0 ? "font-semibold text-[#1D1D1F]" : "text-[#6E6E73]"}`}>{conv.last_message?.content || "Chưa có tin nhắn"}</p>
 {conv.unread_count > 0 && <div className="w-2.5 h-2.5 bg-[#0071E3] rounded-full shrink-0 ml-2" />}
 </div>
 </div>
 </div>
 );
 }) : <div className="py-24 text-center"><p className="text-[15px] text-[#6E6E73]">Hộp thư trống</p></div>}
 </div>
 </div>
 ...
 {selectedConv.other_user?.avatar_url ? <img src={selectedConv.other_user.avatar_url} className="w-full h-full object-cover" alt="" /> : <User className="w-5 h-5 text-white m-auto mt-2.5" />}
 </div>
 <div>
 <h3 className="text-[17px] font-medium text-[#1D1D1F]">{selectedConv.other_user?.full_name || selectedConv.other_user?.username}</h3>
 <p className="text-[12px] text-[#6E6E73]">{isOnline ? "Trực tuyến" : "Ngoại tuyến"}</p>
 </div>
 </div>
 ...
 return (
 <div key={msg._id || msg.id || i} ref={(el) => (messageRefs.current[msg._id || msg.id] = el)} className={`flex ${isSender ? "justify-end" : "justify-start"}`}>
 <div className={`max-w-[70%] rounded-[18px] px-4 py-2.5 ${msg.is_recalled ? "bg-[#F5F5F7] text-[#6E6E73] italic" : isSender ? "bg-[#0071E3] text-white" : "bg-[#F5F5F7] text-[#1D1D1F]"}`}>
 {msg.image_url && !msg.is_recalled && <img src={msg.image_url.startsWith("http") ? msg.image_url : `${API_URL}/storage/${msg.image_url}`} alt="" className="rounded-[10px] mb-2 max-h-[300px]" />}
 {msg.audio_url && !msg.is_recalled && <CustomAudioPlayer src={msg.audio_url.startsWith("http") ? msg.audio_url : `${API_URL}/storage/${msg.audio_url}`} isSender={isSender} />}
 {!msg.is_recalled && msg.content !== "Tin nhắn thoại" && <p className="text-[15px] leading-[1.4] whitespace-pre-wrap">{msg.content}</p>}
 {msg.is_recalled && "Tin nhắn đã thu hồi"}
 <div className={`text-[11px] mt-1 ${isSender ? "text-blue-200 text-right" : "text-[#6E6E73]"}`}>
 ...
 <div className="text-center"><MessageSquare className="w-12 h-12 text-[#D2D2D7] mx-auto mb-4" /><p className="text-[17px] text-[#6E6E73]">Chọn một hội thoại để bắt đầu</p></div>
 )}
 </div>
 </div>
 </div>
