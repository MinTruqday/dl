    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-500">
      <header className="border-b-2 border-black pb-8 mb-12 flex justify-between items-end">
        <div>
           <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[12px] font-bold tracking-widest text-zinc-400">Hệ thống tối cao</span>
           </div>
           <h1 className="text-4xl font-bold text-black tracking-tighter">Bảng điều khiển Hệ thống</h1>
        </div>
        <button 
           onClick={fetchData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         <div className="border border-black p-8 space-y-6">
            <div className="flex items-center justify-between">
               ...
               <span className={`text-[12px] font-bold tracking-widest px-3 py-1 ${health?.status === 'ok' ? 'bg-black text-white' : 'bg-zinc-200 text-black border border-black'}`}>
                  {health?.status || 'Đang kiểm tra'}
               </span>
            </div>
            <div className="space-y-1">
               <h3 className="text-sm font-bold tracking-tight">Dịch vụ hệ thống</h3>
               <p className="text-[12px] text-zinc-400 font-bold tracking-widest">v{health?.version || '1.0.0'}</p>
            </div>
         ...
         <div className="border border-black p-8 space-y-6">
            <div className="flex items-center justify-between">
               ...
               <span className={`text-[12px] font-bold tracking-widest px-3 py-1 ${health?.services.mongodb === 'ok' ? 'bg-black text-white' : 'bg-zinc-200 text-black border border-black'}`}>
                  {health?.services.mongodb || 'Đang kiểm tra'}
               </span>
            </div>
            <div className="space-y-1">
               <h3 className="text-sm font-bold tracking-tight">Lưu trữ dữ liệu</h3>
               <p className="text-[12px] text-zinc-400 font-bold tracking-widest">Cụm dữ liệu chính</p>
            </div>
            <div className="pt-4 border-t border-zinc-100 flex items-center gap-2">
         ...
         <div className="border border-black p-8 space-y-6">
            <div className="flex items-center justify-between">
               ...
               <span className={`text-[12px] font-bold tracking-widest px-3 py-1 ${health?.services.redis === 'ok' ? 'bg-black text-white' : 'bg-zinc-100 text-zinc-400'}`}>
                  {health?.services.redis || 'Không khả dụng'}
               </span>
            </div>
            <div className="space-y-1">
               <h3 className="text-sm font-bold tracking-tight">Bộ đệm hệ thống</h3>
               <p className="text-[12px] text-zinc-400 font-bold tracking-widest">Bộ đệm phân tán</p>
            </div>
            <div className="pt-4 border-t border-zinc-100 flex items-center gap-2">
      ...
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-12">
         <div className="border border-black p-10 space-y-8">
            <h2 className="text-xl font-bold tracking-tighter border-l-4 border-black pl-4">Tài nguyên máy chủ</h2>
                  ...
                  <div key={i} className="space-y-2">
                     <div className="flex justify-between items-end">
                        <div className="flex items-center gap-2">
                           {stat.icon}
                           <span className="text-[12px] font-bold tracking-widest text-zinc-500">{stat.label}</span>
                        </div>
         ...
         <div className="border border-black p-10 space-y-8">
            <h2 className="text-xl font-bold tracking-tighter border-l-4 border-black pl-4">Bảo trì & Sao lưu</h2>
            <div className="space-y-6">
               <div className="p-6 bg-zinc-50 border border-zinc-100 space-y-4">
                  <div className="flex justify-between items-center">
                     <p className="text-xs font-bold tracking-tight">Sao lưu dữ liệu</p>
                     <span className="text-[13px] font-bold text-zinc-400">Hàng ngày @ 03:00</span>
                  </div>
                  <button className="w-full py-3 border border-black text-[12px] font-bold tracking-widest hover:bg-black hover:text-white transition-all">
                     Thực hiện sao lưu
                  </button>
               </div>
               <div className="p-6 bg-zinc-50 border border-zinc-100 space-y-4">
                  <div className="flex justify-between items-center">
                     <p className="text-xs font-bold tracking-tight">Dọn dẹp bộ đệm</p>
                     <span className="text-[13px] font-bold text-zinc-400">Sử dụng: 42.5 MB</span>
                  </div>
                  <button className="w-full py-3 border border-black text-[12px] font-bold tracking-widest hover:bg-black hover:text-white transition-all">
                     Làm trống bộ đệm
                  </button>
         ...
         <div className="border border-black p-10 space-y-8 lg:col-span-2">
            <div className="flex items-center justify-between">
               <h2 className="text-xl font-bold tracking-tighter border-l-4 border-black pl-4">Nhật ký Hệ thống Toàn cầu</h2>
               <button className="text-[12px] font-bold tracking-widest text-zinc-400 hover:text-black underline">Tải xuống CSV</button>
            </div>
            <div className="space-y-1">
               <div className="grid grid-cols-12 px-4 py-3 bg-zinc-100 text-[12px] font-bold tracking-widest text-zinc-500">
               ...
               <div className="divide-y divide-zinc-100 border border-zinc-100">
                  {logs.length > 0 ? logs.map((log: any, i: number) => (
                     <div key={i} className="grid grid-cols-12 px-4 py-4 text-[13px] font-bold tracking-tight hover:bg-zinc-50 transition-colors items-center">
