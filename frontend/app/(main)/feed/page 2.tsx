...
import { Heart, MessageCircle, Globe, Sparkles, Users, Lock, Share2, PlusSquare, ArrowUp, Send, CheckCircle, XCircle, X, Bookmark, BookText, BarChart2, Trash2, Trophy, EyeOff, Edit3, Flag, Eye, Image as ImageIcon, Quote, PenTool, Book, FileText, HelpCircle, AtSign, Pin, Archive, Link as LinkIcon } from "lucide-react";
              ...
              <div key={post.id} className="bg-card border border-border rounded-sm p-5  ">
                ...
                {post.book_id && (
                  <div className="mt-4 p-4 border border-border bg-muted/30 rounded-sm cursor-pointer border-l-4 border-l-primary hover:bg-muted/50 transition-colors flex items-center justify-between">
                    <div>
                      <span className="text-xs font-semibold text-muted-foreground tracking-wider flex items-center"><BookText className="w-4 h-4 mr-2" /> Tác phẩm đính kèm</span>
                      <h5 className="font-semibold mt-1 text-sm text-foreground truncate">{post.book_title || post.book_id} {post.book_title ? `<${post.book_id}>` : ""}</h5>
                    </div>
