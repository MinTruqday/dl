...
export default function Feed() {
  ...
  useEffect(() => {
    if (viewingStoryMode && activeStoryIndex >= 0 && stories[activeStoryIndex]) {
      ...
      if (storyId) {
        viewStoryAPI(storyId).catch(e => console.error("Error viewing story:", e));
      }
    }
  }, [viewingStoryMode, activeStoryIndex]);
  ...
  const fetchFeed = async (reset = false) => {
    try {
      ...
      const limit = 10;
      const json = await getFeedAPI(tab, skip, limit, itemType, filter === 'trending' ? 'trending' : undefined);
  ...
  const handleVote = async (postId: string, amount: number) => {
    try {
      ...
      showToast(data.message || `Đã gửi tặng ${amount} C thành công`, "success");
      fetchWallet(); fetchFeed(true);
    } catch (e: any) { 
      showToast(e.message || "Bạn không đủ số dư để thực hiện, xin nạp thêm.", "error"); 
    }
  };
  ...
  const togglePinPost = async (postId: string) => {
    try { await pinPostAPI(postId); fetchFeed(true); } catch(e) { console.error("API error:", e); }
  };

  const reportPost = async (postId: string) => {
    const reason = prompt("Vui lòng nhập lý do báo cáo để Quản trị viên xem xét:");
    if (!reason || !reason.trim()) return;
    try { await reportPostAPI(postId, reason); showToast("Cảm ơn, báo cáo đã được ghi nhận.", "success"); } catch(e) { console.error("API error:", e); }
  };

  const hidePost = async (postId: string) => {
    try { await hidePostAPI(postId); showToast("Đã ẩn.", "info"); fetchFeed(true); } catch(e) { console.error("API error:", e); }
  };

  const followUser = async (userId: string) => {
    try { const data = await followUserAPI(userId); showToast(data.message, "success"); fetchSuggestions(); } catch(e) { console.error("API error:", e); }
  };
