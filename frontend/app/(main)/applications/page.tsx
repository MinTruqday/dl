        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            ...
            <p className="text-zinc-400 text-sm font-bold uppercase tracking-widest flex items-center gap-2">
              Quy trình xét duyệt tác giả tri thức{" "}
              <ClipboardCheck className="w-3.5 h-3.5 text-zinc-100" />
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-white p-1 rounded-sm border border-zinc-100">
              {[
                ...
                <button
                  ...
                  className={`px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest rounded-sm ${
                    statusFilter === f.id
                      ? "bg-white text-black "
                      : "text-zinc-400 "
                  }`}
            ...
            <button
              ...
              className="h-12 px-8 bg-black text-white text-[10px] font-bold uppercase tracking-widest active:scale-[0.98] flex items-center gap-3 rounded-sm"
            >
              {isRefreshing ? (
                  ...
                  <tr key={app._id} className=" group">
                    <td className="px-10 py-10">
                      <div className="flex items-center gap-8">
                        <div className="w-14 h-14 bg-white flex items-center justify-center border border-zinc-100 font-black text-zinc-200 rounded-sm">
                          {app.user_name?.[0]?.toUpperCase() || "U"}
                        </div>
                        <div className="flex flex-col gap-2 min-w-0">
                          <span className="font-bold text-black text-base tracking-tighter truncate max-w-xs">
                            {app.user_name || "Ứng viên ẩn danh"}
                          </span>
                    ...
                    <td className="px-10 py-10">
                      <div className="bg-white p-4 border-l-2 border-zinc-100 rounded-sm transition-colors">
                        <p className="text-[12px] text-zinc-500 font-medium italic line-clamp-2 max-w-md leading-relaxed">
                          "
                          {app.motivation ||
                            "Ứng viên chưa cung cấp mô tả chi tiết về động lực ứng tuyển."}
                          "
                        </p>
                      </div>
                    </td>
                    ...
                    <td className="px-10 py-10 text-right">
                      {statusFilter === "PENDING" ? (
                        ...
                        <span
                          className={`inline-block px-4 py-1.5 text-[9px] font-bold uppercase tracking-widest rounded-sm border ${
