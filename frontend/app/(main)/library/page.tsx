...
export default function LibraryPage() {
  ...
  const canManageSeries = ["author", "moderator", "admin"].includes(
  ...
  const fetchLibraryData = useCallback(async () => {
    ...
    try {
      const [pinnedRes, continueRes, foldersRes, historyRes, listsRes, seriesRes] = await Promise.all([
        getPinnedDocumentsAPI().catch((err) => {
          console.error("Pinned API failed:", err);
          return { data: [] };
        }),
        getContinueReadingAPI().catch((err) => {
          console.error("Continue API failed:", err);
          return { data: [] };
        }),
        getBookmarkFoldersAPI().catch((err) => {
          console.error("Folders API failed:", err);
          return { data: [] };
        }),
        getReadingHistoryAPI().catch((err) => {
          console.error("History API failed:", err);
          return { data: [] };
        }),
        getReadingListsAPI().catch((err) => {
          console.error("Lists API failed:", err);
          return { data: [] };
        }),
        canManageSeries
          ? getMySeriesAPI().catch((err) => {
              console.error("Series API failed:", err);
              return { data: [] };
            })
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast("Khởi tạo thất bại, vui lòng thử lại sau", "error");
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDeleteHistoryItem = async (documentId: string) => {
    ...
    try {
      ...
      console.error("Không thể xóa mục lịch sử:", err);
      showToast("Xóa mục lịch sử thất bại", "error");
    } finally {
  ...
  const filteredHistory = history.filter((h) =>
    (h.document_title || "").toLowerCase().includes(searchQuery.toLowerCase()),
  );

  const filteredReadingLists = readingLists.filter((l) =>
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 py-12 font-sans text-black selection:bg-black selection:text-white">
          ...
          <nav className="flex bg-white border border-zinc-100 p-1 rounded-sm overflow-x-auto no-scrollbar max-w-[600px]">
            {[
              ...
              <button
                ...
                className={`px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest rounded-sm transition-all shrink-0 ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div
          className="lg:col-span-8 space-y-12 delay-75 duration-300"
          style={{
            opacity: visible ? 1 : 0,
            transform: visible ? "translateY(0)" : "translateY(10px)",
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest transition-colors"
                  >
                    Xem toàn bộ lịch sử
                  </button>
                </div>
                  ...
                  >
                    {filteredContinueDocs.map((doc) => (
                      <Link
                        ...
                        className={`group border border-zinc-100 p-6 bg-white flex ${viewMode === "grid" ? "flex-col justify-between" : "items-center gap-6"} rounded-sm`}
                      >
                        <div
                          className={`flex ${viewMode === "grid" ? "flex-col space-y-4" : "items-center gap-6 flex-1"}`}
                        >
                          <div
                            className={`${viewMode === "grid" ? "aspect-[3/4]" : "w-16 h-20"} bg-white border border-zinc-100 overflow-hidden grayscale relative rounded-sm`}
                          >
                            {doc.cover_url ? (
                              <img
                                src={
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button
                      ...
                      className="h-10 px-6 rounded-sm text-[10px] font-bold uppercase tracking-widest border border-zinc-100 flex items-center gap-2 active:scale-95 transition-transform"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[...folders.slice(0, 2), ...readingLists.slice(0, 2)].length > 0 ? (
                    [...folders.slice(0, 2), ...readingLists.slice(0, 2)].map((item, i) => (
                      <Link
                        key={item.id || item._id}
                        href={item.id ? `/library/folder/${item.id}` : `/collection/${item._id}`}
                        className="group p-8 border border-zinc-100 bg-white flex items-center justify-between rounded-sm"
                      >
                        <div className="space-y-2">
                          <h4 className="text-lg font-bold tracking-tight text-black transition-transform ">
                            {item.name}
                          </h4>
                          <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest">
            ...
            <section className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="h-10 px-6 text-[10px] font-bold text-zinc-400 uppercase tracking-widest border border-zinc-100 rounded-sm active:scale-95"
                >
                ...
                {filteredHistory.length > 0 ? (
                  filteredHistory.map((item, idx) => (
                    <div
                      ...
                      className={`group flex items-center justify-between p-8 border border-zinc-100 bg-white rounded-sm ${isDeletingHistory === item.document_id ? "opacity-50" : ""}`}
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <Link
                          ...
                          className="h-12 px-8 bg-black text-white text-[10px] font-bold uppercase tracking-widest flex items-center rounded-sm active:scale-95"
                        >
                          Đọc lại
                        </Link>
                        <button
                          onClick={() => handleDeleteHistoryItem(item.document_id)}
                          className="w-12 h-12 border border-zinc-100 flex items-center justify-center text-zinc-200 active:scale-95 rounded-sm"
                        >
            ...
            <section className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="h-10 px-8 bg-black text-white text-[10px] font-bold uppercase tracking-widest flex items-center gap-3 rounded-sm active:scale-95"
                >
                ...
                {folders.length > 0 ? (
                  folders.map((folder) => (
                    <Link
                      ...
                      className="group p-10 border border-zinc-100 bg-white flex flex-col justify-between min-h-[200px] rounded-sm transition-all hover:border-black"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredReadingLists.length > 0 ? (
                  filteredReadingLists.map((list) => (
                    <div
                      key={list._id}
                      className="group border border-zinc-100 p-10 bg-white flex flex-col justify-between min-h-[300px] rounded-sm hover:border-black transition-all"
                    >
                      <div className="space-y-4">
                ...
                {filteredSeries.length > 0 ? (
                  filteredSeries.map((s) => (
                    <Link
                      key={s._id}
                      href={`/series/${s._id}`}
                      className="group border border-zinc-100 p-10 bg-white flex flex-col justify-between min-h-[240px] rounded-sm hover:border-black transition-all"
                    >
                      <div className="space-y-4">
        ...
        <aside
          className="lg:col-span-4 space-y-10 delay-150 duration-300"
          style={{
            opacity: visible ? 1 : 0,
            transform: visible ? "translateY(0)" : "translateY(10px)",
        ...
        >
              ...
              {pinnedDocs.length > 0 ? (
                pinnedDocs.map((doc) => (
                  <Link
                    ...
                    className="flex items-center gap-4 p-4 bg-white border border-zinc-100 group rounded-sm"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold truncate underline-offset-4 decoration-1">
                        {doc.title}
                      </h4>
                      <p className="text-[8px] font-bold text-zinc-300 uppercase tracking-widest mt-1 italic">
                        Đã ghim
                      </p>
                    </div>
                  </Link>
          ...
          <div className="p-8 border border-zinc-100 bg-white space-y-6 rounded-sm">
            ...
            <input
              ...
              className="w-full h-12 bg-white border border-zinc-100 px-5 text-[11px] font-bold outline-none focus:border-black rounded-sm"
            />
          </div>
      ...
      >
        <ModalHeader>
          <ModalTitle>
            {createType === "folder"
              ...
              : createType === "list"
                ? "Tạo danh sách mới"
                : "Khởi tạo chuỗi tri thức"}
          </ModalTitle>
        </ModalHeader>
        <ModalContent className="space-y-6 pt-4">
          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest leading-relaxed">
            {createType === "folder"
              ? "Đặt tên cho bộ sưu tập tri thức cá nhân của bạn để quản lý tài liệu hiệu quả hơn."
              : "Phân loại và kết nối các thực thể tri thức chuyên sâu để tối ưu hóa việc tiếp cận thông tin."}
          </p>
          <div className="space-y-3">
            <label className="text-[9px] font-bold text-black uppercase tracking-widest">
              {createType === "folder" ? "Tên thư mục" : "Tiêu đề thực thể"}
            </label>
            <input
              type="text"
          ...
          {createType !== "folder" && (
            <div className="space-y-3">
              <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-[0.2em]">
                Mô tả tóm lược
              </label>
              <textarea
                ...
                className="w-full min-h-[120px] p-6 bg-zinc-50/30 border border-zinc-100 text-sm font-medium focus:border-black outline-none rounded-sm resize-none"
                placeholder=""
              />
            </div>
          )}
            ...
            <div className="flex items-center justify-between p-6 border border-zinc-100 rounded-sm">
              <div className="space-y-1">
                ...
                <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-tight">
                  Công khai bộ sưu tập với cộng đồng
                </p>
              </div>
              <button
                onClick={() =>
                ...
                className={`w-14 h-8 relative rounded-sm transition-colors border ${createListForm.is_public ? "bg-black border-black" : "bg-zinc-200 border-zinc-200"}`}
              >
                <div
                  className={`absolute top-1 w-6 h-6 bg-white rounded-sm transition-all ${createListForm.is_public ? "left-7" : "left-1"}`}
                />
              </button>
            </div>
          )}
        ...
        <ModalFooter className="flex gap-4">
          ...
          <button
            ...
            disabled={
              isCreating ||
              (createType === "folder" ? !newFolderName.trim() : !createListForm.name.trim())
            }
            className="flex-1 h-14 bg-black text-white text-[10px] font-bold uppercase tracking-widest active:scale-95 rounded-sm transition-all disabled:opacity-50 flex items-center justify-center"
          >
            {isCreating ? (
              ...
              createType === "folder" ? "Tạo thư mục" : createType === "list" ? "Xác nhận tạo" : "Khởi tạo ngay"
            )}
          </button>
        </ModalFooter>
      </Modal>
