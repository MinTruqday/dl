...
import {
  X, Send, Zap, Paperclip, Image as ImageIcon, FileText, Loader2, Maximize2, Edit2, Trash2, Plus as PlusIcon, Sparkles, MoreVertical, ArrowRight, Activity
} from "lucide-react";
...
export default function TroChuyenPage() {
  ...
  const handleAttach = () => {
    if (!useSmart) {
      showToast("Vui lòng bật Chế độ Suy nghĩ để phân tích tài liệu đính kèm", "info");
      return;
    }
  ...
  const handleSubmit = async (e?: React.FormEvent, retryText?: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) errorText = "Bạn đã hết hạn mức sử dụng hôm nay. Vui lòng quay lại vào ngày mai hoặc nâng cấp gói cước";
        else {
          try {
        ...
        setMessages((prev) => { const updated = [...prev]; updated[updated.length - 1].content = errorText; return updated; });
        return;
      }
          ...
          if (type === "status" && data) {
            try {
              ...
              if (typeof nodeVi === "object" && nodeVi !== null) {
                if (nodeVi.agent && nodeVi.task) nodeVi = `Tác vụ: ${nodeVi.task} (${nodeVi.agent})`;
            ...
            try {
              ...
              setMessages((prev) => {
                const updated = [...prev];
                const lastMsg = { ...updated[updated.length - 1] };
                if (lastMsg.role === "assistant" && !lastMsg.thoughts?.includes(toolMsg)) {
          ...
          <div className="overflow-y-auto px-6 pb-6 flex flex-col gap-2 shrink custom-scrollbar">
            {sessions.length === 0 ? (
              <div className="py-12 flex flex-col items-center justify-center bg-[#F5F5F7] rounded-[18px]">
                <p className="text-[13px] font-medium text-[#6E6E73]">Lịch sử rỗng</p>
              </div>
            ) : (
              sessions.map((s) => (
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div
                      ...
                      onClick={async () => {
                        ...
                        setView("chat");
                        try {
                          const res = await fetch(`${API_URL}/lich-su/${s._id}`, { headers: { Authorization: `Bearer ${token}` } });
                          if (res.ok) {
                            const data = await res.json();
                    ...
                    >
                      {editingTitleId === s._id ? (
                        <input
                          ...
                          onKeyDown={async (e) => {
                            if (e.key === "Enter") {
                              ...
                              try {
                                const token = getToken();
                                const res = await fetch(`${API_URL}/lich-su/${s._id}/tieu-de`, {
                                  method: "PUT",
                                  headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                                  body: JSON.stringify({ title: editingTitleValue }),
                                });
                                if (res.ok) { fetchHistory(); setEditingTitleId(null); }
                              } catch (err) {}
                        ...
                        <p className="text-[15px] font-medium text-[#1D1D1F] truncate">{s.title}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <p className="text-[12px] text-[#6E6E73] whitespace-nowrap">{new Date(s.updated_at).toLocaleDateString("vi-VN")}</p>
                      <div className="relative">
                        <button onClick={(e) => { e.stopPropagation(); setOpenDropdownId(openDropdownId === s._id ? null : s._id); }} className="p-1.5 text-[#6E6E73] hover:text-[#1D1D1F] hover:bg-[#F5F5F7] rounded-full transition-colors">
                            ...
                            <div className="absolute right-0 top-full mt-1 w-36 p-1.5 bg-white border border-[#E8E8ED] rounded-[14px] shadow-sm z-50">
                              ...
                              <button
                                onClick={async (e) => {
                                  ...
                                  try {
                                    const token = getToken();
                                    const res = await fetch(`${API_URL}/lich-su/${s._id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
                                    if (res.ok) {
                                      if (currentSessionId === s._id) { setCurrentSessionId(null); setMessages([]); }
                                      fetchHistory();
                                    }
                                  } catch (err) {}
                                }}
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto flex flex-col min-h-0 custom-scrollbar relative">
            {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-[24px] font-semibold text-[#1D1D1F]">Xin chào, {user.full_name}</p>
                <p className="text-[15px] text-[#6E6E73] mt-2 leading-relaxed max-w-sm">Tôi có thể giúp gì cho bạn hôm nay?</p>
              </div>
            ) : (
                    ...
                    return (
                      <div key={idx} className="flex justify-end">
                        <div className="max-w-[85%] bg-[#0071E3] text-white px-5 py-3.5 rounded-[20px] rounded-tr-[4px]">
                          <p className="text-[15px] whitespace-pre-wrap leading-relaxed min-w-0">{msg.content}</p>
                        </div>
                      </div>
                    );
                  }

                  const cleanText = msg.content.replace(/<think>[\s\S]*?<\/think>/g, "").replace(/<think>[\s\S]*$/, "").trim();
                        ...
                        <div className="py-2 w-full relative group">
                          {msg.thoughts && msg.thoughts.length > 0 && (
                            <div className="mb-3 flex flex-wrap gap-2">
                              {msg.thoughts.map((t, i) => (
                                <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#F5F5F7] border border-[#E8E8ED] text-[12px] font-medium text-[#6E6E73] whitespace-nowrap">
                                  <Zap className="w-3.5 h-3.5 text-[#0071E3]" /> {t}
                                </span>
                              ))}
                            </div>
                          )}
                          {!cleanText && (
                            <div className="flex gap-1.5 h-6 items-center">
                              ...
                              <div className="w-2 h-2 rounded-full bg-[#C7C7CC] animate-pulse" style={{ animationDelay: "0.2s" }} />
                              <div className="w-2 h-2 rounded-full bg-[#C7C7CC] animate-pulse" style={{ animationDelay: "0.4s" }} />
                            </div>
                          )}
                            ...
                            <ReactMarkdown
                              ...
                              components={{
                                a: ({ href, children, ...props }) => {
                                  ...
                                  return <a href={href} className="text-[#0071E3] font-medium hover:underline" target="_blank" rel="noreferrer" {...props}>{children}</a>;
                                }
                              }}
                            >
                              {cleanText}
            ...
            <div className="w-full relative">
                ...
                <div className="flex gap-4 mb-4 overflow-x-auto pb-2 scrollbar-none">
                  {selectedImage && (
                    <div className="relative group shrink-0">
                      <img src={selectedImage.data} alt="" className="h-16 w-16 object-cover border border-[#E8E8ED] rounded-[14px]" />
                      <button onClick={() => setSelectedImage(null)} className="absolute -top-2 -right-2 w-6 h-6 bg-[#1D1D1F] text-white flex items-center justify-center rounded-full"><X className="w-3 h-3" /></button>
                    </div>
                  )}
                  {selectedFile && (
                    <div className="relative group shrink-0 h-16 px-4 bg-white border border-[#E8E8ED] flex items-center gap-3 rounded-[14px]">
                      <FileText className="w-5 h-5 text-[#0071E3] shrink-0" />
                      <span className="text-[13px] font-medium text-[#1D1D1F] truncate max-w-[150px]">{selectedFile.name}</span>
                      <button onClick={() => setSelectedFile(null)} className="absolute -top-2 -right-2 w-6 h-6 bg-[#1D1D1F] text-white flex items-center justify-center rounded-full"><X className="w-3 h-3" /></button>
                    </div>
                  )}
                </div>
              )}
              ...
              {showAttachments && (
                <div className="absolute bottom-full left-0 mb-2 w-40 bg-white rounded-[14px] shadow-sm py-2 z-50">
                  ...
                  <button onClick={() => { imageInputRef.current?.click(); setShowAttachments(false); }} className="w-full text-left px-4 py-2.5 text-[13px] font-medium text-[#1D1D1F] hover:bg-[#F5F5F7] transition-colors flex items-center gap-3">
              ...
              <form onSubmit={handleSubmit} className="flex gap-3">
                <div className="flex-1 min-h-[56px] bg-white flex items-center px-4 gap-3 focus-within:border-[#0071E3] border border-transparent rounded-[20px] transition-colors shadow-sm">
                  ...
                  />
                  <label className={`flex items-center gap-2 ${user?.ai_tier !== 'PREMIUM' && user?.role !== 'admin' ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'} group shrink-0 pl-4 border-l border-[#E8E8ED]`}>
