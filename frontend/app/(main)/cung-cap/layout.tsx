...
export default function CreationLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`w-full max-w-[1280px] mx-auto px-6 py-6 h-[calc(100vh-44px)] font-sans text-[#1D1D1F] transition-opacity duration-500 ${visible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="grid lg:grid-cols-12 gap-6 h-full items-start">
        <aside className="lg:col-span-4 xl:col-span-3 bg-white border border-[#E8E8ED] rounded-[24px] shadow-sm flex flex-col overflow-hidden shrink-0 self-start w-full h-full">
          <div className="p-6 flex items-center gap-3 shrink-0 border-b border-[#E8E8ED]">
          ...
          <div className="mt-auto p-6 border-t border-[#E8E8ED] shrink-0 bg-[#F5F5F7]/50">
            <div className="flex items-center gap-3">
