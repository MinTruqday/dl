...
const CustomAudioPlayer = ({
  ...
  return (
    <div
      className={`flex items-center gap-3 w-full rounded-full py-1 min-w-[200px]`}
    >
      <button
        onClick={togglePlay}
        className={`flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full ${isSender ? "bg-white text-black" : "bg-black text-white"}`}
      >
        {isPlaying ? (
      ...
      <div className="flex-1 flex items-center gap-3">
        <div
          ...
          style={{
            background: isSender ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.1)",
          }}
        >
          <div
            ...
            style={{
              width: `${progress}%`,
              background: isSender ? "white" : "black",
            }}
          ></div>
        </div>
...
export default function MessagesPage() {
        ...
        if (type === "new_message") {
          ...
          setConversations((prev) =>
            prev.map((c) => {
              if (
                c.last_message &&
                (c.last_message._id || c.last_message.id) ===
                  (data._id || data.id)
              ) {
                return {
                  ...c,
                  last_message: { ...c.last_message, content: data.content },
                };
              }
              return c;
            }),
  ...
  const handleSend = async () => {
    if (isBlocked) {
      showToast(
        "Không thể gửi tin nhắn khi bị chặn hoặc đang chặn người dùng này.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const attachRes = await getSharedAttachmentsAPI(
        selectedConv.other_user_id,
      );
      setSharedAttachments(attachRes.data || attachRes || []);
    } catch (err: any) {
    ...
    try {
      ...
      setConversations((prev) =>
        prev.map((c) => {
          if (
            c.last_message &&
            (c.last_message._id || c.last_message.id) === messageId
          ) {
            return {
              ...c,
              last_message: {
  ...
  const handleDeleteConv = async (otherUserId: string) => {
    if (
      !confirm("Bạn có chắc chắn muốn xóa cuộc hội thoại này? (Nhóm sẽ bị rời)")
    )
      return;
    try {
  ...
  const handleToggleMute = async () => {
    ...
    try {
      ...
      showToast(
        status.is_muted
          ? "Đã tắt âm thông báo cuộc trò chuyện."
          : "Đã bật âm thông báo cuộc trò chuyện.",
        "success",
      );
    } catch (err: any) {
  ...
  const handleUpdateSelfDestruct = async (seconds: number) => {
    ...
    try {
      ...
      showToast(
        seconds > 0
          ? `Đã thiết lập tin nhắn tự hủy sau ${seconds} giây.`
          : "Đã tắt chế độ tin nhắn tự hủy.",
        "success",
      );
    } catch (err: any) {
    ...
    <div className="w-full max-w-[1280px] mx-auto px-6 py-6 h-[calc(100dvh-var(--navbar-height))] flex flex-col font-sans text-black selection:bg-black selection:text-white">
      ...
      >
        ...
        <ModalContent className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-black uppercase block">
              Chọn thành viên
            </label>
        ...
        <ModalContent className="p-6 max-h-[350px] overflow-y-auto space-y-2">
          {loadingShareDocs ? (
            ...
            <p className="text-xs text-zinc-400 py-6 text-center">
              Bạn chưa có tài liệu sáng tác nào để chia sẻ
            </p>
          )}
        </ModalContent>
      </Modal>
          ...
          >
            {loadingConv ? (
              ...
              sortedConversations.map((conv) => {
                const isConvPinned = user?.pinned_conversations?.includes(
                  conv.other_user_id,
                );
                return (
                  <div
                    ...
                    className={`p-3 rounded-2xl cursor-pointer flex items-center gap-3 group relative transition-colors ${
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="flex justify-between items-center mb-1">
                        <span
                          className={`text-sm text-black truncate flex items-center gap-1.5 ${conv.unread_count > 0 ? "font-semibold" : "font-medium"}`}
                        >
                          {conv.other_user?.full_name ||
                            conv.other_user?.username}
                          {isConvPinned && (
                            <Pin className="w-3.5 h-3.5 text-black shrink-0 fill-current" />
                          )}
                        </span>
                        <span
                          className={`text-[10px] shrink-0 ${conv.unread_count > 0 ? "text-black font-semibold" : "text-zinc-500 font-medium"}`}
                        >
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <p
                          className={`text-xs truncate ${conv.unread_count > 0 ? "text-black font-semibold" : "text-zinc-500 font-medium"}`}
                        >
          ...
          {selectedConv ? (
              ...
              <div className="flex items-center justify-between shrink-0 relative z-30">
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 border border-zinc-200 overflow-hidden bg-white flex items-center justify-center relative rounded-full">
                    ...
                    <div
                      className={`absolute bottom-0 right-0 w-2.5 h-2.5 border-2 border-white rounded-2xl ${isOnline ? "bg-black" : "bg-zinc-300"}`}
                    />
                  </div>
                  <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 relative">
                  {showConvMenu && (
                    <>
                      ...
                      />
                      <div className="absolute right-0 top-full mt-2 w-52 p-1.5 bg-white border border-zinc-200 rounded-2xl shadow-lg z-50">
                        <button
                          onClick={() =>
                            setShowSelfDestructMenu(!showSelfDestructMenu)
                          }
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <Flame
                              className={`w-4 h-4 ${selfDestructSeconds > 0 ? "text-red-500" : ""}`}
                            />
                            Tin nhắn tự hủy
                          </div>
                        ...
                        {showSelfDestructMenu && (
                          <div className="bg-zinc-50 border-y border-zinc-100 flex flex-col text-left text-xs my-1 rounded-xl overflow-hidden">
                            <button
                              onClick={() => {
                                handleUpdateSelfDestruct(0);
                                setShowConvMenu(false);
                              }}
                              className="px-9 py-2 hover:bg-zinc-100 transition-colors"
                            >
                              Tắt tự hủy
                            </button>
                            <button
                              onClick={() => {
                                handleUpdateSelfDestruct(5);
                                setShowConvMenu(false);
                              }}
                              className="px-9 py-2 hover:bg-zinc-100 transition-colors"
                            >
                        ...
                        <button
                          onClick={() => {
                            setShowSearchMsgBar(!showSearchMsgBar);
                            setShowConvMenu(false);
                          }}
                          className="w-full text-left px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-100 rounded-xl transition-colors flex items-center gap-2.5"
                        >
                          <Search
                            className={`w-4 h-4 ${showSearchMsgBar ? "text-black" : "text-zinc-500"}`}
                          />
                          Tìm kiếm tin nhắn
                        </button>
                        ...
                        <button
                          onClick={() => {
                            setShowSharedSidebar(!showSharedSidebar);
                            setShowConvMenu(false);
                          }}
                          className="w-full text-left px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-100 rounded-xl transition-colors flex items-center gap-2.5"
                        >
                          <Paperclip
                            className={`w-4 h-4 ${showSharedSidebar ? "text-black" : "text-zinc-500"}`}
                          />
                          Tệp đính kèm
                        </button>
                        ...
                        <button
                          onClick={() => {
                            handleTogglePinConv(selectedConv.other_user_id);
                            setShowConvMenu(false);
                          }}
                          className="w-full text-left px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-100 rounded-xl transition-colors flex items-center gap-2.5"
                        >
                          {user?.pinned_conversations?.includes(
                            selectedConv.other_user_id,
                          ) ? (
                        ...
                        <button
                          onClick={() => {
                            handleMarkAsRead(selectedConv.other_user_id);
                            setShowConvMenu(false);
                          }}
                          className="w-full text-left px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-100 rounded-xl transition-colors flex items-center gap-2.5"
                        >
                        ...
                        <button
                          onClick={() => {
                            handleDeleteConv(selectedConv.other_user_id);
                            setShowConvMenu(false);
                          }}
                      ...
                      {showSearchMsgBar ? (
                              ...
                              {searchedMsgResults.map((sm) => (
                                ...
                                >
                                  ...
                                  <span className="font-mono text-[8px] text-zinc-400">
                                    {new Date(sm.created_at).toLocaleTimeString(
                                      "vi-VN",
                                    )}
                                  </span>
                                </div>
                        ...
                        <>
                          <div className="flex items-center">
                            <span className="text-[10px] font-bold text-black tracking-wider shrink-0">
                              Đã ghim:
                            </span>
                          </div>
                              ...
                              .map((pm: any) => (
                                ...
                                >
                                  {pm.is_recalled ? (
                                    ...
                                    pm.content || "[Hình ảnh / Tệp đính kèm]"
                                  )}
                                </div>
                              ))}
                          </div>
                      ...
                      messages.map((msg, i) => {
                        ...
                        if (showTimeDivider) {
                          ...
                          const isYesterday =
                            ...
                            currDate.getFullYear() === yesterday.getFullYear();
                        ...
                        const marginTopClass = showTimeDivider
                          ? "mt-1"
                          : "mt-1.5";
                          ...
                          <React.Fragment key={msg._id || msg.id || i}>
                            ...
                            >
                              <div className="relative max-w-[85%] sm:max-w-[400px]">
                                {msg.replied_message && (
                                  <div
                                    className={`mb-1 px-3 py-1.5 border-l-2 border-zinc-300 bg-zinc-50/50 text-[11px] text-zinc-500 truncate`}
                                  >
                                ...
                                <div
                                  className={`px-4 py-3 text-sm leading-relaxed border rounded-2xl break-words whitespace-pre-wrap ${isSender ? "rounded-tr-sm" : "rounded-tl-sm"} ${
                                    msg.is_recalled
                                ...
                                >
                                  ...
                                  {(() => {
                                    ...
                                    const docMatch = msg.content.match(
                                      /^Đã chia sẻ tài liệu: \*\*\[([^\]]+)\](?:\(([^)]+)\))?\*\*(?:\n\n([\s\S]*))?/,
                                    );
                                    if (docMatch) {
                                      return (
                                        ...
                                        >
                                          <span className="font-bold text-[14px] text-blue-400">
                                            {docMatch[1]}
                                          </span>
                                        </div>
                                      );
                                  ...
                                  {msg.is_edited && !msg.is_recalled && (
                                    <span className="block text-[9px] mt-1 opacity-50 italic">
                                      (Đã chỉnh sửa)
                                    </span>
                                  )}
                                </div>
                                ...
                                {reactions.length > 0 && !msg.is_recalled && (
                                  <div
                                    className={`flex gap-1 mt-1.5 flex-wrap ${isSender ? "justify-end" : "justify-start"}`}
                                  >
                              ...
                              <div
                                className={`flex items-center gap-2 mt-1 px-1 ${isSender ? "flex-row-reverse" : "flex-row"}`}
                              >
                                ...
                                {!msg.is_recalled && (
                                  <div
                                    className={`flex items-center gap-1 transition-opacity ${isSender ? "flex-row-reverse" : "flex-row"} ${showMsgMenu === (msg._id || msg.id) ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                                  >
                                    <button
                                      onClick={() =>
                                        handleAddReaction(
                                    ...
                                    <div className="relative">
                                      ...
                                      {showMsgMenu === (msg._id || msg.id) && (
                                        <>
                                          ...
                                          />
                                          <div
                                            className={`absolute z-50 w-48 bg-white border border-zinc-200 shadow-xl rounded-2xl py-1.5 ${isSender ? "right-0" : "left-0"} top-full mt-1`}
                                          >
                                            <button
                                              onClick={() => {
                                                setReplyingTo(msg);
                ...
                {showSharedSidebar && (
                  <>
                    <div
                      ...
                      onClick={() => setShowSharedSidebar(false)}
                    />
                    <div className="absolute right-4 top-4 bottom-4 w-[280px] border border-zinc-200 p-4 bg-white/95 backdrop-blur-md flex flex-col shrink-0 overflow-y-auto z-30 shadow-sm transition-all rounded-2xl">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <button
                          onClick={() => setShowSharedSidebar(false)}
                          className="text-zinc-400 hover:text-black p-1.5 rounded-full hover:bg-zinc-100 transition-colors"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="space-y-3">
                        {sharedAttachments.length > 0 ? (
                          sharedAttachments.map((att) => {
                            const isDoc =
                              att.content && att.content.includes("](/truyen/");
                                ...
                                onClick={() => {
                                  if (att.image_url) {
                                    ...
                                    if (
                                      ...
                                      window.open(
                                        `${API_URL}/storage/${docUrl}`,
                                        "_blank",
                                      );
                                  } else if (att.audio_url) {
                                    window.open(
                                      `${API_URL}/storage/${att.audio_url}`,
                                      "_blank",
                                    );
                                  }
                                }}
                  ...
                  <div className="mb-4 p-3 bg-zinc-50 border-l-2 border-black flex items-center justify-between">
                    <div className="min-w-0">
                      <span className="text-[11px] font-bold text-black block mb-1">
                        {editingMsg
                          ? "Chỉnh sửa tin nhắn"
                          : replyingTo.sender_id === user?._id
                            ? user?.full_name
                            : selectedConv.other_user?.full_name}
                      </span>
                      <p className="text-xs text-zinc-500 truncate">
                        {(editingMsg || replyingTo).content || "[Hình ảnh]"}
                      </p>
                    </div>
                    <button
                      onClick={() => {
                ...
                <div className="flex gap-3 relative items-end">
                    ...
                    {!isRecording ? (
                      <>
                        ...
                        <textarea
                          ...
                          disabled={isBlocked}
                          placeholder={
                            isBlocked ? "Hội thoại bị vô hiệu hóa" : ""
                          }
                      ...
                      <div className="flex-1 flex items-center justify-between py-1 h-full">
                        ...
                        {/* Placeholder to perfectly center the text */}
                      </div>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    disabled={
                      sending ||
                      isBlocked ||
                      (!isRecording && !newMessage.trim() && !imageFile)
                    }
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-white border border-zinc-200 rounded-2xl shadow-sm animate-in fade-in slide-in-from-bottom-8 duration-300">
              ...
              <p className="text-xs font-medium text-zinc-500 mt-1">
                Chọn một cuộc hội thoại từ hộp thư để bắt đầu kết nối
              </p>
            </div>
          )}
        </div>
