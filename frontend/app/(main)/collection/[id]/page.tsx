    ...
    <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 py-12 font-sans text-black selection:bg-black selection:text-white">
      ...
      >
        <button
          ...
          className="flex items-center gap-3 text-[10px] font-bold tracking-widest uppercase text-zinc-400 group"
        >
        ...
        <div className="flex gap-2">
          <button className="p-3 border border-zinc-100 rounded-sm">
      ...
      <div
        className="mb-16 border-b border-zinc-100 pb-12 delay-75"
        style={{
          opacity: visible ? 1 : 0,
          transform: visible ? "translateY(0)" : "translateY(10px)",
        }}
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-bold tracking-widest uppercase bg-black text-white px-4 py-2 rounded-sm">
              {collection.documents_detailed?.length || 0} Tài liệu trong danh
              sách
            </span>
          </div>
          <button className="h-12 px-8 bg-white text-black border border-zinc-100 text-[10px] font-bold tracking-widest uppercase active:scale-95 rounded-sm flex items-center gap-2">
        ...
        {collection.documents_detailed?.length > 0 ? (
          <div className="grid grid-cols-1 gap-4">
            {collection.documents_detailed.map((doc: any, index: number) => (
              <div
                key={doc._id}
                className="group flex items-center gap-8 p-6 border border-zinc-100 bg-white rounded-sm"
              >
                ...
                <div className="flex-1 min-w-0">
                  <Link
                    ...
                    className="text-lg font-bold text-black underline-offset-8 decoration-1 tracking-tight truncate block"
                  >
                    {doc.title}
                  </Link>
                  <div className="flex items-center gap-6 mt-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                        Tác giả: {doc.author_name || "Tri thức DocLib"}
                      </span>
                    </div>
                    <div className="flex items-center gap-4">
                ...
                <Link
                  href={`/documents/${doc.slug}`}
                  className="w-12 h-12 flex items-center justify-center border border-zinc-100 rounded-sm"
                >
          ...
          <div className="py-32 flex flex-col items-center justify-center border border-dashed border-zinc-100 bg-white rounded-sm">
            ...
            <h2 className="text-2xl font-bold tracking-tighter text-black mb-3">
            ...
            <p className="text-sm font-medium text-zinc-400 mb-8 max-w-xs text-center uppercase tracking-widest leading-loose">
              Hãy thêm những tài liệu giá trị vào đây để xây dựng kho tàng tri
              thức của riêng bạn.
            </p>
            <Link href="/">
              <button className="h-14 px-12 bg-black text-white text-[11px] font-bold tracking-widest uppercase rounded-sm">
