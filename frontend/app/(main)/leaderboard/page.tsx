...
import { Trophy, Star, TrendingUp, Users, BookOpen, ChevronRight, User } from "lucide-react";
...
interface LeaderboardData {
  top_books_by_views: LeaderboardBook[];
  top_books_by_rating: LeaderboardBook[];
  top_authors: any[];
}
        ...
        {loading ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {(activeTab === "views" || activeTab === "rating") ? (
              <div className="space-y-4">
                {((activeTab === "views" ? data?.top_books_by_views : data?.top_books_by_rating) || []).map((book, index) => (
                  <div
                    key={book._id}
                  ...
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <div className="relative w-16 h-24 bg-zinc-50 border border-border  overflow-hidden flex items-center justify-center shrink-0 grayscale group-hover:grayscale-0 transition-all duration-500">
                        {book.cover_image ? (
                          <img
                            src={book.cover_image.startsWith("http") ? book.cover_image : `${process.env.NEXT_PUBLIC_API_URL}/storage/${book.cover_image}`}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-zinc-500 mt-2 text-sm font-medium">
                        Tác giả:{" "}
                        <Link href={`/profile/${book.author?.slug}`} className="text-black font-bold hover:bg-black hover:text-white px-1.5 py-0.5 transition-all">
                          {book.author?.display_name || "Vô danh"}
                        </Link>
                      </p>
                    </div>
