...
export default function ReadingHistoryPage() {
  ...
  const filteredHistory = history.filter((item) => {
    ...
    if (activeTab === "reading")
      return matchesSearch && (item.progress_percentage || 0) < 100;
      ...
      >
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <button
            ...
            className="h-14 px-8 bg-white text-zinc-400 border border-zinc-100 text-[10px] font-bold tracking-widest uppercase rounded-sm active:scale-95 transition-transform"
          >
          ...
          <div className="space-y-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-14 h-14 bg-white border border-zinc-100 focus:bg-white focus:border-black outline-none text-xs font-bold rounded-sm px-4"
              />
            </div>
          </div>
          ...
          <div className="p-8 border border-zinc-100 bg-white space-y-4 rounded-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-[11px] font-medium">
                ...
                <span className="text-black font-bold">
                  {
                    history.filter((h) => (h.progress_percentage || 0) >= 100)
                      .length
                  }
                </span>
              </div>
          ...
          {filteredHistory.length > 0 ? (
            <div className="space-y-4">
              {filteredHistory.map((item, index) => (
                <div
                  ...
                  className={`group flex flex-col md:flex-row items-start md:items-center justify-between p-8 border border-zinc-100 bg-white rounded-sm ${isDeleting === item.document_id ? "opacity-50 pointer-events-none" : ""}`}
                >
                  <div className="flex items-center gap-8 flex-1 min-w-0 w-full md:w-auto">
                    <div className="w-16 h-20 bg-white border border-zinc-100 shrink-0 overflow-hidden grayscale rounded-sm">
                      {item.cover_url ? (
                        <img
                          src={
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Link
                        ...
                        className="text-xl font-bold text-black underline-offset-4 decoration-1 tracking-tight truncate block mb-4 transition-transform"
                      >
                        {item.document_title}
                      </Link>

                      <div className="max-w-md space-y-2">
                        <div className="w-full bg-white h-1 relative overflow-hidden rounded-full">
                          <div
                  ...
                  <div className="flex items-center gap-4 mt-6 md:mt-0 ml-0 md:ml-8 w-full md:w-auto">
                    <Link
                      href={`/documents/${item.document_slug}`}
                      className="flex-1 md:flex-none bg-black text-white text-[10px] font-bold tracking-[0.2em] uppercase px-10 h-14 flex items-center justify-center active:scale-95 rounded-sm"
                    >
                      Tiếp tục đọc
                    </Link>
                    <button
                      onClick={() => handleDeleteItem(item.document_id)}
                      className="w-14 h-14 border border-zinc-100 flex items-center justify-center text-zinc-200 active:scale-95 group/trash rounded-sm"
                    >
            ...
            <div className="py-48 flex flex-col items-center justify-center border border-dashed border-zinc-100 bg-white rounded-sm">
              ...
              <p className="text-sm font-medium text-zinc-400 mb-10 max-w-xs text-center uppercase tracking-widest leading-loose">
                Bạn chưa đọc tài liệu nào gần đây. Hãy bắt đầu khám phá kho tàng
                tri thức của DocLib.
              </p>
              <Link href="/">
                <button className="h-16 px-14 bg-black text-white text-[11px] font-bold tracking-[0.2em] uppercase rounded-sm">
                  Khám phá ngay
                </button>
              </Link>
            </div>
