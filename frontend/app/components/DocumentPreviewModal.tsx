...
import { X, ChevronRight, BookOpen, Lock } from "lucide-react";
      ...
      <div className="bg-white w-full max-w-5xl h-full max-h-[85vh] shadow-[0_0_100px_rgba(0,0,0,0.3)] flex flex-col relative overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-700">
        ...
        <div className="p-8 border-b border-zinc-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h3 className="text-xl font-bold tracking-tighter uppercase">{document.title}</h3>
              <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest mt-1">Bản xem trước giới hạn</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="w-12 h-12 flex items-center justify-center hover:bg-zinc-50 transition-all active:scale-90"
          >
        ...
        {/* Content Preview */}
        <div className="flex-1 overflow-y-auto p-12 md:p-20 bg-zinc-50/20 scrollbar-none relative">
          <div className="max-w-3xl mx-auto space-y-12">
            <div className="prose prose-zinc max-w-none">
              <h2 className="text-4xl font-bold text-black mb-10 tracking-tight leading-tight">Mở đầu tri thức</h2>
              <div className="text-zinc-800 leading-relaxed text-lg font-medium space-y-6">
                {document.content ? (
                  <div dangerouslySetInnerHTML={{ __html: document.content.slice(0, 2000).replace(/\n/g, '<br/>') }} />
                ) : (
            ...
            <div className="relative h-64 mt-10">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-20 gap-6">
                ...
                <div className="text-center">
                  <p className="text-sm font-bold text-black uppercase tracking-tight mb-2">Đã đạt đến giới hạn xem trước</p>
                  <p className="text-[11px] font-bold text-zinc-400">Sở hữu tài liệu để khai mở toàn bộ kho tàng tri thức này</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="p-8 border-t border-zinc-100 bg-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
             <span className="text-[11px] font-bold text-zinc-300 uppercase tracking-widest italic">DocLib Premium Preview</span>
          </div>
          <button 
            onClick={onClose}
            className="h-14 px-10 bg-black text-white text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-zinc-800 transition-all active:scale-95 flex items-center gap-3"
          >
