...
export default function ReportModal({ itemId, itemType, onClose }: ReportModalProps) {
  ...
  const handleSubmit = async () => {
    if (!reason.trim()) {
        setNotification({ type: "error", text: "Vui lòng nhập lý do báo cáo" });
        return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setNotification({ type: "success", text: "Báo cáo đã được gửi tới hội đồng điều hành" });
        setTimeout(onClose, 2000);
      } else {
        const data = await res.json();
        setNotification({ type: "error", text: data.message || "Không thể gửi báo cáo vào lúc này" });
      }
    } catch (err) {
      setNotification({ type: "error", text: "Mất kết nối với hệ thống điều hành" });
    } finally {
