        ...
        <div className="flex justify-between items-center mt-4">
          {replyTo && (
            <button 
              ...
              className="text-[13px] font-bold text-zinc-400 hover:text-black tracking-widest transition-colors"
            >
              Hủy phản hồi
            </button>
          )}
          <button 
            type="submit" 
            className="bg-black text-white px-6 py-2.5  ml-auto text-[13px] font-bold tracking-widest hover:bg-zinc-800 transition-all flex items-center gap-2"
          >
            Gửi bình luận
            ...
            >
              ...
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7  bg-zinc-100 flex items-center justify-center text-[12px] font-bold text-black">
                ...
                <span className="text-[12px] text-zinc-400 font-bold tracking-widest">{new Date(c.created_at).toLocaleDateString("vi-VN")}</span>
              </div>
              <p className="text-zinc-700 text-sm leading-relaxed font-medium">{c.text}</p>
              <button 
                onClick={() => setReplyTo(c._id)} 
                className="text-[12px] font-bold text-zinc-400 hover:text-black mt-4 flex items-center gap-1.5 tracking-widest transition-colors"
              >
