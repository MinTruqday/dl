...
export default function ReviewSection({ documentId }: ReviewSectionProps) {
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      ...
      setReviews(Array.isArray(res.data) ? res.data : []);
        ...
        setNotification({ type: "error", text: "Không thể kết nối mạng lưới đánh giá" });
    } finally {
      setLoading(false);
    }
  }, [documentId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) {
      setNotification({ type: "error", text: "Vui lòng đăng nhập để gửi nhận xét tri thức" });
      return;
    }
    setSubmitting(true);
    try {
      await createDocumentReviewAPI(documentId, rating, comment);
      setNotification({ type: "success", text: "Đã đăng nhận xét tri thức thành công" });
      setComment("");
      setRating(5);
      loadReviews();
    } catch (e) {
      setNotification({ type: "error", text: "Gửi nhận xét thất bại, vui lòng thử lại sau" });
    } finally {
      setSubmitting(false);
    }
  };
