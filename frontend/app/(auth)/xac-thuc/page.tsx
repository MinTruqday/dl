...
import { useVerifyCode } from "../_hooks/usePasswordRecovery";

function VerifyCodeContent() {
  const email = useSearchParams().get("email") || "";
  const [token, setToken] = useState("");
  const { submitting, resending, countdown, error, verify, resend } =
    useVerifyCode(email);
  ...
  return (
    <AuthFrame
      title="Xác thực mã"
      description={
        email ? `Mã đã được gửi đến ${email}` : "Nhập mã đã nhận qua email"
      }
    >
      {error && (
        <div className="mb-5">
