...
import { BarChart3, DocumentOpen, TrendingUp, Tag, Ticket, Layers, ChevronRight, Eye, Star, Clock, Brain, CheckCircle, Image, FolderOpen, Loader2, Trash2 } from "lucide-react";
    ...
    <div className="w-full max-w-[1100px] mx-auto px-6 lg:px-8 py-12 bg-white min-h-screen animate-in fade-in duration-300">
        ...
        <div className="animate-in fade-in duration-300">
          {revenue && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
              ...
              <div className="border border-border p-5"><DocumentOpen className="w-5 h-5 text-zinc-400 mb-3" /><span className="text-3xl font-bold text-black">{revenue.total_documents || 0}</span><p className="text-[10px] text-zinc-400 font-bold tracking-widest mt-1">Tài liệu</p></div>
            </div>
          )}
          <h2 className="text-xs font-bold tracking-widest text-black mb-6 flex items-center gap-2"><DocumentOpen className="w-4 h-4" /> Tài liệu của bạn</h2>
          {documents.length === 0 ? (
            ...
            <div className="space-y-3">{documents.map((document: any) => (
              <Link key={document.id} href={`/studio?document=${document.id}`} className="group flex items-center justify-between p-5 border border-border hover:border-black transition-all duration-300">
                <div className="flex items-center gap-4 min-w-0">
                  <DocumentOpen className="w-5 h-5 text-zinc-300 group-hover:text-black transition-colors shrink-0" />
                  <div className="min-w-0">
                    <h3 className="font-bold text-black truncate group-hover:underline underline-offset-4">{document.title}</h3>
                    <div className="flex items-center gap-3 mt-1 text-[10px] text-zinc-400 font-bold tracking-widest">
                      <span className={`px-2 py-0.5 border ${document.status === "published" ? "border-black text-black" : "border-zinc-200 text-zinc-400"}`}>{document.status === "published" ? "Xuất bản" : document.status === "draft" ? "Bản nháp" : document.status}</span>
                ...
                <div className="flex items-center gap-6 text-[10px] font-bold text-zinc-400 tracking-widest shrink-0">
                  <span className="flex items-center gap-1.5"><Eye className="w-3.5 h-3.5" /> {document.views || 0}</span>
        ...
        <div className="animate-in fade-in duration-300">
          ...
          {series.length === 0 ? (
            ...
            <div className="space-y-3">{series.map((s: any) => (
              <div key={s.id} className="p-5 border border-border"><h3 className="font-bold text-black">{s.title}</h3><p className="text-sm text-zinc-500 mt-1">{s.description}</p><span className="text-[10px] text-zinc-400 font-bold tracking-widest mt-2 block">{s.document_count} tài liệu</span></div>
            ))}</div>
          )}
        </div>
      )}
        ...
        <div className="animate-in fade-in duration-300 space-y-6">
          <div className="border border-border p-6">
            ...
            <select value={selectedDocumentId} onChange={(e) => setSelectedDocumentId(e.target.value)} className="w-full px-4 py-3 border border-border text-sm focus:outline-none focus:border-black transition-all mb-4 bg-white">
              ...
              {documents.map((b) => <option key={b.id} value={b.id}>{b.title}</option>)}
            </select>
            <button onClick={analyzeSentiment} disabled={processing || !selectedDocumentId} className="w-full py-3 bg-black text-white text-[10px] font-bold tracking-widest hover:bg-zinc-800 disabled:bg-zinc-300 transition-all flex items-center justify-center gap-2">
      ...
      {activeTab === "grammar" && (
        <div className="animate-in fade-in duration-300 space-y-6">
          <div className="border border-border p-6">
            ...
            <button onClick={checkGrammar} disabled={processing || !selectedDocumentId || !selectedChapterId} className="w-full py-3 bg-black text-white text-[10px] font-bold tracking-widest hover:bg-zinc-800 disabled:bg-zinc-300 transition-all flex items-center justify-center gap-2">
      ...
      {activeTab === "cover" && (
        <div className="animate-in fade-in duration-300 border border-border p-6">
          ...
          <button onClick={generateCover} disabled={processing || !selectedDocumentId} className="w-full py-3 bg-black text-white text-[10px] font-bold tracking-widest hover:bg-zinc-800 disabled:bg-zinc-300 transition-all flex items-center justify-center gap-2">
