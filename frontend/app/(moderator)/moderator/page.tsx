...
export default function ModerationDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<ModTab>(tabFromUrl || "documents");
    ...
    const reviewDocument = async (documentId: string, status: string) => {
        try {
            ...
            setNotification({ type: "success", text: status === "PUBLISHED" ? "Đã phê duyệt tài liệu thành công." : "Đã từ chối tài liệu." });
            fetchData();
        } catch (err: any) {
            setNotification({ type: "error", text: err.message || "Lỗi thao tác phê duyệt." });
        }
    };
    ...
    const resolveReport = async (reportId: string, action: string) => {
        try {
            ...
            setNotification({ type: "success", text: "Báo cáo vi phạm đã được xử lý." });
            fetchData();
        } catch (err: any) {
            setNotification({ type: "error", text: err.message || "Lỗi xử lý báo cáo." });
        }
    };
