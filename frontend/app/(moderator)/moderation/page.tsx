            ...
            <Tabs defaultValue="documents" className="space-y-8">
                <TabsList className="bg-muted/50 p-1 rounded-sm border border-border inline-flex h-auto w-full md:w-auto overflow-x-auto whitespace-nowrap">
                    <TabsTrigger value="documents" className="gap-2 px-6 py-2.5 data-[state=active]:bg-white data-[state=active]: transition-all duration-200">
                        <Clock className="w-4 h-4" /> Duyệt tác phẩm ({pendingDocuments.length})
                    </TabsTrigger>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {pendingDocuments.length === 0 ? (
                            ...
                            pendingDocuments.map((document) => (
                                <div key={document._id} className="bg-white border border-border rounded-sm overflow-hidden flex flex-col group hover:border-foreground transition-colors duration-300">
                                    <div className="p-6 flex-1">
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1">
                                                <h3 className="text-lg font-black tracking-tight leading-tight mb-2">{document.title}</h3>
                                                <div className="flex items-center gap-4 text-[10px] font-bold text-muted-foreground tracking-widest mb-4">
                                                    <span className="flex items-center gap-1"><User className="w-3 h-3" /> {document.author_name || "Unknown"}</span>
                                                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {new Date(document.created_at).toLocaleDateString("vi-VN")}</span>
                                                </div>
                                            ...
                                            <div className="w-20 h-28 bg-muted rounded-sm flex-shrink-0 border border-border overflow-hidden">
                                                {document.cover_url ? (
                                                    <img src={document.cover_url} alt={document.title} className="w-full h-full object-cover" />
                                                ) : (
                                    ...
                                    <div className="p-4 bg-muted/20 border-t border-border flex items-center justify-between gap-2">
                                        ...
                                        <div className="flex gap-2">
                                            <Button 
                                                ...
                                                onClick={() => reviewDocument(document._id, "PUBLISHED")}
                                            ...
                                            <Button 
                                                ...
                                                onClick={() => reviewDocument(document._id, "REJECTED")}
                                                disabled={isProcessing}
                                            >
