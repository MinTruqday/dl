...
import { Heart, MessageCircle, Globe, Sparkles, Users, Lock, Share2, PlusSquare, ArrowUp, Send, CheckCircle, XCircle, X, Bookmark, DocumentText, BarChart2, Trash2, Trophy, EyeOff, Edit3, Flag, Eye, Image as ImageIcon, Quote, PenTool, Document, FileText, HelpCircle, AtSign, Pin, Archive, Link as LinkIcon } from "lucide-react";
...
function ToastContainer({ toasts, removeToast }: { toasts: any[], removeToast: (id: string) => void }) {
  return (
    <div className="fixed bottom-4 left-4 z-50 flex flex-col gap-2">
      {toasts.map((t) => (
        <div key={t.id} className={`flex items-center gap-3 px-4 py-3 rounded-sm  border text-sm font-medium transition-all animate-in slide-in-from-left-8 ${t.type === 'success' ? 'bg-background border-border text-foreground' :
            t.type === 'error' ? 'bg-destructive text-gray-700-foreground border-gray-400' :
...
export default function Feed() {
  ...
  const reactToStory = async (storyId: string) => {
    try {
      ...
      showToast("Đã thả tim story", "success");
  ...
  const handleContentChange = async (e: any) => {
    ...
    const match = val.match(/\/document\s+([^\n]+)$/);
    if (match && match[1].length > 1) {
      try {
        const res = await fetch(`${API_URL}/documents?q=${encodeURIComponent(match[1])}&limit=5`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
  ...
  const selectAttachedDocument = (document: any) => {
    setAttachedDocumentId(document.slug || document.id);
    setAttachedDocumentTitle(document.title);
    setContent(content.replace(/\/document\s+[^\n]+$/, ''));
    setDocumentSuggestions([]);
  };
  ...
  const renderContentWithTags = (text: string) => {
    ...
    return parts.map((part, i) => {
      ...
      if (ytMatch) {
        return (
          <div key={i} className="my-3 rounded-sm overflow-hidden border border-border aspect-video">
            <iframe width="100%" height="100%" src={`https:
          </div>
        );
      }
  ...
  const fetchFeed = async (reset = false) => {
    try {
      ...
      const limit = 10;
      const res = await fetch(`${API_URL}/social/feed?tab=${tab}&skip=${skip}&limit=${limit}${itemType ? `&item_type=${itemType}` : ''}`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
      if (res.ok) {
      ...
      const documentsRes = await fetch(`${API_URL}/social/suggested-documents`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
      if (documentsRes.ok) setDocumentSuggestions(await documentsRes.json());
    } catch (error) {
  ...
  const createStory = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast("Đã tạo Story thành công.", "success");
        fetchStories();
      }
    } catch(e) { console.error("API error:", e); }
  };
  ...
  const createPost = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAttachedDocumentId(""); setAttachedDocumentTitle("");
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.message === "Đã thích") {
          ...
          confetti({ particleCount: 50, spread: 60, origin: { x, y }, colors: ['#ff0000', '#ffc0cb', '#ff69b4'], disableForReducedMotion: true });
        }
        fetchFeed(true);
      }
  ...
  const handleVote = async (postId: string, amount: number) => {
    try {
      ...
      if (res.ok) {
        showToast((await res.json()).message || `Đã gửi tặng ${amount} Coin thành công.`, "success");
  ...
  const repostPost = async (postId: string) => {
    try {
      ...
      if (res.ok) { showToast("Đã chia sẻ (Repost) bài viết thành công!", "success"); fetchFeed(true); } else showToast("Chia sẻ thất bại", "error");
    } catch(e) { console.error("API error:", e); }
  };
  ...
  const reportPost = async (postId: string) => {
    const reason = prompt("Vui lòng nhập lý do báo cáo để Moderator xem xét:");
  ...
  const updatePost = async (postId: string) => {
    ...
    try { const res = await fetch(`${API_URL}/social/posts/${postId}`, { method: "PUT", headers: { 'Authorization': `Bearer ${getToken()}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ content: editingContent }) }); if (res.ok) { showToast("Cập nhật thành công", "success"); setEditingPostId(null); fetchFeed(true); } } catch(e) { console.error("API error:", e); }
  };
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="flex gap-4 border-b border-border text-sm font-semibold">
            <div 
              ...
              className={`pb-3 cursor-pointer transition-all ${tab === 'foryou' ? 'border-b-2 border-foreground text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Dành cho bạn
            </div>
            <div 
              onClick={() => setTab("following")}
              className={`pb-3 cursor-pointer transition-all ${tab === 'following' ? 'border-b-2 border-foreground text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Đang theo dõi
            </div>
          </div>
          ...
          {currentUser && (
            <div className="flex gap-2 overflow-x-auto pb-6 pt-2 hide-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
              <div 
                ...
                className="relative w-28 h-48 rounded-sm overflow-hidden cursor-pointer shrink-0 group bg-card border border-border  flex flex-col hover:opacity-90 transition-opacity"
              >
                <div className="flex-1 bg-muted relative overflow-hidden group-hover:brightness-95 transition-all">
                  {currentUser?.avatar_url ? (
                    <img src={currentUser.avatar_url} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full bg-secondary flex items-center justify-center font-bold text-3xl text-secondary-foreground">
                      {currentUser?.email?.[0]?.toUpperCase() || "U"}
                    </div>
                  )}
                </div>
                <div className="h-[46px] bg-card flex flex-col justify-end pb-2 items-center relative">
                  <div className="absolute -top-4 w-9 h-9 bg-primary text-white rounded-full flex items-center justify-center border-4 border-card  z-10">
                    <PlusSquare className="w-5 h-5" />
                  </div>
                  <span className="text-[12px] font-semibold text-foreground mt-4">Tạo tin</span>
                </div>
              </div>
              ...
              {stories.map((story, idx) => (
                <div 
                  key={story.id} 
                  className="relative w-28 h-48 rounded-sm overflow-hidden cursor-pointer shrink-0 group bg-black border border-border  flex flex-col"
                  onClick={() => { setActiveStoryIndex(idx); setViewingStoryMode(true); setStoryProgress(0); }}
                >
                  <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-black/60 z-10"></div>
                  
                  {story.media_url ? (
                    <img src={story.media_url} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 pointer-events-none" />
                  ) : (
                    <div 
                      className="absolute inset-0 flex items-center justify-center p-2 text-center text-white" 
                      style={{ backgroundColor: story.background_color || '#18181b' }}
                    >
                      <h3 className="z-20 font-bold text-[11px] drop- break-words line-clamp-6">{story.text_content}</h3>
                    </div>
                  )}

                  <div className="absolute top-2 left-2 z-20 w-8 h-8 rounded-full border border-border overflow-hidden bg-muted">
                    {story.user?.avatar_url ? (
                      <img src={story.user.avatar_url} className="w-full h-full object-cover rounded-full" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center bg-secondary font-bold text-[13px] text-foreground rounded-full">
                        {story.user?.name?.[0]?.toUpperCase() || "A"}
                      </div>
                    )}
                  </div>
                  <span className="absolute bottom-2 left-2 z-20 text-[12px] font-semibold text-white drop- truncate w-[90%]">{story.user?.name || 'Ai đó'}</span>
                </div>
              ))}
            </div>
          )}
...
{}
          {currentUser && (
            <div className="bg-card border border-border rounded-sm p-4  flex flex-col mb-4">
              <div className="flex gap-3 items-start p-1">
                <div className="w-10 h-10 bg-secondary rounded-full border border-border flex shrink-0 items-center justify-center text-secondary-foreground font-bold text-base overflow-hidden relative cursor-pointer hover:brightness-95 transition-all">
                  {currentUser?.avatar_url ? (
                    <img src={currentUser.avatar_url} className="w-full h-full object-cover" />
                  ) : (
                    currentUser?.email?.[0]?.toUpperCase() || "U"
                ...
                <div className="flex-1">
                  ...
                  {mediaUrls.length > 0 && (
                    <div className="grid grid-cols-2 gap-1 mt-2 rounded-sm overflow-hidden border border-border">
                      {mediaUrls.map((url, i) => (
                        <div key={i} className={`relative w-full aspect-square ${mediaUrls.length === 1 ? 'col-span-2 aspect-video' : ''}`}>
                          ...
                          <button onClick={() => setMediaUrls(mediaUrls.filter((_, idx) => idx !== i))} className="absolute top-2 right-2 bg-black/60 hover:bg-black/80 text-white rounded-full w-8 h-8 flex items-center justify-center  backdrop-blur-sm transition-colors">
                            <X className="w-5 h-5"/>
                          </button>
                        </div>
                      ))}
                  ...
                  {documentSuggestions.length > 0 && (
                    <div className="absolute z-50 bg-background border border-border rounded-sm  mt-1 overflow-hidden w-full max-w-md">
                      <div className="p-2 bg-muted/50 text-xs font-semibold text-muted-foreground border-b border-border">Đính kèm sách</div>
                      {documentSuggestions.map((document: any, i: number) => (
                        <div key={i} className="px-4 py-2 hover:bg-muted cursor-pointer text-sm font-medium flex justify-between" onClick={() => selectAttachedDocument(document)}>
                          {document.title} 
                          <span className="text-muted-foreground text-xs">{document.author}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {showExtras && (
                <div className="mt-3 p-4 bg-muted/30 rounded-sm space-y-3 border border-border">
              ...
              <div className="mt-3 pt-3 border-t border-border">
                <div className="flex items-center justify-between border border-border rounded-sm p-2  font-semibold text-sm">
                  ...
                  <div className="flex gap-1 justify-end flex-1">
                    <label className="cursor-pointer p-2 hover:bg-muted rounded-full transition-colors group flex items-center justify-center gap-2" title="Đính kèm Ảnh/Video">
                       <ImageIcon className="w-6 h-6 text-foreground scale-100 group-hover:scale-110 transition-transform" />
                       <input type="file" className="hidden" accept="image}
          {currentUser && (
            <>
              <div className="bg-muted text-muted-foreground text-sm py-2 px-4 rounded-sm flex items-center justify-between border border-border">
                <span className="font-medium">Tóm tắt bảng tin hôm nay với AI?</span>
                <button 
                  onClick={async () => {
                    try { showToast("Đang tóm tắt feed", "info"); const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/social/ai/feed-summary`, { headers: { 'Authorization': `Bearer ${getToken()}`}}); const data = await res.json(); if(data.summary) { console.log(data.summary); console.log(true); } } catch (e) { showToast("Lỗi tóm tắt AI", "error"); }
                  }}
          ...
          <div className="flex flex-col gap-5 w-full">
            {loading ? (
              ...
              <div className="text-center p-10 text-muted-foreground border border-border border-dashed rounded-sm">Chưa có bài viết nào trong bảng tin.</div>
            ) : posts.map(post => (
              <div key={post.id} className="bg-card border border-border rounded-sm p-5  ">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-secondary rounded-full flex shrink-0 items-center justify-center text-secondary-foreground font-bold border border-border overflow-hidden">
                    {post.user?.avatar_url ? (
                      <img src={post.user.avatar_url} className="w-full h-full object-cover rounded-full" />
                    ) : (
                  ...
                  {currentUser?._id && currentUser._id === post.author_id ? (
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => deletePost(post.id)} title="Xóa bài viết" className="text-muted-foreground opacity-50 hover:opacity-100 transition-all hover:text-red-600">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => reportPost(post.id)} title="Báo cáo vi phạm" className="text-muted-foreground opacity-50 hover:opacity-100 hover:text-red-600 transition-all">
                ...
                {editingPostId === post.id ? (
                  <div className="mb-4">
                     <textarea
                      className="w-full bg-background border border-border outline-none p-3 rounded-sm text-foreground resize-y h-24 focus:ring-1 focus:ring-black dark:ring-white transition-all text-sm mb-2"
                    ...
                    <>
                      ...
                      {post.is_locked && (
                        <div className="absolute inset-0 bg-background/60 backdrop-blur-md flex flex-col items-center justify-center z-10 rounded-sm">
                          ...
                          <Button onClick={async () => {
                              try {
                                 ...
                                 if(res.ok) { fetchFeed(true); fetchWallet(); } else { showToast("Mở khóa thất bại, hãy nạp thêm Coin.", "error"); }
                              } catch(e) { console.error("API error:", e); }
                          }}> Mở khóa với {post.price} Coin</Button>
                        </div>
                      )}
                      {post.read_progress && (
                        <div className="mt-3 bg-muted/30 rounded-sm p-2">
                          <div className="flex justify-between text-xs text-muted-foreground mb-1">
                            <span>Tiến độ đọc:</span>
                            <span className="font-bold text-black dark:text-white">{post.read_progress}%</span>
                          </div>
                          <div className="w-full bg-secondary rounded-full h-1.5 overflow-hidden">
                             <div className="bg-black dark:bg-white h-1.5 rounded-full" style={{ width: `${post.read_progress}%` }}></div>
                          </div>
                        </div>
                      )}
                    </>
                ...
                {post.media_urls && post.media_urls.length > 0 && (
                  <div className="mt-3 flex overflow-x-auto snap-x snap-mandatory gap-2 pb-2 hide-scrollbar rounded-sm">
                    {post.media_urls.map((url: string, i: number) => (
                      <div key={i} className="relative aspect-video shrink-0 bg-muted snap-center w-[85%] rounded-sm overflow-hidden border border-border">
                        {url.match(/\.(mp4|webm)$/i) ? (
                ...
                {post.document_id && (
                  <div className="mt-4 p-4 border border-border bg-muted/30 rounded-sm cursor-pointer border-l-4 border-l-primary hover:bg-muted/50 transition-colors flex items-center justify-between">
                    <div>
                      <span className="text-xs font-semibold text-muted-foreground tracking-wider flex items-center"><DocumentText className="w-4 h-4 mr-2" /> Tác phẩm đính kèm</span>
                      <h5 className="font-semibold mt-1 text-sm text-foreground truncate">{post.document_title || post.document_id} {post.document_title ? `<${post.document_id}>` : ""}</h5>
                    </div>
                ...
                {post.poll_options && post.poll_options.length > 0 && (
                  <div className="mt-4 space-y-2">
                     {post.poll_options.map((opt: any) => {
                       ...
                       return (
                         <div key={opt.id} onClick={() => { if(currentUser) submitPollVote(post.id, opt.id); else showToast("Vui lòng đăng nhập để thực hiện.", "error"); }} className={`relative overflow-hidden cursor-pointer rounded-sm border ${isVoted ? 'border-foreground ring-1 ring-black dark:ring-white/20' : 'border-border'} bg-muted/30 p-3 hover:bg-muted transition-colors`}>
                           <div className="absolute top-0 left-0 bottom-0 bg-black dark:bg-white/10 transition-all duration-500 rounded-sm" style={{ width: `${percentage}%` }}></div>
                ...
                <div className="mt-5 border-t border-border pt-4 flex gap-2 text-sm items-center">
                  <div className="relative group flex items-center">
                    <Button variant="ghost" size="sm" onClick={(e) => { if(currentUser) toggleLike(post.id, "like", e); else showToast("Vui lòng đăng nhập để thực hiện.", "error"); }} className={`text-muted-foreground hover:text-foreground ${post.likes?.includes(currentUser?._id) ? 'text-red-500 font-semibold' : ''}`}>
                      <Heart className={`w-4 h-4 mr-1.5 transition-colors ${post.likes?.includes(currentUser?._id) ? 'fill-red-500 text-red-500' : 'hover:text-red-500'}`} /> {post.likes?.length || 0}
                    </Button>
                    {currentUser && (
                      <div className="absolute bottom-full left-0 mb-1 hidden group-hover:flex bg-background border border-border rounded-full  p-1 gap-0.5 z-10">
                        <button onClick={(e) => toggleLike(post.id, 'like', e)} className="p-1.5 hover:bg-muted rounded-full transition-transform hover:scale-110 text-base" title="Tim">❤️</button>
                        <button onClick={(e) => toggleLike(post.id, 'lightbulb', e)} className="p-1.5 hover:bg-muted rounded-full transition-transform hover:scale-110 text-base" title="Sáng kiến">💡</button>
                        <button onClick={(e) => toggleLike(post.id, 'fire', e)} className="p-1.5 hover:bg-muted rounded-full transition-transform hover:scale-110 text-base" title="Lửa">🔥</button>
                        <button onClick={(e) => toggleLike(post.id, 'laugh', e)} className="p-1.5 hover:bg-muted rounded-full transition-transform hover:scale-110 text-base" title="Haha">😂</button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 ml-auto">
                    <Button variant="ghost" size="sm" onClick={() => { if(currentUser) handleVote(post.id, 50); else showToast("Vui lòng đăng nhập để thực hiện.", "error"); }} className="text-muted-foreground hover:text-foreground" title="Tặng 50 Coin">
                      <ArrowUp className="w-4 h-4 mr-1" /> 50
                    </Button>
                ...
                {expandedComments === post.id && (
                  <div className="mt-4 bg-muted/30 rounded-sm p-4 border border-border animate-in slide-in-from-top-2">
                    ...
                    {currentUser ? (
                      <>
                        {replyToContext && replyToContext.postId === post.id && (
                          <div className="text-xs text-muted-foreground mb-2 flex justify-between bg-muted/50 p-2 rounded-sm">
                        ...
                        <div className="flex gap-2 items-center pt-2">
                          {post.comment_privacy === 'private' && currentUser?._id !== post.author_id ? (
                            <div className="flex-1 text-xs text-muted-foreground italic py-2 px-4 bg-muted rounded-full border border-border text-center">
                      ...
                      <div className="text-xs text-center text-muted-foreground mt-2 bg-muted py-2 rounded-sm border border-border">
            ...
            {!loading && hasMore && (
              <div className="flex justify-center p-4">
                <Button variant="outline" size="sm" onClick={() => fetchFeed()} disabled={loading} className="text-xs font-semibold px-6 py-2 rounded-full border-border hover:bg-muted text-muted-foreground transition-colors">
                  Tải thêm bài viết
                </Button>
              </div>
            )}
        ...
        <div className="hidden lg:col-span-4 lg:flex flex-col gap-4 sticky top-6 self-start">
          ...
          {currentUser && (
            <div className="bg-card border border-border rounded-sm p-5  text-center">
              ...
              <p className="text-3xl font-black text-foreground">{walletBalance.toLocaleString("vi-VN")} <span className="text-sm font-bold text-muted-foreground">Coin</span></p>
              <Button size="sm" variant="outline" className="mt-4 w-full" onClick={() => window.location.href = '/wallet'}>Quản lý ví</Button>
            </div>
          )}

          <div className="bg-card border border-border rounded-sm p-5 ">
            <h3 className="text-xs font-bold text-foreground tracking-wider mb-4 border-b border-border pb-3">Bảng Xếp Hạng Coin</h3>
            
            {ranking.length === 0 ? (
              <p className="text-sm text-muted-foreground mb-2">Chưa có ai trong top Donate</p>
            ) : ranking.map((r, i) => (
              <div key={i} className="flex gap-3 items-center group border-b border-border last:border-0 pb-3 mb-3 last:pb-0 last:mb-0">
                <div className="w-8 h-8 rounded-full bg-secondary text-secondary-foreground font-bold flex items-center shrink-0 justify-center text-xs border border-border">
                  #{i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-sm text-foreground truncate">{r.display_name || "Ẩn danh"}</h4>
                  <span className="text-xs text-gray-600 font-bold truncate"> {r.total_coins || 0} Coins</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-card border border-border rounded-sm p-5 ">
            <h3 className="text-xs font-bold text-foreground tracking-wider mb-4 border-b border-border pb-3">Sách Đáng Đọc Tuần Này</h3>
            {documentSuggestions.length === 0 ? (
              <p className="text-sm text-muted-foreground">Chưa có gợi ý sách nào.</p>
            ) : (
              <div className="space-y-4 pt-1">
                {documentSuggestions.map((b, i) => (
                  <div key={i} className="flex gap-3 items-center group cursor-pointer">
                    <div className="w-10 h-14 bg-muted border border-border rounded  shrink-0 flex items-center justify-center overflow-hidden">
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm text-foreground truncate group-hover:text-black dark:text-white transition-colors">{b.title}</h4>
                      <span className="text-xs text-muted-foreground">{b.mentions} người nhắc đến</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-card border border-border rounded-sm p-5 ">
            <h3 className="text-xs font-bold text-foreground tracking-wider mb-4 border-b border-border pb-3">Hashtag Thịnh Hành</h3>
            {trendingTags.length === 0 ? (
              <p className="text-sm text-muted-foreground">Chưa có xu hướng nào.</p>
            ) : (
              <div className="space-y-3">
                {trendingTags.map((t, i) => (
                  <div key={i} className="flex justify-between items-center group cursor-pointer">
                    <span className="text-sm text-muted-foreground group-hover:text-black dark:text-white transition-colors font-medium">#{t.tag}</span>
                    <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{t.count}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-card border border-border rounded-sm p-5 ">
            <h3 className="text-xs font-bold text-foreground tracking-wider mb-4 border-b border-border pb-3">Gợi Ý Bạn bè</h3>
            {suggestions.length === 0 ? (
              <p className="text-sm text-muted-foreground leading-relaxed">Hãy cập nhật sở thích thẻ Bookmark để tìm vòng tròn bạn bè cùng gu sách.</p>
            ) : suggestions.map((s, i) => (
              <div key={i} className="flex gap-3 items-center group cursor-pointer border-b border-border last:border-0 pb-3 last:pb-0 mb-3 last:mb-0">
                <div className="w-10 h-10 rounded-full bg-secondary text-secondary-foreground font-bold flex items-center shrink-0 justify-center text-sm border-border border">
                  {s.display_name?.[0]?.toUpperCase() || "A"}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-sm text-foreground truncate">{s.display_name || "Ẩn danh"}</h4>
                  <span className="text-xs text-muted-foreground truncate">{s.total_match || 0} điểm Bookmark chung</span>
                </div>
                <Button size="sm" variant="outline" onClick={() => { if(currentUser) followUser(s._id); else showToast("Vui lòng đăng nhập để thực hiện.", "error"); }} title="Theo dõi" className="h-7 px-3 text-xs rounded-full shrink-0 hover:bg-foreground hover:text-background transition-all">
                  Theo dõi
                </Button>
              </div>
            ))}
      ...
      {currentUser && showStoryModal && (
        <div className="fixed inset-0 z-[300] bg-background/80 backdrop-blur-sm flex items-center justify-center animate-in fade-in-0 duration-200">
          <div className="bg-card w-full h-[100dvh] md:h-[85vh] max-w-sm md:rounded-sm  md:border border-border flex flex-col relative overflow-hidden">
            {}
            <div className="absolute z-10 top-0 left-0 right-0 p-3 flex justify-between items-center bg-gradient-to-b from-black/60 to-transparent text-white">
              <div className="flex items-center gap-2">
                <button onClick={() => setShowStoryModal(false)} className="p-2 backdrop-blur-md bg-black/20 hover:bg-black/40 rounded-full transition-colors">
                  <X className="w-5 h-5" />
                </button>
                <button
                  onClick={() => { setShowStoryArchive(!showStoryArchive); if (!showStoryArchive) fetchArchivedStories(); }}
                  className={`p-2 backdrop-blur-md rounded-full transition-colors ${showStoryArchive ? 'bg-white/30' : 'bg-black/20 hover:bg-black/40'}`}
              ...
              <div className="flex gap-2 items-center">
                <select
                  ...
                  className="bg-black/20 text-white text-xs px-3 py-1.5 rounded-full backdrop-blur-md outline-none cursor-pointer hover:bg-black/40"
                >
            ...
            {}
            {showStoryArchive && (
              <div className="absolute z-20 top-16 left-0 right-0 bottom-0 bg-card/95 backdrop-blur-md overflow-y-auto p-4">
                ...
                {archivedStories.length === 0 ? (
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {archivedStories.map((s, i) => (
                      <div
                        key={i}
                        className="aspect-[9/16] rounded-sm overflow-hidden relative border border-border cursor-pointer group"
            ...
            >
              ...
              {storyLinkUrl && (
                <div className="mt-4 px-4 py-2 bg-white/20 backdrop-blur-md rounded-full border border-white/20 flex gap-2 items-center max-w-[80%] z-10 ">
              ...
              {storyAddPoll && (
                <div className="mt-6 w-full max-w-[280px] bg-black/40 backdrop-blur-md rounded-sm border border-white/20 p-4 z-10  flex flex-col gap-3">
                  ...
                  {storyPollOptions.map((opt, idx) => (
                    <input
                      ...
                      className="w-full bg-white/10 rounded-sm text-white text-sm border border-white/20 outline-none px-3 py-2 placeholder:text-white/50 focus:bg-white/20 transition-all font-medium text-center"
                    />
                  ))}
                  {storyPollOptions.length < 4 && (
                    <button 
                      onClick={() => setStoryPollOptions([...storyPollOptions, ""])} 
                      className="text-white/70 text-xs hover:text-white font-medium py-1"
                    >
                      + Thêm lựa chọn
                    </button>
                  )}
                  ...
                  {storyQuizOptions.map((opt, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                       <button
                         ...
                         className={`w-6 h-6 flex items-center justify-center rounded-full border ${storyQuizCorrectIdx === idx ? 'bg-green-500 border-green-500 text-white' : 'bg-white/10 border-white/30'}`}
                       >
            ...
            <div className="bg-card w-full border-t border-border p-3 flex flex-col gap-2.5 z-10">
                ...
                <div className="flex items-center gap-2 animate-in slide-in-from-bottom-2">
                  <div className="flex-1 relative flex items-center">
                    ...
                    <Input
                      ...
                      className="pl-9 h-9 w-full bg-muted/50 border-border text-sm rounded-full"
                      value={storyLinkUrl}
              ...
              {}
              <div className="flex items-center gap-2">
                <label className="cursor-pointer h-10 w-10 flex items-center justify-center bg-muted/50 hover:bg-muted rounded-full transition-colors border border-border shrink-0" title="Ảnh / Video">
                  {isStoryUploading ? <div className="w-4 h-4 rounded-full border-2 border-foreground border-t-transparent animate-spin"/> : <ImageIcon className="w-5 h-5 text-foreground" />}
                  <input type="file" className="hidden" accept="image}
      {viewingStoryMode && activeStoryIndex >= 0 && stories[activeStoryIndex] && (
            <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-sm flex justify-center items-center animate-in fade-in-0 duration-200 text-white">
              <div className="absolute top-4 right-4 z-[210] flex gap-4 hidden md:flex">
                <button onClick={() => { setViewingStoryMode(false); setStoryProgress(0); }} className="text-white hover:text-gray-300 p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors  backdrop-blur-md">
                  <X className="w-6 h-6"/>
                </button>
              </div>
              
              <div className="flex-1 flex flex-col justify-between items-center relative overflow-hidden w-full max-w-sm mx-auto h-[100dvh] md:h-[85vh] md:w-[400px] group md:rounded-sm  md:border border-border/50"
                 ...
                 <div className="absolute top-0 left-0 right-0 px-2 pt-2 flex gap-1 z-[205] w-full bg-gradient-to-b from-black/50 to-transparent pb-4">
                    {stories.map((s, idx) => (
                      <div key={s.id} className="flex-1 h-[3px] bg-white/30 rounded-full overflow-hidden backdrop-blur-md">
                        <div 
                 ...
                 {}
                 <button onClick={() => { setViewingStoryMode(false); setStoryProgress(0); }} className="absolute top-4 right-4 z-[210] text-white hover:text-gray-300 p-1 bg-black/20 hover:bg-black/40 rounded-full transition-colors  backdrop-blur-md md:hidden">
                    <X className="w-5 h-5"/>
                 </button>
                    ...
                    <div className="absolute inset-0 w-full h-full flex flex-col justify-center items-center">
                      ...
                      <div className="w-full px-6 flex-1 flex flex-col gap-6 justify-center items-center overflow-hidden z-10">
                        {stories[activeStoryIndex].text_content && (
                          <h2 className="text-2xl font-bold text-center max-w-full leading-snug drop- break-words mb-4" 
                              style={{ 
                        ...
                        {stories[activeStoryIndex].link_url && (
                            <a 
                              ...
                              className="flex items-center gap-2 px-5 py-2.5 bg-white/20 hover:bg-white/30 backdrop-blur-md rounded-full text-white font-semibold  transition-all border border-white/20 max-w-[80%]"
                        ...
                        {stories[activeStoryIndex].poll_data ? (
                          <div className="w-full max-w-[280px] bg-black/40 backdrop-blur-md rounded-sm border border-white/20 p-4 z-10  flex flex-col gap-2 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                            ...
                            {stories[activeStoryIndex].poll_data.options.map((opt: string, idx: number) => {
                              ...
                              return (
                                <button 
                                  ...
                                  className={`relative w-full rounded-sm text-white text-sm border overflow-hidden font-medium transition-all ${myVote === idx ? 'border-primary bg-primary/20' : 'border-white/20 bg-white/10'} ${(hasVoted) ? 'cursor-default' : 'hover:bg-white/20 cursor-pointer'} `}
                                >
                          ...
                          <div className="w-full max-w-[280px] bg-black/40 backdrop-blur-md rounded-sm border border-white/20 p-4 z-10  flex flex-col gap-2 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                            ...
                            {stories[activeStoryIndex].quiz_data.options.map((opt: string, idx: number) => {
                              ...
                              if (hasAnswered) {
                                buttonClass = isCorrect ? 'border-green-500 bg-green-500/20 cursor-default' : (myAnswer === idx ? 'border-red-500 bg-red-500/20 cursor-default' : 'border-white/20 bg-black/20 opacity-50 cursor-default');
                              }
                              ...
                              return (
                                <button 
                                  ...
                                  className={`relative w-full rounded-sm text-white text-sm border overflow-hidden font-medium transition-all ${buttonClass}`}
                                >
                                  <div className="relative px-3 py-2.5 flex justify-between items-center z-10">
                                    <span className="truncate pr-2">{opt}</span>
                                    {hasAnswered && isCorrect && <CheckCircle className="w-4 h-4 text-green-400" />}
                                    {hasAnswered && !isCorrect && myAnswer === idx && <XCircle className="w-4 h-4 text-red-400" />}
                                  </div>
                                </button>
                              );
                            })}
                 ...
                 <div className="absolute top-6 left-4 flex gap-2.5 items-center z-[210] p-1.5 pr-4 max-w-[80%]">
                   <div className="w-10 h-10 rounded-full flex justify-center items-center overflow-hidden shrink-0 bg-secondary relative">
                      {stories[activeStoryIndex].user?.avatar_url ? (
                        <img src={stories[activeStoryIndex].user.avatar_url} className="w-full h-full object-cover rounded-full" />
                      ) : (
                        <span className="text-foreground font-bold text-sm bg-muted/50 w-full h-full flex justify-center items-center rounded-full backdrop-blur-md">
                          {stories[activeStoryIndex].user?.name?.[0]?.toUpperCase() || "A"}
                        </span>
                      )}
                   </div>
                   <div className="flex flex-col justify-center">
                     <span className="text-sm font-semibold tracking-tight text-white drop-">
                     ...
                     <span className="text-[10px] font-medium opacity-80 text-white drop-">{new Date(stories[activeStoryIndex].created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                   </div>
                 </div>
                 
                 {!stories[activeStoryIndex].media_url && (
                   <div className="w-full px-6 flex-1 flex flex-col gap-6 justify-center items-center overflow-hidden z-10">
                     <h2 className="text-2xl font-bold text-center max-w-full leading-snug drop- break-words" 
                        style={{ 
                          color: stories[activeStoryIndex].text_color || '#ffffff',
                          fontFamily: stories[activeStoryIndex].font_style === 'mono' ? 'Courier New, monospace' : 'inherit' 
                        }}>
                 ...
                 <div className="absolute bottom-4 left-0 right-0 w-full px-4 z-[205] flex justify-between items-center gap-3">
                    {(stories[activeStoryIndex].user_id === currentUser?._id || stories[activeStoryIndex].user_id === currentUser?.id) ? (
                      <div className="bg-black/40 backdrop-blur-md rounded-full px-4 py-2.5 text-sm text-white border border-white/20 w-full flex justify-between items-center cursor-pointer hover:bg-black/50 transition-colors">
                        <span className="flex items-center justify-center gap-2 font-medium">
                          <Eye className="w-4 h-4" /> 
                          {stories[activeStoryIndex]?.viewers?.length || 0} lượt xem
                        </span>
                        {stories[activeStoryIndex]?.viewers?.length > 0 && (
                          <div className="flex -space-x-1.5">
                            {(stories[activeStoryIndex]?.viewers?.slice(0, 3) || []).map((v: any, i: number) => (
                               <div key={i} className="w-6 h-6 rounded-full bg-white/20 border border-white/40 overflow-hidden flex items-center justify-center text-[10px] font-bold">
                                 {i===0 ? "A" : "B"}
                               </div>
                            ))}
                          </div>
                      ...
                      <>
                        <div className="flex-1 relative flex items-center">
                          <input 
                            type="text" 
                            placeholder={`Trả lời ${stories[activeStoryIndex].user?.name || 'Story'}...`} 
                            className="w-full bg-black/30 border border-white/30 rounded-full px-4 py-2.5 pr-12 text-sm text-white placeholder-white/70 outline-none focus:bg-black/50 focus:border-white/50 backdrop-blur-md transition-all"
                            value={replyMessage}
                          ...
                          {replyMessage.trim() && (
                            <button 
                              ...
                              disabled={isReplying}
                              className="absolute right-2 p-1.5 text-primary hover:text-primary/80 transition-colors"
                            >
                              <Send className="w-4 h-4" />
                            </button>
                          )}
                        ...
                        <button 
                          ...
                          className="text-white hover:scale-105 active:scale-95 transition-transform bg-white/10 hover:bg-white/20 p-2.5 rounded-full"
                        >
      ...
      {}
      {translationModal && (
        <div className="fixed inset-0 z-[400] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center animate-in fade-in-0 duration-150 p-4">
          <div className="bg-card border border-border rounded-sm w-full max-w-lg  animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <h3 className="font-semibold text-sm text-foreground">Bản dịch tự động</h3>
              <button onClick={() => setTranslationModal(null)} className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded-sm hover:bg-muted">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-5">
            ...
            <div className="px-5 py-3 border-t border-border flex justify-end">
              <button onClick={() => setTranslationModal(null)} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-4 py-1.5 rounded-sm hover:bg-muted">
                Đóng
              </button>
            </div>
          </div>
