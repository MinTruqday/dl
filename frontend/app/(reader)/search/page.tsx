...
import { Search, Filter, DocumentOpen, User, Clock, Star, Trash2, X } from "lucide-react";
          ...
          <div className="grid grid-cols-1 gap-6">
            {loading ? (
              ...
              results.map((document) => (
                <Link key={document._id} href={`/preview?slug=${document.slug}`} className="group p-6 bg-white border border-border hover:border-black transition-all duration-300 flex gap-6">
                  <div className="w-24 h-32 bg-zinc-100 border border-border shrink-0 overflow-hidden">
                    {document.cover_url ? (
                       <img src={document.cover_url} alt={document.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3 text-[9px] font-bold tracking-widest text-zinc-400">
                      <span className="flex items-center gap-1"><DocumentOpen className="w-3.5 h-3.5" /> {document.categories?.[0] || "Tài liệu"}</span>
                      <span className="w-1 h-1 rounded-none bg-zinc-200" />
                      <span className="flex items-center gap-1"><User className="w-3.5 h-3.5" /> {document.author_name || "Tác giả ẩn danh"}</span>
                    </div>
                    <h3 className="text-lg font-black tracking-tight group-hover:underline underline-offset-4 decoration-2">{document.title}</h3>
                    ...
                    <div className="pt-2 flex items-center gap-6">
                       <div className="flex items-center gap-1.5 text-[9px] font-bold tracking-widest text-zinc-300">
                          <Clock className="w-3.5 h-3.5" /> {new Date(document.created_at).toLocaleDateString("vi-VN")}
                       </div>
                       <div className="flex items-center gap-1.5 text-[9px] font-bold tracking-widest text-black">
                          <Star className="w-3.5 h-3.5" /> {document.average_rating?.toFixed(1) || "0.0"}
                       </div>
                    </div>
                  </div>
                </Link>
