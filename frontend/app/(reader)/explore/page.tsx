...
import { getDocumentsAPI, getTagsCategoriesAPI } from "@/app/lib/api";
          ...
          <div className="p-6 bg-zinc-50 border border-border space-y-4">
             ...
             <div className="space-y-4">
                {trending.map((document, i) => (
                   <Link key={document._id} href={`/preview?slug=${document.slug}`} className="group cursor-pointer block">
                      <div className="text-[10px] font-bold text-zinc-400 mb-1">0{i+1}</div>
                      <h4 className="text-xs font-bold leading-tight group-hover:underline underline-offset-4">{document.title}</h4>
                   </Link>
                ))}
             </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {documents.map((document) => (
                <Link key={document._id} href={`/preview?slug=${document.slug}`} className="group space-y-4">
                  <div className="aspect-[3/4] bg-zinc-100 border border-border relative overflow-hidden">
                    {document.cover_url ? (
                      <img src={document.cover_url} alt={document.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    ) : (
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                       <span className="text-[9px] font-bold tracking-widest text-zinc-400">{document.categories?.[0] || "Tổng hợp"}</span>
                       <span className="text-[9px] font-bold tracking-widest text-zinc-400">{document.views || 0} lượt xem</span>
                    </div>
                    <h3 className="text-sm font-bold leading-tight group-hover:underline underline-offset-4 decoration-1">{document.title}</h3>
                    <p className="text-[11px] text-muted-foreground line-clamp-2 leading-relaxed">{document.description}</p>
                  </div>
                </Link>
              ))}
            </div>
