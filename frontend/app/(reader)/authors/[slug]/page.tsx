...
export default function AuthorProfilePage() {
  ...
  const fetchAuthorProfile = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const docRes = await fetch(`${API_URL}/documents?author_slug=${slug}`, {
          headers: { Authorization: `Bearer ${getToken()}` }
        });
        if (docRes.ok) {
        ...
        setError(data.message || "Không thể tải thông tin tác giả");
      }
    } catch (err: any) {
  ...
  const toggleFollow = async () => {
    if (!author) return;
    try {
      const res = await fetch(`${API_URL}/social/users/${author._id || author.id || slug}/follow`, {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` }
      });
      if (res.ok) {
        ...
        console.error("Lỗi thực hiện theo dõi");
      }
    } catch (err: any) {
