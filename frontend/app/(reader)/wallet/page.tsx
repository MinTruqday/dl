...
export default function WalletPage() {
  ...
  const handleRedeemVoucher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotification({ type: "error", text: error.message || "Voucher không hợp lệ hoặc đã hết hạn." });
    } finally {
  ...
  const handleTopup = async () => {
    if (topupAmount < 10000) {
      setNotification({ type: "error", text: "Số tiền nạp tối thiểu là 10.000 VNĐ." });
      return;
    }
    ...
    try {
      ...
      if (res.data?.payment_url || res.payUrl) {
        ...
        setNotification({ type: "error", text: "Không thể khởi tạo thanh toán lúc này." });
      }
    } catch (e: any) {
