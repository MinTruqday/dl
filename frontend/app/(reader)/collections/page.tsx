    ...
    <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 py-12 font-sans text-black selection:bg-black selection:text-white">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-3">
            ...
            <p className="text-zinc-400 text-sm font-bold uppercase tracking-widest flex items-center gap-2">
              Knowledge Repository <Sparkles className="w-3.5 h-3.5 text-zinc-100" />
            </p>
          </div>
          
          <Button className="h-16 px-12 bg-black text-white text-[11px] font-bold tracking-[0.2em] uppercase hover:bg-zinc-800 transition-all active:scale-95 flex items-center gap-4 rounded-none shadow-xl shadow-black/5">
      ...
      <div className="grid lg:grid-cols-12 gap-12">
        ...
        >
            ...
            <nav className="flex flex-col gap-1">
              {[
                ...
                <button
                  ...
                  className={`text-left px-6 py-4 text-[11px] font-bold uppercase tracking-widest transition-all border ${
                    activeTab === tab.id
          ...
          <div className="p-8 border border-zinc-100 bg-zinc-50/30">
          ...
          {filteredCollections.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredCollections.map((col) => (
                <div
                  key={col._id}
                  className="group border border-zinc-100 p-10 bg-white hover:border-black transition-all duration-700 flex flex-col justify-between min-h-[380px] relative overflow-hidden"
                >
                  {/* Decorative background element */}
                  <div className="absolute -top-10 -right-10 w-40 h-40 bg-zinc-50/50 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-10">
                      <div className="w-16 h-16 border border-zinc-100 bg-zinc-50 flex items-center justify-center grayscale group-hover:grayscale-0 group-hover:bg-black group-hover:text-white group-hover:border-black transition-all duration-700">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-4">
                        <h3 className="text-2xl font-bold tracking-tighter text-black group-hover:translate-x-1 transition-transform duration-500">
                          {col.name}
                        </h3>
                        {col.is_public ? (
                  ...
                  <div className="pt-10 border-t border-zinc-50 mt-10 relative z-10">
                    ...
                    <Link
                      ...
                      className="flex items-center justify-between w-full h-16 px-8 bg-zinc-50 border border-transparent group-hover:border-zinc-100 group-hover:bg-white transition-all duration-500"
                    >
                      <span className="text-[11px] font-bold tracking-[0.2em] uppercase">Truy cập tri thức</span>
                      <ChevronRight className="w-5 h-5 text-zinc-300 group-hover:text-black group-hover:translate-x-2 transition-all duration-500" />
                    </Link>
                  </div>
                </div>
              ))}
              
              {/* Create New Card - Standardized Placeholder */}
              <button className="group border border-dashed border-zinc-200 p-10 flex flex-col items-center justify-center space-y-8 hover:border-black hover:bg-zinc-50/50 transition-all min-h-[380px] active:scale-[0.98]">
                <div className="w-16 h-16 border border-zinc-100 flex items-center justify-center bg-white group-hover:border-black transition-all duration-500">
