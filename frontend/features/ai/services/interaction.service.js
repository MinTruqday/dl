...
export async function createAiSessionAPI(documentId = "", firstQuery = "", mode = "chat") {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể tạo phiên trò chuyện");
    return data;
}
...
export async function getAiWorkspaceAPI(sessionId) {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể tải tiến trình");
    return data.data;
}
export async function updateAiSessionTitleAPI(sessionId, title) {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể đổi tên phiên trò chuyện");
    return data;
}
...
export async function getAiCapabilitiesAPI() {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể tải khả năng AI");
    return data;
}
export async function deleteAiSessionAPI(sessionId) {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể xóa phiên trò chuyện");
    return data;
}
...
export async function getPendingAiApprovalsAPI(sessionId) {
    var _a;
    const res = await fetch(`${API_URL}/ngat-qua-trinh/phe-duyet/${sessionId}`, { headers: getAuthHeaders() });
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể tải yêu cầu xác nhận");
...
export async function resolveAiApprovalAPI(approvalId, status, scope = "once") {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể gửi lựa chọn xác nhận");
    return data.data;
}
export async function getUserInstructionsAPI() {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể tải chỉ dẫn cá nhân");
    return data;
}
export async function saveUserInstructionsAPI(instructions) {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể lưu chỉ dẫn cá nhân");
    return data;
}
export async function clearUserInstructionsAPI() {
    ...
    const data = await res.json();
    if (!res.ok)
        throw new Error(data.message || "Không thể xóa chỉ dẫn cá nhân");
    return data;
}
