...
export default function ChatInstructionsModal({ open, close, initial, save, }) {
    ...
    return (<Modal isOpen={open} onClose={close}>
      ...
      <ModalContent>
        <label htmlFor="chat-instructions" className="mb-2 block text-[13px] font-semibold text-ink">
          Cách trợ lý nên phản hồi
        </label>
        <textarea id="chat-instructions" value={value} onChange={(event) => setValue(event.target.value)} maxLength={4000} className="apple-input min-h-40 w-full resize-y"/>
      </ModalContent>
      <ModalFooter>
