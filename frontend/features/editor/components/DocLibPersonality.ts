...
export default class DocLibPersonality implements BlockTool {
  ...
  render() {
    ...
    if (!document.getElementById("doclib-personality-styles")) {
      ...
      style.innerHTML = `
        ...
        .doclib-personality-name:empty::before { content: 'Character name'; color: #94a3b8; pointer-events: none; }
        .doclib-personality-desc { font-size: 0.9em; color: #64748b; outline: none; }
        .doclib-personality-desc:empty::before { content: 'Short description'; color: #94a3b8; pointer-events: none; }
        .doclib-personality-link { font-size: 0.85em; color: #3b82f6; outline: none; }
        .doclib-personality-link:empty::before { content: 'Link (optional)'; color: #94a3b8; pointer-events: none; }
      `;
  ...
  private buildUI() {
    ...
    img.addEventListener("click", () => {
      const newUrl = prompt("Enter avatar URL:", this.data.photo);
      if (newUrl !== null) {
