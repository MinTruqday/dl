    ...
    if (this.data.file.url) {
      ...
      this.wrapper.appendChild(card);
              ...
              if (fileInput.files && fileInput.files[0]) {
                  ...
                  .then(res => {
                      if (res.success === 1 && res.file && res.file.url) {
                          ...
                          this.data.file = { url: res.file.url, name: file.name, size: file.size, extension: file.name.split(".").pop().toUpperCase() }; this.data.title = file.name;
                      } else {
                          this.data.url = res.url || res.data?.url || URL.createObjectURL(file);
                          this.data.file = { url: this.data.url, name: file.name, size: file.size, extension: file.name.split(".").pop().toUpperCase() }; this.data.title = file.name;
                      }
                      this.buildUI();
                  })
                  .catch(err => {
          ...
          uploader.addEventListener('contextmenu', (e) => {
              ...
              if (url) {
                  this.data.url = url;
                  this.data.file = { url, name: url.split("/").pop(), size: 0, extension: "FILE" }; this.data.title = url.split("/").pop();
                  this.buildUI();
              }
          });
