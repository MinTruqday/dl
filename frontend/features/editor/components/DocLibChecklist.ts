...
export default class DocLibChecklist implements BlockTool {
  ...
  constructor({ api, data }: { api: API, data: any }) {
    this.api = api;
    this.data = {
      items: Array.isArray(data?.items) ? data.items : [{ text: '', checked: false }]
    };
  }
      ...
      input.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') {
              ...
              if (this.wrapper!.children.length > 1) {
                  ...
                  row.remove();
                  if (prev) {
                      const prevInput = prev.querySelector('.doclib-checklist-text') as HTMLElement;
                      prevInput.focus();
  ...
  save() {
      ...
      if (this.wrapper) {
          Array.from(this.wrapper.children).forEach(row => {
              ...
              const text = (row.querySelector('.doclib-checklist-text') as HTMLElement).innerHTML;
