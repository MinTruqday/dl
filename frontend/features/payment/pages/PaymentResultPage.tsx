...
export default function PaymentResultPage() {
  ...
  return (
    <div className="w-full h-[calc(100vh-56px)] flex flex-col items-center justify-center font-sans text-[var(--ink)] px-6 md:px-0">
      <div className="w-full max-w-[480px] bg-[var(--surface-quiet)] border-[var(--border)] rounded-[var(--radius-panel)] p-10 flex flex-col items-center text-center">
        <div
        ...
        <h1 className="text-[24px] font-semibold text-[var(--ink)] mb-2">
          {current.title}
        </h1>
        <p className="text-[15px] text-[var(--ink-muted)] mb-8">{current.description}</p>

        {status === "success" && paymentInfo && (
          <div className="w-full bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] p-6 space-y-4 mb-8">
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-[var(--ink-muted)] font-medium">Mã giao dịch</span>
              <span className="text-[15px] font-medium text-[var(--ink)]">#{paymentInfo.order_code}</span>
            </div>
            <div className="w-full h-px bg-[var(--border)]"></div>
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-[var(--ink-muted)] font-medium">Số tiền nạp</span>
              ...
              : <span className="text-[var(--ink-muted)] text-[13px]">Đang cập nhật</span>
                }
              </span>
            </div>
            {paymentInfo.dl > 0 && (
              <>
                <div className="w-full h-px bg-[var(--border)]"></div>
                <div className="flex items-center justify-between">
                  <span className="text-[13px] text-[var(--ink-muted)] font-medium">Số dl nhận được</span>
                  <span className="text-[15px] font-semibold text-[var(--success)]">+{paymentInfo.dl.toLocaleString()} dl</span>
                </div>
              </>
            )}
          </div>
        ...
        <div className="flex items-center gap-4 w-full">
          <Link
            href="/vi-tien"
            className="flex-1 h-12 bg-[var(--surface-quiet)] text-[var(--ink)] text-[15px] font-medium flex items-center justify-center gap-2 rounded-full hover:bg-[var(--border)] transition-colors"
          >
          ...
          {(status === "failed" || status === "cancelled") && (
            <button
              ...
              className="flex-1 h-12 bg-[var(--brand)] text-white text-[15px] font-medium flex items-center justify-center rounded-full hover:bg-[var(--brand-hover)] transition-colors"
            >
              Thử lại
            </button>
          )}
