...
export default class DocLibSmartArtList implements BlockTool {
  ...
  render() {
    ...
    style.innerHTML = `
      ...
      .doclib-sal-item { display: flex; border: 1px solid hsl(var(--border)); border-radius: 8px; overflow: hidden; position: relative; }
      .doclib-sal-title { width: 150px; background: #6366f1; color: hsl(var(--surface)); padding: 16px; font-weight: bold; font-size: 16px; display: flex; align-items: center; justify-content: center; text-align: center; outline: none; }
      .doclib-sal-desc { flex: 1; background: hsl(var(--surface-raised)); padding: 16px; font-size: 14px; color: hsl(var(--ink)); display: flex; align-items: center; outline: none; }
      .doclib-sal-title:empty:before { content: "DocLib Title"; color: #c7d2fe; font-weight: normal; }
      .doclib-sal-desc:empty:before { content: "DocLib Description text goes here"; color: hsl(var(--ink-faint)); font-style: italic; }
      .doclib-sal-del { position: absolute; top: 8px; right: 8px; font-size: 12px; color: hsl(var(--danger)); background: transparent; border: none; cursor: pointer; display: none; }
      .doclib-sal-item:hover .doclib-sal-del { display: block; }
      .doclib-sal-add { padding: 12px; background: hsl(var(--surface-quiet)); border: 1px dashed hsl(var(--border)); border-radius: 8px; text-align: center; color: hsl(var(--ink-muted)); font-weight: bold; cursor: pointer; font-size: 14px; }
      .doclib-sal-add:hover { background: hsl(var(--border)); }
    `;
