...
export default function TestGovernancePage({ project }) {
  ...
  const load = useCallback(async () => {
    try {
      const result = await testingApi.listTestStrategies(project._id, { q: query, status, page_size: 200 });
      setItems(result.items || []);
      setSelected((current) => result.items?.find((item) => item._id === current?._id) || current || result.items?.[0] || null);
    } catch (reason) {
  ...
  const transition = async (title, confirmLabel, action, field = "note") => {
    if (!selected) return;
    const answer = await ask({ title, confirmLabel, fields: [{ name: field, label: "Lý do hoặc ghi chú", required: true, multiline: true }] });
    if (!answer) return;
    try {
  ...
  return (
    <WorkspacePage
      ...
      actions={<><ProjectCrumb projectId={project._id} />{can("teststrategy.create") && <button className="apple-button" type="button" onClick={() => { setSelected(null); setEditing(true); }}>Tạo chiến lược</button>}</>}
    >
      {dialog}
      ...
      <Panel title="Kho chiến lược kiểm thử">
        <form className="grid gap-3 p-5 sm:grid-cols-[1fr_220px_auto]" onSubmit={(event) => { event.preventDefault(); void load(); }}>
          <input className="apple-input" aria-label="Tìm chiến lược" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm theo mã hoặc tên" />
          <select className="apple-input" aria-label="Lọc trạng thái" value={status} onChange={(event) => setStatus(event.target.value)}>
            <option value="">Tất cả trạng thái</option>
            {["DRAFT", "IN_REVIEW", "APPROVED", "SUPERSEDED", "ARCHIVED"].map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: "active", label: "Hiệu lực", render: (item) => item.active_approved ? "Đang áp dụng" : "" },
            { key: "actions", label: "Thao tác", render: (item) => <button className="secondary-button" type="button" onClick={() => setSelected(item)}>Mở</button> },
          ]}
        />
      </Panel>
      {selected && (
        <Panel title={`${selected.key} phiên bản ${selected.version}`} actions={<div className="flex flex-wrap gap-2">
          ...
          <div className="grid gap-5 p-5 md:grid-cols-2">
            <div><p className="field-label">Mục tiêu</p><p className="mt-2 whitespace-pre-wrap text-sm">{selected.objective}</p></div>
            <div><p className="field-label">Cách tiếp cận</p><p className="mt-2 whitespace-pre-wrap text-sm">{selected.approach}</p></div>
            <div><p className="field-label">Cấp kiểm thử</p><p className="mt-2 text-sm">{selected.test_levels?.join(" · ")}</p></div>
            <div><p className="field-label">Loại kiểm thử</p><p className="mt-2 text-sm">{selected.test_types?.join(" · ")}</p></div>
      ...
      {selected && project.current_permissions?.includes("reviewsession.read") && <FormalReviewPanel project={project} artifactType="TEST_STRATEGY" artifactId={selected._id} artifactVersionId={selected._id} reviewType="TEST_STRATEGY_REVIEW" />}
      <Modal isOpen={editing} onClose={() => setEditing(false)} ariaLabel="Biên tập chiến lược kiểm thử" className="max-h-[94dvh] max-w-5xl overflow-y-auto">
        <ModalHeader><ModalTitle>{selected ? "Chỉnh sửa chiến lược" : "Tạo chiến lược"}</ModalTitle></ModalHeader>
        <TestStrategyEditor
          strategy={selected}
          onCancel={() => setEditing(false)}
          onSave={async (payload) => {
            try {
              const value = selected ? await testingApi.updateTestStrategy(selected._id, payload) : await testingApi.createTestStrategy(project._id, payload);
              setSelected(value);
              setEditing(false);
              await load();
            } catch (reason) {
