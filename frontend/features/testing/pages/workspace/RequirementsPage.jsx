...
export default function RequirementsPage({ project, section }) {
  ...
  const [generatingRequirementDraft, setGeneratingRequirementDraft] = useState(false);
  const [aiDraftWarnings, setAiDraftWarnings] = useState([]);
  ...
  const generateRequirementDraft = async () => {
    if (form.title.trim().length < 2) {
      setError("Nhập tên yêu cầu trước khi nhờ AI tạo bản nháp.");
      return;
    }
    setGeneratingRequirementDraft(true);
    setError("");
    try {
      const result = await testingApi.generateRequirementAiDraft(project._id, {
        title: form.title.trim(),
        type: form.type,
        priority: form.priority,
        risk: form.risk,
        content: docText(form.content_doc),
        acceptance_criteria: splitBlocks(form.acceptance),
        business_rules: splitBlocks(form.businessRules),
        actors: form.actors
          .split(",")
          .map((value) => value.trim())
          .filter(Boolean),
        dependencies: splitBlocks(form.dependencies),
        tags: form.tags
          .split(",")
          .map((value) => value.trim())
          .filter(Boolean),
      ...
      setForm((value) => ({
        ...
        content_doc: candidate.content ? textDoc(candidate.content) : value.content_doc,
        acceptance: candidate.acceptance_criteria?.length
          ? candidate.acceptance_criteria.join("\n")
          : value.acceptance,
        businessRules: candidate.business_rules?.length
          ? candidate.business_rules.join("\n")
          : value.businessRules,
        actors: candidate.actors?.length ? candidate.actors.join(", ") : value.actors,
        dependencies: candidate.dependencies?.length
          ? candidate.dependencies.join("\n")
          : value.dependencies,
        tags: candidate.tags?.length ? candidate.tags.join(", ") : value.tags,
      }));
      setAiDraftWarnings(result.warnings || []);
    } catch (reason) {
      setError(messageOf(reason));
    } finally {
      setGeneratingRequirementDraft(false);
    }
  };
        ...
        <>
                ...
                {can("ai.run_lint") && (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        // The editor autosaves on a short delay. Persist the current
                        // draft first so a just-cleared field is not linted as the old
                        // saved Requirement.
                        let versionToCheck = current;
                        if (draftDirty && draft) {
                          const saved = await persistDraft(draft, draftSequence.current);
                          if (!saved?.current_version) return;
                          versionToCheck = saved.current_version;
                        }
                        const result = await testingApi.lintRequirement(versionToCheck._id, {
                          idempotency_key: crypto.randomUUID(),
                        ...
                        setLintMessage(
                          result.findings?.length
                            ? `Đã kiểm tra xong: phát hiện ${result.findings.length} vấn đề.`
                            : "Đã kiểm tra xong: không phát hiện vấn đề cần chỉnh sửa.",
                        );
                      } catch (reason) {
                        setError(messageOf(reason));
                      } finally {
                  ...
                  >
                    {checkingWithAi ? "Đang kiểm tra chất lượng…" : "Kiểm tra chất lượng"}
                  </button>
                )}
          ...
          {lint && (
            ...
            >
              {lintMessage && (
                <p
                  className="mb-4 rounded-control bg-brand-soft p-3 text-sm text-brand"
                  role="status"
                >
                  {lintMessage}
                </p>
                ...
                <DataTable
                  ...
                  columns={[
                    ...
                    { key: "revised_content", label: "Nội dung đề xuất" },
                    {
                      ...
                      render: (item) => {
                        const fields = [
                          item.revised_content && "Nội dung",
                          item.acceptance_criteria?.length && "Tiêu chí",
                          item.business_rules?.length && "Quy tắc",
                          item.actors?.length && "Tác nhân",
                          item.dependencies?.length && "Phụ thuộc",
                          item.tags?.length && "Nhãn",
                        ].filter(Boolean);
                        return fields.join(", ") || "Nội dung";
                      },
                    },
                      ...
                      render: (item) =>
                        current.status === "DRAFT" && can("requirement.update") ? (
                          <button
                            ...
                            onClick={async () => {
                              try {
                                // Cancel any pending autosave based on the old editor text before
                                // loading the AI-applied version back into the controlled draft.
                                draftSequence.current += 1;
                                setDraftDirty(false);
                                setSaveState("saved");
                                ...
                                // The version id does not change when a suggestion is applied.
                                // Clear the guard so the draft-loading effect also refreshes the
                                // editor for a new revision of that same version.
                ...
                <form className="space-y-4 p-5" onSubmit={create}>
                  ...
                  <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm">
                        AI dùng tên và nội dung bạn vừa nhập để điền riêng từng ô bên dưới.
                      </p>
                      <button
                        className="secondary-button"
                        type="button"
                        disabled={generatingRequirementDraft || form.title.trim().length < 2}
                        onClick={() => void generateRequirementDraft()}
                      >
                        {generatingRequirementDraft ? "AI đang tạo" : "AI điền bản nháp"}
                      </button>
                    </div>
                    {aiDraftWarnings.length > 0 && (
                      <p className="mt-2 text-sm text-warning">
                        AI cần BA xác nhận: {aiDraftWarnings.join(" ")}
                      </p>
                    )}
                  </div>
                  <label className="field-label">
