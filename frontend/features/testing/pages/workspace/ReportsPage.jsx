...
export default function ReportsPage({ project }) {
  ...
  return (
    <QaPage
      ...
      description="Các chỉ số được tính xác định từ dữ liệu truy vết thực thi lỗi và quyết định bảo trì trong phạm vi dự án"
          ...
          <button
            ...
            onClick={async () => {
              const answer = await ask({
                ...
                description: `Release ${scope.release || "mọi release"} build ${scope.build || "mọi build"}`,
      ...
      {!value ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            ...
            <Metric label="Độ phủ tiêu chí chấp nhận" value={`${value.coverage.acceptance_criterion_coverage}%`} />
            ...
            <Metric
              ...
              value={value.maintenance.proposal_acceptance_rate === null ? "Chưa đủ dữ liệu" : `${Math.round(value.maintenance.proposal_acceptance_rate * 100)}%`}
            />
          </div>
          <div className="grid gap-5 xl:grid-cols-2">
            <Panel title="Độ phủ theo kỹ thuật">
              <DataTable
                items={Object.entries(value.coverage.category_coverage || {}).map(([category, coverage]) => ({ _id: category, category, coverage }))}
                empty="Chưa có dữ liệu kỹ thuật"
                columns={[
                  { key: "category", label: "Kỹ thuật", render: (item) => valueLabel(item.category) },
            ...
            <Panel title="Trạng thái đề xuất AI">
              <DataTable
                items={(value.maintenance.proposal_status || []).map((item) => ({ ...item, status: item._id }))}
          ...
          <Panel title="Ảnh chụp độ phủ">
            <DataTable
              ...
              columns={[
                ...
                { key: "requirement_coverage", label: "Độ phủ yêu cầu", render: (item) => `${item.metrics?.requirement_coverage ?? 0}%` },
                { key: "fresh_coverage", label: "Độ phủ còn hiệu lực", render: (item) => `${item.metrics?.fresh_coverage ?? 0}%` },
                { key: "execution_coverage", label: "Độ phủ thực thi", render: (item) => `${item.metrics?.execution_coverage ?? 0}%` },
              ]}
            />
          </Panel>
        </>
