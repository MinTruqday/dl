...
export default function TestabilityFindingsPanel({ condition, canResolve, onResolve }) {
  return <DataTable items={condition.analysis_findings || []} empty="Không có finding về khả năng kiểm thử" columns={[
    ...
    ...(canResolve ? [{ key: "actions", label: "Thao tác", render: (item) => item.status === "OPEN" ? <button className="secondary-button" type="button" onClick={() => onResolve(item)}>Giải quyết</button> : null }] : []),
  ]} />;
}
