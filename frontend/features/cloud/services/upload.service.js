...
async function doDirectUpload(file, isSystem, isMessageAttachment) {
    ...
    if (!reqRes.ok)
        throw new Error(reqData.detail ||
            reqData.message ||
...
export async function getChatAttachmentBlobUrlAPI(filePath) {
    if (!filePath || /^(https?:|blob:|data:)/.test(filePath))
        return filePath;
    const response = await fetch(`${API_URL}/tai-len/noi-dung/${filePath.replace(/^\/+/, "")}`, { headers: getAuthHeaders() });
    if (!response.ok)
        throw new Error("Không thể tải tệp tin nhắn");
