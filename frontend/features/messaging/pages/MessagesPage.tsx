  ...
  return (
    <Modal isOpen={true} onClose={onClose}>
        ...
        {filtered.map(c => {
          ...
          return (
            <div 
              ...
              className="flex items-center gap-3 p-2 rounded-[var(--radius-control)] hover:bg-[var(--surface-quiet)] cursor-pointer transition-colors"
            >
              <input 
                type="checkbox" 
                checked={selectedIds.includes(targetId)}
                readOnly
                className="w-4 h-4 rounded-full border-[var(--border-strong)] text-[var(--brand)] focus:ring-[var(--brand)]"
              />
              {avatar ? (
                <img 
                  src={avatar} 
                ...
                <div className="w-10 h-10 rounded-full bg-[var(--border)] flex items-center justify-center border border-[var(--border)]">
                  <User className="w-5 h-5 text-[var(--ink-muted)]" />
                </div>
              )}
              <span className="text-[15px] text-[var(--ink)] font-medium truncate">{name || "Người dùng"}</span>
            </div>
          );
        })}
        {filtered.length === 0 && (
          <div className="p-4 text-center text-[var(--ink-muted)] text-[15px]">Không tìm thấy cuộc trò chuyện nào</div>
        )}
      </div>
      ...
      <div className="flex justify-end gap-3 px-6 py-4 border-t border-[var(--border)]">
        <button
          onClick={onClose}
          className="px-4 py-2 text-[14px] font-medium text-[var(--ink)] bg-[var(--border)] hover:bg-[var(--border-strong)] rounded-full transition-colors"
        >
          Hủy
        </button>
        <button
          ...
          className="px-4 py-2 text-[14px] font-medium text-white bg-[var(--brand)] hover:opacity-80 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-sm">
      ...
      <ModalContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[var(--ink)] mb-1">Ngày gửi</label>
            <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setError(""); }} required className="field-control" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--ink)] mb-1">Giờ gửi</label>
            <input type="time" value={time} onChange={(e) => { setTime(e.target.value); setError(""); }} required className="field-control" />
          </div>
    ...
    <Modal isOpen={true} onClose={onClose}>
      ...
      <ModalContent>
        <div className="space-y-4 max-h-[50vh] overflow-y-auto hide-scrollbar">
          <div>
            <label className="block text-[13px] font-medium text-[var(--ink-muted)] mb-1">Câu hỏi</label>
            <input 
              type="text" 
              value={question}
          ...
          <div className="space-y-2">
            <label className="block text-[13px] font-medium text-[var(--ink-muted)] mb-1">Các lựa chọn</label>
            {options.map((opt, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                />
                {options.length > 2 && (
                  <button onClick={() => handleRemoveOption(idx)} className="p-2 text-[var(--ink-muted)] hover:text-red-500 rounded-full hover:bg-[#FFF5F5] transition-colors">
            ...
            <button 
              ...
              className="flex items-center gap-2 text-[var(--brand)] text-[14px] font-medium mt-2 hover:opacity-80 transition-opacity p-1"
            >
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-[var(--border)]">
          ...
          <button
            ...
            className="px-4 py-2 text-[14px] font-medium text-white bg-[var(--brand)] hover:opacity-80 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Tạo bình chọn
          </button>
        </div>
...
function PollMessage({ messageId, pollData, currentUserId, onVote }: PollMessageProps) {
  ...
  return (
    <div className="w-full min-w-[260px] max-w-[320px] bg-white rounded-[var(--radius-panel)] border border-[var(--border)] overflow-hidden shadow-sm flex flex-col">
      <div className="p-4 border-b border-[var(--border)] bg-[var(--surface-quiet)]">
        <h4 className="font-semibold text-[var(--ink)] text-[15px] leading-snug">
          {pollData.question}
        </h4>
        <span className="text-[12px] text-[var(--ink-muted)] mt-1 block">Bình chọn • {totalVotes} lượt vote</span>
      </div>
        ...
        {pollData.options.map((opt) => {
          ...
          return (
            <div 
              ...
              className="relative rounded-[var(--radius-control)] overflow-hidden cursor-pointer group/pollopt transition-colors border border-transparent hover:border-[var(--border)]"
            >

              <div 
                className={`absolute inset-y-0 left-0 transition-all duration-500 ease-out ${hasVoted ? "bg-[var(--brand)]/15" : "bg-[var(--surface-quiet)]"}`}
              ...
              <div className="relative flex items-center justify-between p-3 z-10">
                <div className="flex items-center gap-3">
                  {hasVoted ? (
                    <CheckCircle2 className="w-[18px] h-[18px] text-[var(--brand)] shrink-0" />
                  ) : (
                    <Circle className="w-[18px] h-[18px] text-[var(--ink-muted)] shrink-0" />
                  )}
                  <span className={`text-[15px] font-medium ${hasVoted ? "text-[var(--brand)]" : "text-[var(--ink)]"}`}>
                    {opt.text}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  {voteCount > 0 && (
                     <div className="flex -space-x-1.5 mr-1">
                        ...
                        <div className="w-5 h-5 rounded-full bg-white border border-[var(--border-strong)] flex items-center justify-center text-[9px] font-bold text-[var(--ink-muted)]">
                          {voteCount}
                        </div>
                     </div>
                  )}
...
function ReplyBlock({ replyingTo, onCancel }: ReplyBlockProps) {
  ...
  return (
    <div className="absolute bottom-full left-0 w-full bg-white/95 backdrop-blur-md border-t border-[var(--border)] px-4 py-2.5 flex items-center justify-between z-10 animate-in slide-in-from-bottom-2 fade-in duration-200">
      <div className="flex flex-col flex-1 overflow-hidden pr-4 border-l-2 border-[var(--brand)] pl-3">
        <div className="flex items-center gap-1.5 text-[13px] font-semibold text-[var(--brand)] mb-0.5">
          <Reply className="w-3.5 h-3.5" />
          <span>Đang trả lời tin nhắn</span>
        </div>
        <p className="text-[14px] text-[var(--ink-muted)] truncate">
          {replyingTo.content || (replyingTo.image_url ? "[Hình ảnh]" : replyingTo.poll_data ? "[Bình chọn]" : "[Đính kèm]")}
        </p>
      </div>
      <button 
        onClick={onCancel}
        className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-[var(--surface-quiet)] text-[var(--ink-muted)] transition-colors shrink-0"
      >
        <X className="w-4 h-4" />
      </button>
    </div>
...
const CustomAudioPlayer = ({
  ...
  return (
    <div className="flex items-center gap-3 w-full py-1 min-w-[200px]">
      <button
        onClick={togglePlay}
        className={`flex shrink-0 items-center justify-center w-6 h-6 rounded-full ${isSender ? "bg-white text-[var(--brand)]" : "bg-[var(--brand)] text-white"}`}
      >
        {isPlaying ? (
      ...
      <div className="flex-1 flex items-center gap-3">
        ...
        >
          <div
            ...
            style={{
              width: `${progress}%`,
              background: isSender ? "white" : "var(--brand)",
            }}
          ></div>
        </div>
...
function SmartQuickReplies({ replies, onSelect }: { replies: string[], onSelect: (r: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-white border-t border-[var(--border)] overflow-x-auto no-scrollbar">
      <Sparkles className="w-4 h-4 text-[var(--brand)] shrink-0" />
      {replies.map((reply, idx) => (
        <button
          key={idx}
          onClick={() => onSelect(reply)}
          className="whitespace-nowrap px-4 py-1.5 bg-[var(--surface-quiet)] hover:bg-[var(--border)] text-[var(--ink)] text-[14px] font-medium rounded-[980px] transition-colors"
        >
          {reply}
        </button>
      ))}
...
export default function MessagesPage() {
  ...
  const scrollToMessage = (id: string) => {
    ...
    if (el) {
      ...
      el.classList.add("bg-[var(--surface-quiet)]");
      setTimeout(() => el.classList.remove("bg-[var(--surface-quiet)]"), 2000);
    }
  };
    ...
    try {
      ...
      if (data.data?.url) {
        await updateGroupInfoAPI(
          selectedConv.other_user_id,
          selectedConv.group_name || selectedConv.other_user?.full_name || "",
          data.data.url,
        );
  ...
  const handleDeleteConv = async (otherUserId: string) => {
    setDeleteConversationTarget(otherUserId);
    setActiveConvMenuId(null);
  };
  ...
  const executeDeleteConversation = async () => {
    ...
    try {
      await deleteConversationAPI(deleteConversationTarget);
      if (selectedConv?.other_user_id === deleteConversationTarget)
        setSelectedConv(null);
      setConversations((prev) =>
        prev.filter((c) => c.other_user_id !== deleteConversationTarget),
      );
  ...
  return (
    <div className="w-full h-full flex flex-col font-sans text-[var(--ink)]">
        ...
        <ModalContent>
          <div className="relative mb-4">
            ...
            <input
              ...
              placeholder=""
              className="w-full bg-[var(--border)] text-[var(--ink)] placeholder:text-[var(--ink-faint)] pl-9 pr-4 py-2 rounded-[var(--radius-control)] focus:outline-none focus:ring-2 focus:ring-[var(--brand)] transition-all text-[15px]"
            />
          </div>
            ...
            {searching ? (
              ...
              searchResults.map((u) => (
                <div
                  ...
                  className="flex items-center justify-between p-4 bg-white rounded-[var(--radius-control)] cursor-pointer hover:bg-[var(--surface-quiet)]"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-6 h-6 bg-[var(--surface-quiet)] rounded-full overflow-hidden flex items-center justify-center">
                      {u.avatar_url ? (
                        <img
                          src={u.avatar_url}
                        ...
                        <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[9px] font-semibold uppercase">
              ...
              <p className="text-center text-[13px] text-[var(--ink-muted)] py-12">
      ...
      >
        ...
        <ModalContent>
          <input
            ...
            className="apple-input w-full"
          />
          <div className="max-h-48 overflow-y-auto space-y-2 bg-white rounded-[var(--radius-control)] p-2 border border-[var(--border-strong)]">
            {loadingGroupUsers ? (
              ...
              allUsersForGroup.map((u) => (
                <div
                  key={u._id || u.id}
                  className="flex items-center gap-3 p-2 hover:bg-[var(--surface-quiet)] rounded-[var(--radius-control)]"
                >
                  <input
                    type="checkbox"
                    ...
                    className="w-6 h-6 rounded text-[var(--brand)] focus:ring-[var(--brand)]"
                  />
          ...
          {loadingShareDocs ? (
            ...
            myDocsForShare.map((doc) => (
              <div
                ...
                className="p-4 bg-white rounded-[var(--radius-control)] cursor-pointer hover:flex justify-between items-center"
              >
                <div className="flex flex-col">
                  <span className="text-[15px] font-medium text-[var(--ink)]">
                    {doc.title}
        ...
        <ModalContent>
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-[15px] font-medium text-[var(--ink)]">Hạn chế nhắn tin</span>
                <span className="text-[13px] text-[var(--ink-muted)]">Chỉ trưởng nhóm và phó nhóm được gửi tin</span>
              </div>
              <input
                type="checkbox"
                ...
                className="w-5 h-5 rounded border-[var(--border-strong)] text-[var(--brand)] focus:ring-[var(--brand)]"
              />
            </div>
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-[15px] font-medium text-[var(--ink)]">Yêu cầu phê duyệt</span>
                <span className="text-[13px] text-[var(--ink-muted)]">Người mới dùng link tham gia cần được duyệt</span>
              </div>
              <input
                type="checkbox"
            ...
            <button
              onClick={async () => {
                try {
                  const data = await updateGroupSettingsAPI(
                    selectedConv.other_user_id,
                    tempGroupSettings,
                  );
                    showToast(data.message || "Đã lưu cài đặt nhóm", "success");
                    setShowGroupSettingsModal(false);
                } catch(error: any) {
                  showToast(error.message || "Không thể lưu cài đặt nhóm", "error");
                }
              }}
              className="w-full mt-4 bg-[var(--brand)] text-white font-medium py-3 rounded-[980px] hover:bg-[var(--brand-hover)] transition-colors"
            >
              Lưu thay đổi
            </button>
          </div>
        ...
        <ModalContent>
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="w-full bg-[var(--surface-quiet)] text-[var(--ink)] font-medium py-3 rounded-[980px] hover:bg-[var(--border)] transition-colors"
            >
      ...
      <div className="flex flex-1 min-h-0 gap-6">
        <div
          className={`w-full md:w-[320px] bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] flex flex-col overflow-hidden shrink-0 ${selectedConv ? "hidden md:flex" : "flex"}`}
        >
          ...
          <div className="flex-1 overflow-y-auto px-6 md:px-0 pb-4 space-y-2 hide-scrollbar">
            {isGlobalSearching ? (
              ...
              globalSearchResults.length > 0 ? (
                globalSearchResults.map((msg) => (
                  <div
                    ...
                    className="p-4 bg-white rounded-[var(--radius-panel)] cursor-pointer hover:bg-[var(--surface-quiet)] border border-transparent hover:border-[var(--border)] transition-all"
                  >
                    <div className="text-[13px] text-[var(--ink-muted)] mb-1">{formatRelativeTime(parseUTC(msg.created_at))}</div>
                    <div className="text-[15px] text-[var(--ink)] line-clamp-2">{msg.content}</div>
                  </div>
                ))
              ) : (
                <p className="text-center text-[13px] text-[var(--ink-muted)] py-12">
              ...
              <div className="space-y-2">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="p-4 rounded-[var(--radius-panel)] flex items-center gap-4 animate-pulse bg-white/50 border border-transparent">
                    <div className="w-[48px] h-[48px] bg-[var(--border)] rounded-full shrink-0" />
                    <div className="flex-1 space-y-2.5">
                      <div className="h-3.5 bg-[var(--border)] rounded-full w-[40%]" />
                      <div className="h-2.5 bg-[var(--border)] rounded-full w-[70%]" />
                    </div>
                  </div>
                ))}
              </div>
              ...
              sortedConversations.map((conv) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-[var(--radius-panel)] cursor-pointer flex items-center gap-4 transition-colors group/conv relative ${active ? "bg-white" : "hover:bg-white/50"}`}
                  >
                    <div className="w-10 h-10 rounded-full overflow-hidden shrink-0">
                      {conv.other_user?.avatar_url ? (
                        ...
                        <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[16px] font-semibold uppercase">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[15px] font-medium text-[var(--ink)] truncate pr-2">
                          {aliases[conv.other_user_id] || conv.other_user?.full_name ||
                            conv.other_user?.username}
                        </span>
                        <span className="text-[12px] text-[var(--ink-muted)] shrink-0">
                      ...
                      <div className="flex justify-between items-center relative group-hover/conv:pr-6">
                        <p
                          className={`text-[13px] truncate transition-all duration-300 ${conv.unread_count > 0 ? "font-semibold text-[var(--ink)]" : "text-[var(--ink-muted)]"}`}
                        >
                        ...
                        {conv.unread_count > 0 && (
                          <div className="w-2.5 h-2.5 bg-[var(--brand)] rounded-full shrink-0 ml-2" />
                        )}
                        <div className="absolute right-0 top-1/2 -translate-y-1/2 opacity-0 group-hover/conv:opacity-100 transition-opacity">
                          <button
                            ...
                            className="p-1 text-[var(--ink-muted)] hover:text-[var(--ink)] hover:bg-[var(--border)] rounded-full bg-[var(--surface-quiet)] md:bg-white shadow-sm"
                          >
                            <MoreHorizontal className="w-4 h-4" />
                          </button>
                          {activeConvMenuId === conv.other_user_id && (
                            <div className="absolute z-50 w-48 bg-white/90 backdrop-blur-md rounded-[var(--radius-panel)] shadow-[0_8px_32px_rgba(0,0,0,0.15)] border border-[var(--border)] py-1.5 flex flex-col right-0 top-full mt-1">
                              <button 
                                onClick={(e) => { e.stopPropagation(); handleTogglePinConv(conv.other_user_id); }}
                                className="flex items-center gap-3 px-4 py-2.5 text-[13px] hover:bg-[var(--surface-quiet)] text-[var(--ink)] text-left rounded-t-[10px] transition-colors"
                              >
                                <Pin className="w-3.5 h-3.5 text-[var(--ink-muted)]" />
                                {isPinned ? "Bỏ ghim" : "Ghim"}
                              </button>
                              <button 
                                onClick={(e) => { e.stopPropagation(); handleMarkAsRead(conv.other_user_id); }}
                                className="flex items-center gap-3 px-4 py-2.5 text-[13px] hover:bg-[var(--surface-quiet)] text-[var(--ink)] text-left transition-colors"
                              >
                                <CheckCheck className="w-3.5 h-3.5 text-[var(--ink-muted)]" />
                                Đánh dấu đã đọc
                              </button>
                              <div className="h-px bg-[var(--surface-quiet)] mx-3 my-1" />
                              <button 
        ...
        <div
          className={`flex-1 flex min-w-0 rounded-[var(--radius-panel)] overflow-hidden ${!selectedConv ? "hidden md:flex items-center justify-center bg-[var(--surface-quiet)]" : ""}`}
        >
          {selectedConv ? (
            <>
              <div className="flex-1 flex flex-col min-w-0 bg-[var(--surface-quiet)] relative">
              <div className="h-[64px] px-6 md:px-0 flex items-center justify-between bg-transparent">
                <div className="flex items-center gap-4">
                  {activeThreadParentId ? (
                    <button onClick={() => setActiveThreadParentId(null)} className="text-[var(--brand)] flex items-center gap-1 hover:bg-[var(--surface-quiet)] p-1.5 rounded-full transition-colors">
                    ...
                    <button
                      ...
                      className="md:hidden text-[var(--brand)]"
                    >
                      <ArrowLeft className="w-4 h-4" />
                    </button>
                  )}
                  <button 
                    ...
                    className="flex items-center gap-3 text-left hover:bg-[var(--border)] p-1 -ml-1 rounded-[var(--radius-panel)] transition-colors"
                  >
                    <div className="w-10 h-10 rounded-full overflow-hidden relative">
                      {isGroupConv ? (
                        selectedConv.group_avatar ? (
                          ...
                          <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[18px] font-semibold uppercase">
                ...
                <div className="relative flex items-center gap-1 sm:gap-2">
                  <button
                    onClick={() => setShowSharedSidebar(!showSharedSidebar)}
                    className={`p-2 rounded-full transition-colors ${showSharedSidebar ? "bg-[var(--border)] text-[var(--ink)]" : "text-[var(--brand)] hover:bg-[var(--surface-quiet)] "}`}
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full transition-colors hidden sm:block ${showSearchMsgBar ? "bg-[var(--border)] text-[var(--ink)]" : "text-[var(--brand)] hover:bg-[var(--surface-quiet)]"}`}
                    title="Tìm kiếm"
                  >
                    <Search className="w-4 h-4" />
                  </button>
              ...
              {showSearchMsgBar && (
                <div className="px-6 md:px-0 py-3 bg-[var(--surface-quiet)]">
                  <div className="relative">
                    <Search className="w-4 h-4 text-[var(--ink-muted)] absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                    <input
                      type="text"
                      value={searchMsgQuery}
                      onChange={(e) => handleSearchMessages(e.target.value)}
                      className="w-full bg-white border border-[var(--border-strong)] text-[var(--ink)] pl-9 pr-8 py-2 rounded-[var(--radius-control)] text-[15px] focus:outline-none focus:border-[var(--brand)] transition-colors"
                    />
                    {searchMsgQuery && (
                      <button
                        onClick={() => { setSearchMsgQuery(""); setSearchedMsgResults([]); }}
                        className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--ink-faint)] hover:text-[var(--ink-muted)] transition-colors"
                      >
                    ...
                    <div className="mt-2 max-h-[180px] overflow-y-auto space-y-1 hide-scrollbar">
                      {searchedMsgResults.map((m) => (
                        <div
                          ...
                          className="px-3 py-2 bg-white rounded-[var(--radius-control)] cursor-pointer hover:bg-[var(--surface-quiet)] transition-colors"
                        >
                          <p className="text-[13px] text-[var(--ink)] truncate">{m.content}</p>
                  ...
                  {searchMsgQuery.length > 0 && searchedMsgResults.length === 0 && (
                    <p className="text-[12px] text-[var(--ink-faint)] mt-2 text-center">Không tìm thấy kết quả</p>
                  )}
                </div>
              )}
                  ...
                  <div className="z-10 sticky top-0 bg-transparent flex flex-col w-full transition-all duration-300">
                    ...
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        <span className="text-[13px] text-[var(--ink)] opacity-90 truncate">
                          {pinnedMsgs[pinnedMsgs.length - 1].content || "Đính kèm"}
                        </span>
                      </div>
                      {pinnedMsgs.length > 1 && (
                        <div className="text-[var(--ink-muted)] flex items-center gap-1 shrink-0 ml-4">
                    ...
                    {isPinnedExpanded && pinnedMsgs.length > 1 && (
                      <div className="absolute top-full left-0 w-full bg-[var(--surface-quiet)] flex flex-col z-50">
                  ...
                  <div className="space-y-4 flex flex-col h-full justify-end pb-4">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className={`flex ${i % 2 === 0 ? "justify-end" : "justify-start"} animate-pulse`}>
                        <div className={`w-48 h-10 rounded-[var(--radius-panel)] ${i % 2 === 0 ? "bg-[var(--border-strong)]" : "bg-[var(--border)]"}`} />
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="space-y-4">
                        ...
                        <div className="flex flex-col mb-4 w-full">
                          <div className={`flex flex-col w-full ${isSender ? "items-end" : "items-start"}`}>
                            <div className={`group relative max-w-[85%] flex flex-col ${isSender ? "items-end" : "items-start"}`}>
                              <div className="text-[12px] font-semibold text-[var(--ink-muted)] mb-1 px-1">Bản tin gốc</div>
                              <div className={`rounded-[var(--radius-panel)] p-4 ${isSender ? `${getThemeBgClass(conversationTheme)} text-white` : "bg-[var(--border)] text-[var(--ink)]"} opacity-90`}>
                                <p className="text-[15px] leading-relaxed whitespace-pre-wrap">{parentMessage.content || "[Đính kèm]"}</p>
                              </div>
                            </div>
                          </div>
                          <div className="w-full h-[1px] bg-[var(--border)] my-6 relative">
                            <span className="absolute left-1/2 -translate-x-1/2 -top-2.5 bg-[var(--surface-quiet)] px-4 text-[12px] font-medium text-[var(--ink-muted)]">
                    ...
                    {activeThreadParentId && loadingThread ? (
                      <div className="flex justify-center p-4">
                        <Circle className="w-6 h-6 animate-spin text-[var(--brand)]" />
                      </div>
                      ...
                      if (msg.is_system) {
                        return (
                          <div key={msg._id || msg.id || i} className="flex justify-center w-full my-4">
                            <span className="bg-[var(--border)] text-[var(--ink)] px-4 py-1.5 rounded-full text-[13px] font-medium">
                              {msg.content}
                            </span>
                          </div>
                        );
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div
                              className={`rounded-[var(--radius-panel)] flex flex-col gap-2 ${
                                msg.is_recalled
                                  ? "bg-transparent border border-dashed border-[var(--border-strong)] text-[var(--ink-muted)] min-h-[38px] p-4 justify-center"
                                  ...
                                  : "bg-[var(--border)] text-[var(--ink)] p-4"
                            ...
                            >
                              {msg.reply_to && !msg.is_recalled && (
                                <div 
                                  ...
                                  className={`text-[12px] px-2 py-1.5 rounded-[var(--radius-control)] truncate max-w-[250px] opacity-80 cursor-pointer hover:opacity-100 transition-opacity ${isSender ? "bg-[var(--brand-hover)] text-white" : "bg-[var(--border)] text-[var(--ink-muted)]"}`}
                                >
                              ...
                              {msg.image_url && !msg.is_recalled && (
                                <img
                                  ...
                                  alt=""
                                  className="rounded-[var(--radius-control)] max-h-[300px] object-cover"
                                />
                              )}
                              {msg.attachments && msg.attachments.length > 0 && !msg.is_recalled && (
                                <div className="space-y-2">
                                  {msg.attachments.map((att: any, idx: number) => (
                                    <a key={idx} href={att.url && att.url.startsWith("http") ? att.url : `${API_URL}/storage/${att.url}`} target="_blank" rel="noreferrer" className={`flex items-center gap-2 p-2 rounded-[var(--radius-control)] ${isSender ? "bg-[var(--brand-hover)] text-white" : "bg-[var(--border)] text-[var(--ink)]"}`}>
                              ...
                              {!msg.is_recalled && !msg.poll_data && msg.content && msg.content !== "Tin nhắn thoại" && (
                                msg.content.startsWith("Shared document preview and link to access") ? (
                                  (() => {
                                    ...
                                    if (match) {
                                      return (
                                        <a href={`/tai-lieu/${match[2]}`} target="_blank" rel="noreferrer" className={`flex flex-col gap-2 p-3 rounded-[var(--radius-workspace)] border shadow-sm ${isSender ? "bg-[var(--brand)] border-white/20 text-white" : "bg-white border-[var(--border-strong)] text-[var(--ink)]"} transition-all hover:opacity-90 w-full min-w-[240px] max-w-[280px]`}>
                                          <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 shrink-0 rounded-[var(--radius-control)] flex items-center justify-center ${isSender ? "bg-white/20" : "bg-[var(--surface-quiet)]"}`}>
                                              <FileText className={`w-5 h-5 ${isSender ? "text-white" : "text-[var(--brand)]"}`} />
                                            </div>
                                            <div className="flex flex-col overflow-hidden">
                                              <span className="text-[14px] font-semibold truncate leading-tight">{match[1]}</span>
                                              <span className={`text-[12px] mt-0.5 ${isSender ? "text-white/80" : "text-[var(--ink-muted)]"}`}>Tài liệu DocLib</span>
                                            </div>
                                          </div>
                                          <div className={`text-[12px] px-3 py-1.5 rounded-full text-center font-medium mt-1 ${isSender ? "bg-white/10 text-white" : "bg-[var(--surface-quiet)] text-[var(--brand)]"}`}>
                                            Xem chi tiết
                                          </div>
                                        </a>
                                      );
                            ...
                            {!msg.poll_data && (
                              <div className={`flex items-center gap-2 mt-1 ${isSender ? "flex-row-reverse mr-1" : "flex-row ml-1"}`}>
                                ...
                                {!msg.is_recalled && msg.reactions && msg.reactions.length > 0 && (
                                  <div className="bg-white border border-[var(--border-strong)] rounded-full px-1.5 py-0.5 text-[11px] flex items-center gap-1 shadow-sm text-[var(--ink)]">
                                    {(() => {
                                      ...
                                      return Object.entries(counts).map(([emoji, count]) => (
                                        <span key={emoji} className="flex items-center gap-0.5 font-medium leading-none">
                                          ...
                                          <span className="text-[var(--ink-muted)] text-[11px] tabular-nums leading-none">{count}</span>
                                        </span>
                                      ));
                                    })()}
                                  </div>
                              ...
                              <div className="flex justify-end mt-1 mr-1">
                                <div className="w-5 h-5 rounded-full overflow-hidden border-[1.5px] border-white shadow-sm">
                                  {selectedConv.other_user?.avatar_url ? (
                                    ...
                                    <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[12px] font-semibold uppercase">
                                      {(aliases[selectedConv.other_user_id] || selectedConv.other_user?.full_name || selectedConv.other_user?.username || "U").charAt(0)}
                                    </div>
                                  )}
                                </div>
                    ...
                    {typingUsers[selectedConv.other_user_id] && (
                      <div className="flex w-full mb-4 justify-start">
                        <div className="flex gap-2 items-end max-w-[70%]">
                          <div className="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--border)] flex items-center justify-center overflow-hidden">
                            {selectedConv.other_user?.avatar_url ? (
                              <img src={selectedConv.other_user.avatar_url} className="w-full h-full object-cover" alt="" />
                            ) : (
                              <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[11px] font-semibold uppercase">
                                {(aliases[selectedConv.other_user_id] || selectedConv.other_user?.full_name || selectedConv.other_user?.username || "U").charAt(0)}
                              </div>
                            )}
                          </div>
                          <div className="px-4 py-3 rounded-[var(--radius-panel)] bg-[var(--surface-quiet)] rounded-bl-[4px]">
                            <div className="flex gap-1.5 h-2 items-center">
                              <span className="w-1.5 h-1.5 bg-[var(--ink-muted)] rounded-full animate-bounce [animation-delay:-0.3s]"></span>
                              <span className="w-1.5 h-1.5 bg-[var(--ink-muted)] rounded-full animate-bounce [animation-delay:-0.15s]"></span>
                              <span className="w-1.5 h-1.5 bg-[var(--ink-muted)] rounded-full animate-bounce"></span>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                if (cannotMessage) {
                  return (
                    <div className="flex justify-center p-4 py-8">
                      <span className="text-[13px] text-[var(--ink-muted)] bg-[var(--border)] px-4 py-2 rounded-full font-medium">Chỉ trưởng nhóm và phó nhóm mới được gửi tin nhắn</span>
                    </div>
                  );
                }
              ...
              <div className="px-4 pb-4 pt-2 bg-transparent relative">
                ...
                {imageFiles.length > 0 && (
                  <div className="flex gap-2 mb-3 overflow-x-auto hide-scrollbar">
                    {imageFiles.map((file, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative w-16 h-16 shrink-0 rounded-[var(--radius-control)] overflow-hidden border border-[var(--border-strong)] bg-white flex items-center justify-center">
                          {isImg && objectUrl ? (
                            <img src={objectUrl} alt="" className="w-full h-full object-cover" />
                          ) : (
                            <FileText className="w-6 h-6 text-[var(--ink-muted)]" />
                          )}
                ...
                <div className="flex items-center gap-3 h-[44px]">
                  ...
                  <div className="flex-1 relative">
                    {isRecording ? (
                      <div className="w-full bg-[var(--border)] border border-transparent rounded-[980px] pl-4 pr-1.5 h-[44px] text-[15px] flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-0.5">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center text-[var(--brand)] hover:bg-black/5 rounded-full transition-colors"
                          >
                            {isRecordingPaused ? <Mic className="w-[18px] h-[18px]" /> : <Pause className="w-[18px] h-[18px]" />}
                          </button>
                          <button
                            onClick={handleCancelRecording}
                            className="w-8 h-8 flex items-center justify-center text-[var(--ink-muted)] hover:text-red-500 hover:bg-black/5 rounded-full transition-colors"
                          >
                      ...
                      <>
                        <button
                          ...
                          className="absolute left-1.5 top-1/2 -translate-y-1/2 w-[36px] h-[36px] flex items-center justify-center text-[var(--brand)] hover:bg-[var(--surface-quiet)] rounded-full z-10 transition-colors"
                        >
                          <Paperclip className="w-[18px] h-[18px]" />
                        </button>
                        <button
                          onClick={openShareDoc}
                          className="absolute left-[40px] top-1/2 -translate-y-1/2 w-[36px] h-[36px] flex items-center justify-center text-[var(--brand)] hover:bg-[var(--surface-quiet)] rounded-full z-10 transition-colors"
                        >
                          <FileText className="w-[18px] h-[18px]" />
                        </button>
                        <button
                          onClick={() => setShowPollModal(true)}
                          className="absolute left-[78px] top-1/2 -translate-y-1/2 w-[36px] h-[36px] flex items-center justify-center text-[var(--brand)] hover:bg-[var(--surface-quiet)] rounded-full z-10 transition-colors"
                        >
                          <BarChart2 className="w-[18px] h-[18px]" />
                        </button>
                        <button
                          onClick={() => setShowScheduleModal(true)}
                          className="absolute left-[116px] top-1/2 -translate-y-1/2 w-[36px] h-[36px] flex items-center justify-center text-[var(--brand)] hover:bg-[var(--surface-quiet)] rounded-full z-10 transition-colors"
                        >
                        ...
                        <input
                          ...
                          placeholder=""
                          className="w-full h-[44px] bg-white border border-transparent rounded-[980px] pl-[158px] pr-[44px] text-[15px] focus:outline-none focus:border-[var(--border-strong)]"
                        />
                        <button
                          onClick={handleStartRecording}
                          className="absolute right-1.5 top-1/2 -translate-y-1/2 w-[36px] h-[36px] flex items-center justify-center text-[var(--brand)] hover:bg-[var(--surface-quiet)] rounded-full z-10 transition-colors"
                        >
            ...
            <div className="flex items-center justify-center w-full h-full min-h-[500px]">
              <span className="text-[17px] text-[var(--ink-muted)]">
                Chưa có dữ liệu
              </span>
            </div>
          )}
        ...
        {showSharedSidebar && selectedConv && (
          <div className="w-[300px] md:w-[320px] shrink-0 bg-[var(--surface-quiet)] border-l border-[var(--border-strong)] flex flex-col h-full overflow-hidden">
            <div className="flex-1 overflow-y-auto hide-scrollbar pt-6">
              <div className="flex flex-col items-center px-4 pb-6">
                <div className="w-24 h-24 rounded-full bg-[var(--border-strong)] overflow-hidden mb-3 relative shadow-sm group">
                  {isGroupConv ? (
                    (selectedConv.group_avatar || selectedConv.other_user?.avatar_url) ? (
                      ...
                      <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[40px] font-semibold uppercase">
                        {(selectedConv.group_name || selectedConv.other_user?.full_name || "G").charAt(0)}
                      </div>
                    )
                  ) : (
                  ...
                  {!isGroupConv && onlineUsers[selectedConv.other_user_id] === true && (
                    <span className="absolute bottom-1 right-1 w-4 h-4 bg-green-500 border-[3px] border-[var(--surface-quiet)] rounded-full"></span>
                  )}
                  {isGroupConv && (
                ...
                <h4 className="text-[20px] font-semibold text-[var(--ink)] text-center leading-tight mt-1 mb-1">
                  {isGroupConv 
                    ? (selectedConv.group_name || selectedConv.other_user?.full_name || "Nhóm trò chuyện") 
                    : (aliases[selectedConv.other_user_id] || selectedConv.other_user?.full_name || selectedConv.other_user?.username)}
                </h4>
                <p className="text-[14px] text-[var(--ink-muted)] mt-1 text-center px-4">
                  {renderOnlineStatus(selectedConv.other_user_id, isGroupConv, selectedConv.participant_ids?.length)}
                </p>
              </div>

              <div className="mt-2 px-3 space-y-4">
                <div className="bg-white rounded-[var(--radius-workspace)] shadow-sm border border-[var(--border)] overflow-hidden divide-y divide-[var(--border)]">
                  {!isGroupConv && (
                    <button onClick={() => { setAliasInput(aliases[selectedConv.other_user_id] || ""); setShowAliasModal(true); }} className="w-full px-4 py-3 flex items-center justify-between hover:bg-[var(--surface-quiet)] active:bg-[var(--border)] transition-colors">
                      <span className="text-[15px] text-[var(--ink)] font-medium">Đổi biệt danh</span>
                      <ChevronRight className="w-4 h-4 text-[var(--ink-faint)]" />
                    </button>
                  )}
                  <button onClick={handleToggleMute} className="w-full px-4 py-3 flex items-center justify-between hover:bg-[var(--surface-quiet)] active:bg-[var(--border)] transition-colors">
                    <span className="text-[15px] text-[var(--ink)] font-medium">Âm báo</span>
                    <span className="text-[14px] text-[var(--ink-muted)]">{isMuted ? "Tắt" : "Bật"}</span>
                  </button>
                  <div className="px-4 py-3 flex items-center justify-between">
                    <span className="text-[15px] text-[var(--ink)] font-medium">Chủ đề</span>
                    <div className="flex gap-2">
                      <button onClick={() => updateTheme("default")} className="w-5 h-5 rounded-full bg-[var(--brand)] border border-transparent hover:border-black" />
                  ...
                  <div>
                    <button onClick={() => setShowSelfDestructMenu(!showSelfDestructMenu)} className="w-full px-4 py-3 flex items-center justify-between hover:bg-[var(--surface-quiet)] active:bg-[var(--border)] transition-colors">
                      <span className="text-[15px] text-[var(--ink)] font-medium">Tự hủy tin</span>
                    ...
                    {showSelfDestructMenu && (
                      <div className="px-3 pb-2 pt-1 bg-[var(--surface-quiet)]/50">
                        {[
                          { label: "Tắt", value: 0 },
                          ...
                          <button
                            ...
                            className={`w-full text-left py-2 px-3 rounded-[var(--radius-control)] flex items-center justify-between text-[14px] ${selfDestructSeconds === opt.value ? "bg-[var(--brand)]/10 text-[var(--brand)] font-medium" : "text-[var(--ink)] hover:bg-black/5"}`}
                          >
                            {opt.label}
                            {selfDestructSeconds === opt.value && <Check className="w-4 h-4 text-[var(--brand)]" />}
                          </button>
                        ))}
                      </div>
                    )}
                ...
                <div className="bg-white rounded-[var(--radius-workspace)] shadow-sm border border-[var(--border)] p-4">
                  <h5 className="text-[13px] font-semibold text-[var(--ink-muted)] uppercase tracking-wider mb-3">File phương tiện</h5>
                  {sharedAttachments.length > 0 ? (
                    <div className="grid grid-cols-3 gap-2">
                      {sharedAttachments.slice(0, 6).map((att: any, idx: number) => {
                        ...
                        return (
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="aspect-square bg-[var(--surface-quiet)] rounded-[var(--radius-control)] flex flex-col items-center justify-center text-[var(--ink-faint)] hover:bg-[var(--border)] transition-colors p-1 cursor-pointer overflow-hidden border border-[var(--border-strong)]/50"
                          >
                            {isImg ? (
                              <img src={href} className="w-full h-full object-cover rounded-[6px]" alt="" />
                            ) : url.match(/\.(mp4|webm|mov)$/i) ? (
                              <Video className="w-6 h-6 mb-1 text-[var(--ink-muted)]" />
                            ) : (
                              <FileText className="w-6 h-6 mb-1 text-[var(--ink-muted)]" />
                            )}
                            {!isImg && (
                              <span className="text-[10px] text-[var(--ink-muted)] truncate w-full text-center px-1">
                                {att.name || "File"}
                              </span>
                            )}
                          </a>
                      ...
                      {sharedAttachments.length > 6 && (
                        <div className="col-span-3 text-center mt-2 pt-2 border-t border-[var(--surface-quiet)]">
                          <button className="text-[14px] text-[var(--brand)] font-medium hover:underline">Xem tất cả ({sharedAttachments.length})</button>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="text-center py-4 bg-[var(--surface-quiet)] rounded-[var(--radius-control)]">
                      <p className="text-[13px] text-[var(--ink-muted)]">Chưa có file nào</p>
                    </div>
                  )}
                </div>
                ...
                {isGroupConv && (
                  <>
                    <div className="mt-4 bg-white rounded-[var(--radius-workspace)] shadow-sm border border-[var(--border)] p-4">
                      <h5 className="text-[13px] font-semibold text-[var(--ink-muted)] uppercase tracking-wider mb-3">Thành viên ({selectedConv.participant_ids?.length || 0})</h5>
                      <div className="flex flex-col max-h-[150px] overflow-y-auto hide-scrollbar divide-y divide-[var(--border)]/50 -mx-2 px-2">
                        {(selectedConv.participant_ids || []).map((mId: string) => {
                          ...
                          return (
                            <div key={mId} className="flex items-center gap-3 py-2 hover:bg-[var(--surface-quiet)] rounded-[var(--radius-control)] transition-colors cursor-pointer px-2">
                              <div className="w-8 h-8 rounded-full bg-[var(--border-strong)] overflow-hidden shrink-0">
                                {mUser?.avatar_url ? (
                              ...
                              <div className="flex flex-col flex-1 min-w-0">
                                <span className="text-[14px] font-medium text-[var(--ink)] truncate">{mUser?.full_name || mUser?.username || "Thành viên"}</span>
                                {isGroupAdmin ? (
                                  <span className="text-[11px] text-[var(--brand)] font-medium mt-0.5">Trưởng nhóm</span>
                                ) : isGroupDeputy ? (
                                  <span className="text-[11px] text-[var(--brand)] mt-0.5">Phó nhóm</span>
                                ) : null}
                              </div>
                            </div>
                          )
                    ...
                    <div className="bg-white rounded-[var(--radius-workspace)] shadow-sm border border-[var(--border)] overflow-hidden divide-y divide-[var(--border)] mt-4 mb-2">
                       <button onClick={async () => {
                         try {
                           let token = selectedConv.invite_token;
                           if (!token) {
                             const data = await generateGroupInviteAPI(
                               selectedConv.other_user_id,
                             );
                       ...
                       }} className="w-full px-4 py-3 flex items-center gap-3 text-[15px] font-medium text-[var(--brand)] hover:bg-[var(--surface-quiet)] active:bg-[var(--border)] transition-colors">
                ...
                {!isGroupConv && (
                  <div className="bg-white rounded-[var(--radius-workspace)] shadow-sm overflow-hidden border border-[var(--border)] mb-6">
                    <button onClick={handleBlockUser} className="w-full px-4 py-3 flex items-center gap-3 text-[15px] font-medium text-red-500 hover:bg-[var(--surface-quiet)] active:bg-[var(--border)] transition-colors">
      ...
      {activeMsgMenuId && activeMsgRect && activeMsgObj && (() => {
        ...
        return (
          <>
            ...
            >
               <div
                  className={`rounded-[var(--radius-panel)] flex flex-col gap-2 p-4 ${
                    activeMsgObj.is_recalled
                      ? "bg-white/90 border border-dashed border-[var(--border-strong)] text-[var(--ink-muted)] justify-center min-h-[38px]"
                      : isSender
                ...
                >
                  {activeMsgObj.reply_to && !activeMsgObj.is_recalled && (
                    <div className={`text-[12px] px-2 py-1.5 rounded-[var(--radius-control)] truncate opacity-80 ${isSender ? "bg-[var(--brand-hover)] text-white" : "bg-[var(--border)] text-[var(--ink-muted)]"}`}>
                      <span className="font-semibold block mb-0.5">Trích dẫn:</span>
                  ...
                  {activeMsgObj.image_url && !activeMsgObj.is_recalled && (
                    <img src={activeMsgObj.image_url.startsWith("http") ? activeMsgObj.image_url : `${API_URL}/storage/${activeMsgObj.image_url}`} alt="" className="rounded-[var(--radius-control)] max-h-[300px] object-cover" />
                  )}
                  {activeMsgObj.attachments && activeMsgObj.attachments.length > 0 && !activeMsgObj.is_recalled && (
                    <div className="space-y-2">
                      {activeMsgObj.attachments.map((att: any, idx: number) => (
                        <div key={idx} className={`flex items-center gap-2 p-2 rounded-[var(--radius-control)] ${isSender ? "bg-[var(--brand-hover)] text-white" : "bg-[var(--border)] text-[var(--ink)]"}`}>
                          <FileText className="w-5 h-5 shrink-0" />
                          <span className="text-[13px] truncate">{att.name || "Tài liệu đính kèm"}</span>
                        </div>
                      ))}
                  ...
                  {activeMsgObj.audio_url && !activeMsgObj.is_recalled && (
                    <div className={`text-[13px] ${isSender ? "text-white/80" : "text-[var(--ink-muted)]"}`}>[Tin nhắn thoại]</div>
                  )}
               ...
               <div className={`flex items-center gap-2 mt-1 ${isSender ? "flex-row-reverse mr-1" : "flex-row ml-1"}`}>
                  ...
                  {!activeMsgObj.is_recalled && activeMsgObj.reactions && activeMsgObj.reactions.length > 0 && (
                    <div className="bg-white border border-[var(--border-strong)] rounded-full px-1.5 py-0.5 text-[11px] flex items-center gap-1 shadow-md text-[var(--ink)]">
                      {(() => {
                        const counts: Record<string, number> = {};
            ...
            >
              ...
              {!isRecalled && (
                <div className="flex items-center gap-1 bg-white/95 backdrop-blur-md border border-[var(--border)] rounded-full px-3 py-2 shadow-[0_8px_32px_rgba(0,0,0,0.18)] self-start">
              ...
              <div className="flex flex-col bg-white/95 backdrop-blur-md border border-[var(--border)] rounded-[var(--radius-workspace)] shadow-[0_8px_32px_rgba(0,0,0,0.15)] overflow-hidden">
                {!isRecalled && (
                  <button
                    onClick={() => { setReplyingTo(activeMsgObj); dismiss(); }}
                    className="flex items-center gap-3 w-full px-4 py-3 text-[15px] text-[var(--ink)] hover:bg-[var(--surface-quiet)] border-b border-[var(--surface-quiet)] text-left transition-colors"
                  >
                    <Reply className="w-[18px] h-[18px] text-[var(--ink-muted)]" />
                    Trả lời
                  </button>
                )}
                ...
                {!isRecalled && (
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 text-[15px] text-[var(--ink)] hover:bg-[var(--surface-quiet)] border-b border-[var(--surface-quiet)] text-left transition-colors"
                  >
                    <MessageSquareReply className="w-[18px] h-[18px] text-[var(--ink-muted)]" />
                ...
                {!isRecalled && (
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 text-[15px] text-[var(--ink)] hover:bg-[var(--surface-quiet)] border-b border-[var(--surface-quiet)] text-left transition-colors"
                  >
                    <Share2 className="w-[18px] h-[18px] text-[var(--ink-muted)]" />
                    Chuyển tiếp
                  </button>
                )}
                ...
                {!isRecalled && (
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 text-[15px] text-[var(--ink)] hover:bg-[var(--surface-quiet)] border-b border-[var(--surface-quiet)] text-left transition-colors"
                  >
                    {activeMsgObj.is_pinned ? <PinOff className="w-[18px] h-[18px] text-[var(--ink-muted)]" /> : <Pin className="w-[18px] h-[18px] text-[var(--ink-muted)]" />}
                ...
                {!isRecalled && isSender && (
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 text-[15px] text-[var(--ink)] hover:bg-[var(--surface-quiet)] border-b border-[var(--surface-quiet)] text-left transition-colors"
                  >
                    <Edit2 className="w-[18px] h-[18px] text-[var(--ink-muted)]" />
                    Chỉnh sửa
                  </button>
                )}
                <button
                  onClick={() => setShowDeleteSubMenu(showDeleteSubMenu === msgId ? null : msgId)}
                  className={`flex items-center justify-between gap-3 w-full px-4 py-3 text-[15px] text-red-500 hover:bg-[#FFF5F5] text-left transition-colors ${showDeleteSubMenu === msgId ? "border-b border-[var(--surface-quiet)]" : ""}`}
                >
                  <div className="flex items-center gap-3">
                    <Trash2 className="w-[18px] h-[18px]" />
                    Xóa
                  ...
                  <div className="overflow-hidden">
                    {isSender && !isRecalled && (
                      <button
                        ...
                        className="flex items-center gap-3 w-full px-5 py-2.5 text-[14px] text-orange-500 hover:bg-orange-50 border-b border-[var(--surface-quiet)] text-left transition-colors"
                      >
      ...
      />
      <Modal
        isOpen={!!deleteConversationTarget}
        onClose={() => setDeleteConversationTarget(null)}
      >
        <ModalHeader>
          <ModalTitle>Xóa cuộc trò chuyện</ModalTitle>
        </ModalHeader>
        <ModalContent>
