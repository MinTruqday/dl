...
export async function assessmentRequest(path, options = {}) {
    const response = await authenticatedFetch(`${API_URL}${path}`, Object.assign(Object.assign({}, options), { headers: Object.assign({ "Content-Type": "application/json" }, options.headers) }));
    const expected = options.expectedStatus;
    if (expected ? response.status !== expected : !response.ok) {
        const body = await response.json().catch(() => null);
        const error = new Error(typeof (body === null || body === void 0 ? void 0 : body.detail) === "string" ? body.detail : "Không thể hoàn tất yêu cầu");
        error.status = response.status;
        error.detail = body === null || body === void 0 ? void 0 : body.detail;
        throw error;
    }
...
export async function exportAssessmentVersion(versionId, format) {
    ...
    if (!response.ok) {
        const body = await response.json().catch(() => null);
        throw new Error(typeof (body === null || body === void 0 ? void 0 : body.detail) === "string" ? body.detail : "Không thể xuất bài đánh giá");
    }
...
export function restoreQuestionDraftVersion(id, versionId) {
    return assessmentRequest(`/question-drafts/${id}/restore-version/${versionId}`, { method: "POST" });
}
...
export function createTeacherMaterialMapping(documentId, payload) {
    return assessmentRequest(`/education/sources/${documentId}/map`, {
        method: "POST",
        body: JSON.stringify(Object.assign(Object.assign({}, payload), { document_id: documentId, source_type: "teacher_material", authority: "supplementary" })),
    });
}
...
export function createAssessment(assessmentDraftId, deliveryPolicy) {
    return assessmentRequest("/assessments", { method: "POST", body: JSON.stringify({ assessment_draft_id: assessmentDraftId, delivery_policy: deliveryPolicy }) });
}
export function publishAssessment(assessmentId, assessmentDraftId, expectedRevision, scheduledFor) {
    return assessmentRequest(`/assessments/${assessmentId}/publish`, { method: "POST", body: JSON.stringify({ assessment_draft_id: assessmentDraftId, expected_revision: expectedRevision, idempotency_key: `publish-${crypto.randomUUID()}`, scheduled_for: scheduledFor || null }) });
}
