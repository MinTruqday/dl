...
export default function UserSettingsPage() {
  ...
  useEffect(() => {
    assessmentRequest("/education/profiles/me/settings")
      .then((value) =>
        setSettings((current) => ({
          ...current,
          ...value,
  ...
  const save = async () => {
    ...
    try {
      const value = await assessmentRequest("/education/profiles/me/settings", {
        ...
        body: JSON.stringify({
          ...settings,
          default_subject: settings.default_subject || null,
        }),
      });
      setSettings((current) => ({
        ...current,
  ...
  const exportData = async () => {
    ...
    try {
      ...
      const csvRows = Object.entries(value).flatMap(([section, sectionValue]) =>
      ...
      const blob = new Blob([useCsv ? csv : JSON.stringify(value, null, 2)], {
        type: useCsv ? "text/csv" : "application/json",
      });
  ...
  const updateAccessibility = (key, value) =>
    setSettings((current) => ({
      ...
      accessibility_preferences: {
        ...(current.accessibility_preferences || {}),
        [key]: value,
      },
    }));
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-5 md:p-8">
      <div>
        ...
        <h1 className="mt-2 text-[30px] font-semibold">Cài đặt tài khoản và trải nghiệm</h1>
        <p className="mt-2 text-[13px] text-ink-muted">
          Môn gần đây chỉ là tiện ích và không trở thành identity cố định
        </p>
      </div>
      <section className="grid gap-4 rounded-panel border border-border bg-surface p-5 sm:grid-cols-2">
        ...
        <label className="text-[12px] font-semibold text-ink-muted">
          ...
          <input
            ...
            onChange={(event) =>
              setSettings((current) => ({
                ...current,
                default_subject: event.target.value,
              }))
            }
          />
        </label>
        <label className="text-[12px] font-semibold text-ink-muted">
          ...
          <select
            ...
            onChange={(event) =>
              setSettings((current) => ({
                ...current,
                data_export_format: event.target.value,
              }))
            }
          >
        ...
        <label className="flex items-center gap-2 text-[12px] font-semibold">
          <input
            type="checkbox"
            checked={Boolean(settings.privacy_mode)}
            onChange={(event) =>
              setSettings((current) => ({
                ...current,
          ...
          />{" "}
          Chế độ riêng tư tăng cường
        </label>
        <label className="flex items-center gap-2 text-[12px] font-semibold">
          <input
            type="checkbox"
            checked={Boolean(settings.accessibility_preferences?.reduced_motion)}
            onChange={(event) => updateAccessibility("reduced_motion", event.target.checked)}
          />{" "}
          Giảm chuyển động
        </label>
        <label className="flex items-center gap-2 text-[12px] font-semibold">
          <input
            type="checkbox"
            checked={Boolean(settings.accessibility_preferences?.high_contrast)}
            onChange={(event) => updateAccessibility("high_contrast", event.target.checked)}
          />{" "}
          Tương phản cao
        </label>
        <div className="flex flex-wrap gap-2 sm:col-span-2">
          <button className="apple-button" onClick={() => void save()}>
            Lưu cài đặt
          </button>
          <button className="apple-button-secondary" onClick={() => void exportData()}>
            Xuất dữ liệu cá nhân
          </button>
          <Link className="apple-button-secondary" href="/cai-dat/vai-tro">
            Vai trò và cá nhân hóa
          </Link>
        </div>
      </section>
      {status && (
