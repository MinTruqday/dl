...
export default function GenerateAssessmentPage() {
  ...
  const [difficultyDistribution, setDifficultyDistribution] = useState(distributeDifficulty(3));
  ...
  const generate = async () => {
    setError("");
    try {
      if (useDistribution && !validDifficultyDistribution(count, difficultyDistribution))
        throw new Error("Tổng phân bố độ khó phải bằng số câu cần tạo");
      const value = await assessmentRequest(`/assessment-drafts/${draftId}/generate`, {
        method: "POST",
        body: JSON.stringify({
          ...
          count,
          target_difficulty: useDistribution ? null : difficulty,
          difficulty_distribution: useDistribution ? difficultyDistribution : {},
          cognitive_level: cognitiveLevel || null,
          intended_purpose: intendedPurpose,
          time_constraint_minutes: Number(timeConstraint) || null,
          target_learner_band: { minimum: Number(abilityMinimum), maximum: Number(abilityMaximum) },
          use_teacher_materials: useTeacherMaterials,
          source_scope: useTeacherMaterials ? "curriculum_and_owned_material" : "curriculum_only",
          source_evidence: [],
        }),
      });
      setResult(value);
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 p-5 md:p-8">
      <div>
        ...
        <h1 className="mt-2 text-[30px] font-semibold">AI tạo câu hỏi</h1>
        <p className="mt-2 text-[13px] text-ink-muted">
          Mọi câu sinh ra là QuestionDraft cần giáo viên rà soát và không tự xuất bản
        </p>
      </div>
      <section className="grid gap-4 rounded-panel border border-border bg-surface p-5 md:grid-cols-2">
        ...
        <label className="text-[13px] font-semibold">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              setCount(next);
              setDifficultyDistribution(distributeDifficulty(next));
            }}
          />
        </label>
        <label className="text-[13px] font-semibold">
          ...
          >
            <option value="">Hệ thống đề xuất</option>
        ...
        <label className="flex items-center gap-2 text-[13px] font-semibold">
          <input
            ...
            onChange={(event) => setUseDistribution(event.target.checked)}
          />{" "}
          Dùng phân bố năm mức
        </label>
        {useDistribution ? (
          <div className="grid grid-cols-5 gap-2 md:col-span-2">
            {[1, 2, 3, 4, 5].map((level) => (
              <label key={level} className="text-center text-[12px] font-semibold">
                Mức {level}
                <input
                  className="apple-input mt-2 w-full text-center"
                  type="number"
                  min="0"
        ...
        <label className="flex items-center gap-2 text-[13px] font-semibold md:col-span-2">
          <input
            type="checkbox"
            checked={useTeacherMaterials}
            onChange={(event) => setUseTeacherMaterials(event.target.checked)}
          />{" "}
          Dùng curriculum và tài liệu riêng thuộc sở hữu của tôi
        </label>
        <p className="text-[12px] text-ink-muted md:col-span-2">
          Phạm vi nguồn{" "}
          {useTeacherMaterials ? "curriculum và tài liệu riêng" : "chỉ curriculum chính thống"}
        </p>
        <button
          className="apple-button md:col-span-2"
          disabled={!draftId || !topic.trim()}
          onClick={generate}
        >
          Tạo candidate có provenance
        </button>
      </section>
      {error && (
      ...
      {result && (
        <section className="rounded-panel border border-border bg-surface">
          <h2 className="border-b border-border px-5 py-4 font-semibold">Candidate chờ rà soát</h2>
          <div className="divide-y divide-border">
            {result.questions?.map((question) => {
              return (
                <div key={question._id} className="px-5 py-4">
                  <p className="font-semibold">{question._id}</p>
                  <p className="mt-1 text-[12px] text-ink-muted">
                    Độ khó dự đoán{" "}
                    {question.difficulty_prediction?.predicted_difficulty ??
                      "Ẩn theo research blind mode"}{" "}
                    · trạng thái needs teacher review
                  </p>
                </div>
              );
            })}
