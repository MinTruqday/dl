...
export default function AdminOperationsPage({ view }: { view: "curriculum" | "models" | "security" }) {
    ...
    try {
      if (view === "curriculum") {
        const [curriculum, mappings, stats, jobs] = await Promise.all([
          assessmentRequest<Record<string, any>[]>("/education/curriculum?include_obsolete=true"),
          assessmentRequest<Record<string, any>>("/education/mappings/review"),
          assessmentRequest<Record<string, any>>("/thu-thap/thong-ke"),
          assessmentRequest<Record<string, any>[]>("/thu-thap/jobs"),
        ]);
        setData({ curriculum, mappings, stats, jobs });
      } else if (view === "models") {
        const [models, health] = await Promise.all([
          assessmentRequest<Record<string, any>>("/operations/models"),
          assessmentRequest<Record<string, any>>("/operations/health"),
        ]);
        setData({ ...models, health });
      } else {
        const [events, privacy] = await Promise.all([
          assessmentRequest<Record<string, any>[]>("/audit"),
          assessmentRequest<Record<string, any>>("/operations/privacy-policy"),
        ]);
  ...
  const startCollection = async () => {
    try {
      const result = await assessmentRequest<Record<string, any>>("/thu-thap/kich-hoat", {
        method: "POST",
  ...
  const confirmMapping = async (mapping: Record<string, any>) => {
  ...
  const rejectMapping = async (mapping: Record<string, any>) => {
  ...
  const reindexSource = async (documentId: string) => {
    try {
      const result = await assessmentRequest<Record<string, any>>(`/education/sources/${documentId}/reindex`, { method: "POST" });
  ...
  const editCurriculumNode = async (node: Record<string, any>) => {
  ...
  const mergeCurriculumNode = async (target: Record<string, any>) => {
    const sourceText = window.prompt("Node IDs cần gộp vào node này");
    if (!sourceText?.trim()) return;
    const sourceNodeIds = sourceText.split(/[,;\n]+/).map((value) => value.trim()).filter(Boolean);
    const nodesById = Object.fromEntries((data.curriculum || []).map((node: any) => [node._id, node]));
  ...
  const splitCurriculumNode = async (node: Record<string, any>) => {
    ...
    try {
      const result = await assessmentRequest<Record<string, any>>(`/education/curriculum/${node._id}/split`, {
        method: "POST",
        body: JSON.stringify({ expected_revision: Number(node.revision || 1), parts }),
      });
      setMessage(`Đã tách thành ${result.parts?.length || 0} node và chuyển quan hệ hiện hữu sang phần đầu tiên`);
      await load();
    } catch (reason) {
      setError(messageOf(reason));
    }
  ...
  const obsoleteSource = async (documentId: string) => {
  ...
  const purgeExpiredPii = async () => {
    ...
    try {
      const result = await assessmentRequest<Record<string, any>>("/operations/privacy/purge", {
        method: "POST",
  ...
  if (loading) return <div className="mx-auto max-w-[1300px] p-8"><div className="skeleton h-72" /></div>;
