...
export default function TeacherDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([getTeacherDashboard(), getMyDocumentsAPI("", "", 20)])
  ...
  const metrics = [
    ["Đề đã xuất bản", data.published_assessments?.length || 0, ClipboardCheck],
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-7 p-5 md:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-brand">
            Không gian giáo viên
          </p>
          <h1 className="mt-2 text-[32px] font-semibold tracking-[-0.04em]">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link className="apple-button-secondary" href="/giao-vien/cau-hoi">
            Ngân hàng câu hỏi
          </Link>
          <Link className="apple-button" href="/giao-vien/de/soan-thao">
            <FilePlus2 size={16} /> Tạo đề mới
          </Link>
        </div>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(([label, value, Icon]) => (
          <section key={label} className="rounded-panel border border-border bg-surface p-5">
            ...
            <p className="mt-5 text-[30px] font-semibold">{value}</p>
      ...
      <section className="rounded-panel border border-border bg-surface">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <h2 className="font-semibold">Bản nháp gần đây</h2>
          <Link className="text-[13px] font-semibold text-brand" href="/giao-vien/de">
            Xem tất cả
          </Link>
        </div>
        <div className="divide-y divide-border">
          {(data.recent_drafts || []).map((draft) => (
            <Link
              ...
              className="flex items-center gap-4 px-5 py-4 hover:bg-surface-quiet"
            >
              <div className="min-w-0 flex-1">
                <p className="truncate text-[14px] font-semibold">{draft.title}</p>
                <p className="mt-1 text-[12px] text-ink-muted">{draft.status}</p>
              </div>
              <ArrowRight size={17} />
            </Link>
          ))}
          {!data.recent_drafts?.length && (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-panel border border-border bg-surface">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <h2 className="font-semibold">Cảnh báo dự đoán so với thực nghiệm</h2>
            <Link className="text-[13px] font-semibold text-brand" href="/giao-vien/hieu-chinh">
              Mở hiệu chỉnh
            </Link>
          </div>
          <div className="divide-y divide-border">
            {(data.predicted_empirical_alerts || []).map((alert) => (
              <div key={alert.question_version_id} className="px-5 py-4 text-[13px]">
                <p className="font-semibold">{alert.question_version_id}</p>
                <p className="mt-1 text-ink-muted">
                  AI {alert.predicted_difficulty} · Thực nghiệm {alert.empirical_difficulty} · Gap{" "}
                  {alert.gap} · Mẫu {alert.sample_size} {alert.drift_flag ? "· Có drift" : ""}
                </p>
              </div>
            ))}
            {!data.predicted_empirical_alerts?.length && (
              <p className="px-5 py-10 text-center text-[13px] text-ink-muted">
                Chưa có cảnh báo sai lệch
              </p>
            )}
          </div>
        </section>
        <section className="rounded-panel border border-border bg-surface">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <h2 className="font-semibold">Tài liệu gần đây</h2>
            <Link className="text-[13px] font-semibold text-brand" href="/giao-vien/tai-lieu">
              Mở thư viện
            </Link>
          </div>
          <div className="divide-y divide-border">
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 text-[12px] text-ink-muted">
                    ...
                    {material.education_metadata?.subject || "Chưa gắn môn"}
                  </p>
                </Link>
              );
            })}
