...
type Dashboard = {
  recent_drafts?: { _id: string; title: string; status: string; updated_at: string }[];
  published_assessments?: { _id: string; status: string }[];
  review_queue_count?: number;
  flagged_item_count?: number;
  calibration_ready_count?: number;
  predicted_empirical_alerts?: { question_version_id: string; predicted_difficulty: number; empirical_difficulty: number; gap: number; sample_size: number; drift_flag: boolean }[];
};
...
export default function TeacherDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([getTeacherDashboard(), getMyDocumentsAPI("", "", 20)]).then(([dashboard, documents]) => {
      ...
      setMaterials(rows.filter((row: any) => row.education_metadata?.source_type === "teacher_material").slice(0, 5));
    ...
    <div className="mx-auto max-w-[1400px] space-y-7 p-5 md:p-8">
      ...
      <section className="rounded-panel border border-border bg-surface">
        ...
        <div className="divide-y divide-border">
          ...
          {!data.recent_drafts?.length && <p className="px-5 py-10 text-center text-[13px] text-ink-muted">Chưa có bản nháp</p>}
        </div>
      </section>
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-panel border border-border bg-surface"><div className="flex items-center justify-between border-b border-border px-5 py-4"><h2 className="font-semibold">Cảnh báo dự đoán so với thực nghiệm</h2><Link className="text-[13px] font-semibold text-brand" href="/giao-vien/hieu-chinh">Mở hiệu chỉnh</Link></div><div className="divide-y divide-border">{(data.predicted_empirical_alerts || []).map((alert) => <div key={alert.question_version_id} className="px-5 py-4 text-[13px]"><p className="font-semibold">{alert.question_version_id}</p><p className="mt-1 text-ink-muted">AI {alert.predicted_difficulty} · Thực nghiệm {alert.empirical_difficulty} · Gap {alert.gap} · Mẫu {alert.sample_size} {alert.drift_flag ? "· Có drift" : ""}</p></div>)}{!data.predicted_empirical_alerts?.length && <p className="px-5 py-10 text-center text-[13px] text-ink-muted">Chưa có cảnh báo sai lệch</p>}</div></section>
        <section className="rounded-panel border border-border bg-surface"><div className="flex items-center justify-between border-b border-border px-5 py-4"><h2 className="font-semibold">Tài liệu gần đây</h2><Link className="text-[13px] font-semibold text-brand" href="/giao-vien/tai-lieu">Mở thư viện</Link></div><div className="divide-y divide-border">{materials.map((material) => <Link key={material._id} href={`/tai-lieu/xem-truoc/${material._id}`} className="block px-5 py-4 hover:bg-surface-quiet"><p className="font-semibold">{material.title}</p><p className="mt-1 text-[12px] text-ink-muted">{material.indexing_status || (material.is_indexed ? "indexed" : "queued")} · {material.education_metadata?.subject || "Chưa gắn môn"}</p></Link>)}{!materials.length && <p className="px-5 py-10 text-center text-[13px] text-ink-muted">Chưa có tài liệu bổ trợ</p>}</div></section>
      </div>
