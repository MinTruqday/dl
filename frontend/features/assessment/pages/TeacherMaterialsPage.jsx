...
import {
  ...
  getDocumentDraftAPI,
  getMyDocumentsAPI,
  retryDocumentIndexingAPI,
  updateDocumentAPI,
} from "@/features/content/services/document.service";
import {
  createTeacherMaterialMapping,
  listSourceMappings,
  reviewSourceMapping,
...
export default function TeacherMaterialsPage() {
  ...
  const load = async () => {
    ...
    const teacherMaterials = rows.filter((row) => {
      return row.education_metadata?.source_type === "teacher_material";
    });
    const mappingRows = await Promise.all(
      teacherMaterials.map(async (row) => [row._id, (await listSourceMappings(row._id))[0]]),
    );
    setMaterials(teacherMaterials);
    setMappings(Object.fromEntries(mappingRows.filter(([, mapping]) => mapping)));
    setMappingConcepts(
      Object.fromEntries(
        mappingRows.map(([documentId, mapping]) => [
          documentId,
          (mapping?.concept_ids || []).join(", "),
        ]),
      ),
    );
    setMappingNodes(
    ...
    try {
      for (const [index, file] of files.entries()) {
        ...
        try {
          ...
          const [uploaded, sourceVersion] = await Promise.all([
    ...
    return (
      (!materialSearch ||
        String(material.title || "")
          .toLocaleLowerCase()
          .includes(materialSearch.toLocaleLowerCase())) &&
  ...
  const searchIndexedContent = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Không thể tìm trong nội dung đã lập chỉ mục",
      );
    } finally {
      setBusy(false);
    }
  ...
  const reviewMapping = async (documentId, mappingStatus) => {
    ...
    setNotice("");
    try {
      const concepts = (mappingConcepts[documentId] || "")
        .split(/[,;\n]+/)
        .map((value) => value.trim())
        .filter(Boolean);
      const curriculumNodes = (mappingNodes[documentId] || "")
        .split(/[,;\n]+/)
        .map((value) => value.trim())
        .filter(Boolean);
      const skills = (mappingSkills[documentId] || "")
        .split(/[,;\n]+/)
        .map((value) => value.trim())
        .filter(Boolean);
      ...
      setNotice(
        mappingStatus === "confirmed"
          ? "Đã xác nhận ánh xạ tài liệu"
          : "Đã đưa ánh xạ về trạng thái cần rà soát",
      );
      await load();
    } catch (reason) {
  ...
  const loadExtractedText = async (documentId) => {
    ...
    try {
      ...
      setExtractedText((current) => ({
        ...current,
        [documentId]: String(document.extracted_text || ""),
      }));
    } catch (reason) {
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 p-5 md:p-8">
      <div>
        ...
        <h1 className="mt-2 text-[30px] font-semibold">Tài liệu bổ trợ riêng</h1>
        <p className="mt-2 text-[13px] text-ink-muted">
          Tài liệu chỉ được truy xuất trong phạm vi chủ sở hữu và không thay thế curriculum chính
          thống
        </p>
      </div>
      <form
        className="grid gap-4 rounded-panel border border-border bg-surface p-5 md:grid-cols-2"
        onSubmit={submit}
      >
        ...
        <label className="text-[13px] font-semibold">
          ...
          <input
            ...
            type="file"
            multiple
            accept=".pdf,.docx,.pptx,.txt,.md,.tex,.png,.jpg,.jpeg,.webp"
        ...
        {busy && (
          <div className="md:col-span-2">
            <div className="h-2 overflow-hidden rounded-full bg-surface-quiet">
              <div
                className="h-full bg-brand transition-[width]"
                style={{ width: `${progress}%` }}
              />
            </div>
            <p className="mt-1 text-[11px] text-ink-muted">Tiến độ {progress} phần trăm</p>
          </div>
        )}
        <button
          className="apple-button md:col-span-2"
          disabled={busy || !files.length || !title.trim()}
        >
          {busy ? "Đang xử lý" : `Tải lên và lập chỉ mục ${files.length || ""}`}
        </button>
      </form>
      ...
      <section className="rounded-panel border border-border bg-surface">
        <div className="grid gap-3 border-b border-border p-4 md:grid-cols-4">
          ...
          <button
            type="button"
            className="apple-button-secondary md:col-span-4"
            disabled={busy || !materialSearch.trim()}
            onClick={() => void searchIndexedContent()}
          >
            Tìm trong nội dung đã lập chỉ mục
          </button>
        </div>
        {semanticResults.length > 0 && (
          <div className="border-b border-border p-4">
            <p className="text-[12px] font-semibold">Kết quả tìm trong nội dung riêng</p>
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              {semanticResults.map((result, index) => {
                return (
                  <div
                    key={`${String(result.metadata?.chunk_id || result.metadata?.document_id)}-${index}`}
                    className="rounded-control bg-surface-quiet p-3 text-[12px]"
                  >
                    <p className="font-semibold">
                      {result.metadata?.title || result.metadata?.document_id}
                    </p>
                    <p className="mt-1 line-clamp-4 text-ink-muted">{result.text}</p>
                    <p className="mt-1">Score {Number(result.score || 0).toFixed(3)}</p>
                  </div>
                );
              })}
            </div>
            {semanticConflicts.length > 0 && (
              <p className="mt-3 rounded-control bg-warning-soft p-3 text-[12px] text-warning">
                Có {semanticConflicts.length} xung đột với nguồn khác cần giáo viên rà soát và tài
                liệu riêng không tự ghi đè curriculum
              </p>
            )}
          </div>
        )}
        <div className="divide-y divide-border">
          {visibleMaterials.map((material) => {
            const mapping = mappings[material._id];
            const indexingStatus =
              material.indexing_status || (material.is_indexed ? "indexed" : "queued");
            return (
              <div key={material._id} className="space-y-3 px-5 py-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="font-semibold">{material.title}</p>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-surface-quiet px-2 py-1 text-[11px] font-semibold">
                    ...
                    <a
                      ...
                      href={`/tai-lieu/xem-truoc/${material._id}`}
                    >
                      Xem trước
                    </a>
                    <button
                      type="button"
                      className="apple-button-secondary"
                      onClick={() => void renameMaterial(material)}
                    >
                      Đổi tên
                    </button>
                    <button
                      ...
                      onClick={() => void removeMaterial(material._id)}
                    >
                      Xóa
                    </button>
                    {indexingStatus === "failed" && (
                      <button
                        ...
                        className="apple-button-secondary"
                        disabled={busy}
                        onClick={() => retryIndexing(material._id)}
                      >
                        Thử lại
                      </button>
                    )}
                ...
                {material.indexing_error && (
                  <p className="text-[12px] text-danger">Mã lỗi {material.indexing_error}</p>
                )}
                {material.extracted_text && (
                  <details className="rounded-control border border-border p-3 text-[12px]">
                    <summary className="cursor-pointer font-semibold">
                      Văn bản đã trích xuất
                    </summary>
                    <pre className="mt-2 max-h-60 overflow-auto whitespace-pre-wrap">
                      {String(material.extracted_text)}
                    </pre>
                  </details>
                )}
                <label className="block text-[12px] font-semibold text-ink-muted">
                  Thay phiên bản nguồn
                  <input
                    className="apple-input mt-1 w-full"
                    type="file"
                    accept=".pdf,.docx,.pptx,.txt,.md,.tex,.png,.jpg,.jpeg,.webp"
                    ...
                    onChange={(event) => {
                      ...
                      if (replacement) void replaceMaterialVersion(material, replacement);
                    }}
                  />
                </label>
                {mapping && (
                  <div className="grid gap-2 md:grid-cols-3">
                    <label className="text-[12px] font-semibold text-ink-muted">
                      ...
                      <input
                        className="apple-input mt-1 w-full"
                        value={mappingNodes[material._id] || ""}
                        onChange={(event) =>
                          setMappingNodes((current) => ({
                            ...current,
                    ...
                    <label className="text-[12px] font-semibold text-ink-muted">
                      ...
                      <input
                        className="apple-input mt-1 w-full"
                        value={mappingConcepts[material._id] || ""}
                        onChange={(event) =>
                          setMappingConcepts((current) => ({
                            ...current,
                            [material._id]: event.target.value,
                          }))
                        }
                    ...
                    <label className="text-[12px] font-semibold text-ink-muted">
                      ...
                      <input
                        className="apple-input mt-1 w-full"
                        value={mappingSkills[material._id] || ""}
                        onChange={(event) =>
                          setMappingSkills((current) => ({
                            ...current,
                            [material._id]: event.target.value,
                          }))
                        }
                    ...
                    <div className="flex gap-2 md:col-span-3">
                      <button
                        ...
                        className="apple-button-secondary"
                        disabled={busy}
                        onClick={() => reviewMapping(material._id, "needs_review")}
                      >
                        Cần rà soát
                      </button>
                      <button
                        ...
                        disabled={busy}
                        onClick={() => reviewMapping(material._id, "confirmed")}
                      >
                        Lưu và xác nhận ánh xạ
                      </button>
                    </div>
                  </div>
                )}
      ...
      {visibleMaterials.some((material) => {
        return (
          material.extracted_text_available ||
          material.index_report?.failed_chunks?.length ||
          material.index_report?.quarantined_chunks?.length
        );
      }) && (
        <section className="rounded-panel border border-border bg-surface">
          <div className="border-b border-border px-5 py-4">
            <h2 className="font-semibold">Kết quả trích xuất và lập chỉ mục</h2>
          </div>
          <div className="divide-y divide-border">
            {visibleMaterials
              .filter((material) => {
              ...
              .map((material) => {
                const text = extractedText[material._id];
                return (
                  <div key={material._id} className="space-y-3 px-5 py-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-[13px] font-semibold">{material.title}</p>
                      {material.extracted_text_available && (
                        <button
                          type="button"
                          className="apple-button-secondary"
                          disabled={busy}
                          onClick={() => void loadExtractedText(material._id)}
                        >
                          {text !== undefined ? "Ẩn văn bản" : "Xem văn bản đã trích xuất"}
                        </button>
                      )}
                    </div>
                    {material.index_report?.failed_chunks?.length > 0 && (
                      <p className="text-[12px] text-danger">
                        Có {material.index_report.failed_chunks.length} chunk lập chỉ mục lỗi
                      </p>
                    )}
                    {material.index_report?.quarantined_chunks?.length > 0 && (
                      <p className="text-[12px] text-warning">
                        Có {material.index_report.quarantined_chunks.length} chunk bị cách ly an
                        toàn
                      </p>
                    )}
                    {text !== undefined && (
                      <div className="rounded-control border border-border p-3 text-[12px]">
                        <pre className="max-h-72 overflow-auto whitespace-pre-wrap">
                          {text || "Không có văn bản"}
                        </pre>
                        {material.extracted_text_truncated && (
                          <p className="mt-2 text-warning">
                            Bản xem trước đã được giới hạn kích thước
                          </p>
                        )}
                      </div>
                    )}
