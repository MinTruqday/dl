...
export default function PersonaSettingsPage() {
    ...
    useEffect(() => {
        assessmentRequest("/education/profiles/me").then((profile) => setPersonas(profile.personas || []));
        loadTeacherProfile().catch(() => undefined);
    }, []);
    const toggle = (persona) => setPersonas((current) => current.includes(persona) ? current.filter((value) => value !== persona) : [...current, persona]);
    const save = async () => { if (!personas.length)
        return; await assessmentRequest("/education/profiles/me", { method: "PUT", body: JSON.stringify({ personas }) }); setStatus("Đã lưu vai trò sử dụng"); };
    ...
    const resetPersonalization = async () => {
        if (!window.confirm("Xóa toàn bộ tùy chọn và tín hiệu cá nhân hóa đã suy ra"))
            return;
