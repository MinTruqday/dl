...
function stemText(candidate) {
  return candidate.stem_doc?.content?.[0]?.content?.map((node) => node.text || "").join("") || "";
}
export default function ImportAssessmentPage() {
  ...
  const setCandidates = (candidates) => {
    setSelected(
      Object.fromEntries(
        candidates.map((candidate) => [candidate.candidate_id, Boolean(candidate.recognized)]),
      ),
    );
    setCorrections(
      Object.fromEntries(
        candidates.map((candidate) => [candidate.candidate_id, stemText(candidate)]),
      ),
    );
  };
  const parse = async () => {
  ...
  const mergeNext = (index) => {
    ...
    setCorrections((values) => ({
      ...
      [current.candidate_id]:
        `${values[current.candidate_id] || ""}\n${values[next.candidate_id] || ""}`.trim(),
    }));
    setSelected((values) => ({
      ...values,
      [next.candidate_id]: false,
    }));
  };
  ...
  const confirm = async () => {
    if (!job) return;
    const selectedCandidates = job.candidates.filter(
      (candidate) => selected[candidate.candidate_id],
    );
    const correctedQuestions = Object.fromEntries(
      selectedCandidates.map((candidate) => [
        candidate.candidate_id,
        {
          ...
          stem_doc: {
            type: "doc",
            content: corrections[candidate.candidate_id]
              ? [
                  {
                    type: "paragraph",
                    content: [{ type: "text", text: corrections[candidate.candidate_id] }],
                  },
                ]
              : [],
          },
    ...
    try {
      const value = await assessmentRequest(`/imports/${job._id}/confirm`, {
        method: "POST",
        body: JSON.stringify({
          selected_candidate_ids: selectedCandidates.map((candidate) => candidate.candidate_id),
          corrected_questions: correctedQuestions,
        }),
      });
      setJob(value);
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 p-5 md:p-8">
      <div>
        ...
        <h1 className="mt-2 text-[30px] font-semibold">Nhập đề có sẵn</h1>
        <p className="mt-2 text-[13px] text-ink-muted">
          PDF DOCX và hình ảnh đi qua Docling OCR trước khi giáo viên rà soát
        </p>
      </div>
      <section
      ...
      >
        ...
        <label className="text-[13px] font-semibold">
          Kéo thả hoặc chọn PDF DOCX hay hình ảnh
          <input
            className="apple-input mt-2 w-full"
            type="file"
            accept=".pdf,.docx,image/*"
            onChange={(event) => {
              return setFile(event.target.files?.[0] || null);
            }}
          />
        </label>
        <p className="text-[12px] text-ink-muted md:col-span-2">{file?.name || "Chưa chọn tệp"}</p>
        <button className="apple-button md:col-span-2" disabled={!file || !draftId} onClick={parse}>
          Tách câu hỏi
        </button>
      </section>
      {job?.candidates && (
        <section className="rounded-panel border border-border bg-surface">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <h2 className="font-semibold">Nguồn và câu hỏi đã nhận dạng</h2>
          ...
          <div className="grid lg:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.2fr)]">
            <aside className="border-b border-border p-4 lg:border-b-0 lg:border-r">
              {file?.type.startsWith("image/") && previewUrl ? (
                ...
                <iframe title="Tệp PDF nguồn" src={previewUrl} className="h-[70vh] w-full" />
              ) : (
                <div className="rounded-control bg-surface-quiet p-5 text-[13px]">
                  <p className="font-semibold">{file?.name}</p>
                  <p className="mt-2 text-ink-muted">
                    Đối chiếu số trang công thức và hình ảnh trong từng candidate
                  </p>
                </div>
              )}
            </aside>
            <div className="divide-y divide-border">
              {job.candidates.map((candidate, index) => (
                <div key={candidate.candidate_id} className="px-5 py-5">
                  <label className="flex items-start gap-3 rounded-control bg-surface-quiet p-3 text-[13px]">
                    <input
                      type="checkbox"
                      checked={Boolean(selected[candidate.candidate_id])}
                      onChange={(event) =>
                        setSelected((current) => ({
                          ...current,
                    ...
                    />
                    Trang nguồn {candidate.source_page}
                  </label>
                  <p className="mt-3 font-semibold">{candidate.candidate_id}</p>
                  <textarea
                    className="apple-input mt-2 min-h-24 w-full"
                    value={corrections[candidate.candidate_id] || ""}
                    onChange={(event) =>
                    ...
                    aria-label={`Nội dung ${candidate.candidate_id}`}
                  />
                  <div className="mt-2 flex flex-wrap gap-2">
                    <button
                      className="apple-button-secondary"
                      onClick={() => mergeNext(index)}
                      disabled={!job.candidates[index + 1]}
                    >
                      Ghép câu kế tiếp
                    </button>
                    <button
                      className="apple-button-secondary"
                      onClick={() => splitCandidate(index)}
                    >
                      Tách tại dòng gần giữa
                    </button>
                  </div>
                  <p className="mt-2 text-[12px] text-ink-muted">
                    Độ tin cậy {Math.round(candidate.parse_confidence * 100)} phần trăm{" "}
                    {(candidate.exception_flags || []).join(" ") || "Không có cảnh báo"}
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-2 border-t border-border p-4">
            <button className="apple-button-secondary" onClick={undoStructure}>
              Hoàn tác cấu trúc
            </button>
            <button className="apple-button" onClick={confirm}>
              Xác nhận câu đã chọn
            </button>
          </div>
        </section>
      )}
      {job?.questions && (
        <Link className="apple-button" href={`/giao-vien/de/soan-thao?id=${draftId}`}>
          Mở trong Composer
        </Link>
      )}
      {!job?.candidates && status && (
        <p role="status" className="rounded-control bg-surface-quiet p-3 text-[13px]">
          {status}
        </p>
      )}
    </div>
