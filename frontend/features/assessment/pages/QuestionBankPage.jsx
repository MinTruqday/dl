...
import { addQuestionBankItemsToDraft, archiveQuestionBankItem, duplicateQuestionBankItem, getQuestionUsage, listQuestionBank, listQuestionVersions, } from "../services/assessment.service";
...
export default function QuestionBankPage() {
    ...
    const addSelected = async () => {
        if (!assessmentDraftId || !selected.size)
            return;
        try {
    ...
    const archive = async (questionId) => {
        if (!window.confirm("Lưu trữ câu hỏi này"))
            return;
        try {
    ...
    const inspect = async (questionId) => {
        if (details[questionId]) {
            setDetails((current) => (Object.assign(Object.assign({}, current), { [questionId]: null })));
            return;
        }
        try {
            const [versions, usage] = await Promise.all([listQuestionVersions(questionId), getQuestionUsage(questionId)]);
            setDetails((current) => (Object.assign(Object.assign({}, current), { [questionId]: { versions, usage } })));
        }
        catch (reason) {
            setError(messageOf(reason));
        }
