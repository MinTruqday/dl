...
export default function QuestionBankPage() {
  ...
  const duplicate = async (questionId) => {
    try {
  ...
  const archive = async (questionId) => {
  ...
  const inspect = async (questionId) => {
    ...
    try {
      const [versions, usage] = await Promise.all([
        listQuestionVersions(questionId),
        getQuestionUsage(questionId),
      ]);
      setDetails((current) => ({
        ...current,
        [questionId]: { versions, usage },
      }));
    } catch (reason) {
      setError(messageOf(reason));
    }
    ...
    <div className="mx-auto max-w-[1450px] space-y-6 p-5 md:p-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="mt-2 text-[30px] font-semibold">Ngân hàng câu hỏi</h1>
          <p className="mt-2 text-[13px] text-ink-muted">
            Tìm kiếm lọc theo chất lượng theo dõi phiên bản mức sử dụng và exposure
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            ...
            href={
              assessmentDraftId
                ? `/giao-vien/de/soan-thao?id=${assessmentDraftId}`
                : "/giao-vien/de/soan-thao"
            }
          >
            Mở Composer
          </Link>
      ...
      <section className="grid gap-3 rounded-panel border border-border bg-surface p-4 md:grid-cols-[minmax(240px,1fr)_200px_180px_200px]">
        <input
          ...
          placeholder="Tìm toàn văn nội dung câu hỏi"
        ...
        >
          ...
          <option value="symbolic_math">Toán ký hiệu</option>
        ...
        >
          ...
          <option value="predicted_difficulty">Độ khó dự đoán</option>
          <option value="calibrated_difficulty">Độ khó thực nghiệm</option>
      ...
      <input
        ...
        placeholder="Lọc theo Skill ID"
        aria-label="Lọc theo kỹ năng"
      />
      <details className="rounded-panel border border-border bg-surface p-4">
        <summary className="cursor-pointer text-[13px] font-semibold">Bộ lọc nâng cao</summary>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            <option value="import">Nhập đề</option>
      ...
      {!loading && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {questions.map((item) => {
            ...
            const prediction = item.difficulty_prediction || {};
            const calibration = item.calibration || {};
            const detail = details[item._id];
            return (
              <article
                key={item._id}
                className={`rounded-panel border bg-surface p-5 ${selected.has(item._id) ? "border-brand" : "border-border"}`}
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2 text-[11px] uppercase tracking-wide text-ink-muted">
                    <span>{version.question_type}</span>
                    <span>v{version.version}</span>
                  </div>
                </div>
                <p className="mt-4 line-clamp-3 text-[14px] font-semibold">
                  {version.plain_text_projection || item._id}
                </p>
                <div className="mt-5 grid grid-cols-2 gap-2 text-[12px]">
                  <span className="rounded-control bg-surface-quiet p-2">
                    AI {prediction.predicted_difficulty ?? "Chưa có"}
                  </span>
                  <span className="rounded-control bg-surface-quiet p-2">
                    Empirical {calibration.difficulty ?? "Chưa có"}
                  </span>
                  <span className="rounded-control bg-surface-quiet p-2">
                    Dùng {item.usage_count || 0} đề
                  </span>
                  <span className="rounded-control bg-surface-quiet p-2">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button
                    type="button"
                    className="apple-button-secondary"
                    onClick={() => void inspect(item._id)}
                  >
                    {detail ? "Ẩn lịch sử" : "Lịch sử"}
                  </button>
                  <button
                    type="button"
                    className="apple-button-secondary"
                ...
                {detail && (
                  <div className="mt-4 rounded-control bg-surface-quiet p-3 text-[12px]">
                    <p>{detail.versions.length} phiên bản bất biến</p>
                    <p className="mt-1">
                      {detail.usage.assessment_versions.length} AssessmentVersion sử dụng
                    </p>
                    <p className="mt-1">{detail.usage.exposure_count} lượt exposure</p>
                  </div>
                )}
                <p className="mt-4 text-[11px] text-ink-faint">{version._id}</p>
              </article>
            );
          })}
        </div>
