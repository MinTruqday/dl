...
export default function ReviewQueuePage() {
    const [data, setData] = useState({ questions: [], question_revisions: [], draft_revisions: [] });
    ...
    const archive = async (question) => {
        if (!question.question_id || !window.confirm("Lưu trữ câu hỏi khỏi ngân hàng"))
            return;
    ...
    return (<div className="mx-auto max-w-[1250px] space-y-6 p-5 md:p-8">
      ...
      <section className="rounded-panel border border-border bg-surface"><h2 className="border-b border-border px-5 py-4 font-semibold">Câu hỏi cần quyết định</h2><div className="divide-y divide-border">{data.questions.map((question) => { var _a, _b, _c; return <div key={question._id} className="flex flex-wrap items-center gap-3 px-5 py-4"><div className="min-w-0 flex-1"><p className="font-semibold">{question._id}</p><p className="mt-1 text-[12px] text-ink-muted">{question.authoring_source} · {question.status} · độ tin cậy parse {(_a = question.parse_confidence) !== null && _a !== void 0 ? _a : "không áp dụng"} · validation {((_b = question.validation) === null || _b === void 0 ? void 0 : _b.status) || "chưa chạy"}</p>{((_c = question.review_reason_codes) === null || _c === void 0 ? void 0 : _c.length) > 0 && <p className="mt-1 text-[12px] text-warning">Lý do {question.review_reason_codes.join(" · ")}</p>}</div><Link className="apple-button-secondary" href={`/giao-vien/de/soan-thao?id=${question.assessment_draft_id}`}>Sửa thủ công</Link><Link className="apple-button-secondary" href={`/tro-chuyen?mode=work&prompt=${encodeURIComponent(`Điều tra QuestionDraft ${question._id} rồi chỉ tạo đề xuất sửa đổi có construct check`)}`}>Yêu cầu AI sửa</Link><button className="apple-button-secondary" disabled={Boolean(busy) || question.locked} onClick={() => void lock(question)}>{question.locked ? "Đã khóa AI" : "Khóa khỏi AI"}</button>{question.question_id && <button className="apple-button-secondary" disabled={Boolean(busy)} onClick={() => void archive(question)}>Lưu trữ</button>}<button className="apple-button-secondary" disabled={Boolean(busy)} onClick={() => decide(`/question-drafts/${question._id}/reject`)}>Từ chối</button><button className="apple-button" disabled={Boolean(busy)} onClick={() => decide(`/question-drafts/${question._id}/approve`)}>Phê duyệt</button></div>; })}</div></section>
      <section className="rounded-panel border border-border bg-surface"><h2 className="border-b border-border px-5 py-4 font-semibold">Đề xuất sửa đổi version</h2><div className="divide-y divide-border">{data.question_revisions.map((proposal) => { var _a, _b, _c, _d, _e; return <div key={proposal._id} className="space-y-3 px-5 py-4"><div className="flex flex-wrap items-center gap-3"><div className="min-w-0 flex-1"><p className="font-semibold">Lý do {(_a = proposal.reason_codes) === null || _a === void 0 ? void 0 : _a.join(" · ")}</p><p className="mt-1 text-[12px] text-ink-muted">Target effect {(_b = proposal.target_difficulty) !== null && _b !== void 0 ? _b : "chưa đặt"} · Construct {((_c = proposal.construct_check) === null || _c === void 0 ? void 0 : _c.passed) ? "đạt" : "chưa đạt"} · Evidence {((_d = proposal.evidence_ids) === null || _d === void 0 ? void 0 : _d.length) || 0}</p></div><button className="apple-button-secondary" disabled={Boolean(busy)} onClick={() => decide(`/revisions/${proposal._id}/reject`)}>Từ chối</button><button className="apple-button" disabled={Boolean(busy) || !((_e = proposal.construct_check) === null || _e === void 0 ? void 0 : _e.passed)} onClick={() => decide(`/revisions/${proposal._id}/approve`)}>Tạo version mới</button></div><details className="rounded-control border border-border p-3 text-[12px]"><summary className="cursor-pointer font-semibold">Before After Why Target Effect Construct Check</summary><div className="mt-3 grid gap-3 lg:grid-cols-2"><pre className="overflow-auto whitespace-pre-wrap rounded-control bg-surface-quiet p-3">{JSON.stringify({ before: proposal.original_version, why: proposal.reason_codes, target_effect: proposal.target_difficulty }, null, 2)}</pre><pre className="overflow-auto whitespace-pre-wrap rounded-control bg-surface-quiet p-3">{JSON.stringify({ after: proposal.proposed_version, construct_check: proposal.construct_check }, null, 2)}</pre></div></details></div>; })}</div></section>
      <section className="rounded-panel border border-border bg-surface"><h2 className="border-b border-border px-5 py-4 font-semibold">Đề xuất sửa bản nháp</h2><div className="divide-y divide-border">{data.draft_revisions.map((proposal) => { var _a, _b; return <div key={proposal._id} className="space-y-3 px-5 py-4"><div className="flex flex-wrap items-center gap-3"><div className="min-w-0 flex-1"><p className="font-semibold">{proposal.action}</p><p className="mt-1 text-[12px] text-ink-muted">Construct {((_a = proposal.construct_check) === null || _a === void 0 ? void 0 : _a.passed) ? "đạt" : "chưa đạt"}</p></div><button className="apple-button-secondary" disabled={Boolean(busy)} onClick={() => decide(`/draft-revisions/${proposal._id}/reject`)}>Từ chối</button><button className="apple-button" disabled={Boolean(busy) || !((_b = proposal.construct_check) === null || _b === void 0 ? void 0 : _b.passed)} onClick={() => decide(`/draft-revisions/${proposal._id}/approve`)}>Áp dụng vào bản nháp</button></div><details className="rounded-control border border-border p-3 text-[12px]"><summary className="cursor-pointer font-semibold">So sánh trước và sau</summary><div className="mt-3 grid gap-3 lg:grid-cols-2"><pre className="overflow-auto whitespace-pre-wrap rounded-control bg-surface-quiet p-3">{JSON.stringify(proposal.before, null, 2)}</pre><pre className="overflow-auto whitespace-pre-wrap rounded-control bg-surface-quiet p-3">{JSON.stringify(proposal.after, null, 2)}</pre></div></details></div>; })}</div></section>
    </div>);
}
