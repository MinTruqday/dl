...
export default function CollectorPage() {
  ...
  return (
    <div className="app-page gap-6">
      <PageHeader title="Thu thập" />
      <div className="flex flex-col md:flex-row gap-6 flex-1 min-h-0">
        <aside className="w-full md:w-[320px] shrink-0 xl:col-span-3 flex flex-col bg-[var(--surface-quiet)] md:bg-transparent rounded-[var(--radius-panel)] md:rounded-none overflow-hidden">
          <div className="overflow-y-auto no-scrollbar p-6 md:px-0 md:pt-6 flex flex-col flex-1 gap-8">
            <div>
              <p className="text-[13px] text-[var(--ink-muted)] font-medium mb-4">Tài liệu đã thu thập</p>
              <p className="text-[32px] font-semibold text-[var(--ink)]">
                {collectorStats?.total_documents_collected || 0}
              </p>
            </div>

            <div>
              <p className="text-[13px] text-[var(--ink-muted)] font-medium mb-4">
                Trạng thái hoạt động
              </p>
              <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full w-fit">
                <div
                  className={`w-2.5 h-2.5 rounded-full ${collectorStats?.status === "operational" ? "bg-[var(--success)]" : collectorStats?.status === "paused" ? "bg-[var(--warning)]" : "bg-[var(--danger)]"}`}
                />
              ...
              <button
                ...
                className="w-full py-3 bg-[var(--danger)]/10 text-[var(--danger)] hover:bg-[var(--danger)]/20 font-medium rounded-[var(--radius-workspace)] transition-colors flex items-center justify-center gap-2 disabled:opacity-50 mt-4"
              >
                {isProcessing ? (
        ...
        <main className="flex-1 min-w-0 xl:col-span-9 flex flex-col min-h-0 gap-4">
          <div className="flex justify-end">
            <button
              ...
              className="p-2 bg-[var(--surface-quiet)] text-[var(--ink)] hover:bg-[var(--border)] rounded-full transition-colors disabled:opacity-50 flex-shrink-0 w-8 h-8 flex items-center justify-center"
              title="Đồng bộ"
            >
              <RefreshCcw className={`w-4 h-4 ${isRefreshing ? "animate-spin" : ""}`} />
            </button>
          </div>
          <div className="flex-1 bg-[var(--ink)] rounded-[var(--radius-panel)] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#333336] bg-[#2A2A2D]">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 bg-[var(--ink)] px-3 py-1 rounded-[var(--radius-control)] text-[var(--ink-faint)] text-[12px] font-mono">
                  <Terminal className="w-3.5 h-3.5" /> Live Console
                </div>
              </div>
              <div className="text-[12px] font-mono text-[var(--ink-faint)] flex items-center gap-2">
                <div className="w-2 h-2 bg-[var(--success)] rounded-full animate-pulse" />{" "}
                sys.log
              </div>
            </div>
            <div className="flex-1 overflow-y-auto no-scrollbar p-6 font-mono text-[13px] leading-relaxed text-[#D1D1D6] bg-[var(--ink)]">
            {logs.length === 0 ? (
          ...
          <p>Đang chờ sự kiện thu thập</p>
              ...
              <div className="space-y-2">
                {logs.map((log, i) => {
                  ...
                  let lc = "text-[var(--ink-faint)]",
                    bgc = "bg-[#333336] text-[var(--ink-faint)]";
                  if (l === "INFO") {
                    lc = "text-[#32ADE6]";
                    bgc = "bg-[#32ADE6]/10 text-[#32ADE6]";
                  } else if (l === "WARNING") {
                    lc = "text-[var(--warning)]";
                    bgc = "bg-[var(--warning)]/10 text-[var(--warning)]";
                  } else if (l === "ERROR") {
                    lc = "text-[var(--danger)]";
                    bgc = "bg-[var(--danger)]/10 text-[var(--danger)]";
                  } else if (l === "SUCCESS") {
                    lc = "text-[var(--success)]";
                    bgc = "bg-[var(--success)]/10 text-[var(--success)]";
                  }
                  return (
                    <div
                      key={i}
      ...
      >
        ...
        <ModalContent>
          <div className="bg-[var(--surface-quiet)] p-4 rounded-[var(--radius-workspace)] border-[var(--border)] mb-4">
            <span className="text-[13px] text-[var(--ink-muted)]">Nguồn thu thập: </span>
            <strong className="text-[var(--ink)]">{collectionForm.source}</strong>
          </div>
