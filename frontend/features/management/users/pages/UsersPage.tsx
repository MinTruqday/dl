...
export default function UsersManagementPage() {
  ...
  const handleDeleteUser = async (user: any) => {
    setConfirmModal({ type: "delete", user, value: null });
  };
  ...
  const executeDeleteUser = async () => {
    ...
    setIsUpdating(true);
    try {
      await deleteUserAPI(confirmModal.user._id);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex-1">
              {!showSearch ? (
                <div className="relative inline-block w-fit">
                  <select
                    ...
                    className="w-full bg-transparent h-10 pr-8 text-[20px] font-semibold text-[var(--ink)] focus:outline-none appearance-none cursor-pointer"
                  >
                  ...
                  <ChevronRight className="absolute right-0 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--ink-muted)] pointer-events-none rotate-90" />
                </div>
              ) : (
              ...
              placeholder="Tìm kiếm email, tên"
                    ...
                    className="w-full bg-[var(--surface-quiet)] h-10 px-4 pr-10 text-[15px] rounded-full focus:outline-none focus:ring-2 focus:ring-[var(--brand)]/20 transition-all border border-transparent focus:border-[var(--brand)]/20"
                  />
                  <button
                    onClick={() => {
                      setShowSearch(false);
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-[var(--ink-muted)] hover:text-[var(--ink)] rounded-full transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                  ...
                  filteredUsers.map((u) => (
                    ...
                    >
                      <td className="py-3 px-6 text-center">
                        <span className="font-medium text-[14px] text-[var(--ink)]">@{u.slug || u._id?.substring(0,8)}</span>
                      </td>
                      <td className="py-3 px-6 text-center">
                      ...
                      <td className="py-3 px-6 text-center">
                        <span className="font-medium text-[14px] text-[var(--ink)] truncate max-w-[200px] inline-block">
                      ...
                      <td className="py-3 px-6 text-center">
                        <div className="relative inline-block w-full max-w-[150px] text-left">
                          <select
                            ...
                            className={`w-full bg-transparent h-[32px] px-2 text-[13px] font-medium rounded-md focus:outline-none appearance-none transition-colors hover:bg-[var(--border-strong)]/50 ${u.role === "admin" ? "text-[var(--brand)]" : "text-[var(--ink)]"}`}
                          >
                            <option value="reader">Độc giả</option>
                            <option value="author">Tác giả</option>
                          ...
                          <ChevronRight className="absolute right-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--ink-muted)] pointer-events-none" />
                        </div>
                      </td>
                      <td className="py-3 px-6 text-[var(--ink-muted)] text-[13px] hidden md:table-cell text-center">
                        {u.created_at
                      ...
                      <td className="py-3 px-6 hidden md:table-cell text-center">
                        <div
                          className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-[12px] font-medium whitespace-nowrap ${u.is_active ? "bg-[#E8F5E9] text-[var(--success)]" : "bg-[var(--danger)]/10 text-[var(--danger)]"}`}
                        >
                      ...
                      <td className="py-3 px-6 text-right relative">
                        <button
                          ...
                          className="p-2 text-[var(--ink-muted)] hover:text-[var(--ink)] hover:bg-[var(--border-strong)]/50 rounded-full transition-colors opacity-0 group-hover:opacity-100"
                        >
                        ...
                        {openDropdownId === u._id && (
                          <>
                            ...
                            />
                            <div className="absolute right-6 top-10 mt-1 w-44 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.08)] border border-[var(--border)] rounded-[var(--radius-control)] z-50 overflow-hidden py-1">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="w-full text-left px-4 py-2 text-[14px] text-[var(--ink)] hover:bg-[var(--surface-quiet)] transition-colors flex items-center gap-2"
                              >
                                {u.is_active ? (
                                  <Lock className="w-4 h-4 text-[var(--ink-muted)]" />
                                ) : (
                                  <Unlock className="w-4 h-4 text-[var(--ink-muted)]" />
                                )}{" "}
                                {u.is_active ? "Khóa tài khoản" : "Kích hoạt"}
                              </button>
                              <div className="w-full h-[1px] bg-[var(--border)] my-1"></div>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setOpenDropdownId(null);
                                ...
                                className="w-full text-left px-4 py-2 text-[14px] text-[var(--danger)] hover:bg-[var(--danger)]/10 transition-colors flex items-center gap-2"
                              >
          ...
          <p className="text-[15px] text-[var(--ink-muted)] leading-relaxed">
            {confirmModal?.type === "role"
              ...
              : confirmModal?.type === "delete"
                ? `Xóa tài khoản ${confirmModal.user.email}`
      ...
      >
        ...
        <ModalContent>
          <div className="space-y-4 text-[var(--ink)]">
            <div>
              <label className="block text-[13px] font-medium text-[var(--ink-muted)] mb-1">Họ và tên</label>
              <input
                type="text"
                placeholder="Nhập họ và tên"
                value={newUserForm.full_name}
                onChange={(e) => setNewUserForm({ ...newUserForm, full_name: e.target.value })}
                className="w-full bg-[var(--surface-quiet)] h-10 px-4 text-[15px] rounded-[var(--radius-control)] focus:outline-none focus:ring-2 focus:ring-[var(--brand)]/20 transition-all border border-transparent focus:border-[var(--brand)]/20"
              />
            </div>
            <div>
              <label className="block text-[13px] font-medium text-[var(--ink-muted)] mb-1">Email đăng nhập</label>
              <input
                type="email"
            ...
            <div>
              <label className="block text-[13px] font-medium text-[var(--ink-muted)] mb-1">Mật khẩu</label>
              <input
                type="password"
            ...
            <div>
              <label className="block text-[13px] font-medium text-[var(--ink-muted)] mb-1">Quyền hạn</label>
              <div className="relative inline-block w-full">
                <select
                  value={newUserForm.role}
                  onChange={(e) => setNewUserForm({ ...newUserForm, role: e.target.value })}
                  className="w-full bg-[var(--surface-quiet)] h-10 px-4 pr-10 text-[15px] rounded-[var(--radius-control)] focus:outline-none appearance-none transition-all border border-transparent focus:border-[var(--brand)]/20"
                >
                  <option value="reader">Độc giả</option>
                  <option value="author">Tác giả</option>
                ...
                <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--ink-muted)] pointer-events-none" />
              </div>
            </div>
          </div>
        </ModalContent>
