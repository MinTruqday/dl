    ...
    <div className="w-full h-full flex flex-col font-sans text-[var(--ink)]">
      ...
      >
        <aside className="md:col-span-4 xl:col-span-4 space-y-6">
          <div className="bg-[var(--surface-quiet)] md:bg-transparent rounded-[var(--radius-panel)] md:rounded-none p-6 md:p-0 md:pt-6 flex flex-col items-center text-center">
            <div className="relative group mb-6">
              <div className="w-32 h-32 rounded-full bg-[var(--border-strong)] flex items-center justify-center overflow-hidden transition-transform duration-300 group-hover:scale-105">
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                    alt=""
                  ...
                  <div className="w-full h-full bg-[var(--brand)] text-white flex items-center justify-center text-[56px] font-semibold uppercase">
            ...
            <div className="w-full mb-8">
              <div className="flex items-center justify-center gap-2 mb-1">
                <h1 className="text-[20px] font-semibold text-[var(--ink)] truncate px-2">
                ...
                {user.role === "author" && (
                  <BadgeCheck className="w-5 h-5 text-[var(--brand)]" />
                )}
            ...
            <button
              onClick={logoutState}
              className="w-full py-3 bg-white text-[var(--danger)] text-[15px] font-medium flex items-center justify-center gap-2 rounded-full transition-colors hover:bg-[var(--surface-quiet)]"
            >
        ...
        <main className="md:col-span-8 xl:col-span-8 space-y-6">
          <div className="bg-[var(--surface-quiet)] md:bg-transparent rounded-[var(--radius-panel)] md:rounded-none p-6 md:p-0 md:pt-6 space-y-6">
            <h2 className="text-[20px] font-semibold text-[var(--ink)] mb-6">
              Thông tin cá nhân
            </h2>
          ...
          {user.role === "reader" && (
            <div className="bg-[#EBF4FF] rounded-[var(--radius-panel)] p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div>
                <h3 className="text-[17px] font-medium text-[var(--brand)] flex items-center gap-2 mb-2">
                  <BadgeCheck className="w-5 h-5" /> Trở thành Tác giả
                </h3>
                <p className="text-[14px] text-[var(--brand-hover)]">
              ...
              <button
                ...
                disabled={isSaving}
                className="pill-button shrink-0 bg-[var(--brand)] hover:bg-[var(--brand-hover)]"
              >
                {isSaving ? (
                  <Loader2 className="w-5 h-5 animate-spin" />
                ) : (
          ...
          {user.role === "author" && (
            <div className="bg-[#FFF4E5] rounded-[var(--radius-panel)] p-8 space-y-6">
              <div>
                <h3 className="text-[17px] font-medium text-[var(--warning)] flex items-center gap-2 mb-2">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <input
                  ...
                  placeholder=""
                  className="apple-input flex-1 h-[48px] border-[#FFD699] focus:border-[var(--warning)] bg-white"
                />
                <button
                  onClick={handleApplyAuthor}
                  disabled={isApplying}
                  className="pill-button bg-[var(--warning)] hover:bg-[#CC7700] shrink-0"
                >
                  {isApplying ? (
                    <Loader2 className="w-5 h-5 animate-spin" />
                  ) : (
          ...
          {user.role === "potential_author" && (
            <div className="bg-[var(--ink)] rounded-[var(--radius-panel)] p-8 flex items-center gap-6 relative overflow-hidden">
