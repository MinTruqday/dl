...
import { getAnnouncementsAPI, deleteAnnouncementAPI, markAllAnnouncementsReadAPI, markAnnouncementReadAPI, } from "@/features/notification/services/announcement.service";
export function useAnnouncementsPage() {
    ...
    const markRead = async (id) => {
        ...
        try {
            ...
            setItems((current) => current.map((item) => (item._id || item.id) === id ? Object.assign(Object.assign({}, item), { is_read: true }) : item));
        }
        catch (reason) {
    ...
    const markAllRead = async () => {
        if (!unread || processing)
            return;
        setProcessing(true);
        setError("");
        try {
            await markAllAnnouncementsReadAPI();
            setItems((current) => current.map((item) => (Object.assign(Object.assign({}, item), { is_read: true }))));
        }
        catch (reason) {
            setError(reason instanceof Error
                ? reason.message
