...
export const markAllAnnouncementsReadAPI = async () => {
    ...
    const resultData = await res.json();
    if (!res.ok)
        throw new Error(resultData.message || "Không thể cập nhật trạng thái hàng loạt");
    return resultData;
};
...
export const getAnnouncementSettingsAPI = async () => {
    ...
    if (!res.ok)
        throw new Error("Không thể tải cấu hình hệ thống thông báo");
...
export const updateAnnouncementSettingsAPI = async (settings) => {
    ...
    if (!res.ok)
        throw new Error("Không thể lưu cấu hình hệ thống thông báo");
    return await res.json();
};
