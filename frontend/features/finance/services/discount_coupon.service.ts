...
export async function validateCouponAPI(code: string, documentId?: string) {
  const url =
    `${API_URL}/ma-qua-tang/kiem-tra?code=${encodeURIComponent(code)}` +
...
export async function approveCouponAPI(
  ...
  const res = await fetch(
    `${API_URL}/ma-qua-tang/${couponId}/phe-duyet?action=${action}`,
    {
      method: "POST",
      headers: getAuthHeaders(),
    },
...
export async function toggleCouponStatusAPI(couponId: string) {
  const res = await fetch(`${API_URL}/ma-qua-tang/${couponId}/status`, {
    method: "PATCH",
    headers: getAuthHeaders(),
  });
  const data = await res.json();
