...
function ResetPasswordContent() {
    ...
    return (<AuthFrame title="Đặt mật khẩu mới" description="Mật khẩu mới cần có ít nhất 12 ký tự">
      {error && (<div className="mb-5">
          <InlineState title="Không thể cập nhật" detail={error} tone="danger"/>
        </div>)}
      <form className="space-y-5" onSubmit={handleSubmit}>
        <PasswordField id="new-password" name="new_password" label="Mật khẩu mới" autoComplete="new-password" required minLength={12} maxLength={128} value={password} onChange={(event) => setPassword(event.target.value)}/>
