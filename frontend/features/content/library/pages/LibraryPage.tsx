      ...
      <div className="w-full max-w-[1200px] mx-auto px-6 md:px-0 py-6 min-h-[calc(100dvh-56px)] font-sans">
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full md:w-[320px] shrink-0 space-y-6">
            <div className="bg-[var(--surface-quiet)] md:bg-transparent rounded-[var(--radius-panel)] md:rounded-none p-6 md:p-0 md:pt-6 h-[250px] animate-pulse" />
            <div className="bg-[var(--surface-quiet)] md:bg-transparent rounded-[var(--radius-panel)] md:rounded-none p-6 md:p-0 md:pt-6 h-[300px] animate-pulse" />
          </aside>
          <main className="flex-1 space-y-8 pt-6">
            <div className="h-8 w-48 bg-[var(--surface-quiet)] rounded-full mb-6 animate-pulse" />
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                <div key={i} className="bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] overflow-hidden animate-pulse flex flex-col">
  ...
  return (
    <div className="app-page gap-6">
            ...
            <nav className="flex flex-col gap-1.5">
              {tabs.map((t) => (
                <button
                  ...
                  className={`flex items-center justify-between px-4 py-3 text-[15px] rounded-[var(--radius-control)] transition-colors ${activeTab === t.id ? "bg-white text-[var(--brand)] font-medium" : "text-[var(--ink)] hover:bg-[var(--border)]"}`}
                >
              ...
              {pinnedDocs.length > 0 ? (
                pinnedDocs.map((doc, i) => (
                  <Link
                    ...
                    className="flex gap-4 items-center group p-2 -mx-2 rounded-[var(--radius-panel)] hover:bg-white transition-colors"
                  >
                    <div className="w-12 h-12 bg-white rounded-[var(--radius-control)] overflow-hidden shrink-0">
                      {doc.cover_url ? (
                        <img
                          src={
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[15px] font-medium text-[var(--ink)] line-clamp-2 leading-[1.3] group-hover:text-[var(--brand)] transition-colors">
                        {doc.title}
                      </h4>
                    </div>
                  </Link>
                ...
                <div className="py-12 flex flex-col items-center justify-center bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] w-full text-center">
                  <p className="text-[15px] text-[var(--ink-muted)]">Chưa có dữ liệu</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {continueDocs.map((doc) => (
                  <Link
                    ...
                    className="flex flex-col bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] overflow-hidden transition-transform hover:scale-[1.02]"
                  >
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex bg-[var(--border)] p-[2px] rounded-full shrink-0">
                    <button
                      onClick={() => setViewMode("grid")}
                      className={`p-1 rounded-full transition-colors ${viewMode === "grid" ? "bg-white text-[var(--brand)]" : "text-[var(--ink-muted)] hover:text-[var(--ink)]"}`}
                    >
                      <LayoutGrid className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => setViewMode("list")}
                      className={`p-1 rounded-full transition-colors ${viewMode === "list" ? "bg-white text-[var(--brand)]" : "text-[var(--ink-muted)] hover:text-[var(--ink)]"}`}
                    >
                ...
                >
                  {history.map((item, idx) => (
                    <div
                      ...
                      } bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] overflow-hidden transition-transform hover:scale-[1.02] ${isDeletingHistory === item.document_id ? "opacity-50" : ""}`}
                    >
                      <Link
                        ...
                        className={`${
                          viewMode === "grid"
                            ? "aspect-[4/3] w-full"
                            : "w-[120px] h-[120px] shrink-0 rounded-[var(--radius-control)]"
                      ...
                      >
                        <Link href={`/tai-lieu/${item.document_slug}`}>
                          <h3
                            ...
                            } font-medium text-[var(--ink)] line-clamp-2 leading-snug`}
                          >
                            {item.document_title}
                          </h3>
                        </Link>
                      ...
                      <button
                        ...
                        className={`absolute ${viewMode === "grid" ? "top-2 right-2" : "top-1/2 -translate-y-1/2 right-4"} p-2 bg-white rounded-full text-[var(--ink-muted)] hover:text-[var(--danger)] opacity-0 group-hover:opacity-100 transition-opacity z-10 shadow-sm`}
                      >
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="p-2 bg-[var(--brand)] rounded-full text-white hover:bg-[var(--brand-hover)] transition-colors"
                >
                  ...
                  {folders.map((folder) => (
                    ...
                    >
                      <div className="aspect-[4/3] w-full bg-white flex items-center justify-center">
                        <FolderPlus className="w-12 h-12 text-[var(--brand)]" />
                      </div>
                      <div className="p-5 flex flex-col gap-2">
                        <h3 className="text-[17px] font-medium text-[var(--ink)] line-clamp-1">
                          {folder.name}
                        </h3>
                        <p className="text-[13px] text-[var(--ink-muted)]">
              ...
              {readingLists.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                  {readingLists.map((list) => (
                    ...
                    >
                      <div className="aspect-[4/3] w-full bg-white flex items-center justify-center">
                        <Layers className="w-12 h-12 text-[var(--brand)]" />
                      </div>
                      <div className="p-5 flex flex-col gap-2">
                        <h3 className="text-[17px] font-medium text-[var(--ink)] line-clamp-1">
                          {list.name}
      ...
      >
        ...
        <ModalContent className="text-[15px] text-[var(--ink-muted)]">
        ...
        <ModalFooter>
          <button
            ...
            disabled={isClearing}
            className="px-4 py-2 rounded-full text-[15px] text-[var(--brand)] hover:bg-[var(--surface-quiet)] transition-colors font-medium"
          >
            Hủy
          </button>
          <button
            ...
            disabled={isClearing}
            className="px-4 py-2 bg-[var(--danger)] text-white rounded-full text-[15px] font-medium hover:bg-[#D70015] transition-colors"
          >
