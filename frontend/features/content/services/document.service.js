...
export async function getDocumentDraftAPI(documentId) {
    ...
    if (!response.ok)
        throw new Error(data.detail || "Không thể tải dữ liệu bản thảo");
    return data;
}
export async function getMyDocumentsAPI(search = "", cursor = "", limit = 50) {
    ...
    const data = await response.json();
    if (!response.ok)
        throw new Error(data.message || "Không thể tải danh sách tài liệu cá nhân");
...
export async function createDocumentAPI(data) {
    ...
    if (!response.ok)
        throw new Error(result.message || "Không thể tạo tài liệu mới");
    return result;
}
