...
export const smartSearchAPI = async (query: string, limit: number = 10) => {
  const res = await fetch(`${API_URL}/discovery/tim-kiem-thong-minh?query=${encodeURIComponent(query)}&limit=${limit}`);
  const data = await res.json();
...
export const getTrendingTagsAPI = async (limit: number = 10) => {
  const res = await fetch(`${API_URL}/discovery/hashtag-xu-huong?limit=${limit}`);
  const data = await res.json();
