...
export async function createDocumentAPI(data: unknown) {
...
export async function updateDocumentAPI(id: string, data: unknown) {
...
export async function retryDocumentIndexingAPI(id: string) {
  ...
  const result = await response.json();
  if (!response.ok) throw new Error(result.detail || result.message || "Không thể lập chỉ mục lại tài liệu");
  return result;
}

export async function deleteAuthorDocumentAPI(id: string) {
  const response = await authenticatedFetch(`${API_URL}/tai-lieu/${id}`, {
    method: "DELETE",
  });
  const result = await response.json();
