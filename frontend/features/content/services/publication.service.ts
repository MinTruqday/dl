...
export async function configPremiumAPI(documentId: string, premiumChapters: string[]) {
  const res = await fetch(`${API_URL}/xuat-ban/${documentId}/tinh-phi`, {
    method: "POST",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify({ premium_chapters: premiumChapters }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Thiết lập chương tính phí thất bại");
  return data;
}

export async function setFreePreviewAPI(documentId: string, chapterIds: string[]) {
  const res = await fetch(`${API_URL}/xuat-ban/${documentId}/doc-thu`, {
    method: "POST",
    headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify(chapterIds),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Thiết lập chương đọc thử thất bại");
  return data;
}

export async function updateSeoMetadataAPI(documentId: string, metadata: {
  tags?: string[];
  keywords?: string[];
  slug?: string;
  description?: string;
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Cập nhật thông tin SEO thất bại");
  return data;
}
...
export async function getReadabilityScoreAPI(documentId: string) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || "Không thể lấy điểm độ đọc hiểu");
  return data;
}
