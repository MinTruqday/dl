  ...
  return (
    <div className="app-page gap-6">
      ...
      <div className="surface flex flex-col gap-3 p-3 sm:flex-row sm:items-center">
        ...
        <button
          type="button"
          aria-pressed={filterStar}
          onClick={() => setFilterStar((current) => !current)}
          className={`button-secondary ${filterStar ? "border-[var(--brand)] bg-[var(--brand-soft)] text-[var(--brand)]" : ""}`}
        >
          Đã đánh dấu
        </button>
        <div
          className="flex h-11 items-center rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] p-1"
        ...
        >
          <button
            ...
            className={`flex size-9 items-center justify-center rounded-[7px] ${
              viewMode === "grid"
                ? "bg-[var(--surface-quiet)] text-[var(--ink)]"
                : "text-[var(--ink-muted)]"
            }`}
          >
      ...
      <div className="flex flex-col md:flex-row">
        <main className="min-w-0 flex-1 space-y-6">
              ...
              {!currentFolder && breadcrumbs.length === 0 ? (
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`flex items-center gap-1 transition-colors hover:text-[var(--ink)] text-[var(--ink-muted)]`}
                  >
                    Gốc
                  </button>
          ...
          <div
            ...
            onDrop={handleDrop}
            className={`w-full overflow-x-auto min-h-[400px] transition-colors rounded-[var(--radius-panel)] ${isDraggingOver ? "border-2 border-dashed border-[var(--brand)] bg-[var(--brand)]/5" : ""}`}
          >
            {isLoading ? (
              <div className="py-20 text-center text-[14px] text-[var(--ink-muted)]">
                Đang tải tài liệu
              </div>
            ) : viewMode === "list" ? (
                    ...
                    <>
                      {folders.map((folder) => (
                        <tr
                          ...
                          className="hover:bg-[var(--border)]/60 transition-colors cursor-pointer group border-b border-[var(--surface-quiet)] last:border-0"
                        >
                          <td className="py-3 px-6 max-w-[300px]">
                            <div className="flex items-center gap-3">
                              <button
                                ...
                                className="font-medium text-[var(--ink)] hover:text-[var(--brand)] truncate text-left"
                              >
                                {folder.name}
                              </button>
                            </div>
                          ...
                          <td className="py-3 px-6 text-center hidden md:table-cell">
                            <span className="text-[12px] bg-[var(--surface-quiet)] text-[var(--ink-muted)] px-3 py-1 rounded-full font-medium">Thư mục</span>
                          </td>
                          <td className="py-3 px-6 text-center hidden md:table-cell text-[var(--ink-muted)]">--</td>
                          <td className="py-3 px-6 text-center hidden md:table-cell text-[var(--ink-muted)]">--</td>
                          <td className="py-3 px-6 text-right">
                            <button
                              ...
                              className="p-2 text-[var(--ink-muted)] hover:text-[var(--danger)] hover:bg-[var(--danger)]/10 rounded-full opacity-0 group-hover:opacity-100 transition-all"
                            >
                      ...
                      {documents.map((doc) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                              <div className="flex flex-col min-w-0">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  {doc.is_starred && <Star className="w-4 h-4 text-[var(--warning)] fill-[var(--warning)] shrink-0" />}
                                  <p className="font-medium text-[var(--ink)] hover:text-[var(--brand)] truncate">{doc.title}</p>
                                </div>
                                <p className="text-[12px] text-[var(--ink-muted)] mt-0.5">{doc.publisher_name || "DocLib"}</p>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-6 text-center hidden md:table-cell">
                          ...
                          <td className="py-3 px-6 text-center hidden md:table-cell">
                            {doc.is_protected ? (
                              <span className="inline-flex items-center justify-center gap-1 text-[13px] text-[var(--ink)] bg-[var(--surface-quiet)] px-3 py-1 rounded-full">
                                <Lock className="w-3 h-3" /> Đã khóa
                              </span>
                            ) : (
                              <span className="text-[13px] text-[var(--ink-muted)]">Không</span>
                            )}
                          </td>
                          <td className="py-3 px-6 text-center hidden md:table-cell">
                            <span className={`text-[12px] px-3 py-1 rounded-full font-medium inline-block ${doc.status === "published" ? "bg-[#E8F3FF] text-[var(--brand)]" : "bg-[var(--surface-quiet)] text-[var(--ink-muted)]"}`}>
                          ...
                          <td className="py-3 px-6 text-right">
                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                className={`p-2 rounded-full transition-colors ${doc.is_starred ? "text-[var(--warning)] bg-[var(--warning)]/10" : "text-[var(--ink-muted)] hover:bg-[var(--border)] hover:text-[var(--ink)]"}`}
                              >
                                <Star className={`w-4 h-4 ${doc.is_starred ? "fill-[var(--warning)]" : ""}`} />
                              </button>
                              <button
                                onClick={(e) => { e.stopPropagation(); setLockModal({ show: true, docId: doc._id || doc.id }); }}
                                className="p-2 text-[var(--ink-muted)] hover:bg-[var(--border)] hover:text-[var(--ink)] rounded-full transition-colors"
                              >
                              ...
                              <button
                                ...
                                className="p-2 text-[var(--ink-muted)] hover:bg-[var(--danger)]/10 hover:text-[var(--danger)] rounded-full transition-colors"
                              >
                                <Trash2 className="w-4 h-4" />
                              </button>
                            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4">
                {folders.map((folder) => (
                  <div
                    ...
                    className="group bg-white border border-[var(--border)] hover:border-[var(--brand)] hover:shadow-sm rounded-[var(--radius-panel)] p-4 flex flex-col items-center justify-center text-center cursor-pointer transition-all aspect-square relative"
                  >
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 z-10">
                      <button
                        ...
                        className="p-1.5 text-[var(--ink-muted)] hover:bg-[var(--danger)]/10 hover:text-[var(--danger)] rounded-full bg-white shadow-sm"
                      >
                        <Trash2 className="w-3 h-3" />
                      </button>
                    </div>
                    <Folder className="w-12 h-12 text-[var(--brand)] fill-[var(--brand)]/10 mb-3" />
                    <span className="font-medium text-[var(--ink)] text-[14px] line-clamp-2 w-full px-2">{folder.name}</span>
                    <span className="text-[12px] text-[var(--ink-muted)] mt-1">Thư mục</span>
                  </div>
                ))}
                {documents.map((doc) => (
                  <div
                    key={doc._id || doc.id}
                    onClick={() => window.open(`/tai-lieu/xem-truoc/${doc._id || doc.id}`, "_blank")}
                    className="group bg-white border border-[var(--border)] hover:border-[var(--brand)] hover:shadow-sm rounded-[var(--radius-panel)] p-4 flex flex-col items-center justify-center text-center cursor-pointer transition-all aspect-[3/4] relative"
                  >
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 z-10">
                      <button
                        onClick={(e) => { e.stopPropagation(); toggleStar(doc._id || doc.id); }}
                        className={`p-1.5 rounded-full bg-white shadow-sm transition-colors ${doc.is_starred ? "text-[var(--warning)]" : "text-[var(--ink-muted)] hover:text-[var(--ink)]"}`}
                      >
                        <Star className={`w-3 h-3 ${doc.is_starred ? "fill-[var(--warning)]" : ""}`} />
                      </button>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <div className="w-full flex-1 bg-[var(--surface-quiet)] rounded-[var(--radius-panel)] flex flex-col items-center justify-center mb-3 text-[var(--ink-muted)] overflow-hidden">
                      {doc.cover_url ? (
                    ...
                    <span className="font-medium text-[var(--ink)] text-[14px] line-clamp-2 w-full px-1">{doc.title}</span>
                    <span className="text-[12px] text-[var(--ink-muted)] mt-1 line-clamp-1 w-full px-1">{doc.publisher_name || "DocLib"}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      >
        <ModalHeader>
          <ModalTitle className="text-[var(--danger)]">Xóa dữ liệu</ModalTitle>
        </ModalHeader>
        <ModalContent>
          <p className="text-[15px] text-[var(--ink-muted)]">
            Xác nhận xóa{" "}
            <strong className="text-[var(--ink)]">{confirmModal?.title}</strong>
          </p>
        </ModalContent>
        <ModalFooter>
          <button
          ...
          <button
            ...
            className="pill-button bg-[var(--danger)] hover:bg-[#D70015]"
          >
            Xóa
          </button>
        </ModalFooter>
      ...
      >
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="apple-input w-full bg-white h-24 resize-none p-3 rounded-[var(--radius-workspace)]"
              />
            </div>
          </div>
          <div className="space-y-4">
            <div>
              ...
              <div
                ...
                className="h-32 bg-[var(--surface-quiet)] border-[var(--border)] rounded-[var(--radius-panel)] flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-[var(--brand)] transition-colors border-dashed"
              >
                <div className="w-10 h-10 bg-[var(--surface-quiet)] rounded-full flex items-center justify-center text-[var(--brand)]">
                  {file ? (
        ...
        <ModalFooter>
          ...
          >
            {isCreating ? "Đang tải lên" : "Tải lên"}
          </button>
        </ModalFooter>
      </Modal>
      ...
      >
        <ModalHeader>
          <ModalTitle>Khóa tài liệu</ModalTitle>
        </ModalHeader>
        <ModalContent>
      ...
      >
        <ModalHeader>
          <ModalTitle>Chia sẻ tài liệu</ModalTitle>
        </ModalHeader>
        <ModalContent>
          <form
            id="share-form"
            ...
            className="space-y-4"
          >
            <div className="flex items-center gap-3 bg-[var(--surface-quiet)] p-4 rounded-[var(--radius-workspace)] border-[var(--border)]">
              <input
                type="checkbox"
                checked={isPublic}
                onChange={(e) => setIsPublic(e.target.checked)}
                className="w-5 h-5 rounded-[6px] border-[var(--border-strong)] accent-[var(--brand)]"
              />
            ...
            {publicUrl && (
              <div className="bg-[var(--surface-quiet)] p-6 rounded-[var(--radius-panel)] border-[var(--border)] flex flex-col items-center gap-4 mt-4">
                <input
                  type="text"
                  readOnly
                  value={publicUrl}
                  className="apple-input w-full text-center bg-[var(--surface-quiet)] text-[var(--brand)]"
