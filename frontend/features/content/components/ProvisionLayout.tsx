...
export default function ProvisionLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    ...(isAdminOrMod
      ? [{ id: "step8", label: "Duyệt bản thảo", href: "/soan-thao/duyet-ban-thao", highlight: true }]
      : []),
  ];
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-[14px] bg-[#E8E8ED] flex items-center justify-center overflow-hidden shrink-0">
                {user?.avatar_url ? (
                  ...
                  <span className="text-[14px] font-medium text-[#1D1D1F]">{user?.name?.charAt(0).toUpperCase() || "T"}</span>
                )}
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-[14px] font-medium text-[#1D1D1F] truncate">{user?.name || "Tác giả"}</span>
