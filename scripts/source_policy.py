...
def is_emoji(character):
    value = ord(character)
    return 0x1F000 <= value <= 0x1FAFF or 0x2600 <= value <= 0x27BF or value == 0xFE0F
...
def scan_compilation_registry(issues):
    component_root = ROOT / "frontend" / "features" / "compilation" / "components"
    ...
    for component_name in sorted(component_names):
        ...
        if not component_path.is_file():
            issues.append((relative_editor, 1, f"registered_component_missing:{component_name}"))
            continue
        component_source = component_path.read_text(encoding="utf-8")
        if "export default" not in component_source:
            issues.append((component_path.relative_to(ROOT), 1, "component_default_export_missing"))
        ...
        if (
            ...
            issues.append(
                (component_path.relative_to(ROOT), 1, "component_behavior_contract_missing")
            )
        if (
            "private data: { content: string }" in component_source
            and len(component_source.splitlines()) < 60
        ):
            issues.append((component_path.relative_to(ROOT), 1, "generic_component_registered"))
    catalog_path = component_root / "word-command-catalog.ts"
    if not catalog_path.is_file():
        issues.append((catalog_path.relative_to(ROOT), 1, "word_command_catalog_missing"))
        return
    ...
    if len(component_names) != 153:
        issues.append((relative_editor, 1, f"editor_component_count:{len(component_names)}"))
    if len(command_names) != 2296:
        issues.append(
            (catalog_path.relative_to(ROOT), 1, f"word_command_count:{len(command_names)}")
        )
    ...
    if duplicate_commands:
        issues.append(
            (catalog_path.relative_to(ROOT), 1, f"word_command_duplicates:{duplicate_commands}")
        )
    if overlap:
        issues.append((catalog_path.relative_to(ROOT), 1, f"word_feature_overlap:{len(overlap)}"))
    if total_features != 2449:
        issues.append((catalog_path.relative_to(ROOT), 1, f"word_feature_total:{total_features}"))
    ...
    for command_name in sorted(set(command_names)):
        ...
        if not command_path.is_file():
            issues.append(
                (catalog_path.relative_to(ROOT), 1, f"word_command_file_missing:{command_name}")
            )
            continue
        ...
        for contract in command_contracts:
            if contract not in command_source:
                issues.append(
                    (command_path.relative_to(ROOT), 1, f"word_command_file_contract:{contract}")
                )
        if (
            'from "./word-command-engine"' in command_source
            or "createWordCommand" in command_source
        ):
            issues.append((command_path.relative_to(ROOT), 1, "word_command_wrapper_import"))
        if len(command_source.splitlines()) < 60:
            issues.append((command_path.relative_to(ROOT), 1, "word_command_file_behavior_missing"))
    manifest_path = component_root / "word-feature-manifest.json"
    if not manifest_path.is_file():
        issues.append((manifest_path.relative_to(ROOT), 1, "word_feature_manifest_missing"))
        return
    try:
        ...
        issues.append((manifest_path.relative_to(ROOT), 1, "word_feature_manifest_invalid"))
        return
    ...
    if (
        ...
        issues.append(
            (backend_manifest_path.relative_to(ROOT), 1, "word_feature_backend_manifest_mismatch")
        )
    record_ids = [record.get("id") for record in records]
    record_titles = [record.get("title") for record in records]
    record_icons = [record.get("icon") for record in records]
    command_records = [record for record in records if record.get("toolKey") is not None]
    if len(records) != 2449 or set(record_ids) != feature_names:
        issues.append(
            (manifest_path.relative_to(ROOT), 1, f"word_feature_manifest_count:{len(records)}")
        )
    if len(set(record_ids)) != 2449:
        issues.append((manifest_path.relative_to(ROOT), 1, "word_feature_manifest_duplicate_id"))
    if len(set(record_icons)) != 2449 or any(
        not isinstance(icon, str) or "<svg" not in icon for icon in record_icons
    ):
    ...
    if legacy_fields.intersection(manifest) or any(
        ...
        issues.append((manifest_path.relative_to(ROOT), 1, "word_feature_legacy_metadata"))
    if (
    ...
    for record in records:
        ...
        for marker, issue in (
            ...
            if not marker or marker not in feature_source:
                issues.append((feature_path.relative_to(ROOT), 1, issue))
...
def scan_fail_open_contracts(issues):
    ...
    roots = [ROOT / "backend" / "ai" / "src", ROOT / "backend" / "messaging" / "src"]
    for root in roots:
...
def main():
    ...
    for path in tracked_files():
        ...
        if (
            suffix not in PYTHON_SUFFIXES | WEB_SUFFIXES | CONFIG_SUFFIXES
