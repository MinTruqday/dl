...
for srv in services:
    ...
    db_file = os.path.join(core_path, "db_client.py")
    if os.path.exists(db_file):
        with open(db_file, "w") as f:
            f.write(proxy_code)
        print(f"Updated proxy in {srv}")
        
    core_path_alt = os.path.join("backend", srv, "src", "core")
    db_file_alt = os.path.join(core_path_alt, "db_client.py")
    if os.path.exists(db_file_alt):
        with open(db_file_alt, "w") as f:
            f.write(proxy_code)
        print(f"Updated proxy in {srv} alt")
