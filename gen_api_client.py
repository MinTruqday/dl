...
for srv in services:
    ...
    if os.path.exists(core_path):
        with open(os.path.join(core_path, "api_client.py"), "w") as f:
            f.write(api_client_code)
        print(f"Created in {srv}")
    core_path_alt = os.path.join("backend", srv, "src", "core")
    if os.path.exists(core_path_alt) and not os.path.exists(core_path):
        with open(os.path.join(core_path_alt, "api_client.py"), "w") as f:
            f.write(api_client_code)
        print(f"Created in {srv} alt")
