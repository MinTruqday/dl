...
import src.services.operations as training_service
from src.core.auth import get_current_user, require_admin, CurrentUser
...
async def create_dataset(
    ...
    return await training_service.create_dataset(payload)
...
async def list_datasets(current_user: CurrentUser = Depends(get_current_user)):
    ...
    return await training_service.list_datasets(str(current_user.id))
...
async def get_dataset(dataset_id: str, current_user: CurrentUser = Depends(get_current_user)):
    ...
    return await training_service.get_dataset(dataset_id, str(current_user.id))
...
async def delete_dataset(dataset_id: str, current_user: CurrentUser = Depends(get_current_user)):
    ...
    return await training_service.delete_dataset(dataset_id, str(current_user.id))
...
async def add_samples(
    ...
    payload = req.model_dump()
    payload["user_id"] = str(current_user.id)
    return await training_service.add_samples(dataset_id, payload)
...
async def get_samples(
    ...
    return await training_service.get_samples(dataset_id, str(current_user.id), skip, limit)
...
async def delete_sample(dataset_id: str, sample_id: str, current_user: CurrentUser = Depends(get_current_user)):
    ...
    return await training_service.delete_sample(dataset_id, sample_id, str(current_user.id))
...
async def import_feedback(
    ...
    return await training_service.import_feedback({"user_id": str(current_user.id)})
...
async def import_documents(
    ...
    payload = req.model_dump()
    payload["user_id"] = str(current_user.id)
    return await training_service.import_documents(payload)

@router.post("/tien-trinh")
async def create_job(
    req: TrainingJobCreate, current_user: CurrentUser = Depends(get_current_user)
):
    """Create a fine tuning job owned by the authenticated user"""
    payload = req.model_dump()
    payload["user_id"] = str(current_user.id)
    return await training_service.create_job(payload)
...
async def start_job(
    ...
    return await training_service.start_job(
        job_id,
...
async def list_jobs(current_user: CurrentUser = Depends(get_current_user)):
    ...
    return await training_service.list_jobs(str(current_user.id))
...
async def get_job(job_id: str, current_user: CurrentUser = Depends(get_current_user)):
    ...
    return await training_service.get_job(job_id, str(current_user.id))
...
async def cancel_job(
    ...
    return await training_service.cancel_job(
        job_id,
        {"user_id": str(current_user.id)},
    )
...
async def deploy_model(
    ...
    return await training_service.deploy_model(
        job_id,
        {"user_id": str(current_user.id)},
    )
...
async def evaluate_model(
    ...
    payload = req.model_dump()
    payload["user_id"] = str(current_user.id)
    return await training_service.evaluate_model(job_id, payload)
