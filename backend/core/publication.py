...
async def publish_compile_task(document_id: str, author_id: str, content_raw: str):
    if not db_client.rabbitmq:
        logger.error("Lỗi kết nối RabbitMQ, không thể thêm tác vụ biên dịch vào hàng đợi")
        return False
    ...
    try:
        ...
        logger.error(f"Không thể khởi tạo tiến trình biên dịch Tectonic: {str(e)}")
        return False

async def trigger_document_publish_job(document_id: str, author_id: str):
    if not db_client.rabbitmq:
        logger.error("Lỗi kết nối RabbitMQ, không thể thêm tác vụ xuất bản vào hàng đợi")
        return False
...
async def publish_event(queue_name: str, payload: dict):
    if not db_client.rabbitmq:
        logger.error("Lỗi kết nối RabbitMQ, không thể thêm sự kiện vào hàng đợi")
        return False
