...
async def process_tectonic_compile(message: AbstractIncomingMessage):
    async with message.process():
        try:
            ...
            work_dir = f"/tmp/doclib_build_{document_id}"
...
async def process_document_publish(message: AbstractIncomingMessage):
    async with message.process():
        try:
            ...
            logger.info(f"Background publication complete. Document {document_id}. Notified {len(followers)} followers.")
            
        except Exception as e:
            logger.error(f"Document Publish queue error: {str(e)}")

async def start_workers():
    if not db_client.rabbitmq:
        logger.warning("RabbitMQ not active, Worker waiting...")
        return
    ...
    try:
        ...
        logger.info("All background workers (Tectonic, Publish) are active.")
    except Exception as e:
        logger.error(f"Worker startup error: {str(e)}")
