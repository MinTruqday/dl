...
class DRMPolicyOutput(BaseModel):
    decision: str = Field(
        description="CRITICAL: MUST be exactly one of: LEVEL_0 (No DRM), LEVEL_1 (Basic tracking), LEVEL_2 (Watermarking & tracking), LEVEL_3 (Encryption & strict tracking), or BLOCKED (Access denied). Determine this based on the trust profile and document risk."
    )
    reasoning: str = Field(
    ...
    hardware_binding_strict: bool = Field(
        description="CRITICAL: Set to true to lock the decryption key strictly to the client's hardware signature (MAC address, CPU ID). Use ONLY for LEVEL_3."
    )
