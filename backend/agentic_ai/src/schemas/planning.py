...
class PlanNode(BaseModel):
    ...
    dependencies: List[str] = Field(default_factory=list, description="<critical_instructions>List of node IDs that must finish before this node can start. Empty list means it can start immediately.</critical_instructions>")
