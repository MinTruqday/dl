...
class SecurityEvaluation(BaseModel):
    ...
    reason: str = Field(description="<internal_thought>A brief explanation of why the input was flagged, citing the specific heuristic triggered (e.g., 'Contains API key pattern', 'Prompt injection detected'). Leave empty if all flags are False.</internal_thought>")
