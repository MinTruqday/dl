...
class CreateDocument(BaseModel):
    title: str = Field(description="The formal title of the document. CRITICAL: MUST be a concise string (max 100 characters) that accurately reflects the topic.")
    ...
    format: str = Field(description="CRITICAL: MUST be exactly 'json' (for standard interactive UI block editing) or 'latex' (for mathematics/scientific typesetting).")
    content: str = Field(
        description=(
            "The main body of the document. "
            "CRITICAL: For JSON format: MUST be a valid JSON STRING representing an ARRAY of Editor blocks. "
            "CRITICAL: For LaTeX format: MUST be raw LaTeX body content ONLY (no documentclass or begin document). "
...
class UpdateDocument(BaseModel):
    ...
    new_content: str = Field(
        description=(
            ...
            "CRITICAL: For JSON format: MUST be a valid JSON string of an array containing FULL Editor block objects. "
            "CRITICAL: For LaTeX format: MUST be valid raw LaTeX code. "
            "Calling the `inspect_ui_components` tool is a required first step before generating advanced UI blocks. "
