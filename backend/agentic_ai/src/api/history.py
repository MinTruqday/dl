...
async def get_user_sessions(
    ...
    return await HistoryService.get_user_sessions(
        str(current_user.id),
        document_id,
        skip,
        limit,
...
async def update_title(session_id: str, data: SessionTitleUpdate, current_user: CurrentUser = Depends(get_current_user)):
    """Update the title of an owned conversation session"""
    return await HistoryService.update_title(
        session_id,
        data.model_dump(),
        str(current_user.id),
    )
