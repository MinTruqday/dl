...
async def chat_endpoint(req: ChatRequest, request: Request):
    ...
    if not scan.passed:
        return {"answer": "The submitted request contains strictly prohibited malicious content and cannot be processed safely", "route": "blocked"}
...
async def stream_endpoint(req: ChatRequest, request: Request):
    ...
    async def response_generator():
        ...
        if not scan.passed:
            ...
            yield f"event: message\ndata: {json.dumps({'chunk': 'The submitted operational request structurally contains prohibited malicious content obstructing processing capability'})}\n\n"
            yield "event: done\ndata: [DONE]\n\n"
            return
        ...
        try:
            ...
            if route == "chat":
                yield f"event: status\ndata: {json.dumps({'node': 'The system is logically responding directly resolving your submitted conversational network query sequence'})}\n\n"
