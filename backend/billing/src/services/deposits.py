...
class FiatDeposit:
    ...
    async def create_deposit_link(req, current_user, db=None):
        ...
        signature = FiatDeposit._generate_payos_signature(signature_data)
    ...
    async def deposit_webhook(request, db=None):
        ...
        if data.get("code") == "00" and data.get("data"):
            ...
            try:
                ...
                expected_signature = FiatDeposit._generate_payos_signature(
                    signature_data
                )
                ...
                await FiatDeposit.process_success_order(order_code, paid_amount)
            except HTTPException:
                raise
            except Exception:
    ...
    async def verify_deposit(order_code: int, current_user, db=None):
        ...
        try:
            ...
            if res_data.get("code") == "00":
                ...
                if status == "PAID":
                    await FiatDeposit.process_success_order(
    ...
    async def process_success_order(
        ...
        try:
            ...
            try:
                ...
                if settings.ALERTS_URL:
                    async with httpx.AsyncClient() as client:
                        await client.post(
                            f"{settings.ALERTS_URL}/thong-bao/gui-di",
                            json={
