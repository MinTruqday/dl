...
class DatabaseRepository:
    ...
    async def find_one(self, query: Dict[str, Any], **kwargs):
        return await mongo.find_one("collection", query, **kwargs)

    def find(self, *args, **kwargs):
        return mongo.query("collection").filter(*args, **kwargs)

    async def insert_one(self, document: Dict[str, Any], **kwargs):
        return await mongo.insert_one("collection", document, **kwargs)
    ...
    async def update_one(
        ...
        return await mongo.update_one("collection", filter, update, **kwargs)

    async def update_many(
        self, filter: Dict[str, Any], update: Dict[str, Any], **kwargs
    ):
        return await mongo.update_many("collection", filter, update, **kwargs)

    async def delete_one(self, filter: Dict[str, Any], **kwargs):
        return await mongo.delete_one("collection", filter, **kwargs)

    async def delete_many(self, filter: Dict[str, Any], **kwargs):
        return await mongo.delete_many("collection", filter, **kwargs)

    async def count_documents(self, filter: Dict[str, Any], **kwargs) -> int:
        return await mongo.count_documents("collection", filter, **kwargs)
