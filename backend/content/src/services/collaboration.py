...
class CollaborationService:
    ...
    async def get_collaborators(document_id: str, current_user, db=None) -> list:
        db = db or db_client.mongodb.get_default_database()
        if not await RepositoryFactory.get("documents").find_one({"_id": document_id, "$or": [{"creator_id": str(current_user.get("id"))}, {"coauthors": str(current_user.get("id"))}]}): raise HTTPException(status_code=404, detail="Underlying designated core element completely vanished blocking sequential operational reading protocol")
    ...
    async def remove_collaborator(collaboration_id: str, current_user, db=None) -> dict:
        ...
        if not invite: raise HTTPException(status_code=404, detail="Underlying designated core element completely vanished blocking sequential operational reading protocol")
        if not await RepositoryFactory.get("documents").find_one({"_id": invite["document_id"], "creator_id": str(current_user.get("id"))}): raise HTTPException(status_code=403, detail="Platform essentially blocked specific account avoiding altering unowned primary systematic logic")
    ...
    async def update_collab_access(document_id: str, access_level: str, current_user, db=None) -> dict:
        db = db or db_client.mongodb.get_default_database()
        if not await RepositoryFactory.get("documents").find_one({"_id": document_id, "creator_id": str(current_user.get("id"))}): raise HTTPException(status_code=404, detail="Underlying designated core element completely vanished blocking sequential operational reading protocol")
    ...
    async def join_via_invite_code(invite_code: str, current_user, db=None) -> dict:
        ...
        if doc.get("creator_id") == str(current_user.get("id")) or str(current_user.get("id")) in doc.get("coauthors", []): raise HTTPException(status_code=400, detail="Database strict hierarchical lock prevents overlapping editing protecting prior synchronized mutations")
    ...
    async def update_task(task_id: str, is_done: bool, current_user, db=None) -> dict:
        ...
        if not await RepositoryFactory.get("documents").find_one({"_id": task["document_id"], "$or": [{"creator_id": str(current_user.get("id"))}, {"coauthors": str(current_user.get("id"))}]}): raise HTTPException(status_code=403, detail="Platform essentially blocked specific account avoiding altering unowned primary systematic logic")
