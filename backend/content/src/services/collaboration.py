...
class CollaborationService:
    ...
    async def log_activity(document_id: str, user_name: str, action: str, details: str, db=None):
        ...
        await db['collaboration_activities'].insert_one({'_id': str(uuid7()), 'document_id': document_id, 'user_name': user_name, 'action': action, 'details': details, 'timestamp': datetime.now(timezone.utc)})
    ...
    async def send_collaboration_invite(document_id: str, invitee_email: str, role: str, current_user, db=None) -> dict:
        ...
        if invitee_id == str(current_user.id):
            raise HTTPException(status_code=400, detail='Không thể tự mời bản thân làm cộng tác viên')
        existing_invite = await db['collaboration_invites'].find_one({'document_id': document_id, 'invitee_id': invitee_id, 'status': 'PENDING'})
        if existing_invite:
            raise HTTPException(status_code=400, detail='Có lời mời đang chờ người này xác nhận')
        ...
        logger.info(f'Người dùng {current_user.id} mời {invitee_id} chỉnh sửa tài liệu {document_id}')
    ...
    async def get_my_collaboration_invites(current_user, db=None) -> list:
        if db is None:
            db = db_client.mongodb.get_default_database()
        invites = await db['collaboration_invites'].find({'invitee_id': str(current_user.id), 'status': 'PENDING'}).sort('created_at', -1).to_list(length=100)
        return invites
    ...
    async def respond_to_collaboration_invite(invite_id: str, status: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        invite = await db['collaboration_invites'].find_one({'_id': invite_id, 'invitee_id': str(current_user.id), 'status': 'PENDING'})
        if not invite:
        ...
        await db['collaboration_invites'].update_one({'_id': invite_id}, {'$set': {'status': status, 'responded_at': datetime.now(timezone.utc)}})
        if status == 'ACCEPTED':
        ...
        logger.info(f'Người dùng {current_user.id} {status} lời mời cộng tác {invite_id}')
    ...
    async def remove_collaborator(collaboration_id: str, current_user, db=None) -> dict:
        ...
        if not doc:
            raise HTTPException(status_code=403, detail='Không có quyền quản lý cộng tác viên tài liệu này')
        ...
        logger.info(f"Chủ sở hữu {current_user.id} xóa cộng tác viên {invite['invitee_id']} khỏi tài liệu {invite['document_id']}")
    ...
    async def transfer_ownership(document_id: str, target_user_id: str, current_user, db=None) -> dict:
        ...
        doc = await db['documents'].find_one({'_id': document_id, 'author_id': str(current_user.id)})
        if not doc:
            raise HTTPException(status_code=404, detail='Tài liệu không tồn tại hoặc không có quyền chuyển sở hữu')
        import httpx
        target_user = None
        try:
            async with httpx.AsyncClient() as client:
        ...
        if target_user_id not in doc.get('coauthors', []):
            raise HTTPException(status_code=400, detail='Chỉ cộng tác viên mới được nhận chuyển nhượng tài liệu')
    ...
    async def get_online_collaborators(document_id: str, db=None) -> list:
        ...
        online_users = await db['collaboration_status'].find({'document_id': document_id}).to_list(length=100)
        result = []
        for u in online_users:
            last_seen = u.get('last_seen')
            last_seen_ts = last_seen.timestamp() if isinstance(last_seen, datetime) else 0
            is_online = last_seen_ts > cutoff
            result.append({'user_id': u['user_id'], 'full_name': u.get('full_name', 'Cộng tác viên'), 'status': 'online' if is_online else 'offline'})
        return result
    ...
    async def update_collab_access(document_id: str, access_level: str, current_user, db=None) -> dict:
        ...
        doc = await db['documents'].find_one({'_id': document_id, 'author_id': str(current_user.id)})
        if not doc:
            raise HTTPException(status_code=404, detail='Tài liệu không tồn tại hoặc không có quyền cập nhật cài đặt')
        ...
        return {'message': 'Đã cập nhật quyền truy cập mặc định', 'collab_access_level': access_level}
    ...
    async def revoke_invite(invite_id: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        invite = await db['collaboration_invites'].find_one({'_id': invite_id, 'status': 'PENDING'})
        if not invite:
            raise HTTPException(status_code=404, detail='Lời mời không tồn tại hoặc đã được chấp nhận')
        doc = await db['documents'].find_one({'_id': invite['document_id'], 'author_id': str(current_user.id)})
        if not doc:
            raise HTTPException(status_code=403, detail='Không có quyền thu hồi lời mời này')
    ...
    async def get_contribution_stats(document_id: str, current_user, db=None) -> list:
        ...
        doc = await db['documents'].find_one({'_id': document_id, '$or': [{'author_id': str(current_user.id)}, {'coauthors': str(current_user.id)}]})
        if not doc:
            raise HTTPException(status_code=404, detail='Tài liệu không tồn tại hoặc không có quyền xem thống kê')
    ...
    async def generate_invite_code(document_id: str, current_user, db=None) -> dict:
        ...
        doc = await db['documents'].find_one({'_id': document_id, 'author_id': str(current_user.id)})
        if not doc:
            raise HTTPException(status_code=404, detail='Tài liệu không tồn tại hoặc không có quyền sở hữu')
    ...
    async def join_via_invite_code(invite_code: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        code_entry = await db['collaboration_invite_codes'].find_one({'invite_code': invite_code.upper()})
        if not code_entry:
            raise HTTPException(status_code=404, detail='Mã cộng tác không tồn tại hoặc đã hết hạn')
        ...
        if str(current_user.id) in doc.get('coauthors', []):
            raise HTTPException(status_code=400, detail='Đã là cộng tác viên tài liệu này')
    ...
    async def add_task_comment(task_id: str, comment_text: str, current_user, db=None) -> dict:
        ...
        doc = await db['documents'].find_one({'_id': task['document_id'], '$or': [{'author_id': str(current_user.id)}, {'coauthors': str(current_user.id)}]})
        if not doc:
            raise HTTPException(status_code=403, detail='Không có quyền thảo luận trong nhiệm vụ này')
    ...
    async def get_task_comments(task_id: str, current_user, db=None) -> list:
        ...
        doc = await db['documents'].find_one({'_id': task['document_id'], '$or': [{'author_id': str(current_user.id)}, {'coauthors': str(current_user.id)}]})
        if not doc:
            raise HTTPException(status_code=403, detail='Không có quyền truy cập thảo luận nhiệm vụ này')
