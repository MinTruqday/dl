...
class HighlightService:
    ...
    async def get_all_notes(current_user, cursor: str=None, limit: int=50, skip: int=0, db=None) -> list:
        ...
        for h in highlights:
            document = h.get('doc', {})
            result.append({'_id': str(h['_id']), 'document_id': h['document_id'], 'document_title': document.get('title', '') if document else '', 'document_slug': document.get('slug', '') if document else '', 'text': h.get('text', ''), 'note': h.get('note', ''), 'color': h.get('color', '#e4e4e7'), 'created_at': h['created_at'].isoformat() if isinstance(h.get('created_at'), datetime) else h.get('created_at')})
        return result
    ...
    async def export_highlights_markdown(document_id: str, current_user, db=None) -> dict:
        ...
        highlights = await db['highlights'].find({'user_id': str(current_user.id), 'document_id': document_id}).sort('created_at', 1).to_list(length=500)
