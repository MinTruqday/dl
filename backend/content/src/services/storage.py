...
from src.schemas.storage import StorageItemInDB, StorageItemCreate, StorageItemUpdate
...
class StorageService:
    ...
    async def create_shortcut(item_id: str, parent_id: Optional[str], owner_id: str, db=None) -> Optional[StorageItemInDB]:
        ...
        if not target:
            return None
        shortcut = StorageItemInDB(name=f'Lối tắt của {target.name}', parent_id=parent_id, owner_id=owner_id, is_folder=False, is_shortcut=True, target_id=item_id)
    ...
    async def update_item(item_id: str, owner_id: str, update_data: StorageItemUpdate, db=None) -> Optional[StorageItemInDB]:
        ...
        result = await db_client.mongodb.get_default_database().storage_items.find_one_and_update({'_id': item_id, 'owner_id': owner_id}, {'$set': update_dict}, return_document=True)
        if result:
    ...
    async def add_version(item_id: str, owner_id: str, url: str, size: int, db=None) -> Optional[StorageItemInDB]:
        ...
        update_dict = {'url': url, 'size': size, 'updated_at': datetime.now(timezone.utc)}
    ...
    async def share_item(item_id: str, email: str, role: str, owner_id: str, db=None) -> dict:
        ...
        if target_user_id == owner_id:
            from fastapi import HTTPException
            raise HTTPException(status_code=400, detail='Không thể tự chia sẻ tệp cho bản thân')
        item = await db_client.mongodb.get_default_database().storage_items.find_one({'_id': item_id, 'owner_id': owner_id})
        if not item:
            from fastapi import HTTPException
            raise HTTPException(status_code=404, detail='Tệp không tồn tại hoặc không có quyền chia sẻ')
    ...
    async def get_recent_items(owner_id: str, limit: int=20, db=None) -> List[StorageItemInDB]:
        query = {'$or': [{'owner_id': owner_id}, {'shared_with.user_id': owner_id}], 'is_trashed': False, 'is_folder': False}
