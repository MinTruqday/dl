...
class ReviewService:
    ...
    @staticmethod
    async def rate_chapter(document_id: str, data, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        if data.rating < 1 or data.rating > 5:
            raise HTTPException(status_code=400, detail='Điểm đánh giá phải từ 1 đến 5.')
        await db['chapter_ratings'].update_one({'user_id': str(current_user.id), 'document_id': document_id, 'chapter_slug': data.chapter_slug}, {'$set': {'rating': data.rating, 'updated_at': datetime.now(timezone.utc)}}, upsert=True)
    ...
    async def report_typo(document_id: str, data, current_user, db=None) -> dict:
        ...
        report = {'_id': str(uuid7()), 'user_id': str(current_user.id), 'document_id': document_id, 'chapter_slug': chapter_slug, 'text_excerpt': text_excerpt[:500] if text_excerpt else '', 'description': description[:300] if description else '', 'status': 'pending', 'created_at': datetime.now(timezone.utc)}
    ...
    async def get_typo_reports(document_id: str, current_user, db=None) -> list:
        ...
        return [{'_id': str(r['_id']), 'chapter_slug': r.get('chapter_slug', ''), 'text_excerpt': r.get('text_excerpt', ''), 'description': r.get('description', ''), 'status': r.get('status', 'pending'), 'created_at': r['created_at'].isoformat() if isinstance(r.get('created_at'), datetime) else ''} for r in reports]
