...
class ReviewService:
    ...
    async def report_typo(document_id: str, data, current_user, db=None) -> dict:
        ...
        logger.info(f'Người dùng {current_user.id} báo lỗi chính tả tài liệu {document_id}')
    ...
    async def create_review(document_id: str, review_in, current_user, db=None) -> dict:
        ...
        logger.info(f'Người dùng {current_user.id} đánh giá {review_in.rating} sao tài liệu {document_id}')
        return review_item
