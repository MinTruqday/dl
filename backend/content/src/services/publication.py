...
class PublicationService:
    ...
    async def update_seo_metadata(document_id: str, seo_data: dict, current_user, db=None):
        ...
        if not doc:
            raise HTTPException(status_code=403, detail='Tài liệu không tồn tại hoặc không có quyền truy cập')
    ...
    async def get_readability_score(document_id: str, current_user, db=None):
        ...
        try:
            ...
            return {'ease_score': score, 'complexity_grade': grade, 'target_audience': target, 'total_words': words, 'analysis': 'Cấu trúc dễ đọc' if score > 60 else 'Cấu trúc phức tạp'}
        except ImportError:
    ...
    async def schedule_publish(document_id: str, publish_at: str, current_user, db=None):
        ...
        logger.info(f'Tài liệu {document_id} được đặt lịch xuất bản lúc {publish_at} bởi {user_id}')
