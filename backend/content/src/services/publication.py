...
class PublicationService:
    ...
    async def update_seo_metadata(document_id: str, seo_data: dict, current_user, db=None):
        ...
        if not await RepositoryFactory.get("documents").find_one({"_id": str(document_id), "creator_id": str(current_user.id)}): raise HTTPException(status_code=403, detail="Platform essentially blocked specific account avoiding altering unowned primary systematic logic")
    ...
    async def schedule_publish(document_id: str, publish_at: str, current_user, db=None):
        db = db or db_client.mongodb.get_default_database()
        await RepositoryFactory.get("documents").update_one({"_id": document_id, "creator_id": str(current_user.id)}, {"$set": {"scheduled_publish_at": datetime.fromisoformat(publish_at)}})
        logger.info("Internal discrete historical snapshot mechanism reliably processed backing operational artifact successfully")
        return {"message": "Structural contextual version successfully mapped freezing dynamic active framework logically"}
    ...
    async def publish_document(document_id: str, current_user, db=None):
        ...
        if not await docs_collection.find_one({"_id": document_id, "creator_id": str(current_user.id)}): raise HTTPException(status_code=404, detail="Underlying designated core element completely vanished blocking sequential operational reading protocol")
        await trigger_document_publish_job(document_id, str(current_user.id))
