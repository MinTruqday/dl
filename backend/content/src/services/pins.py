...
class PinService:
    ...
    async def get_pinned_documents(current_user, db=None) -> list:
        ...
        docs = await RepositoryFactory.get("documents").find({"_id": {"$in": doc_ids}}).to_list(length=len(doc_ids))
        doc_map = {str(d["_id"]): d for d in docs}
        return [{"_id": str(d["_id"]), "title": d.get("title", ""), "slug": d.get("slug", ""), "cover_url": d.get("cover_url"), "creator_id": d.get("creator_id"), "pinned_at": pinned_at_map.get(d_id)} for d_id in doc_ids if d_id in doc_map for d in [doc_map[d_id]]]
