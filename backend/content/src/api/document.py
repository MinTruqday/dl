...
async def create_document(
    ...
    return APIResponse(data=await DocumentService.create_document(doc_in, current_user), message="Đã tạo tài liệu mới", status=status.HTTP_201_CREATED)
...
async def update_document_content(
    ...
    return APIResponse(data=await DocumentService.update_document_content(document_id, content_in, current_user), message="Đã cập nhật nội dung tài liệu", status=status.HTTP_200_OK)
...
async def update_document(
    ...
    return APIResponse(data=await DocumentService.update_document(document_id, doc_update, current_user), message="Đã cập nhật thông tin tài liệu", status=status.HTTP_200_OK)
...
async def list_documents(
    ...
    return APIResponse(data=await DocumentService.list_documents(limit, cursor, q, sort_by, category, tag), message="Đã tải danh sách tài liệu", status=status.HTTP_200_OK)
...
async def get_document_by_id(
    ...
    return APIResponse(data=await DocumentService.get_document_by_id(document_id, current_user, password), message="Đã tải thông tin tài liệu", status=status.HTTP_200_OK)
...
async def get_document_by_slug(
    ...
    return APIResponse(data=await DocumentService.get_document_by_slug(slug, current_user), message="Đã tải tài liệu theo đường dẫn", status=status.HTTP_200_OK)
...
async def toggle_star_document(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data={"starred": not current_starred}, message="Đã đánh dấu sao tài liệu")
...
async def transfer_document(document_id: str, new_owner_id: str = Query(...), current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data={"status": "transferred", "new_owner_id": new_owner_id}, message="Đã chuyển nhượng tài liệu")
...
async def unlock_document(document_id: str, password: str = Body(..., embed=True), current_user: UserInDB = Depends(get_current_user_optional), db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_document_by_id(document_id, current_user, password), message="Đã mở khóa tài liệu", status=status.HTTP_200_OK)
