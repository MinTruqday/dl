...
from src.schemas.series import SeriesCreateRequest, SeriesResponse
...
async def create_document(
    ...
    return APIResponse(data=await DocumentService.create_document(doc_in, current_user), message="Tạo tài liệu mới hoàn tất", status=status.HTTP_201_CREATED)
...
async def update_document_content(
    ...
    return APIResponse(data=await DocumentService.update_document_content(document_id, content_in, current_user), message="Cập nhật nội dung tài liệu hoàn tất", status=status.HTTP_200_OK)
...
async def update_document(
    ...
    return APIResponse(data=await DocumentService.update_document(document_id, doc_update, current_user), message="Cập nhật thông tin tài liệu hoàn tất", status=status.HTTP_200_OK)
...
async def list_documents(
    ...
    return APIResponse(data=await DocumentService.list_documents(limit, cursor, q, sort_by, category, tag), message="Lấy danh sách tài liệu hoàn tất", status=status.HTTP_200_OK)
...
async def get_folders(parent_id: Optional[str] = None, current_user: UserInDB = Depends(get_current_user)):
    ...
    for f in folders:
        f["_id"] = str(f["_id"])
    return APIResponse(data=folders, message="Lấy danh sách thư mục hoàn tất")
...
async def create_folder(req: FolderCreate, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data=folder_doc, message="Tạo thư mục hoàn tất")
...
async def delete_folder(folder_id: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data={"deleted": True}, message="Xóa thư mục hoàn tất")
...
async def get_my_documents(q: Optional[str] = None, cursor: Optional[str] = None, limit: int = Query(50, ge=1, le=100), current_user: UserInDB = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
        ...
        message="Lấy danh sách tài liệu cá nhân hoàn tất"
    )
...
async def get_trash(current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(
        ...
        message="Lấy danh sách thùng rác hoàn tất"
    )
...
async def get_document_by_id(
    ...
    return APIResponse(data=await DocumentService.get_document_by_id(document_id, current_user, password), message="Lấy thông tin tài liệu hoàn tất", status=status.HTTP_200_OK)
...
async def get_document_by_slug(
    ...
    return APIResponse(data=await DocumentService.get_document_by_slug(slug, current_user), message="Lấy tài liệu theo đường dẫn hoàn tất", status=status.HTTP_200_OK)
...
async def get_document_preview(slug: str):
    return APIResponse(
        ...
        message="Lấy bản xem trước tài liệu hoàn tất"
    )
...
async def get_my_series(current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(
        ...
        message="Lấy danh sách chuỗi tài liệu hoàn tất"
    )
...
async def get_series_by_id(series_id: str):
    return APIResponse(
        ...
        message="Lấy chi tiết chuỗi tài liệu hoàn tất"
    )
...
async def reorder_series_documents(series_id: str, document_ids: List[str], current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(
        ...
        message="Sắp xếp lại thứ tự tài liệu hoàn tất"
    )
...
async def set_document_password(document_id: str, req: DocumentPasswordRequest, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(
        ...
        message="Thiết lập mật khẩu tài liệu hoàn tất"
    )
...
async def get_document_audit_logs(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(
        ...
        message="Lấy nhật ký hoạt động tài liệu hoàn tất"
    )
...
async def toggle_star_document(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data={"starred": not current_starred}, message="Cập nhật gắn sao hoàn tất")
...
async def transfer_document(document_id: str, new_owner_id: str = Query(...), current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data={"status": "transferred", "new_owner_id": new_owner_id}, message="Chuyển nhượng tài liệu hoàn tất")
...
async def get_document_analytics(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    }, message="Lấy phân tích độc giả hoàn tất")
...
async def get_document_academic(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    }, message="Lấy chỉ số học thuật hoàn tất")
...
async def update_drm_settings(document_id: str, req: DRMSettingsUpdate, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message="Cập nhật cài đặt bảo vệ bản quyền hoàn tất")
...
async def update_tags(document_id: str, req: TagsUpdate, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message="Cập nhật thẻ hoàn tất")
...
async def schedule_publish(document_id: str, req: ScheduleUpdate, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message="Lên lịch xuất bản hoàn tất")
...
async def update_nsfw(document_id: str, req: NSFWUpdate, current_user: UserInDB = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message="Cập nhật giới hạn độ tuổi hoàn tất")
...
async def unlock_document(document_id: str, password: str = Body(..., embed=True), current_user: UserInDB = Depends(get_current_user_optional), db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_document_by_id(document_id, current_user, password), message="Mở khóa hoàn tất", status=status.HTTP_200_OK)
