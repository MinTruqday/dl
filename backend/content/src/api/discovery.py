...
async def get_trending_documents(limit: int=5, db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_trending_documents(limit), message='Lấy danh sách tài liệu xu hướng success', status=status.HTTP_200_OK)

@router.get('/phan-loai', response_model=APIResponse[Any])
async def get_tags_categories(db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_tags_categories(), message='Lấy danh sách thẻ và danh mục success', status=status.HTTP_200_OK)
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code == 200:
                result = resp.json()
                return APIResponse(data=result, message='Tìm kiếm thông minh AI success')
            else:
...
async def get_ai_recommendations(limit: int=10, current_user: UserInDB=Depends(get_current_user_optional), db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_trending_documents(limit), message='Lấy gợi ý tài liệu từ AI success')

@router.get('/hashtag-xu-huong', response_model=APIResponse[Any])
async def get_trending_tags(limit: int=10, db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_trending_tags(limit), message='Lấy danh sách hashtag xu hướng success', status=status.HTTP_200_OK)