...
async def get_bookmarks(limit: int=Query(100), current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await BookmarkService.get_bookmarks(current_user, limit, db=db), message='Lấy danh sách dấu trang hoàn tất')
...
async def create_bookmark_folder(data: BookmarkFolderCreate, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await BookmarkService.create_bookmark_folder(data.name, current_user, db=db), message='Tạo thư mục dấu trang hoàn tất', status=201)
...
async def get_bookmark_folders(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await BookmarkService.get_bookmark_folders(current_user, db=db), message='Lấy danh sách thư mục dấu trang hoàn tất')
...
async def assign_bookmarks(folder_id: str, data: BookmarkFolderAssign, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await BookmarkService.assign_bookmarks_to_folder(folder_id, data.bookmark_ids, current_user, db=db), message='Cập nhật thư mục dấu trang hoàn tất')
...
async def delete_bookmark_folder(folder_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await BookmarkService.delete_bookmark_folder(folder_id, current_user, db=db), message='Xóa thư mục dấu trang hoàn tất')