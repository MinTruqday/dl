...
from src.schemas.library import TypographyRequest, ProgressUpdate, ReadingGoalCreate, PinnedDocumentRequest
...
async def get_history(cursor: str=None, limit: int=Query(20), current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.get_reading_history(current_user, cursor, limit, db=db), message='Lấy lịch sử đọc success')
...
async def update_progress(data: ProgressUpdate, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.update_progress(data, current_user, db=db), message='Cập nhật tiến độ success')
...
async def set_reading_goal(data: ReadingGoalCreate, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.set_reading_goal(data, current_user, db=db), message='Thiết lập mục tiêu success', status=201)
...
async def get_reading_goal(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.get_reading_goal(current_user, db=db), message='Lấy thông tin mục tiêu success')
...
async def update_typography(data: TypographyRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.update_typography(data, current_user, db=db), message='Cập nhật hiển thị success')
...
async def search_in_document(document_id: str, q: str=Query(...), current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.search_in_document(document_id, q, current_user, db=db), message='Tìm kiếm trong tài liệu success')
...
async def clear_reading_history(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.clear_reading_history(current_user, db=db), message='Xóa toàn bộ lịch sử đọc success')
...
async def delete_history_item(document_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await ReadingService.delete_history_item(document_id, current_user, db=db), message='Xóa mục lịch sử đọc success')
import aiohttp
import zipfile
import io
import socket
            ...
            async with session.get(file_url) as resp:
                if resp.status == 200:
                    ...
                    with zipfile.ZipFile(io.BytesIO(content)) as z:
                        ...
                        return APIResponse(data=tree, message='Lấy cây thư mục success')
                else:
                    ...
                    with zipfile.ZipFile(io.BytesIO(content)) as z:
                        if path in z.namelist():
                            ...
                            try:
                                ...
                                return APIResponse(data={'content': text, 'type': 'text'}, message='success')
                            except UnicodeDecodeError:
                                return APIResponse(data={'content': 'Binary file cannot be displayed', 'type': 'binary'}, message='success')
