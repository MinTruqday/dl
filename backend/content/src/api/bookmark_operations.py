...
from src.schemas.personal_library import BookmarkFolderCreate, BookmarkFolderAssign
from src.api.system_dependency import get_current_user, get_db
from src.services.bookmark_operations import BookmarkOperations
...
async def create_bookmark_folder(
    ...
    return APIResponse(
        data=await BookmarkOperations.create_bookmark_folder(
...
async def get_bookmark_folders(
    ...
    return APIResponse(
        data=await BookmarkOperations.get_bookmark_folders(current_user, db=db),
...
async def assign_bookmarks(
    ...
    return APIResponse(
        data=await BookmarkOperations.assign_bookmarks_to_folder(
...
async def delete_bookmark_folder(
    ...
    return APIResponse(
        data=await BookmarkOperations.delete_bookmark_folder(
