...
async def analyze_internal_plagiarism(document_id: str, payload: PlagiarismCheckRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.analyze_internal_plagiarism(document_id, payload.model_dump(), current_user, db=db), message='success phân tích đạo văn nội bộ', status=200)
...
async def sync_keystroke_buffer(document_id: str, payload: KeystrokeSyncRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.sync_keystroke_buffer(document_id, payload.model_dump(), current_user, db=db), message='Đồng bộ bộ đệm gõ phím success', status=200)

@router.get('/latex', response_model=APIResponse[Any])
async def get_latex(db=Depends(get_db)):
    return APIResponse(data=await EditorService.get_latex(db=db), message='Lấy mã nguồn LaTeX success', status=200)
...
async def add_inline_suggestion(document_id: str, payload: InlineSuggestionRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.add_inline_suggestion(document_id, payload.model_dump(), current_user, db=db), message='Thêm gợi ý nội dòng success', status=201)
...
async def resolve_suggestion(suggestion_id: str, payload: ResolveSuggestionRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.resolve_suggestion(suggestion_id, payload.model_dump(), current_user, db=db), message='Xử lý gợi ý success', status=200)
...
async def sync_pomodoro_session(payload: PomodoroSyncRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.sync_pomodoro_session(payload.model_dump(), current_user, db=db), message='Đồng bộ phiên Pomodoro success', status=200)
...
async def auto_save_draft(document_id: str, payload: AutoSaveRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.auto_save_draft(document_id, payload.content, current_user, db=db), message='Tự động lưu bản nháp success', status=200)
...
async def submit_for_review(document_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.submit_for_review(document_id, current_user, db=db), message='Gửi tài liệu để xem xét success', status=201)
...
async def check_deep_plagiarism(document_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.check_deep_plagiarism(document_id, current_user, db=db), message='Kiểm tra đạo văn chuyên sâu success', status=200)
...
async def global_find_replace(document_id: str, payload: FindReplaceRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.global_find_replace(document_id, payload.search, payload.replace, payload.match_case, current_user, db=db), message='Thay thế toàn cục success', status=200)
...
async def generate_cover(document_id: str, payload: CoverGenerateRequest, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR])), db=Depends(get_db)):
    return APIResponse(data=await EditorService.generate_cover(document_id, payload.style, current_user, db=db), message='Khởi tạo ảnh bìa AI success')
...
async def update_cover(document_id: str, cover_url: str, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)) -> Any:
    return APIResponse(data=await DocumentService.update_cover(document_id, cover_url, current_user, db=db), message='Cập nhật ảnh bìa success', status=200)
...
async def get_ai_suggestions(document_id: str, payload: AISuggestionRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.get_ai_suggestions(document_id, payload.context, current_user, db=db), message='Lấy gợi ý AI success')
...
async def summarize_document(document_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.summarize_document(document_id, current_user, db=db), message='Tóm tắt tài liệu success')
...
async def extract_smart_tags(document_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.extract_smart_tags(document_id, current_user, db=db), message='Tự động phân tích thẻ success')
...
async def check_logic(document_id: str, payload: dict, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.check_logic(document_id, payload.get('content', ''), current_user, db=db), message='Kiểm tra tính nhất quán success')
...
async def add_inline_comment(document_id: str, payload: InlineCommentRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.add_inline_comment(document_id, payload.model_dump(), current_user, db=db), message='Thêm nhận xét success')
...
async def get_inline_comments(document_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.get_inline_comments(document_id, current_user, db=db), message='Lấy danh sách nhận xét success')
...
async def get_version_diff(document_id: str, payload: VersionDiffRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await EditorService.get_version_diff(document_id, payload.version_id_a, payload.version_id_b, current_user, db=db), message='Lấy dữ liệu so sánh phiên bản success')