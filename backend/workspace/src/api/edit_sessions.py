...
from src.schemas.edit_sessions import (
    AutoSaveRequest,
    FindReplaceRequest,
...
from src.services.edit_sessions import EditorProcess
...
async def sync_keystroke_buffer(
    ...
    return {
        "data": await EditorProcess.sync_keystroke_buffer(
...
async def add_inline_suggestion(
    ...
    return {
        "data": await EditorProcess.add_inline_suggestion(
            document_id, payload.model_dump(), current_user
        ),
...
async def resolve_suggestion(
    ...
    return {
        "data": await EditorProcess.resolve_suggestion(
...
async def sync_pomodoro_session(
    ...
    return {
        "data": await EditorProcess.sync_pomodoro_session(
...
async def auto_save_draft(
    ...
    return {
        "data": await EditorProcess.auto_save_draft(
...
async def submit_for_review(document_id: str, current_user=Depends(get_current_user)):
    return {
        "data": await EditorProcess.submit_for_review(document_id, current_user),
...
async def global_find_replace(
    ...
    return {
        "data": await EditorProcess.global_find_replace(
            document_id,
            payload.search,
            payload.replace,
            payload.match_case,
...
async def add_inline_comment(
    ...
    return {
        "data": await EditorProcess.add_inline_comment(
            document_id, payload.model_dump(), current_user
        ),
...
async def get_inline_comments(document_id: str, current_user=Depends(get_current_user)):
    return {
        "data": await EditorProcess.get_inline_comments(document_id, current_user),
...
async def resolve_comment(comment_id: str, current_user=Depends(get_current_user)):
    return {
        "data": await EditorProcess.resolve_comment(comment_id, current_user),
...
async def get_version_diff(
    ...
    return {
        "data": await EditorProcess.get_version_diff(
