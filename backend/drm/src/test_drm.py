...
MONGO_URI = "mongodb://admin:doclib-password@localhost:27017/?authSource=admin"
...
async def main():
    ...
    await client["doclib_content"]["documents"].insert_one({
        "_id": doc_pro_id,
        "title": "Tài liệu PRO",
    ...
    async with httpx.AsyncClient() as http:
        resp = await http.get(f"http://localhost:8000/drm/ket-xuat/{doc_pro_id}/drm", headers={"Authorization": f"Bearer {pro_token}"})
    ...
    async with httpx.AsyncClient() as http:
        resp = await http.get(f"http://localhost:8000/drm/ket-xuat/{doc_prem_id}/drm", headers={"Authorization": f"Bearer {prem_token}"})
