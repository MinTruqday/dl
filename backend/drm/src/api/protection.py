...
router = APIRouter(prefix="/bao-ve", dependencies=[Depends(verify_internal_token)])
...
async def check_network_anomaly(user_id: str, client_ip: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail="Dịch vụ đánh giá bất thường tạm thời không khả dụng"
        )
...
async def get_user_trust_profile(
    ...
    denied_accesses = await drm_db.audit_logs.count_documents(
        {"user_id": user_id, "event": {"$in": ["license_access_denied", "license_access_anomaly"]}}
    )
...
async def analyze_document_risk(
    ...
    drm_db = database.mongodb[settings.DRM_DB_NAME]
    drm_settings = await drm_db.document_drm_settings.find_one({"document_id": document_id})
    open_disputes = await drm_db.copyright_disputes.count_documents(
        {"document_id": document_id, "status": {"$in": ["PENDING", "OPEN", "UNDER_REVIEW"]}}
    )
    revoked_licenses = await drm_db.drm_licenses.count_documents(
    ...
    if drm_settings and any(
        drm_settings.get(field) for field in ["disable_copy", "disable_print", "watermark_enabled"]
    ):
        score += 20
...
async def generate_dynamic_watermark(
    user_id: str, client_ip: str, email: str = ""
) -> Dict[str, Any]:
    try:
        ipaddress.ip_address(client_ip)
    except ValueError:
    ...
    watermark_token = hashlib.sha256(f"{user_id}:{client_ip}:{timestamp_str}".encode()).hexdigest()[
        :16
    ]
...
async def issue_temporary_aes_key(
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Dịch vụ cấp khóa tạm thời không khả dụng")

    return {"key_id": key_id, "key_hex": raw_key, "ttl_seconds": ttl_seconds, "status": "issued"}
...
async def verify_device_fingerprint(
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail="Địa chỉ IP không hợp lệ")
    licenses = (
        await database.mongodb[settings.DRM_DB_NAME]
        .drm_licenses.find(
            {"user_id": user_id, "status": "ACTIVE", "hardware_signature": {"$type": "string"}},
            {"hardware_signature": 1, "recent_accesses": 1},
        )
        .to_list(length=100)
    )
    enrolled = {row["hardware_signature"] for row in licenses if row.get("hardware_signature")}
    known_ips = {
        access.get("ip")
        for row in licenses
...
async def get_internal_ai_content(
    ...
    document = await ContentClient.get_accessible(document_id, user_id, is_admin, edit=False)
    if not document:
        raise HTTPException(status_code=403, detail="Không có quyền đọc tài liệu")
    drm_db = database.mongodb[settings.DRM_DB_NAME]
    policy = await drm_db.document_drm_settings.find_one({"document_id": document_id})
    ...
    return {
        "data": {
            ...
            "profile": (policy or {}).get("profile", "doclib-standard"),
