    ...
    async def __call__(self, request: Request):
        ...
        if current is not None and int(current) >= self.calls:
            raise HTTPException(
                ...
                detail="Hệ thống đang xử lý quá nhiều yêu cầu, vui lòng thử lại sau",
            )
...
def require_permissions(required_permissions: List[str]):
    ...
    async def permission_checker(
        ...
        if missing:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Thao tác bị từ chối do thiếu quyền truy cập",
            )
        return current_user
...
def get_current_user_from_header(
    ...
    if not x_user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Thiếu thông tin xác thực người dùng",
        )
