...
async def run_worker():
    ...
    async def process_msg(message, handler_func):
        try:
            ...
            logger.error(f"Gặp sự cố khi xử lý tin nhắn: {e}\nPayload: {message.body.decode()}")
            raise
    ...
    def signal_handler():
        logger.info("Nhận được lệnh tắt, hệ thống đang tiến hành tắt an toàn")
        stop_event.set()

    await stop_event.wait()

    logger.info("Đang đóng kết nối hệ thống hàng đợi thông điệp")
