...
async def proxy_rag_stream(payload: dict, req: Request, current_user: UserInDB=Depends(get_current_user_optional), db=Depends(get_db)):
    auth_header = req.headers.get('Authorization')
    return await RagService.proxy_rag_stream(payload, auth_header, current_user, db=db)

@router.post('/truy-van', response_model=APIResponse[Any])
async def proxy_rag_chat(payload: dict, req: Request, current_user: UserInDB=Depends(get_current_user_optional), db: AsyncIOMotorDatabase=Depends(get_db)):
    ...
    auth_header = req.headers.get('Authorization')
    result = await RagService.proxy_rag_chat(payload, auth_header, current_user, db=db)
...
@router.post('/dong-bo/{document_id}', response_model=APIResponse[Any])
async def ingest_document(document_id: str, current_user: UserInDB=Depends(get_current_user_optional), db=Depends(get_db)):
    if not current_user:
        raise HTTPException(status_code=401, detail='Bạn cần đăng nhập để thực hiện hành động này')
    result = await RagService.ingest(document_id, db=db)
