...
async def get_trending_documents(limit: int=5, db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_trending_documents(limit, db=db), message='Lấy danh sách tài liệu xu hướng thành công', status=status.HTTP_200_OK)

@router.get('/the-va-danh-muc', response_model=APIResponse[Any])
async def get_tags_categories(db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_tags_categories(db=db), message='Lấy danh sách thẻ và danh mục thành công', status=status.HTTP_200_OK)
...
async def smart_search(query: str, limit: int=10, current_user: UserInDB=Depends(get_current_user_optional), db=Depends(get_db)):
    if not current_user:
        return APIResponse(data=await DocumentService.get_text_search(query, limit, db=db), message='Tìm kiếm văn bản cơ bản (Vui lòng đăng nhập để dùng AI)')
...
async def get_trending_tags(limit: int=10, db=Depends(get_db)):
    return APIResponse(data=await DocumentService.get_trending_tags(limit, db=db), message='Lấy danh sách hashtag xu hướng thành công', status=status.HTTP_200_OK)