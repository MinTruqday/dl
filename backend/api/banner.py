...
router = APIRouter(prefix='/bieu-ngu')

@router.get('', response_model=APIResponse[Any])
async def get_active_banners(db=Depends(get_db)):
    return APIResponse(data=await BannerService.get_banners(active_only=True, db=db), message='Lấy danh sách biểu ngữ thành công')
...
async def get_all_banners(db=Depends(get_db)):
    return APIResponse(data=await BannerService.get_banners(active_only=False, db=db), message='Lấy toàn bộ danh sách biểu ngữ thành công')
...
async def create_banner(data: BannerRequest, db=Depends(get_db)):
    return APIResponse(data=await BannerService.create_banner(data.model_dump(), db=db), message='Tạo biểu ngữ thành công', status=201)
...
async def delete_banner(banner_id: str, db=Depends(get_db)):
    return APIResponse(data=await BannerService.delete_banner(banner_id, db=db), message='Xoá biểu ngữ thành công')