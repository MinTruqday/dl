...
class ConnectionManager:
    ...
    async def _action_sync(self, user_id: str, data: dict):
        ...
        job_payload = json.dumps({
            ...
            "data": {
                "finetune": [{"id": str(j["_id"]), "progress": j.get("progress", 0), "status": j["status"]} for j in active_finetunes],
                "collector": [{"id": str(j["_id"]), "progress": j.get("progress", 0), "status": j["status"]} for j in active_collectors]
            }
        })
        for ws in list(ws_set):
            try:
    ...
    async def _action_send_message(self, sender_id: str, data: dict):
        ...
        sender_doc = await db["users"].find_one(
            {"_id": sender_id}, {"full_name": 1, "username": 1}
        )
        if not sender_doc:
            return
        ...
        msg = await MessageService.send_message(
            receiver_id, content, fake_user, image_url, reply_to_id, audio_url, client_msg_id
        )
    ...
    try:
        while True:
            ...
            if len(frame_times) > 5:
                if db:
                    await db.setex(f"ws_ban:{user_id}", 300, "banned") # Ban for 5 minutes
