...
async def create_highlight(
    ...
    return APIResponse(data=await HighlightService.create_highlight(document_id, data.model_dump(), current_user), message="Tạo nêu bật đoạn văn thành công", status=201)
...
async def get_highlights(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await HighlightService.get_highlights(document_id, current_user), message="Lấy danh sách nêu bật của tài liệu thành công", status=200)
...
async def update_highlight_note(
    ...
    return APIResponse(data=await HighlightService.update_highlight_note(highlight_id, data.note, current_user), message="Cập nhật ghi chú nêu bật thành công", status=200)
...
async def delete_highlight(highlight_id: str, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await HighlightService.delete_highlight(highlight_id, current_user), message="Xóa nêu bật đoạn văn thành công", status=200)
...
async def export_highlights_markdown(
    ...
    return APIResponse(data=await HighlightService.export_highlights_markdown(document_id, current_user), message="Xuất bản danh sách nêu bật (Markdown) thành công", status=200)
