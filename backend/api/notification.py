...
from api.dependency import get_db, get_current_user, get_current_user_token_param, require_role, RateLimiter
...
async def stream_notifications(request: Request, token: str=Query(...), _: Any=Depends(RateLimiter(calls=60, period=60)), db=Depends(get_db)):
...
async def mark_notification_read(notif_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
...
async def trigger_test_notification(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
...
async def trigger_push_notif(title: str, body: str, current_user: UserInDB=Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN])), db=Depends(get_db)):
...
async def get_notification_settings(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
...
async def update_notification_settings(data: NotificationSettingsUpdate, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
