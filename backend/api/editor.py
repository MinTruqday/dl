...
async def analyze_internal_plagiarism(
    ...
    return APIResponse(data=await EditorService.analyze_internal_plagiarism(document_id, payload.model_dump(), current_user), message="Phân tích đạo văn nội bộ thành công", status=200)
...
async def sync_keystroke_buffer(
    ...
    return APIResponse(data=await EditorService.sync_keystroke_buffer(document_id, payload.model_dump(), current_user), message="Đồng bộ bộ đệm gõ phím thành công", status=200)
...
async def add_inline_suggestion(
    ...
    return APIResponse(data=await EditorService.add_inline_suggestion(document_id, payload.model_dump(), current_user), message="Thêm gợi ý nội dòng thành công", status=201)
...
async def resolve_suggestion(
    ...
    return APIResponse(data=await EditorService.resolve_suggestion(suggestion_id, payload.model_dump(), current_user), message="Xử lý gợi ý thành công", status=200)

@router.post("/pomodoro", response_model=APIResponse[Any])
async def sync_pomodoro_session(
    payload: PomodoroSyncRequest, 
    current_user: UserInDB = Depends(require_role([RoleEnum.AUTHOR, RoleEnum.ADMIN]))
):
    return APIResponse(data=await EditorService.sync_pomodoro_session(payload.model_dump(), current_user), message="Đồng bộ phiên Pomodoro thành công", status=200)

@router.post("/{document_id}/tu-dong-luu", response_model=APIResponse[Any])
async def auto_save_draft(document_id: str, payload: AutoSaveRequest, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await EditorService.auto_save_draft(document_id, payload.content, current_user), message="Tự động lưu bản nháp thành công", status=200)
...
async def global_find_replace(
    ...
    return APIResponse(data=await EditorService.global_find_replace(document_id, payload.search, payload.replace, payload.match_case, current_user), message="Thay thế toàn cục thành công", status=200)
...
async def generate_cover(document_id: str, payload: CoverGenerateRequest, current_user: UserInDB = Depends(require_role([RoleEnum.AUTHOR]))):
    return APIResponse(
        data=await EditorService.generate_cover(document_id, payload.style, current_user),
...
async def add_inline_comment(document_id: str, payload: InlineCommentRequest, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await EditorService.add_inline_comment(document_id, payload.model_dump(), current_user), message="Thêm nhận xét thành công")

@router.get("/{document_id}/binh-luan", response_model=APIResponse[Any])
async def get_inline_comments(document_id: str, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await EditorService.get_inline_comments(document_id, current_user), message="Lấy danh sách nhận xét thành công")

@router.put("/binh-luan/{comment_id}/giai-quyet", response_model=APIResponse[Any])
async def resolve_comment(comment_id: str, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await EditorService.resolve_comment(comment_id, current_user), message="Đã xử lý nhận xét")

@router.post("/{document_id}/so-sanh-phien-ban", response_model=APIResponse[Any])
async def get_version_diff(document_id: str, payload: VersionDiffRequest, current_user: UserInDB = Depends(get_current_user)):
    return APIResponse(data=await EditorService.get_version_diff(document_id, payload.version_id_a, payload.version_id_b, current_user), message="Lấy dữ liệu so sánh phiên bản thành công")
