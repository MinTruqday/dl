...
async def get_user_tier(user_id: str, db=Depends(get_db)):
    ...
    return APIResponse(data=tier_info, message="Truy xuất thông tin gói cước tài khoản thành công")
...
async def update_user_tier(user_id: str, req: UpdateTierRequest, db=Depends(get_db)):
    ...
    return APIResponse(data=updated_tier, message="Cập nhật cấu hình gói cước tài khoản thành công")
