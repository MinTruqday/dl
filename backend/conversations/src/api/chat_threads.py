...
from src.schemas.chat_threads import ConversationResponse, MessageCreate, MessageResponse
from src.services.chat_threads import ChatConversation

from shared.infrastructure.database import db_client
from shared.dependencies import AuthenticatedUser, Depends, Header, HTTPException
from shared.dependencies import get_current_user_from_header as get_current_user
...
async def send_message(req: MessageCreate, current_user=Depends(get_current_user)):
    msg = await ChatConversation.send_message(
        req.receiver_id,
        req.content,
        current_user,
        req.image_url,
...
async def get_messages(
    ...
    return APIResponse(
        data=await ChatConversation.get_messages(
...
async def get_conversations(current_user=Depends(get_current_user)):
    return APIResponse(
        data=await ChatConversation.get_conversations(current_user),
...
async def toggle_pin(message_id: str, current_user=Depends(get_current_user)):
    result = await ChatConversation.toggle_pin(message_id, current_user)
    if result is None:
        return APIResponse(
...
async def edit_message(
    ...
    result = await ChatConversation.edit_message(message_id, content, current_user)
    if not result:
...
async def recall_message(message_id: str, current_user=Depends(get_current_user)):
    result = await ChatConversation.recall_message(message_id, current_user)
    if not result:
...
async def search_messages(
    ...
    return APIResponse(
        data=await ChatConversation.search_messages(other_user_id, q, current_user),
...
async def add_reaction(
    ...
    result = await ChatConversation.add_reaction(message_id, reaction, current_user)
    if not result:
...
async def mark_as_read(other_user_id: str, current_user=Depends(get_current_user)):
    result = await ChatConversation.mark_as_read(other_user_id, current_user)
    await publish_personal_message(
...
async def share_document(
    ...
    result = await ChatConversation.share_document(receiver_id, document_id, current_user)
    if not result:
        return APIResponse(
...
async def get_shared_attachments(
    ...
    return APIResponse(
        data=await ChatConversation.get_shared_attachments(other_user_id, current_user),
...
async def block_user(other_user_id: str, current_user=Depends(get_current_user)):
    return APIResponse(
        data=await ChatConversation.block_user(other_user_id, current_user),
...
async def unblock_user(other_user_id: str, current_user=Depends(get_current_user)):
    return APIResponse(
        data=await ChatConversation.unblock_user(other_user_id, current_user),
...
async def get_blocked_status(
    ...
    blocked = await ChatConversation.check_blocked_status(
...
async def toggle_pin_conversation(
    ...
    return APIResponse(
        data=await ChatConversation.toggle_pin_conversation(other_user_id, current_user),
...
async def translate_message(
    ...
    result = await ChatConversation.translate_message(
...
async def create_group(req: dict, current_user=Depends(get_current_user)):
    ...
    result = await ChatConversation.create_group(group_name, member_ids, current_user)
    return APIResponse(
...
async def save_draft(
    ...
    content = req.get("content", "")
    result = await ChatConversation.save_draft(other_user_id, content, current_user)
...
async def get_draft(other_user_id: str, current_user=Depends(get_current_user)):
    result = await ChatConversation.get_draft(other_user_id, current_user)
...
async def toggle_self_destruct(
    ...
    result = await ChatConversation.toggle_self_destruct(
...
async def toggle_mute(other_user_id: str, current_user=Depends(get_current_user)):
    result = await ChatConversation.toggle_mute(other_user_id, current_user)
    return APIResponse(
...
async def get_conversation_settings(
    ...
    result = await ChatConversation.get_conversation_settings(other_user_id, current_user)
    is_online = False
...
async def delete_conversation(
    ...
    result = await ChatConversation.delete_conversation(other_user_id, current_user)
