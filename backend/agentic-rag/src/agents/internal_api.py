...
class InternalAPIAgent:
    ...
    async def execute(self, action: str, params: dict, user_id: str) -> str:
        logger.info(f"InternalAPI: Thực thi tác vụ '{action}' cho user {user_id}")
        ...
        try:
            ...
            logger.error(f"InternalAPI: Lỗi khi xử lý tác vụ: {e}")
            return "Hệ thống hiện đang bảo trì dữ liệu, vui lòng thử lại sau."
