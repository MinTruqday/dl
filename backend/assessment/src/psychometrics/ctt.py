...
def point_biserial(item_scores: list[float], total_scores: list[float]):
    ...
    incorrect_totals = [
        total_scores[index] for index, value in enumerate(item_scores) if value <= 0
    ]
    ...
    if variance <= 0:
        return 0.0
    return ((mean(correct_totals) - mean(incorrect_totals)) / math.sqrt(variance)) * math.sqrt(
        p * q
    )
...
def item_total_correlation(item_scores: list[float], total_scores: list[float]):
    ...
    numerator = sum(
        (item - item_mean) * (total - total_mean) for item, total in zip(item_scores, total_scores)
    )
...
def ctt_snapshot(
    ...
    scores = [
        (
            max(
                0.0, min(1.0, float(response.get("score", 0)) / float(response.get("max_score", 1)))
            )
    ...
    option_counts = Counter(
        str(response.get("answer", {}).get("option_id", "omitted")) for response in responses
    )
    ...
    anomalous_times = sum(
        ...
        if median_time and (value < median_time * 0.1 or value > median_time * 10)
    )
    ...
    functioning_distractors = [
        option_id for option_id in distractors if option_counts.get(option_id, 0) > 0
    ]
    ...
    return {
        ...
        "item_fit_status": "insufficient_context"
        ...
        "standard_error": math.sqrt(p_value * (1 - p_value) / len(responses))
        if responses and p_value is not None
        else None,
        "option_distribution": dict(option_counts),
        "distractor_efficiency": len(functioning_distractors) / len(distractors)
        if distractors
        else None,
        "omission_rate": omitted / len(responses) if responses else None,
        "median_response_time_ms": median_time,
        "response_time_anomaly_rate": anomalous_times / len(response_times)
        if response_times
        else None,
    }
