...
def generated_question(
    ...
    if question_type in {"single_choice", "multiple_choice"}:
        ...
        answer_key = (
            {"option_id": "A"} if question_type == "single_choice" else {"option_ids": ["A"]}
        )
    ...
    primary_concept = (
        str(model_output.get("primary_concept"))
        if model_output
        else payload.get("concept_ids", [topic])[0]
        if payload.get("concept_ids")
        else topic
    )
    primary_skill = (
        str(model_output.get("primary_skill"))
        if model_output
        else payload.get("skill_ids", ["reasoning"])[0]
        if payload.get("skill_ids")
        else "reasoning"
    )
    ...
    construct = {
        ...
        "learning_objective": str(model_output.get("learning_objective"))
        if model_output
        else topic,
        "reasoning_steps": max(1, round(difficulty)),
        "pedagogical_signals": (payload.get("pedagogical_context") or {}).get("signals", []),
    }
    return {
        ...
        "solution_doc": tiptap_doc(
            str(model_output.get("solution"))
            if model_output
            else f"Đáp án được ràng buộc bởi bằng chứng chương trình học về {topic}"
        ),
        ...
        "generation_provenance": {
            ...
            if payload.get("use_teacher_materials")
            or payload.get("source_scope") == "curriculum_and_owned_material"
            else "curriculum_only",
            "pedagogical_context": payload.get("pedagogical_context")
            or {"source_mode": "curriculum_only"},
            "variation_directive": payload.get("variation_directive"),
        },
