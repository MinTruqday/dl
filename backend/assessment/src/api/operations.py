...
async def teacher_dashboard(user: CurrentUser = Depends(require_author)):
    drafts = await database.value.assessment_drafts.find({"owner_id": user.id}).sort("updated_at", -1).limit(10).to_list(10)
    assessments = await database.value.assessments.find({"owner_id": user.id, "status": {"$in": ["published", "scheduled"]}}).sort("updated_at", -1).limit(10).to_list(10)
    review_count = await database.value.question_drafts.count_documents(
        {"owner_id": user.id, "$or": [{"needs_teacher_review": True}, {"status": {"$in": ["needs_revision", "prevalidated"]}}]}
    )
    ...
    owned_version_ids = await database.value.question_versions.distinct("_id", {"owner_id": user.id})
    ...
    response_counts = await database.value.responses.aggregate(
        [
            {"$match": {"question_version_id": {"$in": [version_id for version_id in owned_version_ids if version_id not in set(calibrated_ids)]}, "evidence_eligibility": "eligible"}},
    ...
    prediction_rows = await database.value.difficulty_estimates.find(
        {
    ...
    for version_id, calibration in latest_calibration.items():
        ...
        if not prediction or prediction.get("predicted_difficulty") is None or calibration.get("difficulty") is None:
            continue
    ...
    return {
        ...
        "predicted_empirical_alerts": sorted(discrepancy_alerts, key=lambda row: row["gap"], reverse=True)[:20],
    }
...
async def list_audit_events(
    ...
    events = await database.value.audit_events.find(query).sort("created_at", -1).limit(5000).to_list(5000)
...
async def model_monitoring(user: CurrentUser = Depends(require_admin)):
    prediction_versions = await database.value.difficulty_estimates.aggregate(
        [{"$group": {"_id": "$model_version", "count": {"$sum": 1}, "average_confidence": {"$avg": "$confidence"}}}]
    ).to_list(1000)
    calibration_jobs = await database.value.calibration_runs.find().sort("created_at", -1).limit(100).to_list(100)
    drift_alerts = await database.value.calibrations.find({"drift_flag": True}).sort("created_at", -1).limit(100).to_list(100)
    low_confidence = await database.value.difficulty_estimates.count_documents({"confidence": {"$lt": 0.5}})
    calibration_rows = await database.value.calibrations.find({"status": "calibrated", "difficulty": {"$ne": None}}).sort("created_at", -1).to_list(100000)
    latest_calibration = {}
    for calibration in calibration_rows:
        latest_calibration.setdefault(calibration["question_version_id"], calibration)
    current_version_ids = await database.value.questions.distinct("current_version_id", {"status": {"$ne": "archived"}})
    monitored_version_ids = list(set(latest_calibration) | set(current_version_ids))
    estimate_rows = await database.value.difficulty_estimates.find(
        {"question_version_id": {"$in": monitored_version_ids}, "predicted_difficulty": {"$ne": None}}
    ...
    version_rows = await database.value.question_versions.find({"_id": {"$in": current_version_ids}}).to_list(100000)
    ...
    for version in version_rows:
        ...
        difficulty = calibration.get("difficulty") if calibration else estimate.get("predicted_difficulty") if estimate else None
        level = str(max(1, min(5, round(float(difficulty))))) if difficulty is not None else "unknown"
...
async def purge_expired_pii(payload: PrivacyPurgeInput, user: CurrentUser = Depends(require_admin)):
    ...
    for attempt in attempts:
        ...
        await database.value.attempts.update_one(
            ...
            {"$set": {"student_id": f"anon:{pseudonym}", "participant_id": pseudonym, "pii_purged_at": now()}},
        )
    ...
    assignments = await database.value.assignments.find(
        {"_id": {"$in": list(assignment_ids)}, "student_id": {"$type": "string"}, "pii_purged_at": {"$exists": False}},
        {"_id": 1, "student_id": 1},
    ).to_list(100000)
...
async def research_evaluation(user: CurrentUser = Depends(require_author)):
    ...
    calibration_snapshots = await database.value.calibrations.find(
        {"owner_id": user.id, "question_version_id": {"$in": version_ids}}
    ).sort("created_at", 1).to_list(100000)
    calibrations = [calibration for calibration in calibration_snapshots if calibration.get("status") == "calibrated"]
    latest_calibration = {}
        ...
        rows.append(
            {
                ...
                "nearest_historical": prediction.get("nearest_historical_difficulty") if prediction else None,
                "teacher": teacher_value,
                "teacher_confidence": {"low": 0.33, "medium": 0.66, "high": 1.0}.get(teacher.get("self_confidence")) if teacher else None,
                "hybrid": (ai_value + teacher_value) / 2 if ai_value is not None and teacher_value is not None else None,
                "empirical": calibration.get("difficulty"),
                "sample_size": calibration.get("sample_size", 0),
                "model_version": prediction.get("model_version") if prediction else None,
                "llm_direct_model_version": direct_prediction.get("provider_model_version") or direct_prediction.get("model_version") if direct_prediction else None,
                "feature_schema_version": prediction.get("feature_schema_version") if prediction else None,
                "training_data_window": prediction.get("training_data_window") if prediction else None,
                "curriculum_version": prediction.get("curriculum_version") if prediction else None,
                "normalization_version": prediction.get("normalization_version") if prediction else None,
                "feature_snapshot": prediction.get("feature_snapshot") if prediction else {},
                "prediction_created_at": utc_value(prediction.get("created_at")) if prediction else None,
    ...
    return {
        ...
        "model_versions": [
            {
                "model": key,
                "metrics": evaluation_metrics(values, "llm_direct" if key.startswith("llm_direct:") else "structured"),
            }
