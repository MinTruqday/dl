...
async def create_indexes():
    ...
    await db.assessment_versions.create_index([("assessment_id", 1), ("idempotency_key", 1)], unique=True)
    await db.assignments.create_index([("assessment_version_id", 1), ("student_id", 1)], unique=True)
    ...
    await db.attempts.create_index([("assignment_id", 1), ("student_id", 1), ("idempotency_key", 1)], unique=True)
    await db.attempts.create_index([("student_id", 1), ("assessment_version_id", 1), ("updated_at", -1)])
    ...
    await db.calibrations.create_index([("question_version_id", 1), ("method", 1), ("created_at", -1)])
    ...
    await db.assessment_rebalance_proposals.create_index([("owner_id", 1), ("idempotency_key", 1)], unique=True)
    await db.assessment_rebalance_proposals.create_index([("assessment_draft_id", 1), ("created_at", -1)])
    ...
    await db.teacher_judgments.create_index([("question_version_id", 1), ("teacher_id", 1), ("created_at", -1)])
    ...
    await db.teacher_profile_events.create_index([("teacher_id", 1), ("idempotency_key", 1)], unique=True)
    await db.curriculum_nodes.create_index([("canonical_code", 1), ("curriculum_version", 1)], unique=True)
