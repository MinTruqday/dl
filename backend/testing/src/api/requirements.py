...
def normalized_suggestion_values(values):
    return [" ".join(str(item or "").strip().casefold().split()) for item in values if str(item or "").strip()]
...
def requirement_suggestion_changes(version, suggestion, acceptance_criteria=None):
    ...
    if (
        ...
        return True
    for field in ("business_rules", "actors", "dependencies", "tags"):
        suggested = [str(item).strip() for item in (suggestion.get(field) or []) if str(item).strip()]
        if suggested and suggested != list(version.get(field, [])):
            return True
    suggested_criteria = normalized_suggestion_values(suggestion.get("acceptance_criteria") or [])
    if suggested_criteria:
        current_criteria = normalized_suggestion_values(
            [
        ...
        return suggested_criteria != current_criteria
    return False


def structured_requirement_quality_candidate(version, acceptance_criteria, evidence_refs, rule_ids):
    """Create a minimal, evidence-only repair for form-structure findings.

    Models are useful for semantic review, but a field-placement issue is not a
    creative task.  Rewriting every field through a model made a simple
    "condition belongs in acceptance criteria" warning expand into unrelated
    requirement text.  This repair moves only text already present in the
    Requirement into the appropriate form fields.
    """
    allowed = {"CONDITION_IN_CONTENT", "MISSING_CONDITION"}
    if not rule_ids or not set(rule_ids).issubset(allowed):
        return None
    content = str(
        version.get("plain_text_projection") or plain_text(version.get("content_doc", {})) or ""
    ).strip()
    condition_match = re.match(
        r"^\s*((?:khi|nếu|trong trường hợp)\b[^,.]*),\s*(.+)$",
        content,
    ...
    # The first sentence states the capability.  Subsequent sentences are
    # commonly business rules and are already stored in their dedicated field.
    general_scope = re.split(r"(?<=[.!?])\s+", remainder, maxsplit=1)[0].strip()
    general_scope = re.sub(r"^hệ thống\s+phải\s+", "Hệ thống ", general_scope, flags=re.I)
    if general_scope:
        general_scope = general_scope[0].upper() + general_scope[1:]
    current_criteria = [
        str(item.get("plain_text") or plain_text(item.get("content_doc", {})) or "").strip()
        for item in acceptance_criteria
        if isinstance(item, dict)
        and str(item.get("plain_text") or plain_text(item.get("content_doc", {})) or "").strip()
    ]
    ...
    for criterion in current_criteria:
        ...
        if re.match(r"^\s*(khi|nếu|trong trường hợp)\b", lowered):
            rewritten_criteria.append(criterion)
            continue
        statement = criterion.strip().rstrip(". ")
        if lowered.startswith("giao dịch thành công phải chuyển"):
            statement = "hệ thống phải chuyển" + statement[len("giao dịch thành công phải chuyển") :]
            rewritten_criteria.append(f"Khi giao dịch thành công, {statement}.")
        elif lowered.startswith("giao dịch thất bại không được"):
            statement = "hệ thống không được" + statement[len("giao dịch thất bại không được") :]
            rewritten_criteria.append(f"Khi giao dịch thất bại, {statement}.")
        elif lowered.startswith("khách hàng chỉ thanh toán được"):
            statement = "hệ thống chỉ cho phép khách hàng thanh toán được" + statement[
                len("khách hàng chỉ thanh toán được") :
            ]
            rewritten_criteria.append(f"{condition}, {statement}.")
        elif lowered.startswith("hệ thống "):
            rewritten_criteria.append(f"{condition}, {statement[0].lower() + statement[1:]}.")
        else:
            rewritten_criteria.append(f"{condition}, hệ thống phải {statement[0].lower() + statement[1:]}.")

    candidate = {
        "revised_title": None,
        "revised_content": general_scope,
        "acceptance_criteria": rewritten_criteria,
        "business_rules": [],
        "actors": [],
        "dependencies": [],
        "tags": [],
        "rationale": "Tách điều kiện kiểm thử khỏi Nội dung và bổ sung điều kiện cho từng Tiêu chí chấp nhận; không thêm nghiệp vụ mới.",
        "evidence_refs": evidence_refs,
        "reason_codes": sorted(set(rule_ids)),
    }
    return candidate if requirement_suggestion_changes(version, candidate, acceptance_criteria) else None


def missing_required_field_candidate(version, acceptance_criteria, evidence_refs, rule_ids):
    """Offer a narrowly scoped draft only when existing fields support it.

    Required fields may be blank while the rest of the Requirement contains
    enough approved user input to form a conservative draft.  The candidate
    changes only the missing field and is always left for the user to apply.
    """
    criteria = [
        str(item.get("plain_text") or plain_text(item.get("content_doc", {})) or "").strip()
        for item in acceptance_criteria
    ...
    evidence_text = " ".join(
        [str(version.get("plain_text_projection") or plain_text(version.get("content_doc", {})) or "").strip()]
        + criteria
        + [str(item).strip() for item in version.get("business_rules", []) if str(item).strip()]
    ).lower()
    if "MISSING_ACTOR" in rule_ids:
        actor_terms = (
        ...
        actors = [label for term, label in actor_terms if term in evidence_text]
        if actors:
            candidate = {
                "revised_title": None,
                "revised_content": "",
                ...
                "dependencies": [],
                "tags": [],
                "rationale": "Đề xuất tác nhân chỉ từ các vai trò đã xuất hiện trong Tiêu chí chấp nhận hoặc Quy tắc nghiệp vụ.",
                "evidence_refs": evidence_refs,
                "reason_codes": ["MISSING_ACTOR"],
            }
            return candidate if requirement_suggestion_changes(version, candidate, acceptance_criteria) else None
    if "MISSING_REQUIREMENT_CONTENT" not in rule_ids:
        return None
    # These capability summaries are intentionally broad and are chosen only
    # from terms already provided by the user in other form fields.
    if "thanh toán" in evidence_text:
        revised_content = "Hệ thống hỗ trợ thanh toán đơn hàng."
    elif "giao dịch" in evidence_text:
        revised_content = "Hệ thống hỗ trợ xử lý giao dịch."
    elif criteria:
        first = re.sub(r"^\s*(?:khi|nếu|trong trường hợp)\b[^,.]*,\s*", "", criteria[0], flags=re.I)
        first = re.sub(r"^\s*hệ thống\s+phải\s+", "Hệ thống ", first, flags=re.I).strip()
        revised_content = first[:1].upper() + first[1:] if first else ""
    else:
        return None
    candidate = {
        "revised_title": None,
        ...
        "dependencies": [],
        "tags": [],
        "rationale": "Đề xuất Nội dung yêu cầu từ Tiêu chí chấp nhận và Quy tắc nghiệp vụ hiện có; không thêm nghiệp vụ mới.",
        "evidence_refs": evidence_refs,
        "reason_codes": ["MISSING_REQUIREMENT_CONTENT"],
    }
    return candidate if requirement_suggestion_changes(version, candidate, acceptance_criteria) else None
...
async def create_requirement_record(project_id, payload, user, origin="manual"):
    ...
    try:
        ...
        await database.value.requirement_versions.delete_one(
            {"_id": version["_id"], "project_id": project_id}
        )
        await database.value.requirements.delete_one(
    ...
    requirements = await database.value.requirements.find(
        {
            ...
            "_id": {"$in": transformation.get("result_requirement_ids", [])},
        }
    ).to_list(100)
    versions = await database.value.requirement_versions.find(
        {
            "project_id": transformation["project_id"],
            "_id": {"$in": transformation.get("result_version_ids", [])},
        }
    ).to_list(100)
    versions_by_id = {item["_id"]: item for item in versions}
    return {
...
async def execute_requirement_transformation(
    ...
    try:
        ...
        await database.value.acceptance_criteria.delete_many(
            {"project_id": project_id, "_id": {"$in": [item["_id"] for item in output_criteria]}}
        )
        await database.value.requirement_versions.delete_many(
            {"project_id": project_id, "transformation_id": transformation["_id"]}
        )
        ...
        await database.value.requirement_transformations.update_one(
            {"_id": transformation["_id"], "project_id": project_id},
            {
                "$set": {
                    "status": "FAILED",
                    "error_code": getattr(error, "detail", {"code": type(error).__name__}),
                    "updated_at": now(),
                }
            },
        )
...
async def requirement_detail(requirement_id: str, user: CurrentUser = Depends(get_current_user)):
    ...
    version = await database.value.requirement_versions.find_one(
        {"_id": requirement["current_version_id"]}
    )
    criteria = await database.value.acceptance_criteria.find(
        {"requirement_version_id": version["_id"]}
    ).to_list(500)
    return envelope(
        {**requirement, "current_version": {**version, "acceptance_criteria": criteria}}
    )
...
async def update_requirement_draft(
    ...
    if "requirement_key" in changes and changes["requirement_key"] != requirement.get(
        ...
        duplicate = await database.value.requirements.find_one(
            {
                "project_id": project_id,
                "requirement_key": changes["requirement_key"],
    ...
    if criteria is not None:
        previous_criteria = await database.value.acceptance_criteria.find(
            {"requirement_version_id": version["_id"]}
        ).to_list(500)
        await database.value.acceptance_criteria.delete_many(
            {"requirement_version_id": version["_id"]}
        ...
        try:
            ...
            raise
        updated_version = await database.value.requirement_versions.find_one(
            {"_id": version["_id"]}
        )
    ...
    if parent_changes:
        await database.value.requirements.update_one(
            {"_id": requirement_id, "project_id": project_id},
            {"$set": {**parent_changes, "updated_at": now()}},
        )
    await audit(user.id, "requirement_draft_updated", "Requirement", requirement_id, project_id)
    return envelope(
        {**requirement, **parent_changes, "current_version": updated_version},
        revision=updated_version["revision"],
    )
...
async def add_requirement_dependency(
    ...
    requirement = await get_project_entity(
        "requirements", requirement_id, user, "requirement_dependency.manage"
    )
    ...
    dependency = await database.value.requirements.find_one(
        {"_id": dependency_id, "project_id": requirement["project_id"]}
    )
    if not dependency:
        raise HTTPException(status_code=422, detail={"code": "INVALID_REQUIREMENT_DEPENDENCY"})
    version = await database.value.requirement_versions.find_one(
        {"_id": requirement["current_version_id"], "project_id": requirement["project_id"]}
    )
    if not version or version.get("status") != "DRAFT":
        raise HTTPException(status_code=409, detail={"code": "IMMUTABLE_REQUIREMENT_VERSION"})
    if version.get("revision") != payload.expected_revision:
    ...
    if await requirement_dependency_reaches(
        dependency_id, requirement_id, requirement["project_id"]
    ):
        raise HTTPException(status_code=422, detail={"code": "REQUIREMENT_DEPENDENCY_CYCLE"})
...
async def remove_requirement_dependency(
    ...
    updated = await database.value.requirement_versions.find_one_and_update(
        ...
        },
        {
            "$set": {
                "dependencies": [
                    item for item in dependencies if item != dependency_requirement_id
                ],
                "updated_at": now(),
            },
            "$inc": {"revision": 1},
    ...
    await audit(
        ...
        requirement_id,
        requirement["project_id"],
        {"dependency_requirement_id": dependency_requirement_id},
    )
    return envelope(updated, revision=updated["revision"])
...
async def requirement_dependency_reaches(start_id: str, target_id: str, project_id: str):
    ...
    while pending:
        ...
        requirement = await database.value.requirements.find_one(
            {"_id": current_id, "project_id": project_id}, {"current_version_id": 1}
        )
        if not requirement:
            continue
        version = await database.value.requirement_versions.find_one(
            {"_id": requirement.get("current_version_id"), "project_id": project_id},
            {"dependencies": 1},
        )
...
async def create_requirement_version(
    ...
    parent = await database.value.requirement_versions.find_one(
        {"_id": requirement["current_version_id"]}
    )
...
async def list_requirement_versions(
    ...
    versions = (
        await database.value.requirement_versions.find({"requirement_id": requirement_id})
        .sort("version", -1)
        .to_list(500)
    )
...
async def submit_requirement_review(
    ...
    if lint_blocking and any(item["severity"] == "error" for item in findings):
        raise HTTPException(
            status_code=409, detail={"code": "REQUIREMENT_LINT_BLOCKED", "findings": findings}
        )
    timestamp = now()
...
@router.post("/du-an/{project_id}/yeu-cau/ai/tao-ban-nhap")
async def generate_requirement_ai_draft(
    project_id: str,
    payload: RequirementAIDraftInput,
    user: CurrentUser = Depends(get_current_user),
):
    await get_project(project_id, user, "requirement.create")
    source = {
    ...
    result = await request_design_assistance(
        "requirement_draft_generation",
        project_id,
        payload.instruction
        or "Điền các ô còn thiếu trong form Requirement từ thông tin người dùng đã nhập.",
        [
            {
                "artifact_type": "requirement_form_input",
                "artifact_id": "requirement-draft-form",
                "text": json.dumps(source, ensure_ascii=False),
            }
        ],
    )
    candidate = (result.get("suggestions") or [None])[0]
    if result.get("status") != "SUCCESS" or not isinstance(candidate, dict):
        raise HTTPException(
            status_code=503,
            detail={"code": "AI_DRAFT_GENERATION_UNAVAILABLE", "warnings": result.get("warnings", [])},
        )
    return envelope(
        {
            "draft": candidate,
            "ai": ai_contract_metadata(result),
...
async def lint_requirement(
    ...
    existing = await database.value.ai_findings.find_one(
        {
            "project_id": version["project_id"],
    ...
    evidence = [
        {
            ...
            "authority": "PROJECT_BASELINE" if version.get("status") == "BASELINED" else "DRAFT",
            "text": json.dumps(
                {
    ...
    rule_ids = [item["rule_id"] for item in deterministic_findings]
    structural_candidate = structured_requirement_quality_candidate(
        version,
        acceptance_criteria,
        [version_id],
        rule_ids,
    )
    missing_field_candidate = missing_required_field_candidate(
        version,
        acceptance_criteria,
        [version_id],
        rule_ids,
    )
    # Field-placement findings have an exact, evidence-only repair.  Do not
    # send that repair to a generative model: it may rewrite valid fields or
    # introduce assumptions.  Gemma remains available for genuine semantic
    # findings that cannot be repaired mechanically.
    terminal_rule_ids = {"MISSING_REQUIREMENT_CONTENT"}
    if missing_field_candidate:
        ai_result = {
            "capability": "requirement_quality_analysis",
            "findings": [],
            "suggestions": [missing_field_candidate],
            ...
            "model": {"provider": "evidence-guided", "model": "requirement-field-draft-v1"},
            "reason_codes": rule_ids,
        }
    elif set(rule_ids) & terminal_rule_ids:
        # Missing required form data is a blocking validation error, not an
        # invitation for a model to invent Requirement content.
        ai_result = {
            "capability": "requirement_quality_analysis",
            "findings": [],
            "suggestions": [],
        ...
        ai_result = {
            "capability": "requirement_quality_analysis",
            "findings": [],
            "suggestions": [structural_candidate],
            "evidence_refs": [version_id],
            "confidence": 1,
            "warnings": [],
            "status": "SUCCESS",
            "degraded_mode": None,
            "provider": "evidence-guided",
            "model": {"provider": "evidence-guided", "model": "requirement-structure-v1"},
            "reason_codes": rule_ids,
        }
    # When deterministic quality rules find nothing, do not ask the model to
    # invent another rewrite. This makes a clean, applied draft stay clean on
    ...
    ai_findings = [
        {
            **item,
            "rule_id": item.get("reason_codes", [item.get("category", "AI_FINDING")])[0],
            "origin": "AI",
            "span": None,
        }
    ...
    suggestions = [
        ...
        for index, item in enumerate(
            [
                ...
                if requirement_suggestion_changes(version, candidate, acceptance_criteria)
            ],
            1,
        )
    ]
    ...
    try:
        await database.value.ai_findings.insert_one(result)
    except DuplicateKeyError:
        existing = await database.value.ai_findings.find_one(
            {
                "project_id": version["project_id"],
...
async def apply_requirement_ai_suggestion(
    ...
    ai_result = await database.value.ai_findings.find_one(
        {
    ...
    current_criteria = await database.value.acceptance_criteria.find(
        {"requirement_version_id": version_id}
    ).to_list(200)
    if not requirement_suggestion_changes(version, suggestion, current_criteria):
        raise HTTPException(status_code=422, detail={"code": "AI_SUGGESTION_NO_CHANGE"})
    patch_values = {"expected_revision": payload.expected_revision}
    revised_title = str(suggestion.get("revised_title") or "").strip()
    revised_content = str(suggestion.get("revised_content") or "").strip()
    if revised_title and revised_title != str(version.get("title") or "").strip():
        patch_values["title"] = revised_title
    current_content = str(
        version.get("plain_text_projection") or plain_text(version.get("content_doc", {})) or ""
    ).strip()
    if revised_content and revised_content != current_content:
        patch_values["content_doc"] = text_doc(revised_content)
    for field in ("business_rules", "actors", "dependencies", "tags"):
        suggested = [str(item).strip() for item in (suggestion.get(field) or []) if str(item).strip()]
        if suggested and suggested != list(version.get(field, [])):
            patch_values[field] = suggested
    suggested_criteria = [
        str(item).strip()
        for item in (suggestion.get("acceptance_criteria") or [])
        if str(item).strip()
    ]
    if suggested_criteria:
        current_criteria_text = [str(item.get("plain_text") or "").strip() for item in current_criteria]
        if suggested_criteria != current_criteria_text:
            patch_values["acceptance_criteria"] = [
                {
                    "key": f"AC-AI-{index + 1}",
                    "content_doc": text_doc(item),
                    "status": "draft",
                }
                for index, item in enumerate(suggested_criteria)
            ]
    if len(patch_values) == 1:
        raise HTTPException(status_code=422, detail={"code": "AI_SUGGESTION_NO_CHANGE"})
    updated_result = await update_requirement_draft(
        version["project_id"],
        version["requirement_id"],
        RequirementDraftPatch(**patch_values),
        user,
    )
    ...
    await database.value.ai_findings.update_one(
        ...
        {
            "$addToSet": {"applied_suggestion_ids": payload.suggestion_id},
            "$set": {"updated_at": now()},
        },
    )
    await audit(
    ...
    return envelope(updated, revision=updated["current_version"]["revision"])
...
async def upload_requirement_document(
    ...
    source = await store_raw_requirement_source(
        project_id, document_id, filename, file.content_type, data
    )
    timestamp = now()
    document = {
        "_id": document_id,
    ...
    await audit(
        ...
        document_id,
        project_id,
        {"content_hash": source["sha256"], "object_key": source["object_key"], "format": format},
    )
    try:
        ...
        document = await database.value.requirement_documents.find_one(
            {"_id": document_id, "project_id": project_id}
        )
        await audit(
            user.id,
...
async def list_knowledge_sources(
    ...
    documents = (
        await database.value.requirement_documents.find(query).sort("updated_at", -1).to_list(1000)
    )
    order = (
        (
            await database.value.projects.find_one(
...
async def archive_requirement_document(
    ...
    document = await get_project_entity(
        "requirement_documents", document_id, user, "requirement_document.archive"
    )
    if document.get("status") == "ARCHIVED":
        return envelope(document, revision=document["revision"])
    updated = await database.value.requirement_documents.find_one_and_update(
...
async def restore_requirement_document(
    ...
    document = await get_project_entity(
        "requirement_documents", document_id, user, "requirement_document.restore"
    )
    if document.get("status") != "ARCHIVED":
        return envelope(document, revision=document["revision"])
    updated = await database.value.requirement_documents.find_one_and_update(
...
async def retry_requirement_document_parse(
    ...
    claimed = await database.value.requirement_documents.update_one(
        ...
        {"$set": {"status": "PARSING", "updated_at": now()}, "$inc": {"revision": 1}},
    )
    ...
    try:
        ...
        return envelope(
            failed, revision=failed["revision"], status="DEGRADED", degraded_mode="PARSER_FAILED"
        )
    normalized = serialized_content(content)
    updated = await database.value.requirement_documents.find_one_and_update(
        {"_id": document_id, "project_id": document["project_id"], "status": "PARSING"},
...
async def extract_requirement_document(
    ...
    if document.get("normalized_content") is None:
        raise HTTPException(
            status_code=409,
            detail={"code": "DOCUMENT_PARSE_REQUIRED", "status": document.get("status")},
        )
    ...
    await database.value.requirement_documents.update_one(
        {"_id": document_id, "project_id": document["project_id"]},
        {
            "$set": {"status": "EXTRACTED", "last_import_job_id": job["_id"], "updated_at": now()},
            "$inc": {"revision": 1},
        },
    )
    await audit(
    ...
    merged.update(
        {
            ...
            "extraction_confidence": min(
                (float(item.get("extraction_confidence", 1)) for item in parents), default=1
            ),
        }
    )
    first_index = min(
...
async def confirm_requirement_import(
    ...
    claim_filter = {"_id": job_id, "project_id": job["project_id"], "status": "PREVIEW_READY"}
        ...
        for index in indexes:
            ...
            created.append(
                await create_requirement_record(
                    job["project_id"], RequirementCreate(**item), user, origin="import"
                )
            )
    except Exception:
...
def atomic_requirement_candidates(document):
    ...
    if document["format"] in {"csv", "xlsx", "openapi", "postman"}:
        candidates = parse_import(
            ImportCreate(filename=document["filename"], format=document["format"], content=content)
        )
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "url_template": redact_url(raw_url),
                                        "header_names": [
                                            header.get("key")
...
async def validate_requirement_sources(project_id, source_refs):
    ...
    documents = await database.value.requirement_documents.find(
        {"project_id": project_id, "_id": {"$in": document_ids}}
    ).to_list(len(document_ids))
    by_id = {item["_id"]: item for item in documents}
    if set(by_id) != set(document_ids):
        raise HTTPException(
            status_code=422, detail={"code": "CROSS_PROJECT_OR_MISSING_REQUIREMENT_DOCUMENT"}
        )
    for reference in references:
        ...
        if expected_hash and expected_hash != by_id[reference["requirement_document_id"]].get(
            ...
            raise HTTPException(
                status_code=422, detail={"code": "REQUIREMENT_SOURCE_HASH_MISMATCH"}
            )
...
async def index_requirement(version):
    ...
    await database.value.requirement_versions.update_one(
        ...
        {
            "$set": {
                "index_status": "READY" if indexed else "FAILED",
                "index_error_code": None if indexed else "KNOWLEDGE_INDEX_FAILED",
                "updated_at": now(),
            }
        },
    )
