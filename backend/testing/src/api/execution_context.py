...
@router.post(
    "/du-an/{project_id}/ban-phat-hanh",
    status_code=201,
    openapi_extra={"x-function-ids": ["REL-03"]},
)
async def create_release(
    project_id: str, payload: ReleaseCreate, user: CurrentUser = Depends(get_current_user)
):
...
async def update_release(
    release_id: str, payload: ReleasePatch, user: CurrentUser = Depends(get_current_user)
):
...
async def release_transition(release_id, payload, user, target, permission, action):
    ...
    if target == "CLOSED":
        try:
            completion_gate_applied = await ensure_release_completion_gate(
                release["project_id"], release_id
            )
    ...
    if target == "ACTIVE":
        await database.value.releases.update_many(
            {"project_id": release["project_id"], "_id": {"$ne": release_id}, "status": "ACTIVE"},
    ...
    event = "release_closed_after_completion" if completion_gate_applied else action
    await audit(
        user.id, event, "Release", release_id, release["project_id"], {"reason": payload.reason}
    )
    return envelope(updated, revision=updated["revision"])
...
async def activate_release(
    release_id: str, payload: ReleaseTransition, user: CurrentUser = Depends(get_current_user)
):
    return await release_transition(
        release_id, payload, user, "ACTIVE", "release.manage", "release_activated"
    )
...
async def close_release(
    ...
    return await release_transition(
        release_id, payload, user, "CLOSED", "release.close", "release_closed"
    )
...
async def archive_release(
    ...
    return await release_transition(
        release_id, payload, user, "ARCHIVED", "release.archive", "release_archived"
    )
...
@router.post(
    "/du-an/{project_id}/ban-dung", status_code=201, openapi_extra={"x-function-ids": ["BLD-02"]}
)
async def create_build(
    project_id: str, payload: BuildCreate, user: CurrentUser = Depends(get_current_user)
):
...
async def update_build(
    build_id: str, payload: BuildPatch, user: CurrentUser = Depends(get_current_user)
):
    build = await get_project_entity("builds", build_id, user, "build.manage")
    updated = await optimistic_patch(
        "builds", build_id, build["project_id"], payload.expected_revision, payload.model_dump()
    )
...
async def set_current_build(
    build_id: str, payload: ReleaseTransition, user: CurrentUser = Depends(get_current_user)
):
    build = await get_project_entity("builds", build_id, user, "build.manage")
    updated = await optimistic_patch(
        "builds", build_id, build["project_id"], payload.expected_revision, {"is_current": True}
    )
    await database.value.builds.update_many(
        {"project_id": build["project_id"], "_id": {"$ne": build_id}},
        {"$set": {"is_current": False}},
    )
...
@router.post(
    "/du-an/{project_id}/moi-truong", status_code=201, openapi_extra={"x-function-ids": ["ENV-02"]}
)
async def create_environment(
    project_id: str, payload: EnvironmentCreate, user: CurrentUser = Depends(get_current_user)
):
...
async def update_environment(
    environment_id: str, payload: EnvironmentPatch, user: CurrentUser = Depends(get_current_user)
):
    environment = await get_project_entity(
        "test_environments", environment_id, user, "environment.update"
    )
    updated = await optimistic_patch(
        "test_environments",
        environment_id,
    ...
    await audit(
        user.id, "environment_updated", "TestEnvironment", environment_id, environment["project_id"]
    )
...
async def update_environment_secrets(
    environment_id: str,
    payload: EnvironmentSecretRefs,
    user: CurrentUser = Depends(get_current_user),
):
    environment = await get_project_entity(
        "test_environments", environment_id, user, "environment.secret_ref.manage"
    )
    updated = await optimistic_patch(
        "test_environments",
        environment_id,
...
async def archive_environment(
    environment_id: str, payload: ReleaseTransition, user: CurrentUser = Depends(get_current_user)
):
    environment = await get_project_entity(
        "test_environments", environment_id, user, "environment.archive"
    )
    updated = await optimistic_patch(
        "test_environments",
        environment_id,
