...
async def get_api_artifact(artifact_id: str, user: CurrentUser = Depends(get_current_user)):
    artifact = await get_project_entity("api_imports", artifact_id, user, "apiartifact.read")
    operations = []
    if artifact.get("status") == "CONFIRMED":
        operations = (
            await database.value.api_operations.find(
...
@router.patch(
    "/dac-ta-giao-dien/{artifact_id}/ra-soat", openapi_extra={"x-function-ids": ["API-04"]}
)
async def review_api_artifact(
    artifact_id: str, payload: APIArtifactReview, user: CurrentUser = Depends(get_current_user)
):
...
@router.post(
    "/dac-ta-giao-dien/{artifact_id}/xac-nhan", openapi_extra={"x-function-ids": ["API-05"]}
)
async def confirm_api_artifact(
    artifact_id: str, payload: APIArtifactConfirm, user: CurrentUser = Depends(get_current_user)
):
...
@router.get(
    "/du-an/{project_id}/dac-ta-giao-dien/thao-tac", openapi_extra={"x-function-ids": ["API-01"]}
)
async def list_api_operations(project_id: str, user: CurrentUser = Depends(get_current_user)):
    await get_project(project_id, user, "apiartifact.read")
    return envelope(
        await database.value.api_operations.find({"project_id": project_id})
        .sort("path", 1)
        .to_list(5000)
    )
    ...
    for case in cases:
        response = await create_test_case_draft(
            ...
            TestCaseDraftCreate(
                ...
                type="api",
                priority="high"
                if case["category"] in {"auth", "forbidden", "conflict"}
                else "medium",
                risk="high" if case["category"] in {"auth", "schema_mismatch"} else "medium",
                preconditions_doc=text_doc(
                    f"API operation {operation['method']} {operation['path']} tồn tại trong đặc tả"
                ),
                steps=[
                    {
                        "id": "step-1",
                        "order": 1,
                        "action_doc": text_doc(case["action"]),
                        "test_data": case["test_data"],
                        "expected_doc": text_doc(case["expected"]),
                    }
                ],
                test_data=case["test_data"],
...
@router.get(
    "/du-an/{project_id}/dac-ta-giao-dien/khac-biet", openapi_extra={"x-function-ids": ["API-07"]}
)
...
async def analyze_api_artifact_impact(
    project_id: str, payload: APIArtifactImpact, user: CurrentUser = Depends(get_current_user)
):
...
@router.post(
    "/dac-ta-giao-dien/{artifact_id}/luu-tru", openapi_extra={"x-function-ids": ["API-09"]}
)
async def archive_api_artifact(
    artifact_id: str, payload: APIArtifactArchive, user: CurrentUser = Depends(get_current_user)
):
...
async def recover_trace_links(project_id: str, user: CurrentUser = Depends(get_current_user)):
    ...
    requirements = await database.value.requirement_versions.find(
        {"project_id": project_id, "status": "BASELINED"}
    ).to_list(5000)
    tests = await database.value.test_case_versions.find(
        {"project_id": project_id, "status": "ACTIVE"}
    ).to_list(10000)
    existing = await database.value.trace_links.find(
...
async def upload_test_import(
    ...
    return await preview_test_import(
        project_id,
        ImportCreate(
            filename=file.filename or f"test-cases.{format}", format=format, content=content
        ),
        user,
    )
    ...
    for index in selected:
        ...
        response = await create_test_case_draft(
            ...
            TestCaseDraftCreate(
                ...
                risk=item["risk"],
                preconditions_doc=text_doc(item["precondition"]),
                steps=[
                    {
                        "id": "step-1",
                        "order": 1,
                        "action_doc": text_doc(item["action"]),
                        "test_data": {},
                        "expected_doc": text_doc(item["expected"]),
                    }
                ],
                test_data={},
                expected_result_doc=text_doc(item["expected"]),
                postconditions_doc=text_doc("Hoàn tất"),
                tags=item["tags"],
                origin="import",
            ),
            user,
...
async def export_test_cases(
    ...
    versions = (
        await database.value.test_case_versions.find({"project_id": project_id})
            ...
            operations.append(
                {
                    ...
                    "title": node.get("name") or f"{request.get('method', 'GET')} {raw}",
                    "folder": folder,
                    "path": raw.split("?", 1)[0],
