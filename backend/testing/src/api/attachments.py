...
async def register_attachment(
    ...
    if payload.artifact_type and payload.artifact_id:
        ...
        if not collection:
            raise HTTPException(
                status_code=422, detail={"code": "ATTACHMENT_ARTIFACT_TYPE_INVALID"}
            )
...
async def list_attachments(
    ...
    return envelope(
        await database.value.attachments.find(query).sort("created_at", -1).to_list(5000)
    )
    ...
    updated = await database.value.attachments.find_one_and_update(
        {
            ...
            "status": "ACTIVE",
            "revision": attachment.get("revision", 1),
        },
        {
            "$set": {
                "status": "DELETED",
