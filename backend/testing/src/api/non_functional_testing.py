...
async def list_non_functional_test_plans(project_id: str, plan_type: str = Query(default=""), user: CurrentUser = Depends(get_current_user)):
...
async def create_non_functional_test_plan(project_id: str, payload: NonFunctionalTestPlanCreate, user: CurrentUser = Depends(get_current_user)):
...
async def get_non_functional_test_plan(plan_id: str, user: CurrentUser = Depends(get_current_user)):
    ...
    value["external_evidence"] = await database.value.non_functional_test_evidence.find({"plan_id": plan_id}).sort("created_at", -1).to_list(1000)
    return envelope(value, revision=value["revision"])


@router.patch("/ke-hoach-phi-chuc-nang/{plan_id}")
async def patch_non_functional_test_plan(plan_id: str, payload: NonFunctionalTestPlanPatch, user: CurrentUser = Depends(get_current_user)):
    value = await update_plan(database.value, plan_id, payload, user)
    return envelope(value, revision=value["revision"])


@router.post("/ke-hoach-phi-chuc-nang/{plan_id}/gui-ra-soat")
async def submit_non_functional_test_plan(plan_id: str, payload: NonFunctionalTransition, user: CurrentUser = Depends(get_current_user)):
    value = await transition_plan(database.value, plan_id, payload, user)
    return envelope(value, revision=value["revision"])


@router.post("/ke-hoach-phi-chuc-nang/{plan_id}/phe-duyet")
async def approve_non_functional_test_plan(plan_id: str, payload: NonFunctionalTransition, user: CurrentUser = Depends(get_current_user)):
...
async def import_non_functional_test_evidence(plan_id: str, payload: ExternalTestEvidenceImport, user: CurrentUser = Depends(get_current_user)):
