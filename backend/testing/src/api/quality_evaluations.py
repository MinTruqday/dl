...
async def list_quality_evaluations(project_id: str, release_id: str = Query(default=""), user: CurrentUser = Depends(get_current_user)):
...
async def create_quality_evaluation(project_id: str, payload: QualityEvaluationCreate, user: CurrentUser = Depends(get_current_user)):
...
async def patch_quality_evaluation(evaluation_id: str, payload: QualityEvaluationPatch, user: CurrentUser = Depends(get_current_user)):
...
async def submit_quality_evaluation(evaluation_id: str, payload: QualityEvaluationTransition, user: CurrentUser = Depends(get_current_user)):
...
async def review_quality_evaluation(evaluation_id: str, payload: QualityEvaluationReview, user: CurrentUser = Depends(get_current_user)):
    value = await review_evaluation(database.value, evaluation_id, payload, user)
    return envelope(value, revision=value["revision"])


@router.post("/danh-gia-chat-luong/{evaluation_id}/waiver", status_code=201)
async def create_quality_waiver(evaluation_id: str, payload: QualityWaiverCreate, user: CurrentUser = Depends(get_current_user)):
    value = await add_waiver(database.value, evaluation_id, payload, user)
    return envelope(value, revision=value["revision"])


@router.post("/danh-gia-chat-luong/{evaluation_id}/waiver/{waiver_id}/quyet-dinh")
async def decide_quality_waiver(evaluation_id: str, waiver_id: str, payload: QualityWaiverDecision, user: CurrentUser = Depends(get_current_user)):
...
async def approve_quality_evaluation(evaluation_id: str, payload: QualityEvaluationTransition, user: CurrentUser = Depends(get_current_user)):
