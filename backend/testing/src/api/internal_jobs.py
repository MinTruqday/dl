...
async def process_job(
    ...
    user = CurrentUser(
        _id=x_requester_id, email=x_requester_email or "worker@internal", system_role="USER"
    )
    ...
    await database.value.worker_events.update_one(
...
async def reindex(project_id, version_id):
    artifact = await database.value.requirement_versions.find_one(
        {"_id": version_id, "project_id": project_id}
    )
    artifact_type = "requirement_version"
    logical_key = "requirement_id"
    if not artifact:
        artifact = await database.value.test_case_versions.find_one(
            {"_id": version_id, "project_id": project_id}
        )
