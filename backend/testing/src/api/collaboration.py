...
async def list_collaboration_presence(
    ...
    items = (
        await database.value.collaboration_sessions.find(
            {
                "project_id": project_id,
...
async def apply_operation(project_id, artifact_type, artifact_id, payload, user):
    ...
    try:
        ...
        if error.status_code != 409:
            raise
        raise HTTPException(
            status_code=409, detail={"code": "COLLABORATION_RETRY_REQUIRED"}
        ) from error
    record = {
        "_id": new_id("COLOP"),
        "project_id": project_id,
...
async def list_collaboration_conflicts(
    ...
    items = (
        await database.value.draft_conflicts.find({"project_id": project_id})
        .sort("created_at", -1)
        .to_list(1000)
    )
    return envelope(items)
...
async def resolve_collaboration_conflict(
    ...
    if current["revision"] != payload.expected_revision:
        raise HTTPException(
            status_code=409,
            detail={"code": "REVISION_CONFLICT", "current_revision": current["revision"]},
        )
    changes = (
        {}
    ...
    updated = await database.value.draft_conflicts.find_one_and_update(
        {
            "_id": conflict_id,
            "project_id": project_id,
            "status": "OPEN",
