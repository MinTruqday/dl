...
from src.api.automation_execution import redact
...
async def create_cicd_binding(
    project_id: str, payload: CiCdBindingCreate, user: CurrentUser = Depends(get_current_user)
):
    await get_project(project_id, user, "cicd.manage")
    connector = await database.value.project_connectors.find_one(
        {"_id": payload.connector_id, "project_id": project_id, "status": "BOUND", "enabled": True}
    )
    if not connector:
...
async def update_cicd_binding(
    ...
    updated = await database.value.cicd_bindings.find_one_and_update(
        {"_id": binding_id, "project_id": project_id, "revision": payload.expected_revision},
...
async def trigger_cicd_run(payload: CiCdTriggerInput, x_internal_token: str = Header(default="")):
    ...
    existing = await database.value.pipeline_runs.find_one(
        {"project_id": payload.project_id, "idempotency_key": payload.idempotency_key}
    )
    if existing:
        return envelope(existing, operation_id=existing["_id"])
    binding = await database.value.cicd_bindings.find_one(
        {"_id": payload.binding_id, "project_id": payload.project_id, "enabled": True}
    )
    if not binding:
    ...
    automation = {
        ...
        "name": f"CI {binding['name']} {payload.external_run_id}",
        "runner": "external_ci",
...
async def ingest_cicd_result(payload: CiCdResultInput, x_internal_token: str = Header(default="")):
    ...
    run = await database.value.pipeline_runs.find_one(
        {"_id": payload.pipeline_run_id, "project_id": payload.project_id}
    )
    if not run:
    ...
    timestamp = now()
    updated = await database.value.pipeline_runs.find_one_and_update(
        {"_id": payload.pipeline_run_id, "project_id": payload.project_id, "status": "RUNNING"},
        {
            "$set": {
    ...
    await database.value.automation_executions.update_one(
        {
            "_id": run["automation_execution_id"],
            "project_id": payload.project_id,
            "status": "RUNNING",
        },
        {
...
@router.post(
    "/du-an/{project_id}/tich-hop-trien-khai-lien-tuc/lan-chay/{run_id}/thu-lai", status_code=202
)
async def retry_cicd_run(
    ...
    if run["project_id"] != project_id:
        raise HTTPException(status_code=422, detail={"code": "PROJECT_MISMATCH"})
    existing = await database.value.cicd_reconciliation_jobs.find_one(
