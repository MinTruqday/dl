...
MetricKey = Literal["REQUIREMENT_COVERAGE", "ACCEPTANCE_CRITERION_COVERAGE", "TEST_CONDITION_COVERAGE", "EXECUTION_PROGRESS", "PASS_RATE", "BLOCKED_RATE", "DEFECT_REOPEN_RATE", "CRITICAL_DEFECT_AGING", "MEAN_TIME_TO_RETEST", "STALE_TEST_RATIO", "REQUIREMENT_VOLATILITY", "IMPACT_PROPOSAL_ACCEPTANCE_RATE", "REGRESSION_EFFECTIVENESS", "AUTOMATION_COVERAGE", "AUTOMATION_PASS_STABILITY", "ESCAPED_DEFECT_RATE", "DEFECT_REMOVAL_EFFICIENCY"]
LOWER_IS_BETTER = {"BLOCKED_RATE", "DEFECT_REOPEN_RATE", "CRITICAL_DEFECT_AGING", "MEAN_TIME_TO_RETEST", "STALE_TEST_RATIO", "REQUIREMENT_VOLATILITY", "ESCAPED_DEFECT_RATE"}


def validate_threshold_order(key, target, warning_threshold, critical_threshold):
    if warning_threshold is not None and critical_threshold is not None and warning_threshold == critical_threshold:
        raise ValueError("Ngưỡng cảnh báo và nghiêm trọng phải khác nhau")
    populated = [value for value in (target, warning_threshold, critical_threshold) if value is not None]
...
class MeasurementDefinitionCreate(BaseModel):
    ...
    aggregation: Literal["LATEST", "SUM", "AVERAGE", "RATIO", "DURATION"]
    ...
    def validate_thresholds(self):
        validate_threshold_order(self.key, self.target, self.warning_threshold, self.critical_threshold)
        return self
...
class MeasurementDefinitionPatch(BaseModel):
    ...
    aggregation: Literal["LATEST", "SUM", "AVERAGE", "RATIO", "DURATION"] | None = None
