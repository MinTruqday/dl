...
class QualityEvaluationCreate(BaseModel):
    ...
    measurement_snapshot_refs: list[str] = Field(min_length=1, max_length=500)
    ...
    recommendation: Literal["GO", "GO_WITH_RISK", "NO_GO", "MORE_TESTING_REQUIRED"]
...
class QualityEvaluationPatch(BaseModel):
    ...
    recommendation: Literal["GO", "GO_WITH_RISK", "NO_GO", "MORE_TESTING_REQUIRED"] | None = None
...
class QualityWaiverCreate(BaseModel):
    metric_or_criterion: str = Field(min_length=1, max_length=500)
    reason: str = Field(min_length=2, max_length=5000)
    risk: str = Field(min_length=2, max_length=5000)
    owner_id: str = Field(min_length=1, max_length=200)
    expiry: datetime
    evidence: list[str] = Field(min_length=1, max_length=500)
