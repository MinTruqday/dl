...
def generated_summaries(snapshot):
    ...
    coverage = "Độ phủ yêu cầu {requirements}% điều kiện kiểm thử {conditions}% và rủi ro {risks}%".format(
    ...
    defects = "Lỗi đang mở gồm {blocker} blocker và {critical} critical với {reopened} lỗi mở lại".format(
...
async def generate_status_report(project_id, payload, user):
    ...
    build = await database.value.builds.find_one({"_id": payload.build_id, "project_id": project_id})
    if not build:
    ...
    actions = await database.value.test_control_actions.find(
    ...
    report = {
        ...
        "snapshot_basis": {
            ...
            "quality_gate_status": snapshot.get("effective_quality_gate_status", snapshot.get("quality_gate_status")),
        ...
        "control_actions": [
            {key: action.get(key) for key in ("_id", "type", "title", "description", "owner_id", "due_at", "priority", "status", "decision_reason", "evidence_refs")}
            for action in actions
        ],
        "forecast": payload.forecast,
        "recommendation": payload.recommendation or default_recommendation(snapshot),
    ...
    await audit(user.id, "test_status_report_generated", "TestStatusReport", report["_id"], project_id, {"snapshot_id": snapshot["_id"], "source_fingerprint": snapshot["source_fingerprint"]})
    return report
...
async def update_status_report(report_id, payload, user):
    ...
    updated = await update_report(report_id, report["project_id"], payload.expected_revision, {"DRAFT"}, changes)
    if not updated:
        raise HTTPException(status_code=409, detail={"code": "STATUS_REPORT_REVISION_CONFLICT"})
    await audit(user.id, "test_status_report_updated", "TestStatusReport", report_id, report["project_id"], {"fields": sorted(changes)})
    return updated
...
async def generate_status_report_narrative(report_id, payload, user):
    ...
    if existing:
        if existing.get("result_type") != "STATUS_REPORT_NARRATIVE" or existing.get("subject_id") != report_id:
    ...
    evidence = [{"artifact_type": "test_status_report", "artifact_id": report_id, "authority": "PROJECT_RECORD", "text": json.dumps(narrative_basis, ensure_ascii=False, default=str)}]
    instruction = json.dumps({"task": "Soạn bản nháp diễn giải báo cáo trạng thái chỉ từ số liệu và bằng chứng đã cung cấp không tự thay đổi số liệu không tự phê duyệt", "user_instruction": payload.instruction}, ensure_ascii=False)
    ai = await request_design_assistance("status_report_narrative", report["project_id"], instruction, evidence)
    result = {"_id": new_id("AIR"), "project_id": report["project_id"], "result_type": "STATUS_REPORT_NARRATIVE", "subject_id": report_id, "candidate_only": True, "human_confirmation_required": True, "suggestions": ai.get("suggestions", []), "evidence_refs": ai.get("evidence_refs", []), "status": ai.get("status", "DEGRADED"), "confidence": ai.get("confidence", 0), "warnings": ai.get("warnings", []), "model": ai.get("model", {}), "idempotency_key": payload.idempotency_key, "created_by": user.id, "created_at": now()}
    try:
    ...
    await audit(user.id, "test_status_report_ai_narrative_generated", "AIResult", result["_id"], report["project_id"], {"report_id": report_id})
    return result
...
async def attach_status_report_evidence(report_id, payload, user):
    ...
    updated = await update_report(report_id, report["project_id"], payload.expected_revision, {"DRAFT"}, {"evidence_refs": evidence_refs, "updated_at": now()})
    if not updated:
        raise HTTPException(status_code=409, detail={"code": "STATUS_REPORT_REVISION_CONFLICT"})
    await audit(user.id, "test_status_report_evidence_attached", "TestStatusReport", report_id, report["project_id"], {"evidence_refs": payload.evidence_refs})
    return updated
...
async def transition_status_report(report_id, payload, user, source, target, permission, action):
    ...
    if target == "IN_REVIEW":
        ...
        changes.update({"change_request": payload.note, "reviewed_by": user.id, "reviewed_at": timestamp})
    elif target == "APPROVED":
        approved_snapshot = status_report_snapshot(report)
        changes.update({"approved_by": user.id, "approved_at": timestamp, "approved_snapshot": approved_snapshot, "approved_snapshot_hash": status_report_hash(report)})
    elif target == "PUBLISHED":
        changes.update({"published_by": user.id, "published_at": timestamp})
    updated = await update_report(report_id, report["project_id"], payload.expected_revision, {source}, changes)
    if not updated:
        raise HTTPException(status_code=409, detail={"code": "STATUS_REPORT_TRANSITION_CONFLICT", "expected_status": source})
    await audit(user.id, action, "TestStatusReport", report_id, report["project_id"], {"from": source, "to": target, "note": payload.note})
    return updated
...
async def compare_status_reports(report_id, other_report_id, user):
    ...
    if not right:
        raise HTTPException(status_code=404, detail={"code": "STATUS_REPORT_COMPARE_TARGET_NOT_FOUND"})
    fields = ("executive_summary", "progress_summary", "coverage_summary", "defect_summary", "deviations", "blockers", "risks", "control_actions", "forecast", "recommendation", "distribution", "status")
    changes = [{"field": field, "from": left.get(field), "to": right.get(field)} for field in fields if left.get(field) != right.get(field)]
    return {"from_report_id": report_id, "to_report_id": other_report_id, "from_sequence": left.get("sequence"), "to_sequence": right.get("sequence"), "changes": changes}
