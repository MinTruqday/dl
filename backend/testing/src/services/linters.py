...
def requirement_findings(version, acceptance_criteria=None):
    ...
    # The form deliberately separates general scope from testable conditions.
    # If conditions remain in the general content, an AI rewrite is warranted;
    # once moved, this finding disappears and no equivalent proposal is made.
    ...
    has_criteria = bool(criteria_text) if acceptance_criteria is not None else bool(
        version.get("acceptance_criterion_ids")
    )
    if not has_criteria:
        findings.append(
            {
    ...
    if has_criteria and not re.search(r"(khi|nếu|given|when|trong trường hợp)", criteria_lowered):
        findings.append(
            {
