...
with httpx.Client(base_url=base_url, timeout=120) as client:
    ...
    strategy = call(client, "POST", f"/kiem-thu/du-an/{project_id}/chien-luoc", expected=201, json={"key": "P1_STRATEGY", "name": "Chiến lược P1", "objective": "Kiểm chứng P1", "test_levels": ["SYSTEM"], "test_types": ["FUNCTIONAL", "SECURITY", "PERFORMANCE"], "approach": "Dựa trên rủi ro", "risk_model": {"probability_scale": [{"value": 1, "label": "Thấp"}], "impact_scale": [{"value": 1, "label": "Thấp"}], "risk_exposure_formula": "probability * impact", "thresholds": [{"level": "LOW", "min": 1}]}, "entry_criteria_defaults": ["Yêu cầu đã baseline"], "exit_criteria_defaults": ["Không còn lỗi blocker"], "suspension_criteria": ["Môi trường không khả dụng"], "resumption_criteria": ["Môi trường đã phục hồi"], "reporting_policy": {"cadence": "WEEKLY"}, "quality_objectives": [{"key": "PASS_RATE", "target": 95}], "reviewer_ids": ["v5-p1-tester"]})
    completeness = call(client, "POST", f"/kiem-thu/chien-luoc/{strategy['_id']}/kiem-tra", headers=tester)
    assert completeness["ready_for_review"] is True and completeness["findings"] == []
    ...
    strategy = call(client, "POST", f"/kiem-thu/chien-luoc/{strategy['_id']}/ra-soat", headers=tester, json={"expected_revision": strategy["revision"], "note": "Đã rà soát"})
    ...
    strategy_v2 = call(client, "POST", f"/kiem-thu/chien-luoc/{strategy['_id']}/tao-phien-ban", expected=201, json={"expected_revision": strategy["revision"], "change_reason": "Kiểm tra version contract"})
    comparison = call(client, "GET", f"/kiem-thu/chien-luoc/{strategy['_id']}/so-sanh", params={"other_strategy_id": strategy_v2["_id"]})
    assert comparison["left_version"] == 1 and comparison["right_version"] == 2
    cloned_strategy = call(client, "POST", f"/kiem-thu/du-an/{project_id}/chien-luoc/nhan-ban", expected=201, json={"source_strategy_id": strategy["_id"], "key": "P1_STRATEGY_CLONE", "name": "Chiến lược P1 nhân bản", "tailoring_rationale": "Kiểm tra clone contract"})
    assert cloned_strategy["cloned_from_strategy_id"] == strategy["_id"] and cloned_strategy["status"] == "DRAFT"
