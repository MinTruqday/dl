...
async def get_notifications(skip: int=Query(0, ge=0), limit: int=Query(20, ge=1, le=100), current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await NotificationService.get_notifications(str(current_user.id), skip, limit, db), message='Đã tải danh sách thông báo')
...
async def mark_as_read(notif_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await NotificationService.mark_as_read(notif_id, str(current_user.id), db), message='Đã đánh dấu đã xem')
...
async def mark_all_as_read(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await NotificationService.mark_all_as_read(str(current_user.id), db), message='Đã đánh dấu xem tất cả')
...
async def delete_notification(notif_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await NotificationService.delete_notification(notif_id, str(current_user.id), db), message='Đã xóa thông báo')
...
async def create_notification(data: NotificationCreate, db=Depends(get_db)):
    return APIResponse(data=await NotificationService.create_notification(data, db), message='Đã tạo thông báo', status=201)
