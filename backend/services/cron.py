...
async def scheduled_publish_worker(db=None):
    from core.database import db_client
    from datetime import datetime, timezone
    if db is None:
        db = db_client.mongodb.get_default_database()
    now = datetime.now(timezone.utc)
    query = {'scheduled_publish_at': {'$lte': now}, 'status': {'$in': ['draft', 'compiling']}, 'is_deleted': {'$ne': True}}
    pending = await db['documents'].find(query).to_list(length=50)
    published_count = 0
    for doc in pending:
        try:
            await db['documents'].update_one({'_id': doc['_id']}, {'$set': {'status': 'published', 'published_at': now, 'updated_at': now}, '$unset': {'scheduled_publish_at': ''}})
            published_count += 1
...
async def integrity_monitor_worker(db=None):
    from core.database import db_client
    ...
    for item in items:
        ...
        if item.get('description') and 'http' in item['description']:
            try:
                ...
                if rag_url:
                    resp = await httpx.AsyncClient().post(f'{rag_url}/inference/kiem-tra-toan-ven', json={'document_id': str(item['_id']), 'text': item.get('description', '')})
                    if resp.status_code == 200:
                        broken_links = resp.json().get('broken_links', [])
            except Exception as e:
                logger.error(f'Integrity Monitor check failed: {e}')
        if broken_links:
            await db['storage_items'].update_one({'_id': item['_id']}, {'$set': {'broken_links': broken_links}})
            logger.info(f"Integrity Monitor: Found broken links in {item.get('name')}")
        else:
            await db['storage_items'].update_one({'_id': item['_id']}, {'$unset': {'broken_links': ''}})
        checked_count += 1
...
def start_cron_service(db=None):
