...
class QuotaService:
    ...
    @staticmethod
    async def get_user_limits(user_id: str, role: str) -> QuotaLimit:
        db = db_client.mongodb[settings.MONGODB_DB_NAME]
    ...
    @staticmethod
    async def check_quota(user_id: str, role: str, feature: str = "chat"):
    ...
    @staticmethod
    async def consume_request(user_id: str, feature: str = "chat"):
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        req_key = f"quota:{user_id}:{feature}:req:{today}"
    ...
    @staticmethod
    async def consume_tokens(user_id: str, tokens: int, feature: str = "chat"):
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        token_key = f"quota:{user_id}:{feature}:token:{today}"
    ...
    @staticmethod
    async def get_current_usage(user_id: str, role: str, feature: str = "chat"):
        limits = await QuotaService.get_user_limits(user_id, role)
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    @staticmethod
    async def update_global_limits(role: str, limits: QuotaLimit):
        db = db_client.mongodb[settings.MONGODB_DB_NAME]
        await db["quota_configs"].update_one(
            {"_id": "global"},
            {"$set": {f"role_limits.{role}": limits.model_dump(), "updated_at": datetime.now(timezone.utc)}},
            upsert=True
        )
