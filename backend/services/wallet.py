...
class WalletService:
        ...
        try:
            async with session.start_transaction():
                deduct_result = await users.update_one(
                    {"_id": current_user.id, "wallet_balance": {"$gte": req.amount}},
    ...
    async def request_payout(amount: int, current_user):
        ...
        try:
            async with session.start_transaction():
                ...
                return {"message": "Đã gửi yêu cầu rút tiền thành công.", "payout_id": payout_id}
        except HTTPException:
            raise
        except Exception as e:
            await session.abort_transaction()
    ...
    async def purchase_chapter(document_id: str, chapter_id: str, current_user) -> dict:
        ...
        try:
            async with session.start_transaction():
                ...
                await session.commit_transaction()
                logger.info(f"Chapter {chapter_id} of document {document_id} purchased by user {current_user.id} for {price} dl (atomic)")
                return {"message": "Mua chương thành công.", "status": "purchased"}
        except HTTPException:
            raise
        except Exception as e:
            await session.abort_transaction()
            logger.error(f"Chapter purchase transaction failed for user {current_user.id}: {e}")
            raise HTTPException(status_code=500, detail="Giao dịch thất bại. Vui lòng thử lại sau.")
        finally:
            await session.end_session()
    ...
    async def cancel_purchase(purchase_id: str, current_user) -> dict:
        """Cancel a purchase and refund within 48 hours of purchase."""
        ...
        # Check if purchase is within refundable window (48 hours)
        purchased_at = purchase.get("purchased_at", datetime.utcnow())
        if isinstance(purchased_at, str):
            purchased_at = datetime.fromisoformat(purchased_at)
        
        refund_window = datetime.utcnow() - timedelta(hours=48)
        if purchased_at < refund_window:
        ...
        try:
            async with session.start_transaction():
                ...
                tx_refund_buyer = Transaction(
                    ...
                    amount=price,
                    note=f"Hoàn tiền {item_type}: {purchase_id}"
                )
                ...
                await session.commit_transaction()
                logger.info(f"Purchase {purchase_id} cancelled and refunded {price} dl for user {current_user.id}")
                return {"message": f"Đã hoàn tiền {price} dl. Giao dịch được hủy thành công.", "refunded_amount": price}
        except HTTPException:
            raise
        except Exception as e:
            await session.abort_transaction()
            logger.error(f"Cancel purchase failed for {purchase_id}: {e}")
            raise HTTPException(status_code=500, detail="Hệ thống đang bảo trì dữ liệu, vui lòng thử lại sau.")
        finally:
            await session.end_session()
