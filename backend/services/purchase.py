...
class PurchaseService:
    ...
    @staticmethod
    async def purchase_document(document_id: str, current_user, db=None) -> dict:
        if db is None:
    ...
    @staticmethod
    async def purchase_chapter(document_id: str, chapter_id: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        doc = await db['documents'].find_one({'_id': document_id})
        if not doc:
    ...
    @staticmethod
    async def cancel_purchase(purchase_id: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
