...
class PaymentService:
    ...
    async def purchase_chapter(req, current_user):
        ...
        await transactions.insert_many([tx_buyer.model_dump(by_alias=True), tx_author.model_dump(by_alias=True)])
        
        record = PurchaseRecord(user_id=user_id, item_id=req.chapter_id, price_paid=price)
        await purchases.insert_one(record.model_dump(by_alias=True))
        
        logger.info(f"Chapter {req.chapter_id} purchased by user {user_id}")
