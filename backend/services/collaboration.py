...
class CollaborationService:
    ...
    async def respond_to_collaboration_invite(invite_id: str, status: str, current_user) -> dict:
        ...
        await CollaborationService.log_activity(
            ...
            "Đã chấp nhận lời mời cộng tác" if status == "ACCEPTED" else "Đã từ chối lời mời cộng tác"
        )
    ...
    async def get_activities(document_id: str, current_user) -> list:
        ...
        activities = await db["collaboration_activities"].find(
            {"document_id": document_id}
        ...
        return [
            {
                ...
                "action": act["action"],
                "details": act["details"],
                "timestamp": act["timestamp"].isoformat() if isinstance(act.get("timestamp"), datetime) else act.get("timestamp")
            }
            for act in activities
        ]

    @staticmethod
    async def transfer_ownership(document_id: str, target_user_id: str, current_user) -> dict:
        db = db_client.mongodb.get_default_database()
        doc = await db["documents"].find_one({"_id": document_id, "author_id": str(current_user.id)})
        if not doc:
        ...
        await db["documents"].update_one(
            {"_id": document_id},
            {
                "$set": {"author_id": target_user_id, "updated_at": datetime.now(timezone.utc)},
    ...
    async def update_status(document_id: str, current_user) -> dict:
        ...
        await db["collaboration_status"].update_one(
            ...
            {"$set": {"last_seen": datetime.now(timezone.utc), "full_name": current_user.full_name}},
            upsert=True
        )
    ...
    async def get_online_collaborators(document_id: str) -> list:
        ...
        online_users = await db["collaboration_status"].find({
            "document_id": document_id
        }).to_list(length=100)
    ...
    @staticmethod
    async def update_collaborator_role(collaboration_id: str, role: str, current_user) -> dict:
        db = db_client.mongodb.get_default_database()
        invite = await db["collaboration_invites"].find_one({"_id": collaboration_id})
        if not invite:
            raise HTTPException(status_code=404, detail="Không tìm thấy thông tin cộng tác.")
    ...
    async def get_memos(document_id: str, current_user) -> list:
        ...
        memos = await db["collaboration_memos"].find(
            {"document_id": document_id}
    ...
    @staticmethod
    async def update_collab_access(document_id: str, access_level: str, current_user) -> dict:
        db = db_client.mongodb.get_default_database()
        doc = await db["documents"].find_one({"_id": document_id, "author_id": str(current_user.id)})
        if not doc:
        ...
        await db["documents"].update_one(
            {"_id": document_id},
            {"$set": {"collab_access_level": access_level}}
        )
    ...
    @staticmethod
    async def create_snapshot(document_id: str, version_name: str, current_user) -> dict:
        db = db_client.mongodb.get_default_database()
        doc = await db["documents"].find_one({
            "_id": document_id,
            "$or": [
    ...
    async def get_snapshots(document_id: str, current_user) -> list:
        ...
        drafts = await db["collaboration_drafts"].find(
            {"document_id": document_id}
    ...
    async def get_lock_status(document_id: str) -> dict:
        ...
        return {
            ...
            "locked_at": existing.get("locked_at").isoformat() if isinstance(existing.get("locked_at"), datetime) else None
        }
    ...
    async def join_via_invite_code(invite_code: str, current_user) -> dict:
        ...
        if doc.get("author_id") == str(current_user.id):
            raise HTTPException(status_code=400, detail="Bạn là chủ sở hữu tài liệu này rồi.")
            
        if str(current_user.id) in doc.get("coauthors", []):
            raise HTTPException(status_code=400, detail="Bạn đã là cộng tác viên của tài liệu này.")
    ...
    @staticmethod
    async def create_task(document_id: str, task_desc: str, assigned_to: str, current_user) -> dict:
        db = db_client.mongodb.get_default_database()
        doc = await db["documents"].find_one({
            "_id": document_id,
            "$or": [
    ...
    async def get_tasks(document_id: str, current_user) -> list:
        ...
        tasks = await db["collaboration_tasks"].find(
            {"document_id": document_id}
        ).sort("created_at", -1).to_list(length=100)
    ...
    async def update_task(task_id: str, is_done: bool, current_user) -> dict:
        ...
        await CollaborationService.log_activity(
            ...
            current_user.full_name,
            "Cập nhật nhiệm vụ",
            f"Đã đánh dấu nhiệm vụ '{task['task_desc']}' thành {'Hoàn thành' if is_done else 'Chưa xong'}"
        )
        
        return {"message": "Cập nhật nhiệm vụ thành công."}

    @staticmethod
    async def add_task_comment(task_id: str, comment_text: str, current_user) -> dict:
        db = db_client.mongodb.get_default_database()
        task = await db["collaboration_tasks"].find_one({"_id": task_id})
        if not task:
            raise HTTPException(status_code=404, detail="Nhiệm vụ không tồn tại.")
    ...
    async def get_task_comments(task_id: str, current_user) -> list:
        ...
        comments = await db["collaboration_task_comments"].find(
            {"task_id": task_id}
        ).sort("timestamp", 1).to_list(length=100)
