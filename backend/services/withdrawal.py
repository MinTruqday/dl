...
class WithdrawalService:
    ...
    @staticmethod
    async def request_withdrawal(data: dict, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
    ...
    @staticmethod
    async def verify_withdrawal(withdrawal_id: str, action: str, current_moderator, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        ...
        try:
            async with session.start_transaction():
                ...
                await db['audit_logs'].insert_one({'action': f'WITHDRAWAL_{status}', 'actor_id': str(current_moderator.id), 'withdrawal_id': withdrawal_id, 'timestamp': datetime.now(timezone.utc)}, session=session)
                await session.commit_transaction()
    ...
    @staticmethod
    async def cancel_withdrawal(withdrawal_id: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
