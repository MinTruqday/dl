...
class PrivacyService:
    ...
    async def request_data_takeout(current_user, db=None):
        ...
        takeout_payload = {'profile': current_user.model_dump(exclude={'password_hash'}), 'authored_documents': [{'_id': str(b['_id']), 'title': b.get('title')} for b in documents], 'comments_written': len(comments), 'reactions_given': len(reactions), 'raw_comments': [{'document_id': c.get('document_id'), 'content': c.get('content')} for c in comments]}
    ...
    async def right_to_be_forgotten(current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        user_id = str(current_user.id)
        await db['comments'].update_many({'user_id': user_id}, {'$set': {'content': '[Nội dung đã bị xóa theo yêu cầu của Quyền lãng quên GDPR]', 'is_shadowbanned_content': True}})
    ...
    async def generate_gdpr_takeout(current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        user_id = str(current_user.id)
        full_data = {'profile': await db['users'].find_one({'_id': str(current_user.id)}, {'password_hash': 0}), 'documents': await db['documents'].find({'author_id': user_id}).to_list(100), 'comments': await db['comments'].find({'user_id': user_id}).to_list(500)}
