...
class PasskeyService:
    ...
    async def register_begin(email: str, db=None):
        ...
        options = generate_registration_options(rp_id=RP_ID, rp_name=RP_NAME, user_id=user_id.encode('utf-8'), user_name=email, user_display_name=user.get('full_name', email), authenticator_selection=AuthenticatorSelectionCriteria(user_verification=UserVerificationRequirement.PREFERRED))
        if db_client.redis:
            try:
    ...
    async def register_finish(email: str, credential_data: dict, db=None):
        ...
        await db['users'].update_one({'_id': user['_id']}, {'$push': {'passkeys': new_passkey}})
    ...
    async def login_finish(email: str, credential_data: dict, db=None):
        ...
        await db['users'].update_one({'_id': user['_id'], 'passkeys.credential_id': credential_id_b64}, {'$set': {'passkeys.$.sign_count': verification.new_sign_count}})
