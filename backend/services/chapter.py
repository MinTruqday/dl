...
class ChapterService:
    ...
    @staticmethod
    async def add_chapter(document_id: str, chapter_in, current_user, db=None):
        if db is None:
        ...
        new_chapter = {'_id': str(uuid7()), 'title': chapter_in.title, 'content': chapter_in.content, 'order': order, 'is_premium': chapter_in.is_premium, 'price_dl': chapter_in.price_dl, 'words_count': len(chapter_in.content.split()), 'readability_score': calculate_flesch_kincaid(chapter_in.content), 'vocabulary_richness': calculate_vocabulary_richness(chapter_in.content), 'created_at': datetime.now(timezone.utc)}
    ...
    @staticmethod
    async def set_free_preview(document_id: str, chapter_ids: list, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
    ...
    @staticmethod
    async def get_document_dropoff(document_id: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        doc = await db['documents'].find_one({'_id': document_id, 'author_id': str(current_user.id)})
        if not doc:
        ...
        for (i, ch) in enumerate(chapters):
            ...
            dropoff_data.append({'chapter_id': ch['id'], 'chapter_title': ch.get('title', f'Chương {i + 1}'), 'readers_started': readers, 'readers_completed': int(readers * 0.9), 'dropoff_rate': round((readers - int(readers * 0.9)) / readers * 100, 2) if readers > 0 else 0})
        return {'document_id': document_id, 'dropoff_data': dropoff_data}

    @staticmethod
    async def get_document_chapters(document_id: str, current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
