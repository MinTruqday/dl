...
class CompilationService:
    ...
    @staticmethod
    async def compile_latex_to_pdf(content: str, db=None):
        if not content:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(compiler_url, json={'content': content})
    ...
    @staticmethod
    async def export_to_format(content: str, format_type: str, db=None):
        if not content:
            raise HTTPException(status_code=400, detail='Nội dung tài liệu đang trống.')
        try:
            compiler_url = f'{settings.COMPILER_SERVICE_URL}/export/{format_type}'
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(compiler_url, json={'content': content, 'format': format_type})
                if response.status_code != 200:
