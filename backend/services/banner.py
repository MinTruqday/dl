...
class BannerService:
    ...
    async def create_banner(data: dict, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        banner = {'_id': str(uuid7()), 'title': data.get('title'), 'image_url': data.get('image_url'), 'link_url': data.get('link_url'), 'priority': data.get('priority', 0), 'is_active': True, 'created_at': datetime.now(timezone.utc)}
