...
class AIService:
    ...
    @staticmethod
    async def smart_search(query: str, current_user, db=None) -> list:
        ...
        try:
            resp = await make_ai_request(f'{rag_url}/tro-chuyen', {'query': f'Tìm kiếm tài liệu liên quan đến: {query}', 'user_id': str(current_user.id), 'useSmart': True})
            result = resp.json()
            if db_client.redis:
    ...
    @staticmethod
    async def process_text(req, current_user, db=None):
        from services.quota import QuotaService
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        if not rag_url:
    ...
    @staticmethod
    async def generate_cover(title: str, description: str, style: str, db=None) -> dict:
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        if not rag_url:
    ...
    @staticmethod
    async def generate_code(prompt: str, language: str='python', db=None) -> dict:
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        if not rag_url:
            return {'message': 'AI không khả dụng.'}
        try:
    ...
    @staticmethod
    async def analyze_reader_sentiment(document_id: str, current_user, db=None) -> dict:
        from services.quota import QuotaService
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        if not rag_url:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                resp = await client.post(f'{rag_url}/inference/phan-tich-cam-xuc', json={'document_id': document_id})
        ...
        return {'sentiment_score': 0.0, 'mood': 'unknown', 'summary': 'Hệ thống đang thu thập thêm dữ liệu từ độc giả để thực hiện phân tích.', 'top_emotions': []}

    @staticmethod
    async def get_ai_recommendations(limit: int=10, current_user=None, db=None) -> list:
        if db is None:
            db = db_client.mongodb.get_default_database()
        from services.document import serialize_document
        cursor = db['documents'].find({'status': 'published', 'is_deleted': {'$ne': True}}).sort([('average_rating', -1), ('views', -1)]).limit(limit)
        documents = await cursor.to_list(length=limit)
        return [serialize_document(d) for d in documents]

    @staticmethod
    async def generate_text_completion(prompt: str, max_tokens: int=300, db=None) -> str:
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        if not rag_url:
    ...
    @staticmethod
    async def transform_tone(text: str, tone: str, expansion: bool, current_user, db=None) -> dict:
        from services.quota import QuotaService
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        try:
    ...
    @staticmethod
    async def peer_review(text: str, criteria: list, current_user, db=None) -> dict:
        from services.quota import QuotaService
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        try:
    ...
    @staticmethod
    async def multi_doc_synthesis(document_ids: list, query: str, current_user, db=None) -> dict:
        from services.quota import QuotaService
        rag_url = getattr(settings, 'AGENTIC_AI_URL', None)
        try:
    ...
    @staticmethod
    async def process_storage_file(item_id: str, owner_id: str, db=None):
        ...
        try:
            ...
            update_data_dict = {'description': summary.strip(), 'name': suggested_name.replace('"', '').strip(), 'tags': tags, 'parent_id': target_parent_id, 'environment_ready': environment_ready, 'is_duplicate': is_duplicate, 'duplicate_of': duplicate_of, 'entities': entities, 'ai_processed': True}
            if is_flagged:
    ...
    @staticmethod
    async def translate_storage_document(item_id: str, target_lang: str, owner_id: str, db=None):
        from services.storage import StorageService
        from models.storage import StorageItemUpdate
        item = await StorageService.get_item(item_id, owner_id)
    ...
    @staticmethod
    async def get_related_storage_items(item_id: str, owner_id: str, db=None) -> list:
        from services.storage import StorageService
        item = await StorageService.get_item(item_id, owner_id)
