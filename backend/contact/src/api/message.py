...
async def get_messages(other_user_id: str, cursor: str=None, limit: int=Query(50), current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.get_messages(other_user_id, current_user, limit, cursor), message='Đã tải lịch sử tin nhắn', status=200)

@router.get('/hoi-thoai', response_model=APIResponse[Any])
async def get_conversations(current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.get_conversations(current_user), message='Đã tải danh sách hội thoại', status=200)
...
async def toggle_pin(message_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result['is_pinned'], message='Đã cập nhật trạng thái ghim', status=200)
...
async def edit_message(message_id: str, req: dict, current_user = Depends(get_current_user)):
    ...
    if not result:
        return APIResponse(message='Tin nhắn không đủ điều kiện để chỉnh sửa', status=403)
    other_id = result['receiver_id'] if result['sender_id'] == current_user.id else result['sender_id']
...
async def search_messages(other_user_id: str, q: str=Query(...), current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.search_messages(other_user_id, q, current_user), message='Đã hoàn tất tìm kiếm tin nhắn')
...
async def add_reaction(message_id: str, req: dict, current_user = Depends(get_current_user)):
    ...
    if not result:
        return APIResponse(message='Thao tác bày tỏ biểu cảm không khả dụng', status=400)
    other_id = result['receiver_id'] if result['sender_id'] == current_user.id else result['sender_id']
...
async def get_shared_attachments(other_user_id: str, current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.get_shared_attachments(other_user_id, current_user), message='Đã tải danh sách tệp chia sẻ')
...
async def get_blocked_status(other_user_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data={'is_blocked': blocked}, message='Đã kiểm tra trạng thái chặn')
...
async def toggle_pin_conversation(other_user_id: str, current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.toggle_pin_conversation(other_user_id, current_user), message='Đã cập nhật trạng thái ghim hội thoại')
