...
async def send_message(req: MessageCreate, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=msg, message='Gửi tin nhắn hoàn tất', status=201)
...
async def get_messages(other_user_id: str, cursor: str=None, limit: int=Query(50), current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.get_messages(other_user_id, current_user, limit, cursor), message='Lấy lịch sử tin nhắn hoàn tất', status=200)

@router.get('/hoi-thoai', response_model=APIResponse[Any])
async def get_conversations(current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.get_conversations(current_user), message='Lấy danh sách hội thoại hoàn tất', status=200)
...
async def toggle_pin(message_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result['is_pinned'], message='Cập nhật trạng thái ghim hoàn tất', status=200)
...
async def edit_message(message_id: str, req: dict, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Chỉnh sửa tin nhắn hoàn tất', status=200)
...
async def search_messages(other_user_id: str, q: str=Query(...), current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.search_messages(other_user_id, q, current_user), message='Tìm kiếm tin nhắn hoàn tất')
...
async def get_shared_attachments(other_user_id: str, current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.get_shared_attachments(other_user_id, current_user), message='Lấy tệp chia sẻ hoàn tất')
...
async def get_blocked_status(other_user_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data={'is_blocked': blocked}, message='Lấy trạng thái chặn hoàn tất')
...
async def toggle_pin_conversation(other_user_id: str, current_user = Depends(get_current_user)):
    return APIResponse(data=await MessageService.toggle_pin_conversation(other_user_id, current_user), message='Cập nhật ghim hội thoại hoàn tất')
...
async def translate_message(message_id: str, req: dict, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Dịch tin nhắn hoàn tất')
...
async def create_group(req: dict, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Tạo nhóm thảo luận hoàn tất', status=201)
...
async def get_draft(other_user_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Lấy tin nhắn nháp hoàn tất')
...
async def toggle_self_destruct(other_user_id: str, req: dict, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Cập nhật bộ tự hủy hoàn tất')
...
async def get_conversation_settings(other_user_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Lấy cài đặt hoàn tất')
...
async def delete_conversation(other_user_id: str, current_user = Depends(get_current_user)):
    ...
    return APIResponse(data=result, message='Xóa cuộc hội thoại hoàn tất')
