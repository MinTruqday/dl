...
class MessageService:
    ...
    async def _upsert_conversation(db, sender_id: str, receiver_id: str, message_data: dict):
        ...
        if receiver_id.startswith('group_'):
            group_doc = await db['message_groups'].find_one({'_id': receiver_id})
        ...
        update_doc = {'$set': {'last_message': message_data, 'updated_at': datetime.now(timezone.utc), 'cleared_by': []}, '$inc': inc_data, '$setOnInsert': {'created_at': datetime.now(timezone.utc), 'pinned_messages': []}}
        if not receiver_id.startswith('group_'):
            update_doc['$set']['participants'] = sorted(participants)
        await db['conversations'].update_one({'_id': participant_key}, update_doc, upsert=True)

    @staticmethod
    async def send_message(receiver_id: str, content: str, current_user, image_url: str=None, reply_to_id: str=None, audio_url: str=None, client_msg_id: str=None, db=None):
        if db is None:
        ...
        message = MessageInDB(sender_id=sender_id, receiver_id=receiver_id, content=content, image_url=image_url, audio_url=audio_url, reply_to_id=reply_to_id, client_msg_id=client_msg_id, self_destruct_seconds=self_destruct_seconds)
        msg_dict = message.model_dump(by_alias=True)
        if reply_msg:
            msg_dict['replied_message'] = {'_id': reply_msg['_id'], 'content': reply_msg.get('content'), 'sender_id': reply_msg.get('sender_id')}
        await db['messages'].insert_one(msg_dict)
        await MessageService._upsert_conversation(db, sender_id, receiver_id, {'_id': msg_dict['_id'], 'sender_id': sender_id, 'receiver_id': receiver_id, 'content': content, 'image_url': image_url, 'audio_url': audio_url, 'is_recalled': False, 'created_at': msg_dict.get('created_at', datetime.now(timezone.utc))})
        return msg_dict

    @staticmethod
    async def get_messages(other_user_id: str, current_user, limit: int=50, cursor: str=None, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        if other_user_id.startswith('group_'):
            query = {'receiver_id': other_user_id}
        else:
            query = {'$or': [{'sender_id': current_user.id, 'receiver_id': other_user_id}, {'sender_id': other_user_id, 'receiver_id': current_user.id}]}
        if cursor:
            query['_id'] = {'$lt': cursor}
    ...
    async def get_conversations(current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        conversations = await db['conversations'].find({'$or': [{'participants': str(current_user.id)}, {'_id': {'$in': [g['_id'] for g in await db['message_groups'].find({'members': str(current_user.id)}).to_list(100)]}}], 'cleared_by': {'$ne': str(current_user.id)}}).sort('updated_at', -1).to_list(length=200)
        other_user_ids = []
        ...
        for conv in conversations:
            ...
            if is_group:
                ...
                if group:
                    results.append({'other_user_id': other_id, 'other_user': {'username': group.get('group_name'), 'full_name': group.get('group_name'), 'avatar_url': '', 'is_group': True}, 'last_message': conv.get('last_message'), 'pinned_messages': pinned_messages, 'unread_count': unread})
            else:
                other_user = user_map.get(other_id)
                if other_user:
                    results.append({'other_user_id': other_id, 'other_user': {'username': other_user.get('username'), 'avatar_url': other_user.get('avatar_url'), 'full_name': other_user.get('full_name')}, 'last_message': conv.get('last_message'), 'pinned_messages': pinned_messages, 'unread_count': unread})
        return results
    ...
    async def toggle_pin(message_id: str, current_user, db=None):
        ...
        if is_pinned:
            await db['conversations'].update_one({'_id': participant_key}, {'$pull': {'pinned_messages': {'_id': message_id}}})
            await db['messages'].update_one({'_id': message_id}, {'$set': {'is_pinned': False}})
        else:
            await db['conversations'].update_one({'_id': participant_key}, {'$push': {'pinned_messages': msg}})
    ...
    async def edit_message(message_id: str, new_content: str, current_user, db=None):
        ...
        if not msg or msg['sender_id'] != str(current_user.id):
            return None
        await db['messages'].update_one({'_id': message_id}, {'$set': {'content': new_content, 'is_edited': True, 'updated_at': datetime.now(timezone.utc)}})
        updated_msg = await db['messages'].find_one({'_id': message_id})
        participant_key = msg['receiver_id'] if msg['receiver_id'].startswith('group_') else f"{min(msg['sender_id'], msg['receiver_id'])}_{max(msg['sender_id'], msg['receiver_id'])}"
        conv = await db['conversations'].find_one({'_id': participant_key})
        if conv and conv.get('last_message', {}).get('_id') == message_id:
            await db['conversations'].update_one({'_id': participant_key}, {'$set': {'last_message.content': new_content}})
        if conv and any((pm['_id'] == message_id for pm in conv.get('pinned_messages', []))):
            await db['conversations'].update_one({'_id': participant_key, 'pinned_messages._id': message_id}, {'$set': {'pinned_messages.$.content': new_content}})
        return updated_msg
    ...
    async def recall_message(message_id: str, current_user, db=None):
        ...
        await db['messages'].update_one({'_id': message_id}, {'$set': {'is_recalled': True, 'content': recalled_content, 'updated_at': datetime.now(timezone.utc)}, '$unset': {'image_url': '', 'audio_url': '', 'file_url': '', 'attachments': ''}})
        participant_key = msg['receiver_id'] if msg['receiver_id'].startswith('group_') else f"{min(msg['sender_id'], msg['receiver_id'])}_{max(msg['sender_id'], msg['receiver_id'])}"
        conv = await db['conversations'].find_one({'_id': participant_key})
        if conv and conv.get('last_message', {}).get('_id') == message_id:
            await db['conversations'].update_one({'_id': participant_key}, {'$set': {'last_message.content': recalled_content, 'last_message.is_recalled': True}, '$unset': {'last_message.image_url': '', 'last_message.audio_url': '', 'last_message.file_url': ''}})
        if conv and any((pm['_id'] == message_id for pm in conv.get('pinned_messages', []))):
            await db['conversations'].update_one({'_id': participant_key, 'pinned_messages._id': message_id}, {'$set': {'pinned_messages.$.content': recalled_content, 'pinned_messages.$.is_recalled': True}, '$unset': {'pinned_messages.$.image_url': '', 'pinned_messages.$.audio_url': '', 'pinned_messages.$.file_url': ''}})
        return await db['messages'].find_one({'_id': message_id})

    @staticmethod
    async def search_messages(other_user_id: str, query_str: str, current_user, db=None) -> list:
        if db is None:
            db = db_client.mongodb.get_default_database()
        ...
        if other_user_id.startswith('group_'):
            ...
            query['$or'] = [{'sender_id': str(current_user.id), 'receiver_id': other_user_id}, {'sender_id': other_user_id, 'receiver_id': str(current_user.id)}]
    ...
    async def delete_conversation(other_user_id: str, current_user, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        if other_user_id.startswith('group_'):
            group = await db['message_groups'].find_one({'_id': other_user_id})
            if group:
                if group.get('created_by') == str(current_user.id):
                    await db['message_groups'].delete_one({'_id': other_user_id})
    ...
    async def add_reaction(message_id: str, reaction: str, current_user, db=None):
        ...
        if not msg:
            return None
        await db['messages'].update_one({'_id': message_id}, {'$pull': {'reactions': {'user_id': str(current_user.id)}}})
        if reaction:
            await db['messages'].update_one({'_id': message_id}, {'$push': {'reactions': {'user_id': str(current_user.id), 'user_name': current_user.full_name, 'reaction': reaction}}})
        return await db['messages'].find_one({'_id': message_id})
    ...
    async def mark_as_read(other_user_id: str, current_user, db=None):
        ...
        last_msg = await db['messages'].find_one({'receiver_id': participant_key if other_user_id.startswith('group_') else user_id}, sort=[('created_at', -1)])
        from datetime import timedelta
        await db['messages'].update_many({'receiver_id': participant_key if other_user_id.startswith('group_') else user_id, 'is_read': False, 'self_destruct_seconds': {'$exists': True, '$ne': None}, 'self_destruct_at': None}, [{'$set': {'is_read': True, 'self_destruct_at': {'$add': [datetime.now(timezone.utc), {'$multiply': ['$self_destruct_seconds', 1000]}]}}}])
        await db['messages'].update_many({'receiver_id': participant_key if other_user_id.startswith('group_') else user_id, 'is_read': False}, {'$set': {'is_read': True}})
    ...
    async def share_document(receiver_id: str, document_id: str, current_user, db=None):
        ...
        message = MessageInDB(sender_id=str(current_user.id), receiver_id=receiver_id, content=content, image_url=None, reply_to_id=None)
        msg_dict = message.model_dump(by_alias=True)
        await db['messages'].insert_one(msg_dict)
        await MessageService._upsert_conversation(db, str(current_user.id), receiver_id, {'_id': msg_dict['_id'], 'sender_id': str(current_user.id), 'receiver_id': receiver_id, 'content': content, 'is_recalled': False, 'created_at': msg_dict.get('created_at', datetime.now(timezone.utc))})
        return msg_dict
    ...
    async def get_shared_attachments(other_user_id: str, current_user, db=None) -> list:
        ...
        query['$or'] = [{'image_url': {'$ne': None, '$ne': ''}}, {'content': {'$regex': 'Đã chia sẻ tài liệu:'}}]
        messages = await db['messages'].find(query).sort('created_at', -1).to_list(length=100)
        attachments = []
        for m in messages:
            if m.get('image_url'):
                attachments.append({'id': m['_id'], 'type': 'image', 'url': m['image_url'], 'created_at': m['created_at'].isoformat() if isinstance(m.get('created_at'), datetime) else m.get('created_at')})
            else:
                attachments.append({'id': m['_id'], 'type': 'document', 'content': m['content'], 'created_at': m['created_at'].isoformat() if isinstance(m.get('created_at'), datetime) else m.get('created_at')})
        return attachments
    ...
    async def toggle_pin_conversation(other_user_id: str, current_user, db=None):
        ...
        if is_pinned:
            await db['conversations'].update_one({'_id': participant_key}, {'$pull': {'pinned_by': str(current_user.id)}})
            return {'is_pinned': False}
        else:
            await db['conversations'].update_one({'_id': participant_key}, {'$addToSet': {'pinned_by': str(current_user.id)}})
            return {'is_pinned': True}

    @staticmethod
    async def translate_message(message_id: str, target_lang: str, current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        msg = await db['messages'].find_one({'_id': message_id})
        if not msg:
        ...
        await db['messages'].update_one({'_id': message_id}, {'$set': {'translations': translations}})
        return {'translated_content': translated_content, 'target_lang': target_lang, 'receiver_id': msg['receiver_id'], 'sender_id': msg['sender_id']}
    ...
    @staticmethod
    async def toggle_self_destruct(other_user_id: str, seconds: int, current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        settings_id = other_user_id if other_user_id.startswith('group_') else f'settings_{min(str(current_user.id), other_user_id)}_{max(str(current_user.id), other_user_id)}'
    ...
    async def toggle_mute(other_user_id: str, current_user, db=None):
        ...
        if is_muted:
            await db['conversations'].update_one({'_id': participant_key}, {'$pull': {'muted_by': str(current_user.id)}})
            return {'is_muted': False}
        else:
            await db['conversations'].update_one({'_id': participant_key}, {'$addToSet': {'muted_by': str(current_user.id)}})
    ...
    async def get_conversation_settings(other_user_id: str, current_user, db=None):
        if db is None:
            db = db_client.mongodb.get_default_database()
        settings_id = other_user_id if other_user_id.startswith('group_') else f'settings_{min(str(current_user.id), other_user_id)}_{max(str(current_user.id), other_user_id)}'
        settings = await db['message_settings'].find_one({'_id': settings_id})
        ...
        return {
            'self_destruct_seconds': settings.get('self_destruct_seconds', 0) if settings else 0,
            'is_muted': is_muted,
            'is_pinned': is_pinned
        }
