...
async def get_my_profile(current_user: CurrentUser = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await UserService.get_user_profile(current_user.id), message="Truy xuất thông tin hồ sơ thành công")
...
async def update_my_profile(data: ProfileUpdate, current_user: CurrentUser = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await UserService.update_profile(current_user.id, data.model_dump(exclude_unset=True)), message="Cập nhật thông tin hồ sơ thành công")
...
async def get_my_settings(current_user: CurrentUser = Depends(get_current_user), db=Depends(get_db)):
    ...
    return APIResponse(data=profile.get("settings", {}), message="Truy xuất dữ liệu cấu hình thành công")
...
async def update_my_settings(data: dict, current_user: CurrentUser = Depends(get_current_user), db=Depends(get_db)):
    ...
    return APIResponse(data=updated.get("settings", {}), message="Cập nhật dữ liệu cấu hình thành công")
...
async def apply_author(data: dict, current_user: CurrentUser = Depends(get_current_user), db=Depends(get_db)):
    ...
    return APIResponse(data=updated, message="Gửi đơn ứng tuyển vai trò tác giả thành công")
...
async def delete_account(current_user: CurrentUser = Depends(get_current_user), db=Depends(get_db)):
    ...
    return APIResponse(data={}, message="Thực hiện xóa tài khoản thành công")
