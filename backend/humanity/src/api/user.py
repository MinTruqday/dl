...
from src.schemas.user import Role, UpdateRoleRequest, UpdateStatusRequest, InternalCreateUserRequest, CreateUserRequest
...
async def get_all_users(limit: int = 50, offset: int = 0, db=Depends(get_db)):
    return APIResponse(data=await UserService.get_all_users(limit, offset), message="Lấy danh sách thành công")

@router.get("/tim-kiem", response_model=APIResponse[Any])
async def search_users(q: str = "", limit: int = 50, db=Depends(get_db)):
    return APIResponse(data=await UserService.search_users(q, limit), message="Tìm kiếm thành công")
...
async def update_user_role(user_id: str, req: UpdateRoleRequest, db=Depends(get_db)):
    return APIResponse(data=await UserService.update_user_role(user_id, req.role), message="Cập nhật quyền thành công")
...
async def update_user_status(user_id: str, req: UpdateStatusRequest, db=Depends(get_db)):
    return APIResponse(data=await UserService.update_user_status(user_id, req.is_active), message="Cập nhật trạng thái thành công")

@router.post("/", response_model=APIResponse[Any], include_in_schema=False)
async def internal_create_user(req: CreateUserRequest, db=Depends(get_db)):
...
async def internal_get_user_by_id(user_id: str, db=Depends(get_db)):
    from src.repositories.user import UserRepository
...
async def internal_update_user(user_id: str, request: Request, db=Depends(get_db)):
    from src.repositories.user import UserRepository
    data = await request.json()
    await UserRepository.update_user(user_id, data)
    return APIResponse(data=None, message="Cập nhật thành công")
