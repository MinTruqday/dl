import redis.asyncio as aioredis
from loguru import logger
from src.core.infrastructure.configuration import settings

class RedisAPIClient:
    def __init__(self):
        self.url = settings.REDIS_URI
        self._client = None

    def get_client(self):
        if self._client is None:
            self._client = aioredis.from_url(self.url, decode_responses=True)
        return self._client

    async def set(self, key: str, value: str):
        try:
            return await self.get_client().set(key, value)
        except Exception:
            logger.exception("Failed to execute SET operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def setex(self, key: str, expire: int, value: str):
        try:
            return await self.get_client().setex(key, expire, value)
        except Exception:
            logger.exception("Failed to execute SETEX operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def get(self, key: str):
        try:
            return await self.get_client().get(key)
        except Exception:
            logger.exception("Failed to execute GET operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def delete(self, key: str):
        try:
            return await self.get_client().delete(key)
        except Exception:
            logger.exception("Failed to execute DELETE operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def sadd(self, key: str, member: str):
        try:
            return await self.get_client().sadd(key, member)
        except Exception:
            logger.exception("Failed to execute SADD operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def sismember(self, key: str, member: str):
        try:
            return await self.get_client().sismember(key, member)
        except Exception:
            logger.exception("Failed to execute SISMEMBER operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def smembers(self, key: str):
        try:
            return await self.get_client().smembers(key)
        except Exception:
            logger.exception("Failed to execute SMEMBERS operation on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def publish(self, channel: str, message: str):
        try:
            return await self.get_client().publish(channel, message)
        except Exception:
            logger.exception("Failed to execute PUBLISH operation to Redis channel")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def pipeline_incr_expire(self, key: str, expire: int):
        try:
            client = self.get_client()
            async with client.pipeline() as pipe:
                await pipe.incr(key)
                await pipe.expire(key, expire)
                res = await pipe.execute()
                return res
        except Exception:
            logger.exception("Failed to execute atomic INCR/EXPIRE pipeline on Redis Cache")
            raise Exception("Dịch vụ bộ đệm tạm thời không khả dụng")

    async def aclose(self):
        if self._client:
            await self._client.aclose()

redis = RedisAPIClient()
