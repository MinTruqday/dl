...
@router.post(
    "/du-an/{project_id}/doi-tuong", description="Chỉ mục hóa artifact trong phạm vi Project"
)
...
@router.post(
    "/du-an/{project_id}/tim-kiem", description="Tìm evidence knowledge theo phạm vi Project"
)
async def search_project_knowledge(project_id: str, req: ProjectKnowledgeSearchRequest):
    ...
    try:
        documents = await retriever.retrieve(
            query=req.query, k=req.limit, requester_id=None, is_admin=True, metadata_filters=filters
        )
