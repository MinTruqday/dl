...
class SecurityEvaluation(SecurityOutput):
    ...
    sanitized_text: str = Field(max_length=200000, description="<critical_instructions>The exact original input text, but with ALL detected PII and credentials completely replaced by [REDACTED]. MUST be provided even if no sanitization was needed.</critical_instructions>")
    reason: str = Field(max_length=2000, description="<decision_context>A brief security classification reason without private reasoning. Leave empty when all flags are false.</decision_context>")
