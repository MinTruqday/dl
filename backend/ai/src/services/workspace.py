...
class WorkspaceService:
    ...
    async def start(
        ...
        await database.mongodb[settings.AI_DB_NAME].ai_workspaces.update_one(
    ...
    async def update_steps(session_id: str, user_id: str, task_status: dict[str, str]) -> None:
        if not session_id or not task_status:
            return
        collection = database.mongodb[settings.AI_DB_NAME].ai_workspaces
    ...
    async def get(session_id: str, user_id: str):
        return await database.mongodb[settings.AI_DB_NAME].ai_workspaces.find_one(
            {"_id": session_id, "user_id": user_id}
        )
