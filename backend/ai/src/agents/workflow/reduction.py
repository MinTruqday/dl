...
async def splitter_node(state: MapReduceState):
    ...
    splitter = RecursiveCharacterTextSplitter(chunk_size=512 * 10, chunk_overlap=64 * 3)
...
async def agent_summarize_long_document(
    document_id: Annotated[
        str, Field(description="Exact identifier of the long document to summarize completely")
    ],
    config: dict,
) -> str:
    """
    ...
    return json.dumps({"status": "success", "summary": res["final_summary"]}, ensure_ascii=False)
