...
async def supervisor_node(state: ActingState):
    ...
    if not ready_tasks:
        active_statuses = [task_status.get(step["id"], "pending") for step in steps]
        ...
        if any(status == "failed" for status in active_statuses):
            ...
            try:
                ...
                revised_nodes = await plan_validator.validate_plan(revised_nodes)
                prefix = f"replan_{replan_count + 1}_"
                id_map = {node["id"]: f"{prefix}{node['id']}" for node in revised_nodes}
                revised_nodes = [
                    {
                        **node,
                        "id": id_map[node["id"]],
                ...
                completed_steps = [step for step in steps if step["id"] in completed_tasks]
                steps = [*completed_steps, *revised_nodes]
                task_status = {
                    **{step["id"]: "completed" for step in completed_steps},
                    **{step["id"]: "pending" for step in revised_nodes},
                }
                replan_count += 1
            except Exception:
                logger.exception("ReAct replanning failed")
                return {
                    "steps": steps,
                    "task_status": task_status,
                    "completed_tasks": completed_tasks,
            ...
            ready_tasks = [
                step
                for step in revised_nodes
                if all(dependency in completed_tasks for dependency in step.get("dependencies", []))
            ]
            for step in ready_tasks:
                task_status[step["id"]] = "running"
        else:
...
class OrchestrationWorkflow:
    ...
    def initialize(self):
        ...
        self.checkpointer = MongoDBSaver(self.sync_client, db_name=settings.AI_DB_NAME)
