...
class EditorJSEngine:
    ...
    def _san(text: str) -> str:
        if not text:
            return ''
        text = re.sub(r'<(script|iframe|object|applet)(.*?)>(.*?)</\1>', '', text, flags=re.IGNORECASE | re.DOTALL)
    ...
    def _render_block(block: dict) -> str:
        ...
        if t == 'paragraph':
            ...
            return '<p style="text-align:{align}">{san(s(d.get("text")))}</p>'
        ...
        if t == 'quote':
            ...
            return (f'<blockquote>{san(s(d.get("text")))}'
        ...
        if t in ('warning', 'alert', 'notice', 'callout'):
            ...
            return ('<div style="border-left:4px solid {c};padding:8px 12px;background:#f9f9f9;margin:8px 0">'
        ...
        if t in ('spoiler', 'toggle'):
            ...
            return '<details style="border:1px solid #ddd;padding:8px;margin:8px 0"><summary><strong>{title}</strong></summary>{content}</details>'

        if t == 'aiText':
            return '<p style="background:#f0f7ff;padding:6px 10px;border-left:3px solid #3498db">{san(s(d.get("text", d.get("content", ""))))}</p>'
        ...
        if t in ('checklist', 'nestedChecklist'):
            ...
            for item in d.get('items', []):
                ...
                parts.append('<p style="margin:2px 0">{chk} {txt}</p>')
            return ''.join(parts)
        ...
        if t in ('codeBox', 'codeMirror'):
            ...
            lbl = '<div style="font-size:9pt;color:#888;margin-bottom:4px">{lang}</div>' if lang else ''
        ...
        if t == 'gist':
            return '<div style="border:1px solid #ddd;padding:8px">[GitHub Gist: {san(s(d.get("url", d.get("gistId", ""))))}]</div>'

        if t in ('latex', 'math'):
            return '<p style="font-family:monospace;text-align:center">{_html.escape(s(d.get("formula", d.get("math", d.get("text", "")))))}</p>'

        if t == 'mermaid':
            return '<pre style="background:#f4f4f4;padding:1em;font-family:monospace;font-size:9pt">[Mermaid]\n{_html.escape(s(d.get("code", "")))}</pre>'

        if t in ('image', 'simpleImage', 'imageCrop', 'imageWithLink'):
            url = san(s(d.get('file', {}).get('url', d.get('url', ''))))
            cap = san(s(d.get('caption', '')))
            link = san(s(d.get('link', '')))
            img = '<img src="{url}" alt="{cap}" style="max-width:100%;display:block;margin:0 auto"/>'
            if link:
                img = '<a href="{link}">{img}</a>'
            cap_tag = '<figcaption style="text-align:center;font-size:0.9em;color:#666">{cap}</figcaption>' if cap else ''
            return '<figure style="margin:1em 0;text-align:center">{img}{cap_tag}</figure>'

        if t in ('gallery', 'groupImage', 'carousel'):
            files = d.get('files', d.get('images', []))
            imgs = ['<img src="{san(s(f.get("url", f.get("file", {}).get("url", ""))))}" style="width:30%;margin:4px;vertical-align:top"/>'
                    for f in files if s(f.get('url', f.get('file', {}).get('url', '')))]
            return '<div style="margin:1em 0">{"".join(imgs)}</div>' if imgs else ''

        if t in ('audio', 'audioPlayer'):
            url = san(s(d.get('file', {}).get('url', d.get('url', ''))))
            title = san(s(d.get('title', '')))
            return '<div style="border:1px solid #ddd;padding:8px">[Audio: {title or url}]</div>'

        if t == 'video':
            url = san(s(d.get('file', {}).get('url', d.get('url', ''))))
            cap = san(s(d.get('caption', '')))
            return '<div style="border:1px solid #ddd;padding:8px;text-align:center">[Video: {cap or url}]</div>'

        if t in ('embed', 'gif', 'telegramPost', 'iframe'):
            src = san(s(d.get('source', d.get('embed', d.get('url', d.get('src', ''))))))
            cap = san(s(d.get('caption', '')))
            return '<div style="border:1px solid #eee;padding:8px;text-align:center;color:#888">[Embed: {cap or src}]</div>'

        if t in ('attaches', 'file'):
            name = san(s(d.get('title', d.get('file', {}).get('name', ''))))
            url = san(s(d.get('file', {}).get('url', '')))
            return '<div style="border:1px solid #ddd;padding:6px 10px">&#128206; <a href="{url}">{name or url}</a></div>'
        ...
        if t == 'linkTool':
            ...
            return '<div style="border:1px solid #ddd;padding:8px"><a href="{link_url}"><strong>{title}</strong></a><br/><small>{desc}</small></div>'
        ...
        if t == 'table':
            ...
            return '<table style="border-collapse:collapse;width:100%;margin:1em 0">{"".join(html_rows)}</table>'

        if t == 'chart':
            return '<div style="border:1px solid #ddd;padding:8px;text-align:center;color:#888">[Chart ({san(s(d.get("type","bar")))}): {san(s(d.get("title","")))}]</div>'
        ...
        if t == 'columns':
            ...
            for col in cols:
                ...
                col_html.append('<td style="vertical-align:top;padding:4px;width:{100//max(len(cols),1)}%">{inner}</td>')
            return '<table style="width:100%;border-collapse:collapse"><tr>{"".join(col_html)}</tr></table>'

        if t == 'button':
            link = san(s(d.get('link', '')))
            text = san(s(d.get('text', '')))
            return '<p><a href="{link}" style="display:inline-block;padding:6px 16px;background:#333;color:#fff;text-decoration:none">{text}</a></p>'

        if t == 'quiz':
            q = san(s(d.get('question', '')))
            opts = ''.join(f'<li>{san(s(o.get("text",o) if isinstance(o,dict) else o))}</li>' for o in d.get('options', []))
            return '<div style="border:1px solid #ddd;padding:8px"><strong>Quiz:</strong> {q}<ol>{opts}</ol></div>'

        if t == 'poll':
            q = san(s(d.get('question', d.get('title', ''))))
            opts = ''.join(f'<li>{san(s(o.get("text",o) if isinstance(o,dict) else o))}</li>' for o in d.get('options', []))
            return '<div style="border:1px solid #ddd;padding:8px"><strong>Kh&#7843;o s&#225;t:</strong> {q}<ul>{opts}</ul></div>'
        ...
        if t == 'kanban':
            ...
            for col in d.get('columns', []):
                ...
                parts.append('<div style="display:inline-block;vertical-align:top;width:30%;margin:4px;border:1px solid #ddd;padding:8px"><strong>{col_title}</strong><ul>{cards}</ul></div>')
        ...
        if t == 'timeline':
            ...
            for ev in d.get('events', d.get('items', [])):
                ...
                parts.append('<div style="padding:6px 0;border-left:3px solid #333;padding-left:12px;margin:4px 0"><strong>{date}</strong> &#8211; {text}<br/><small>{desc}</small></div>')
            return f'<div>{"".join(parts)}</div>'

        if t == 'steps':
            parts = [
                '<div style="margin:4px 0"><strong>B&#432;&#7899;c {i}:</strong> {san(s(item.get("text", item.get("title", "")) if isinstance(item, dict) else item))}</div>'
        ...
        if t == 'pricing':
            parts = [
                '<div style="display:inline-block;border:1px solid #ddd;padding:8px 12px;margin:4px;vertical-align:top"><strong>{san(s(p.get("title",p.get("name",""))))}</strong><br/>{san(s(p.get("price","")))}</div>'
                for p in d.get('plans', d.get('items', []))
            ]
            return f'<div>{"".join(parts)}</div>'

        if t == 'testimonial':
            return '<blockquote style="border-left:4px solid #ddd;padding:8px 12px;font-style:italic">{san(s(d.get("text",d.get("content",""))))}<cite> &#8212; {san(s(d.get("author",d.get("name",""))))}</cite></blockquote>'

        if t == 'personality':
            photo = san(s(d.get('photo', '')))
            img = '<img src="{photo}" style="width:48px;height:48px;border-radius:50%;margin-right:8px;vertical-align:middle"/>' if photo else ''
            return '<div style="border:1px solid #eee;padding:8px">{img}<strong>{san(s(d.get("name","")))}</strong><br/><small>{san(s(d.get("description","")))}</small></div>'
        ...
        if t == 'flipbox':
            ...
            return '<div style="border:1px solid #ddd;padding:8px"><strong>M&#7863;t tr&#432;&#7899;c:</strong> {front}<br/><strong>M&#7863;t sau:</strong> {back}</div>'

        if t == 'badge':
            return '<span style="display:inline-block;background:{d.get("color","#333")};color:#fff;padding:2px 8px;border-radius:12px;font-size:0.8em">{san(s(d.get("text","")))}</span>'
        ...
        if t == 'comment':
            return '<p style="color:#888;font-style:italic">&#128172; {san(s(d.get("author","")))} : {san(s(d.get("text",d.get("content",""))))}</p>'

        if t in ('undo', 'dragDrop', 'multiBlockSelection', 'premium',
                 'alignment', 'indent', 'style', 'notice', 'anchor',
                 'styleTune', 'textVariant', 'textColor', 'colorPicker',
                 'marker', 'underline', 'strikethrough', 'changeCase',
                 'superscript', 'subscript', 'textStyle', 'hyperlink',
                 'linkSearch', 'template'):
            return ''

        logger.warning(f'Bỏ qua khối nội dung không hợp lệ loại {t}')
        return ''
    ...
    def _convert_blocks_to_html(blocks: list) -> str:
        ...
        parts = ['<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8"/><style>{CSS}</style></head><body>']
        for block in blocks:
    ...
    async def compile_to_pdf(content: str) -> bytes:
        ...
        try:
            ...
            if not os.path.exists(pdf_path):
                ...
                logger.error("Lỗi xuất PDF")
                raise Exception({
                    "error": "Lỗi biên dịch EditorJS sang PDF",
                    "logs": error_msg[-2048:]
                })
            ...
            raise Exception("Lỗi biên dịch quá thời gian")
