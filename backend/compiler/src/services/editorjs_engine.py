...
class EditorJSEngine:
    ...
    def _render_block(block: dict) -> str:
        ...
        if t in ('warning', 'alert', 'notice', 'callout'):
            ...
            return (f'<div style="border-left:4px solid {c};padding:8px 12px;background:#f9f9f9;margin:8px 0">'
        ...
        if t in ('gallery', 'groupImage', 'carousel'):
            ...
            imgs = [f'<img src="{san(s(f.get("url", f.get("file", {}).get("url", ""))))}" style="width:30%;margin:4px;vertical-align:top"/>'
        ...
        if t == 'poll':
            ...
            opts = ''.join(f'<li>{san(s(o.get("text",o) if isinstance(o,dict) else o))}</li>' for o in d.get('options', []))
            return f'<div style="border:1px solid #ddd;padding:8px"><strong>Kh&#7843;o s&#225;t:</strong> {q}<ul>{opts}</ul></div>'
        ...
        if t == 'steps':
            parts = [
                f'<div style="margin:4px 0"><strong>B&#432;&#7899;c {i}:</strong> {san(s(item.get("text", item.get("title", "")) if isinstance(item, dict) else item))}</div>'
        ...
        if t == 'flipbox':
            ...
            return f'<div style="border:1px solid #ddd;padding:8px"><strong>M&#7863;t tr&#432;&#7899;c:</strong> {front}<br/><strong>M&#7863;t sau:</strong> {back}</div>'
    ...
    async def compile_to_pdf(content: str) -> bytes:
        ...
        try:
            ...
            if not os.path.exists(pdf_path):
                ...
                logger.error(f"Lỗi xuất PDF: {error_msg}")
                raise Exception({
