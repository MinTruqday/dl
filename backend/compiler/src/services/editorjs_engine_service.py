...
class EditorJSEngine:
    ...
    def _render_block(block: dict) -> str:
        ...
        if t in ("warning", "alert", "notice", "callout"):
            ...
            return (
                '<div style="border-left:4px solid {c};padding:8px 12px;background:#f9f9f9;margin:8px 0">'
        ...
        if t in ("codeBox", "codeMirror"):
            ...
            lbl = (
                '<div style="font-size:9pt;color:#888;margin-bottom:4px">{lang}</div>'
                if lang
                else ""
            )
        ...
        if t in ("image", "simpleImage", "imageCrop", "imageWithLink"):
            ...
            cap_tag = (
                '<figcaption style="text-align:center;font-size:0.9em;color:#666">{cap}</figcaption>'
                if cap
                else ""
            )
            return (
                '<figure style="margin:1em 0;text-align:center">{img}{cap_tag}</figure>'
            )

        if t in ("gallery", "groupImage", "carousel"):
            files = d.get("files", d.get("images", []))
            imgs = [
                '<img src="{san(s(f.get("url", f.get("file", {}).get("url", ""))))}" style="width:30%;margin:4px;vertical-align:top"/>'
                for f in files
        ...
        if t == "columns":
            ...
            for col in cols:
                ...
                col_html.append(
                    '<td style="vertical-align:top;padding:4px;width:{100//max(len(cols),1)}%">{inner}</td>'
                )
        ...
        if t == "kanban":
            ...
            for col in d.get("columns", []):
                ...
                parts.append(
                    '<div style="display:inline-block;vertical-align:top;width:30%;margin:4px;border:1px solid #ddd;padding:8px"><strong>{col_title}</strong><ul>{cards}</ul></div>'
                )
        ...
        if t == "timeline":
            ...
            for ev in d.get("events", d.get("items", [])):
                ...
                parts.append(
                    '<div style="padding:6px 0;border-left:3px solid #333;padding-left:12px;margin:4px 0"><strong>{date}</strong> &#8211; {text}<br/><small>{desc}</small></div>'
                )
            return f'<div>{"".join(parts)}</div>'
        ...
        if t == "personality":
            ...
            img = (
                '<img src="{photo}" style="width:48px;height:48px;border-radius:50%;margin-right:8px;vertical-align:middle"/>'
                if photo
                else ""
            )
    ...
    def _convert_blocks_to_html(blocks: list) -> str:
        ...
        parts = [
            '<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8"/><style>{CSS}</style></head><body>'
        ]
        for block in blocks:
    ...
    async def compile_to_pdf(content: str) -> bytes:
        try:
            ...
            raise Exception(
                {"error": "Nội dung EditorJS không hợp lệ (JSON decode error)"}
            )
