...
from provision.src.schemas.user import UserCreate, UserInDB, UserResponse, ForgotPasswordRequest, ResetPasswordRequest, VerifyCodeRequest
...
async def read_users_me(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    ...
    return APIResponse(data=user_data, message='Đã tải thông tin cá nhân', status=status.HTTP_200_OK)
...
async def login(request: Request, form_data: OAuth2PasswordRequestForm=Depends(), db=Depends(get_db)) -> Any:
    client_ip = request.client.host if request.client else 'unknown'
    return APIResponse(data=await AuthenticationService.login_user(form_data.username, form_data.password, client_ip, db=db), message='Đã đăng nhập', status=status.HTTP_200_OK)
...
async def reset_password(payload: ResetPasswordRequest, request: Request, db=Depends(get_db)) -> Any:
    client_ip = request.client.host if request.client else 'unknown'
    return APIResponse(data=await AuthenticationService.reset_password(payload.token, payload.new_password, client_ip, db=db), message='Đã đặt lại mật khẩu', status=status.HTTP_200_OK)
...
async def google_login(db=Depends(get_db)):
    ...
    return APIResponse(data={'url': auth_url}, message='Đã tạo liên kết đăng nhập Google', status=200)
...
async def google_callback(code: str, request: Request, db=Depends(get_db)):
    client_ip = request.client.host if request.client else 'unknown'
    return APIResponse(data=await AuthenticationService.handle_google_callback(code, client_ip, db=db), message='Đã đăng nhập bằng Google', status=200)
