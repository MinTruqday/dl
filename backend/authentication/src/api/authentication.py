from src.api.dependency import get_db, get_current_user, RateLimiter
...
async def read_users_me(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    ...
    return APIResponse(data=user_data, message='Lấy thông tin cá nhân hoàn tất', status=status.HTTP_200_OK)
...
async def register_user(user_in: UserCreate, request: Request, db=Depends(get_db)) -> Any:
    ...
    return APIResponse(data=await AuthenticationService.register_user(user_in, client_ip, db=db), message='Đăng ký tài khoản hoàn tất', status=status.HTTP_201_CREATED)
...
async def login(request: Request, form_data: OAuth2PasswordRequestForm=Depends(), db=Depends(get_db)) -> Any:
    client_ip = request.client.host if request.client else 'unknown'
    return APIResponse(data=await AuthenticationService.login_user(form_data.username, form_data.password, client_ip, db=db), message='Đăng nhập hoàn tất', status=status.HTTP_200_OK)
...
async def reset_password(payload: ResetPasswordRequest, request: Request, db=Depends(get_db)) -> Any:
    client_ip = request.client.host if request.client else 'unknown'
    return APIResponse(data=await AuthenticationService.reset_password(payload.token, payload.new_password, client_ip, db=db), message='Đặt lại mật khẩu hoàn tất', status=status.HTTP_200_OK)
...
async def google_login(db=Depends(get_db)):
    ...
    return APIResponse(data={'url': auth_url}, message='Lấy liên kết đăng nhập Google hoàn tất', status=200)
...
async def google_callback(code: str, request: Request, db=Depends(get_db)):
    client_ip = request.client.host if request.client else 'unknown'
    return APIResponse(data=await AuthenticationService.handle_google_callback(code, client_ip, db=db), message='Đăng nhập Google hoàn tất', status=200)
