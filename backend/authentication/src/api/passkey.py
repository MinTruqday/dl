...
async def passkey_register_begin(payload: PasskeyRequest, db=Depends(get_db)):
    return APIResponse(data=await PasskeyService.register_begin(payload.email, db=db), message='Khởi tạo đăng ký Passkey hoàn tất', status=200)
...
async def passkey_register_finish(payload: PasskeyFinishRequest, db=Depends(get_db)):
    return APIResponse(data=await PasskeyService.register_finish(payload.email, payload.credential, db=db), message='Hoàn tất đăng ký Passkey hoàn tất', status=200)
...
async def passkey_login_begin(payload: PasskeyRequest, db=Depends(get_db)):
    return APIResponse(data=await PasskeyService.login_begin(payload.email, db=db), message='Khởi tạo đăng nhập Passkey hoàn tất', status=200)
...
async def passkey_login_finish(payload: PasskeyFinishRequest, db=Depends(get_db)):
    return APIResponse(data=await PasskeyService.login_finish(payload.email, payload.credential, db=db), message='Đăng nhập bằng Passkey hoàn tất', status=200)
