...
async def register_user(
    ...
    return APIResponse(
        data=await AuthenticationService.register_user(user_in, client_ip, db=db),
...
async def login(
    ...
    client_ip = request.client.host if request.client else "unknown"
    return APIResponse(
        data=await AuthenticationService.login_user(
...
async def forgot_password(
    ...
    client_ip = request.client.host if request.client else "unknown"
    return APIResponse(
        data=await AuthenticationService.forgot_password(
...
async def reset_password(
    ...
    client_ip = request.client.host if request.client else "unknown"
    return APIResponse(
        data=await AuthenticationService.reset_password(
...
async def verify_code(
    ...
    client_ip = request.client.host if request.client else "unknown"
    return APIResponse(
        data=await AuthenticationService.verify_reset_code(
...
async def google_callback(code: str, request: Request, db=Depends(get_db)):
    client_ip = request.client.host if request.client else "unknown"
    return APIResponse(
        data=await AuthenticationService.handle_google_callback(code, client_ip, db=db),
