...
from src.schemas.user import AuthorStatusEnum, KYCStatusEnum, RoleEnum
...
class IdentityService:
    ...
    async def become_author(current_user, db=None):
        ...
        await db['users'].update_one({'_id': user_id}, {'$set': {'role': RoleEnum.AUTHOR, 'updated_at': datetime.now(timezone.utc)}})
    ...
    async def apply_author(application, current_user, db=None):
        ...
        application_data = {'_id': str(uuid7()), 'user_id': user_id, 'portfolio_url': application.get('portfolio_url', '') if isinstance(application, dict) else '', 'reason': application.get('reason', '') if isinstance(application, dict) else application if isinstance(application, str) else '', 'status': AuthorStatusEnum.PENDING, 'created_at': datetime.now(timezone.utc)}
        await db['author_applications'].insert_one(application_data)
        await db['users'].update_one({'_id': user_id}, {'$set': {'author_status': AuthorStatusEnum.PENDING, 'tos_accepted_at': datetime.now(timezone.utc)}})
    ...
    async def upload_kyc(file, current_user, db=None):
        ...
        kyc_data = {'_id': str(uuid7()), 'user_id': user_id, 'document_url': object_name, 'status': KYCStatusEnum.PENDING, 'created_at': datetime.now(timezone.utc)}
        await db['kyc_applications'].insert_one(kyc_data)
        await db['users'].update_one({'_id': user_id}, {'$set': {'kyc_status': KYCStatusEnum.PENDING}})
    ...
    async def get_public_profile(slug: str, db=None) -> dict:
        if db is None:
            db = db_client.mongodb.get_default_database()
        author = await db['users'].find_one({'$or': [{'slug': slug}, {'username': slug}, {'_id': slug}], 'is_active': {'$ne': False}})
        if not author:
        ...
        return {'_id': author_id, 'full_name': author.get('full_name', 'Ẩn danh'), 'slug': author.get('slug', ''), 'role': author.get('role', 'READER'), 'avatar_url': author.get('avatar_url'), 'bio': author.get('bio', ''), 'cover_image_url': author.get('author_profile', {}).get('cover_image_url') or author.get('brand_page', {}).get('banner_url') or author.get('cover_url', ''), 'welcome_video_url': author.get('author_profile', {}).get('welcome_video_url'), 'custom_theme': author.get('author_profile', {}).get('custom_theme'), 'followers_count': 0, 'following_count': 0, 'social_links': author.get('social_links', {}), 'badges': author.get('badges', []), 'wallet_address': author.get('wallet_address', ''), 'recent_documents': [{'_id': str(d['_id']), 'title': d.get('title'), 'slug': d.get('slug'), 'cover_url': d.get('cover_url') or d.get('cover_image'), 'category_name': d.get('category_name', ''), 'views_count': d.get('views_count', 0), 'price_dl': d.get('price_dl', 0)} for d in docs], 'recent_posts': []}