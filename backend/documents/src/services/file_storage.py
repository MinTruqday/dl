...
from src.schemas.storage_ops import StorageItemCreate, StorageItemInDB, StorageItemUpdate
...
class StorageOperations:
    ...
    async def create_item(
        ...
        await db_client.mongodb.get_default_database().storage_items.insert_one(
            db_item.dict(by_alias=True)
        )
        if db_item.size and db_item.size > 0:
            await db_client.mongodb.get_default_database().users.update_one(
    ...
    async def get_storage_quota(owner_id: str, db=None) -> dict:
        user = await db_client.mongodb.get_default_database().users.find_one(
            {"_id": owner_id}
        )
        limit = (
    ...
    async def get_items_by_parent(
        ...
        cursor = (
            db_client.mongodb.get_default_database()
    ...
    async def delete_item(item_id: str, owner_id: str, db=None) -> bool:
        ...
        if not item.is_folder and (not item.is_shortcut) and item.url:
            ref_count = await db_client.mongodb.get_default_database().storage_items.count_documents(
                {"url": item.url}
            )
            if ref_count <= 1:
        ...
        result = (
            await db_client.mongodb.get_default_database().storage_items.delete_one(
                {"_id": item_id, "owner_id": owner_id}
            )
        )
        ...
        await db_client.mongodb.get_default_database().storage_items.delete_many(
            {"target_id": item_id}
        )
    ...
    async def _copy_children(source_parent_id: str, new_parent_id: str, owner_id: str):
        cursor = db_client.mongodb.get_default_database().storage_items.find(
    ...
    async def add_version(
        ...
        if not item:
            return None
        from src.schemas.storage_ops import FileVersion
    ...
    async def share_item(
        ...
        target_user = await db_client.mongodb.get_default_database().users.find_one(
            {"email": email}
        )
        if not target_user:
