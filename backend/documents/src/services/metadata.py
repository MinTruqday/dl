...
class DocumentMetadata:
        ...
        if settings.ALERTS_URL:
            try:
                ...
                async with httpx.AsyncClient() as client:
                    await client.post(
                        f"{settings.ALERTS_URL}/thong-bao/kich-hoat",
                        json={
    ...
    async def get_document_by_slug(slug: str, current_user=None):
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{settings.ADMINISTRATION_URL}/nguoi-dung/{document['creator_id']}",
                    timeout=settings.DEFAULT_HTTP_TIMEOUT,
                )
                if resp.status_code == 200:
    ...
    async def transfer_document(document_id: str, new_owner_id: str, current_user):
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{settings.ADMINISTRATION_URL}/nguoi-dung/{new_owner_id}",
                    timeout=settings.DEFAULT_HTTP_TIMEOUT,
                )
                if resp.status_code == 200:
