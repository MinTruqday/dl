...
class CollaborationSync:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{settings.ADMINISTRATION_URL}/nguoi-dung/email/{invitee_email}",
    ...
    async def get_collaborators(document_id: str, current_user, db=None) -> list:
        ...
        for inv in invites:
            ...
            try:
                async with httpx.AsyncClient() as client:
                    resp = await client.get(
                        f"{settings.ADMINISTRATION_URL}/nguoi-dung/{inv['invitee_id']}",
                        timeout=settings.DEFAULT_HTTP_TIMEOUT,
                    )
                    if resp.status_code == 200:
    ...
    async def transfer_ownership(
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{settings.ADMINISTRATION_URL}/nguoi-dung/{target_user_id}",
                    timeout=settings.DEFAULT_HTTP_TIMEOUT,
                )
                if resp.status_code == 200:
