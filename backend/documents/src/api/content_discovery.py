...
async def get_trending_documents(
    ...
    return APIResponse(
        data=await DocumentMetadata.get_trending_documents(limit),
...
async def get_tags_categories(db=Depends(get_db)):
    return APIResponse(
        data=await DocumentMetadata.get_tags_categories(),
...
async def smart_search(
    ...
    if not current_user:
        return APIResponse(
            data=await DocumentMetadata.get_text_search(query, limit),
    ...
    rag_url = settings.INTELLIGENCE_URL
    if not rag_url:
        return APIResponse(
            data=await DocumentMetadata.get_text_search(query, limit),
...
async def get_trending_tags(
    ...
    return APIResponse(
        data=await DocumentMetadata.get_trending_tags(limit),
