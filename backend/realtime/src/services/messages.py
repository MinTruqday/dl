...
class MessageManager:
    ...
    async def ensure_listener(self):
        ...
        self._listener_task = asyncio.create_task(self.global_listener())

    async def global_listener(self):
        try:
            while True:
    ...
    async def send_personal_message(self, message: dict, receiver_id: str):
        ...
        if receiver_id.startswith("group_"):
            if db_client.mongodb:
                group = await RepositoryFactory.get("chat_groups").find_one({"_id": receiver_id})
                if group:
    ...
    async def handle_action(self, user_id: str, payload: dict):
        action = payload.get("action")
        data = payload.get("data", {})
        if action == "mark_read":
            await self.action_mark_read(user_id, data)
        elif action == "typing":
            await self.action_typing(user_id, data)
        elif action == "sync":
            await self.action_sync(user_id, data)

    async def action_sync(self, user_id: str, data: dict):
        ...
        disconnected = []
        if last_message_id:
            groups = await RepositoryFactory.get("chat_groups").find({"members": user_id}).to_list(100)
            ...
            new_messages = await RepositoryFactory.get("messages").find(query).sort("created_at", 1).to_list(length=200)
    ...
    async def action_mark_read(self, user_id: str, data: dict):
        ...
        await self.send_personal_message({"type": "messages_read", "data": {"viewer_id": user_id}}, other_user_id)

    async def action_typing(self, user_id: str, data: dict):
        other_user_id = data.get("other_user_id")
        if not other_user_id:
            return
