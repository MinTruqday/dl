...
class VersionService:
    ...
    async def create_file_version(file_id: str, owner_id: str, new_url: str, new_size: int) -> dict:
        item = await StorageService.add_version(file_id, owner_id, new_url, new_size)
        return {"status": "success", "file_id": file_id, "version_count": len(item.versions)}
    ...
    async def restore_file_version(file_id: str, version_id: str, owner_id: str) -> dict:
        ...
        if not item or item.is_folder:
            raise HTTPException(status_code=404, detail="Không tìm thấy tệp tin")
        version = next(
            (candidate for candidate in item.versions if candidate.version_id == version_id), None
        )
        if not version:
            raise HTTPException(status_code=404, detail="Không tìm thấy phiên bản tệp tin")
        restored = await StorageService.add_version(file_id, owner_id, version.url, version.size)
        return {
            "status": "success",
            "file_id": file_id,
