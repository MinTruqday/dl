...
class StorageService:
    ...
    async def get_item(item_id: str, owner_id: str = None) -> Optional[StorageItemInDB]:
        ...
        item = await database.mongodb[settings.CLOUD_DB_NAME].storage_items.find_one(query)
        if item:
    ...
    async def update_item(
        ...
        result = await database.mongodb[settings.CLOUD_DB_NAME].storage_items.find_one_and_update(
            {"_id": item_id, "owner_id": owner_id}, {"$set": update_dict}, return_document=True
        )
        if result:
    ...
    async def delete_item(item_id: str, owner_id: str) -> bool:
        ...
        if item.is_locked and item.locked_by != owner_id:
            ...
            raise HTTPException(
                status_code=403, detail="Tệp đang bị khóa bởi người dùng khác, không thể xóa"
            )
        ...
        if item.is_folder:
            ...
            while pending:
                children = (
                    await database.mongodb[settings.CLOUD_DB_NAME]
                    .storage_items.find({"owner_id": owner_id, "parent_id": {"$in": pending}})
                    .to_list(length=None)
                )
                pending = []
        ...
        for entry in items:
            ...
            for url in urls:
                references = await database.mongodb[
                    settings.CLOUD_DB_NAME
                ].storage_items.count_documents({"url": url})
                if references == 0:
                    try:
    ...
    async def add_version(
        ...
        registered = await cloud_db.storage_items.find_one(
            {"owner_id": owner_id, "$or": [{"url": url}, {"versions.url": url}]}, {"_id": 1}
        )
        ...
        async with await database.mongodb.start_session() as session:
            async with session.start_transaction():
                await cloud_db.storage_items.update_one(
                    {"_id": item_id, "owner_id": owner_id}, update_op, session=session
                )
                if registered and str(registered["_id"]) != item_id:
                    await cloud_db.storage_items.delete_one(
                        {"_id": registered["_id"], "owner_id": owner_id}, session=session
                    )
    ...
    async def get_recent_items(
        owner_id: str, limit: int = Query(default=20, le=100)
    ) -> List[StorageItemInDB]:
        query = {
            "$or": [{"owner_id": owner_id}, {"shared_with.user_id": owner_id}],
            "is_trashed": False,
    ...
    async def get_versions(item_id: str, user_id: str) -> List[dict]:
        ...
        if not item or item.is_folder:
            raise HTTPException(
                status_code=404, detail="Không tìm thấy tệp tin hoặc thư mục không hỗ trợ phiên bản"
            )
    ...
    async def rollback_version(
        ...
        archived_current = FileVersion(
            url=item.url, size=item.size, created_at=item.updated_at or item.created_at
        )
    ...
    async def set_tags_and_color(
        ...
        result = await database.mongodb[settings.CLOUD_DB_NAME].storage_items.find_one_and_update(
            {"_id": item_id, "owner_id": owner_id}, {"$set": update_set}, return_document=True
        )
        from src.services.activity import ActivityService
    ...
    async def get_quota_analytics(owner_id: str) -> dict:
        ...
        categories = {
            "documents": {
                "extensions": [".pdf", ".doc", ".docx", ".txt", ".odt", ".rtf", ".pages", ".md"],
                "count": 0,
                "size": 0,
            },
            "images": {
        ...
        for cat_name, cat_data in categories.items():
            breakdown_result[cat_name] = {
                ...
                "size": cat_data["size"],
                "percentage": round(
                    (cat_data["size"] / total_used * 100) if total_used > 0 else 0.0, 2
                ),
            }

        total_folders = await database.mongodb[
            settings.CLOUD_DB_NAME
        ].storage_items.count_documents(
    ...
    async def get_shared_with_me_items(user_id: str) -> List[StorageItemInDB]:
        cursor = (
            database.mongodb[settings.CLOUD_DB_NAME]
            .storage_items.find({"shared_with.user_id": user_id, "is_trashed": False})
            .sort([("updated_at", -1)])
        )
        items = await cursor.to_list(length=None)
        return [StorageItemInDB(**item) for item in items]
