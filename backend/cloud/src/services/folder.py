...
class FolderService:
    ...
    async def get_folder_contents(folder_id: Optional[str], owner_id: str) -> List[dict]:
        ...
        return await storage_repository.find_many(query, sort=[("name", 1)], limit=500)

    @staticmethod
    async def get_folder_tree(owner_id: str) -> List[dict]:
        query = {"owner_id": owner_id, "is_folder": True, "is_trashed": {"$ne": True}}
        return await storage_repository.find_many(query, sort=[("name", 1)], limit=1000)
    ...
    async def move_folder(folder_id: str, new_parent_id: Optional[str], owner_id: str) -> dict:
        ...
        if new_parent_id:
            ...
            while ancestor.get("parent_id"):
                ...
                ancestor = await storage_repository.find_one(
