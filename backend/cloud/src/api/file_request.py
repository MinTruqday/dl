...
async def create_file_request(
    ...
    return APIResponse(data=result, message="Tạo liên kết yêu cầu tải lên hoàn tất", status=201)


@router.get("/yeu-cau-tai-len/{token}", response_model=APIResponse[Any])
async def validate_file_request(
    token: str, password: Optional[str] = Query(default=None), db=Depends(get_db)
):
    result = await FileRequestService.validate_request(token, password)
    if not result:
        raise HTTPException(
            status_code=404, detail="Liên kết yêu cầu tải lên không hợp lệ hoặc đã hết hạn"
        )
    ...
    return APIResponse(
        data=result, message="Xác thực liên kết yêu cầu tải lên thành công", status=200
    )
