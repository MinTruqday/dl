...
async def run_worker():
    ...
    async def route_anna_collector(payload):
        ...
        await AnnaArchiveCollection.run_list_collector(search_query="", pages=pages)

    async def route_ctan_collector(payload):
        pages = int(payload.get("pages", 0))
        await CTANCollection.run_list_collector(pages)

    async def route_list_collector(payload):
        source = payload.get("source", "AnnaArchive")
        pages = int(payload.get("pages", 0))
        if source == "NXBST":
            await NXBSTCollection.run_list_collector(pages)
        elif source == "CTAN":
            await CTANCollection.run_list_collector(pages)
        else:
            await AnnaArchiveCollection.run_list_collector(search_query="", pages=pages)
    ...
    async def route_detail_collector(payload):
        source = payload.get("source", "AnnaArchive")
        if source == "NXBST":
            await NXBSTCollection.run_detail_collector(payload["url"])
        elif source == "CTAN":
            await CTANCollection.run_detail_collector(payload["url"])
        else:
            await AnnaArchiveCollection.run_detail_collector(payload["url"])

    async def route_download_processor(payload):
        source = payload.get("source", "AnnaArchive")
        if source == "CTAN":
            await CTANCollection.run_download_processor(payload)
        else:
            await AnnaArchiveCollection.run_download_processor(payload)

    async def route_nxbst_collector(payload):
        url = payload.get("url")
        if url:
            await NXBSTCollection.run_detail_collector(url)
        else:
            await NXBSTCollection.run_list_collector(int(payload.get("pages", 0)))
    ...
    await queue_nxbgd.consume(
        lambda m: process_msg_with_sem(
            m, lambda p: NXBGDCollection(p.get("target_class", "-1")).execute()
        )
    )
