...
async def get_stats(db=Depends(get_db)):
    return APIResponse(data=await TelemetryService.get_system_stats(db=db), message='Tải thống kê hệ thống thành công')
...
async def get_audit_logs(limit: int=50, offset: int=0, db=Depends(get_db)):
    return APIResponse(data=await TelemetryService.get_activity_stats(days=30, db=db), message='Tải nhật ký hệ thống thành công')
...
async def get_moderator_activity(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await TelemetryService.get_moderator_activity_log(str(current_user.id), db=db), message='Tải nhật ký hoạt động điều hành thành công')