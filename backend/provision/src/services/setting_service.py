...
class SettingService:
    ...
    async def get_settings(current_user, db=None) -> dict:
        ...
        defaults = {'appearance': 'light', 'fontSize': 'medium', 'notifications': True, 'privacyProfile': 'public', 'privacyActivity': True, 'twoFactor': False, 'notifyCommunity': {'email': True, 'inapp': True}, 'notifyFinance': {'email': True, 'inapp': True}, 'notifyUpdates': {'email': False, 'inapp': True}, 'notifyNewsletter': {'email': True, 'inapp': False}}
    ...
    async def update_settings(settings_data: dict, current_user, db=None) -> dict:
        ...
        await db['users'].update_one({'_id': user_id}, {'$set': {'settings': merged_settings, 'updated_at': datetime.now(timezone.utc)}})
