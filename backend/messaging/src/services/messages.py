...
from src.schemas.messages import MessageInDB
from uuid6 import uuid7

class MessageService:

    @staticmethod
    async def _upsert_conversation(sender_id: str, receiver_id: str, message_data: dict):
        participant_key = (
            receiver_id
        ...
        if receiver_id.startswith("group_"):
            group_doc = await RepositoryFactory.get("message_groups").find_one({"_id": receiver_id})
    ...
    async def send_message(
        ...
        user_doc = await RepositoryFactory.get("user_contact_profiles").find_one({"_id": receiver_id})
        ...
        tasks = [RepositoryFactory.get("message_settings").find_one({"_id": settings_id})]
        if reply_to_id:
            tasks.append(RepositoryFactory.get("messages").find_one({"_id": reply_to_id}))
            
        results = await asyncio.gather(*tasks)
        msg_settings = results[0]
        reply_msg = results[1] if reply_to_id and len(results) > 1 else None
        
        self_destruct_seconds = None
        if msg_settings and msg_settings.get("self_destruct_seconds", 0) > 0:
            self_destruct_seconds = msg_settings["self_destruct_seconds"]
    ...
    async def get_messages(
        ...
        if last_msg_id:
            update_data[f"last_read_message_id.{current_user.id}"] = str(last_msg_id)
        await RepositoryFactory.get("conversations").update_one(
    ...
    async def get_conversations(current_user):
        ...
        for conv in conversations:
            ...
            other_id = str(conv["_id"]) if is_group else None
    ...
    async def edit_message(message_id: str, new_content: str, current_user):
        ...
        updated_msg = await RepositoryFactory.get("messages").find_one({"_id": message_id})
        participant_key = (
            msg["receiver_id"]
            if msg["receiver_id"].startswith("group_")
            else f"{min(msg['sender_id'], msg['receiver_id'])}_{max(msg['sender_id'], msg['receiver_id'])}"
        ...
        if conv and str(conv.get("last_message", {}).get("_id")) == message_id:
            await RepositoryFactory.get("conversations").update_one(
                {"_id": participant_key},
    ...
    @staticmethod
    async def search_messages(other_user_id: str, query_str: str, current_user) -> list:
    ...
    async def mark_as_read(other_user_id: str, current_user):
        ...
        if last_msg:
            update_data[f"last_read_message_id.{user_id}"] = str(last_msg["_id"])
    ...
    async def check_blocked_status(user_id: str, other_user_id: str) -> bool:
        user_doc = await RepositoryFactory.get("user_contact_profiles").find_one({"_id": user_id})
        other_user_doc = await RepositoryFactory.get("user_contact_profiles").find_one({"_id": other_user_id})
    ...
    @staticmethod
    async def translate_message(message_id: str, target_lang: str, current_user):
        msg = await RepositoryFactory.get("messages").find_one({"_id": message_id})
        if not msg:
            return None
        ...
        try:
            ...
            translated_content = f"Translation fallback string generated for original content"
