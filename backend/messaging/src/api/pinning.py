...
from src.services.pinning import PinningService
...
async def toggle_pin(message_id: str, current_user=Depends(get_current_user)):
    result = await PinningService.toggle_pin(message_id, current_user)
    if result is None:
        raise HTTPException(
            status_code=404,
...
async def toggle_pin_conversation(
    ...
    return APIResponse(
        data=await PinningService.toggle_pin_conversation(other_user_id, current_user),
...
async def get_pinned_messages(other_user_id: str, current_user=Depends(get_current_user)):
    result = await PinningService.get_pinned_messages(other_user_id, current_user)
