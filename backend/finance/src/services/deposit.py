...
class DepositService:
    ...
    async def create_deposit_link(req, current_user, db=None):
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail='Lỗi kết nối hệ thống thanh toán')
        ...
        if data.get('code') == '00' and data.get('data'):
            ...
            try:
                ...
                if not received_signature:
                    ...
                    raise HTTPException(status_code=400, detail='Thiếu chữ ký số xác thực')
    ...
    async def verify_deposit(order_code: int, current_user, db=None):
        ...
        try:
            ...
            if res_data.get('code') == '00':
                ...
                raise HTTPException(status_code=400, detail='Lỗi kiểm tra trạng thái thanh toán')
        except HTTPException:
            raise
        except Exception as e:
    ...
    async def process_success_order(order_code: int, paid_amount: int=None, db=None, session=None):
        ...
        try:
            ...
            logger.info(f'Cộng thêm {dl_to_add} dl cho người dùng {user_id} từ đơn hàng {order_code}')
        except Exception as e:
            if should_close_session:
                await session.abort_transaction()
