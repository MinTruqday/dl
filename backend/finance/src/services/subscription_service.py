...
class SubscriptionService:
    ...
    async def create_subscription_plan(
        plan_data: dict, current_user: UserInDB, db=None
    ):
        if db is None:
        ...
        logger.info(
            f"Subscription: Author {current_user.id} created plan {plan_doc['name']}"
        )
        return plan_doc
    ...
    async def get_my_subscriptions(current_user: UserInDB, db=None):
        ...
        plans = (
            ...
            .to_list(length=len(plan_ids) or 1)
        )
    ...
    async def resume_subscription(
        subscription_id: str, current_user: UserInDB, db=None
    ):
        if db is None:
            db = db_client.mongodb.get_default_database()
        await db["subscriptions"].update_one(
