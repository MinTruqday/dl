...
async def request_withdrawal(data: WithdrawalRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.create_withdrawal_request(data.model_dump(), current_user, db=db), message='Đã gửi yêu cầu rút tiền', status=201)
...
async def get_withdrawal_queue(status: str='PENDING', db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.get_payout_queue(db=db), message='Đã lấy danh sách hàng đợi thanh toán')
...
async def cancel_withdrawal(withdrawal_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.cancel_withdrawal(withdrawal_id, current_user, db=db), message='Đã hủy lệnh rút tiền')
...
async def get_my_withdrawals(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.get_withdrawals(current_user, db=db), message='Đã tải danh sách lệnh rút tiền')