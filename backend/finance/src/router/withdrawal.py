...
async def request_withdrawal(data: WithdrawalRequest, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.request_withdrawal(data.model_dump(), current_user, db=db), message='Yêu cầu rút tiền success', status=201)
...
async def get_withdrawal_queue(status: str='PENDING', db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.get_withdrawal_queue(status, db=db), message='Lấy hàng đợi thanh toán success')
...
async def verify_withdrawal(withdrawal_id: str, action: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.verify_withdrawal(withdrawal_id, action, current_user, db=db), message='Xử lý thanh toán success')
...
async def cancel_withdrawal(withdrawal_id: str, current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.cancel_withdrawal(withdrawal_id, current_user, db=db), message='Hủy yêu cầu rút tiền success')
...
async def get_my_withdrawals(current_user: UserInDB=Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(data=await WithdrawalService.get_my_withdrawals(current_user, db=db), message='Lấy danh sách yêu cầu rút tiền success')