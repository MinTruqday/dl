...
async def request_withdrawal(req: WithdrawalRequest, current_user: UserInDB = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
...
@router.post("/{withdrawal_id}/verify", response_model=APIResponse[Any], dependencies=[Depends(require_role([RoleEnum.ADMIN]))])
async def verify_withdrawal(withdrawal_id: str, action: str, reason: str = "", current_user: UserInDB = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
