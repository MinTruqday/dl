...
class RabbitMQClient:
    ...
    async def connect(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.exception("Failed to connect to RabbitMQ, retrying...")
    ...
    async def _auto_nack_if_timeout(self, ack_id: str, delay: int):
        ...
        if message:
            logger.warning(f"Message acknowledgment timeout for delivery tag {ack_id}, auto-nacking")
            try:
    ...
    async def ack(self, delivery_tag: str) -> bool:
        ...
        if message:
            try:
                ...
                logger.exception("Failed to send ACK response to RabbitMQ")
                return False
        return False
