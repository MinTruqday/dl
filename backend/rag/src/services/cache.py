...
class CacheService:
    async def get_response(self, query_text: str, query_vector: Optional[List[float]] = None) -> CacheGetResponse:
        if not query_vector:
    ...
    async def set_response(self, query_text: str, response_text: str, query_vector: Optional[List[float]] = None) -> None:
        if not query_vector:
            query_vector = await embedder.embed_query(query_text)
