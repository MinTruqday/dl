...
class AIClient:
    async def inspect_rag_chunks(self, texts: list[str]) -> set[int]:
        async with httpx.AsyncClient(
            timeout=min(settings.AI_REQUEST_TIMEOUT_SECONDS, 60.0)
        ) as client:
            response = await client.post(
                f"{settings.AI_URL}/suy-luan/noi-bo/kiem-tra-doan-rag",
    ...
    async def summarize_rag_document(self, text: str) -> str:
        async with httpx.AsyncClient(
            ...
            response = await client.post(
                f"{settings.AI_URL}/suy-luan/noi-bo/tom-tat-tai-lieu-rag",
    ...
    async def expand_retrieval_query(self, question: str) -> dict:
        async with httpx.AsyncClient(timeout=25.0) as client:
            response = await client.post(
                f"{settings.AI_URL}/suy-luan/noi-bo/mo-rong-truy-van",
    ...
    async def decompose_cross_document_query(
        ...
        async with httpx.AsyncClient(timeout=25.0) as client:
            response = await client.post(
                f"{settings.AI_URL}/suy-luan/noi-bo/phan-ra-lien-tai-lieu",
