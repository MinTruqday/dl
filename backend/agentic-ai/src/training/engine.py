...
def run_finetune_job(job_id: str, config: dict, update_callback):
    ...
    if "flux" in base_model_name or "diffusion" in base_model_name:
        logger.info("Detected Diffusion model. Dispatching to Diffusion Engine.")
        ...
        logger.info("Detected Seq2Seq model. Dispatching to Seq2Seq Engine.")
...
def run_seq2seq_training(job_id: str, config: dict, update_callback):
    ...
    logger.info(f"[PyTorch Seq2Seq] Loading base model on CUDA: {base_model_name}")
    update_callback({"progress": 10, "status": "running"})
    ...
    logger.info(f"[PyTorch Seq2Seq] Merging adapter into base model")
...
def run_diffusion_training(job_id: str, config: dict, update_callback):
    ...
    logger.info(f"[PyTorch Diffusion] Loading base model on CUDA: {base_model_name}")
    update_callback({"progress": 10, "status": "running"})
