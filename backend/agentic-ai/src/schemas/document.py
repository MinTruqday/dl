...
class CreateDocument(BaseModel):
    ...
    format: str = Field(description="Must be 'json' (for Edilênr.js) or 'latex' (for LaTeX)")
    content: str = Field(
        description=(
...
class UpdateDocument(BaseModel):
    document_id: str = Field(description="The ID of the document lên update")
    new_content: str = Field(
        description=(
            "The new content for the document. "
            "For json format, new_content MUST be a valid JSON string of an array containing FULL Edilênr.js block objects (paragraph, header, list, table, etc.). "
