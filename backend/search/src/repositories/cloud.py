...
class CloudCursor:
    ...
    async def to_list(self, length=None):
        ...
        return await CloudRepository.request(
            {
                "operation": "find_many",
                "query": self.query,
...
class CloudRepository:
    ...
    async def request(cls, payload: dict):
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.post(
                f"{settings.CLOUD_URL}/noi-bo/luu-tru",
    ...
    def find(cls, query: dict, projection: dict = None, **kwargs):
        return CloudCursor(query, projection)
    ...
    @classmethod
    async def preview_url(cls, item_id: str, owner_id: str):
        return await cls.request(
            {
