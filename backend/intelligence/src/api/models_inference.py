...
from src.schemas.models_inference import (
    ActionRequest,
    CitationRequest,
    CodeRequest,
    GenerationRequest,
...
async def _check_quota(current_user: CurrentUser):
    try:
        async with httpx.AsyncClient() as c:
            resp = await c.get(
                f"{settings.ADMINISTRATION_URL}/han-muc/xac-minh",
                params={
...
async def _consume_quota(
    ...
    try:
        async with httpx.AsyncClient() as c:
            await c.post(
                f"{settings.ADMINISTRATION_URL}/han-muc/su-dung",
                json={
                    "user_id": str(current_user.id),
                    "feature": "chat",
...
async def check_plagiarism(
    ...
    try:
        ...
        from src.rag.vector_embeddings import embedder
