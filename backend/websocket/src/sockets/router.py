...
async def chat_websocket(websocket: WebSocket, user_id: str, token: str = Query(...)):
    try:
        # Validate token and user_id matches
        ...
        while True:
            # For chat, the server pushes to client. Client might send ping or typing status.
            data = await websocket.receive_text()
            # If needed, process incoming data (e.g., typing indicator)
...
@router.websocket("/editor/o-cam-crdt/{document_id}")
