...
class OperationManager:
    ...
    async def get_collector_logs(db=None) -> list:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{settings.COLLECTOR_URL}/nhat-ky-hoat-dong",
                    timeout=settings.DEFAULT_HTTP_TIMEOUT,
                )
                if resp.status_code == 200:
                    return resp.json()
        except Exception:
            logger.error("Lỗi tải nhật ký hoạt động")
        return []
    ...
    async def get_active_collector_jobs(db=None) -> list:
        ...
        try:
            ...
            logger.error("Lỗi tải danh sách tác vụ nền")
        return []
    ...
    async def get_withdrawal_requests(
        ...
        try:
            ...
            logger.error("Lỗi truy xuất hàng đợi rút tiền")
        return []
    ...
    async def approve_withdrawal(withdrawal_id: str, system_id: str, db=None) -> dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.post(
                    f"{settings.FINANCE_URL}/rut-tien/{withdrawal_id}/xac-minh",
        ...
        return {
            "status": "error",
            "message": "Lỗi giao dịch, vui lòng thử lại sau",
        }
