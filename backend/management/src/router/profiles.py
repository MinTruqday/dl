...
async def apply_author(data: Any, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
...
async def get_reading_streaks(current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
...
async def get_badges(current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
...
async def block_user(target_id: str, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
...
async def update_brand_page(data: BrandPageUpdate, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    return APIResponse(
