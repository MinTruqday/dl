...
async def get_sys_health(db=Depends(get_db)):
    return APIResponse(
        data=await SystemTelemetry.get_sys_health(db=db),
...
async def get_audit_logs(
    ...
    return APIResponse(
        data=await SystemTelemetry.get_activity_stats(days=30, db=db),
...
async def get_activity(
    ...
    return APIResponse(
        data=await SystemTelemetry.get_activity_log(str(current_user.id), db=db),
