...
async def get_announcements(
    ...
    return APIResponse(
        data=await AnnouncementService.get_announcements(str(current_user.id), skip, limit),
...
async def mark_as_read(
    notif_id: str, current_user: CurrentUser = Depends(get_current_user)
):
    return APIResponse(
        data=await AnnouncementService.mark_as_read(notif_id, str(current_user.id)),
...
async def mark_all_as_read(current_user: CurrentUser = Depends(get_current_user)):
    return APIResponse(
        data=await AnnouncementService.mark_all_as_read(str(current_user.id)),
...
async def delete_announcement(
    ...
    return APIResponse(
        data=await AnnouncementService.delete_announcement(notif_id, str(current_user.id)),
...
async def create_announcement(data: AnnouncementCreate):
    return APIResponse(
        data=await AnnouncementService.create_announcement(data),
...
async def get_settings(current_user: CurrentUser = Depends(get_current_user)):
    return APIResponse(
        data=await AnnouncementService.settings(str(current_user.id)),
...
async def update_settings(
    ...
    return APIResponse(
        data=await AnnouncementService.update_settings(str(current_user.id), settings),
